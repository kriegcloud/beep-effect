...
>() {
  ...
  static testLayer = Layer.succeed(TaskRepo, {
    ...
    add: (text) => Effect.succeed(Task.make({ id: TaskId.make(1), text, done: false })),
...
const listCommand = Command.make("list", { all: Options.boolean("all").pipe(Options.withAlias("a")) }, ({ all }) =>
  Effect.gen(function* () {
    const repo = yield* TaskRepo
  ...
  describe("TaskRepo Live (with temp files)", () => {
    it.scoped("add persists task to file", () =>
      Effect.gen(function* () {
        ...
        const repo = yield* Effect.provide(TaskRepo, TaskRepo.layer(path).pipe(Layer.provide(NodeContext.layer)))
    ...
    it.scoped("loads existing tasks from file", () =>
      Effect.gen(function* () {
        ...
        const initial = TaskList.make({
          tasks: [Task.make({ id: TaskId.make(1), text: "Existing", done: false })],
        })
