...
export const context = <TState, TConnParams, TConnState, TVars, TInput>(): Effect.Effect<
	ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
	never,
	RivetActorContext
> => RivetActorContext as any

export const state = <TState, TConnParams, TConnState, TVars, TInput>(
...
export const updateState = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
	f: (state: TState) => void,
): Effect.Effect<void, never, never> => Effect.sync(() => f(c.state))

export const vars = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
): Effect.Effect<TVars, never, never> => Effect.succeed(c.vars)

export const updateVars = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
	f: (vars: TVars) => void,
...
export const getLog = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.log)

export const getActorId = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.actorId)

export const getName = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.name)

export const getKey = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.key)

export const getRegion = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.region)

export const getSchedule = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.schedule)

export const getConns = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.conns)

export const getClient = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.client())

export const getDb = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.db)

export const getKv = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
) => Effect.succeed(c.kv)
...
export const getAbortSignal = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
): Effect.Effect<AbortSignal, never, never> => Effect.succeed(c.abortSignal)

export const sleep = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
): Effect.Effect<void, never, never> => Effect.sync(() => c.sleep())

export const destroy = <TState, TConnParams, TConnState, TVars, TInput>(
	c: ActorContext<TState, TConnParams, TConnState, TVars, TInput, undefined>,
