...
export class FileValidationError extends Data.TaggedError("FileValidationError")<{
  ...
  override get message(): string {
    return Match.value(this.reason).pipe(
      Match.when("InvalidName", () => "Directory names cannot be empty or contain '/'."),
      Match.when("UnsupportedType", () => "The playground currently only supports creation of `.ts` files."),
      Match.exhaustive
    );
  }
