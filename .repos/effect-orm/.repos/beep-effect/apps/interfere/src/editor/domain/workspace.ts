...
export class File extends Schema.TaggedClass<File>()("File", {
  ...
  language: Schema.String.pipe(Schema.optionalWith({ default: () => "typescript" })),
  userManaged: Schema.Boolean.pipe(Schema.optionalWith({ default: () => false })),
}) {
...
const Directory_: Schema.Struct<{
  ...
  userManaged: Schema.optionalWith<typeof Schema.Boolean, { default: () => false }>;
  children: Schema.Array$<Schema.Union<[typeof File, Schema.Schema<Directory>]>>;
}> = Schema.Struct({
  _tag: Schema.tag("Directory"),
  name: Schema.String,
  userManaged: Schema.Boolean.pipe(Schema.optionalWith({ default: () => false })),
  children: Schema.Array(Schema.Union(File, Schema.suspend(() => Directory_) as Schema.Schema<Directory>)),
});
export const Directory: Schema.Schema<Directory, Schema.Schema.Encoded<typeof Directory_>> = Directory_;
export function makeDirectory(name: string, children: ReadonlyArray<File | Directory>, userManaged = false): Directory {
      ...
      Effect.gen(this, function* () {
        ...
        for (const node of tree) {
          if (node._tag === "File") {
            ...
            out.push(makeDirectory(node.name, yield* walk(node.children), node.userManaged));
          }
        }
        return out;
      });
    return Effect.map(walk(this.tree), (tree) => new Workspace({ ...this, tree }));
  }
...
function filterMapTree(tree: FileTree, f: (node: File | Directory) => Option.Option<File | Directory>): FileTree {
  ...
  for (const node of tree) {
    ...
    if (result.value === node && result.value._tag === "Directory") {
      out.push(makeDirectory(result.value.name, filterMapTree(result.value.children, f), result.value.userManaged));
    } else {
