...
export interface BuildQueryConfig {
  ...
  prepareTyping?: undefined | ((encoder: DriverValueEncoder<unknown, unknown>) => QueryTypingsValue);
  paramStartIndex?: undefined | { value: number };
  inlineParams?: undefined | boolean;
...
export interface QueryWithTypings extends Query {
  typings?: undefined | QueryTypingsValue[];
}
...
export class SQL<T = unknown> implements SQLWrapper<T> {
    ...
    for (const chunk of queryChunks) {
      if (is(chunk, Table)) {
        ...
        this.usedTables.push(
          schemaName === undefined ? chunk[Table.Symbol.Name] : `${schemaName}.${chunk[Table.Symbol.Name]}`
        );
      }
    }
  }
  ...
  buildQueryFromSourceParams(chunks: SQLChunk[], _config: BuildQueryConfig): Query {
    ...
    return mergeQueries(
      chunks.map((chunk): QueryWithTypings => {
          ...
          return {
            sql: chunk.isAlias
              ...
              : chunk.table[IsAlias] || schemaName === undefined
                ...
                : `${escapeName(schemaName)}.${escapeName(chunk.table[Table.Symbol.Name])}.${escapeName(columnName)}`,
            params: [],
          };
        }
  ...
  private mapInlineParam(chunk: unknown, { escapeString }: BuildQueryConfig): string {
    if (chunk === null) {
      return "null";
    }
  ...
  as(alias?: undefined | string): SQL<T> | SQL.Aliased<T> {
  ...
  mapWith<TDecoder extends DriverValueDecoder<any, any> | DriverValueDecoder<any, any>["mapFromDriverValue"]>(
    decoder: TDecoder
...
export function param<TData, TDriver>(
  value: TData,
  encoder?: undefined | DriverValueEncoder<TData, TDriver>
): Param<TData, TDriver> {
...
export namespace sql {
   ...
   * ```
   */
  export function join(chunks: SQLChunk[], separator?: undefined | SQLChunk): SQL {
