...
const ConstraintsSchema = S.Struct({
  email: S.String.pipe(S.pattern(/^[^\s@]+@[^\s@]+\.[^\s@]+$/, { message: () => "Invalid email format" })),
  count: S.Number.pipe(S.positive()),
  description: S.optional(S.String.pipe(S.minLength(10))),
});

const DeepSchema = S.Struct({
  a: S.Struct({
    b: S.Struct({
      c: S.Struct({
        d: S.String,
      }),
      e: S.optional(S.Number),
    }),
  }),
});
...
describe("validateWithSchema", () => {
  describe("Simple Schema", () => {
    ...
    it("should return errors for invalid types", () => {
      ...
      const expectedErrors: Partial<Record<Paths<S.Schema.Encoded<typeof SimpleSchema>>, string>> = {
  ...
  describe("Nested Schema", () => {
    ...
    it("should return null when optional nested object is missing", () => {
      const submission = {
        value: {
          // address is missing, which is allowed by S.optional
    ...
    it("should return errors for invalid data in nested objects", () => {
      ...
      const expectedErrors: Partial<Record<Paths<S.Schema.Encoded<typeof NestedSchema>>, string>> = {
  ...
  describe("Array Schema", () => {
    ...
    it("should return errors for invalid types within array elements", () => {
      ...
      const expectedErrors: Partial<Record<Paths<S.Schema.Encoded<typeof ArraySchema>>, string>> = {
  ...
  describe("Constraints Schema", () => {
    ...
    it("should return errors for violated constraints", () => {
      ...
      const expectedErrors: Partial<Record<Paths<S.Schema.Encoded<typeof ConstraintsSchema>>, string>> = {
  ...
  describe("Deeply Nested Schema", () => {
    ...
    it("should return errors with correct deep path", () => {
      ...
      const expectedErrors: Partial<Record<Paths<S.Schema.Encoded<typeof DeepSchema>>, string>> = {
  ...
  describe("Filter and Brand Schema", () => {
    const DomainSchema = S.String.pipe(
      S.filter((s): s is string => /^[a-zA-Z0-9-]+\.[a-zA-Z]{2,}$/.test(s), {
    ...
    it("should return an error if the filter condition is not met", () => {
      ...
      const expectedErrors: Partial<Record<Paths<S.Schema.Encoded<typeof FilterBrandSchema>>, string>> = {
  ...
  describe("Root Level Errors (Filters/Refinements)", () => {
    const PasswordSchema = S.Struct({
      password: S.String,
      confirmPassword: S.String,
    }).pipe(
      S.filter((input) => input.password === input.confirmPassword, {
    ...
    it("should return a root error if the refinement fails", () => {
      ...
      const expectedErrors: Partial<Record<Paths<S.Schema.Encoded<typeof PasswordSchema>> | "", string>> = {
    ...
    it("should return field errors alongside root errors if both fail", () => {
      const ComplexRootSchema = S.Struct({
        value: S.Number,
        limit: S.Number,
      }).pipe(
        S.filter((input) => input.value <= input.limit, {
      ...
      const expectedErrors: Partial<Record<Paths<S.Schema.Encoded<typeof ComplexRootSchema>> | "", string>> = {
      ...
      const expectedErrorsRefinementFail: Partial<
        Record<Paths<S.Schema.Encoded<typeof ComplexRootSchema>> | "", string>
      > = {
      ...
      const expectedErrorsBothFail: Partial<Record<Paths<S.Schema.Encoded<typeof ComplexRootSchema>> | "", string>> = {
