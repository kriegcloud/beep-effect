...
Based on industry consensus (arXiv:2512.13564), handoff documents should organize context into four memory types:

| Memory Type | Definition | Spec Equivalent | Token Budget |
|-------------|------------|-----------------|--------------|
| **Working** | Current context window content | Phase-specific tasks, success criteria | ≤2,000 |
| **Episodic** | Specific interaction history | Previous phase outcomes, decisions | ≤1,000 |
| **Semantic** | Accumulated facts/knowledge | Project constants, tech stack | ≤500 |
| **Procedural** | Learned action patterns | Links to patterns, standards | Links only |

**Total handoff budget**: ≤4,000 tokens (well under degradation threshold)

### Memory Type Template Sections

Every handoff document SHOULD include these labeled sections:

```markdown
## Context for Phase [N]

### Working Context (≤2K tokens)
Include directly in prompt. Most critical for immediate task success.

- Current task: [specific action to complete]
- Success criteria: [measurable outcomes]
- Blocking issues: [if any]
- Immediate dependencies: [files/services needed]

### Episodic Context (≤1K tokens)
Reference when historical context matters.

- Phase [N-1] outcome: [1-2 sentence summary]
- Key decisions made: [bullet list of decisions]
- Patterns discovered: [if any]

### Semantic Context (≤500 tokens)
Persistent project knowledge that rarely changes.

- Tech stack: [core technologies]
- Architectural constraints: [key boundaries]
- Naming conventions: [if non-standard]

### Procedural Context (links only)
Point to documentation, never inline.

- Effect patterns: `.claude/rules/effect-patterns.md`
- Testing patterns: `@effect/vitest` conventions
- [Domain-specific docs]: `path/to/relevant/doc.md`
```

### Context Placement Guidelines

Research shows models recall beginning/end content better than middle content ("lost in the middle" effect).

| Document Position | Content Priority |
|-------------------|------------------|
| First 25% | Critical: Success criteria, blocking issues, constraints |
| Middle 50% | Supporting: Episodic history, background context |
| Last 25% | Actionable: Next steps, verification commands |
...
- [ ] Semantic context ≤500 tokens (or links)
- [ ] Procedural context uses links, not inline content
...
- [Spec Guide](README.md) - Full spec creation workflow
