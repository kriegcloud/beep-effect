...
import { ColumnConfig } from '../core/filters'
import { DEFAULT_OPERATORS, determineNewOperator } from '../core/Operators'
import { Filter, type FiltersState } from '../core/types'
...
const optionColumn = ColumnConfig.option<TestData, 'status'>({
  id: 'status',
...
const textColumn = ColumnConfig.text<TestData, 'name'>({
  id: 'name',
...
describe('useDataTableFilters', () => {
  ...
  it('should update the filter operator using setFilterOperator', () => {
    ...
    act(() => {
      result.current.actions.setFilterOperator('name', 'DoesNotContain')
    })
