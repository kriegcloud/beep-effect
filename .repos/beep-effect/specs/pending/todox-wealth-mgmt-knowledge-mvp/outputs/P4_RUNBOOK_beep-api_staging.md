...
- `beep-api` is the backend API for the TodoX WM Knowledge MVP: it handles auth/session flows, ingests Gmail material into Documents, serves Knowledge queries, and supports evidence-first meeting prep grounded in persisted spans.
...
- Dependencies:
  ...
  - OTLP backend: `Grafana Cloud` or self-hosted `Grafana LGTM` (dev uses LGTM container; staging should target a reachable collector)
...
- How to run migrations:
  - Recommended: Cloud Run Job (e.g. `beep-api-migrate-staging`) that runs `bun run db:migrate`.
...
- Restore steps:
  ...
  2. Update `DB_PG_URL` to point to the restored instance (staging secrets).
  3. Run migrations if needed to align schema.
  4. Validate the `/knowledge` demo path and evidence resolvability.
- Verification:
...
- Staging should be treated as the canary for env var contract drift; enforce `@beep/shared-env` schema names at deploy time.
