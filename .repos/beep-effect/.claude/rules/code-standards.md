...
```typescript
// Parse + validate in one step (avoid JSON.parse + casting)
import * as S from "effect/Schema";
const UserSchema = S.Struct({ id: S.String, name: S.String });
const data = yield* S.decode(S.parseJson(UserSchema))(response);
```
...
```typescript
// Parse JSON with Schema (avoid JSON.parse + null returns)
import * as Effect from "effect/Effect";
import * as S from "effect/Schema";

const parseJsonUnknown = (input: string) =>
  S.decode(S.parseJson(S.Unknown))(input).pipe(Effect.mapError((error) => new ParseError({ cause: error })));
```
