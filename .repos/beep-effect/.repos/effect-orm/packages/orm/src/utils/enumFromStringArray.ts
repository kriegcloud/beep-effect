...
export const enumOf: EnumOf = <T extends string>(
  ...
  const enumObj = A.reduce(
    literals,
    {} as { readonly [K in T]: K },
    (acc, k) => ({ ...acc, [k]: k }),
  )
  invariant(
    P.isReadonlyObject(enumObj),
    "Expected enum to be a readonly record",
    {
 ...
 * @since 0.1.0
 */
export type EnumValues = <K extends string, A extends string>(
  o: R.ReadonlyRecord<K, A>,
...
export const enumValues: EnumValues = F.flow(
  <K extends string, A extends string>(
    ...
    invariant(
      A.every(values, P.isString),
      "Expected enum values to be strings",
      {
        file: "packages/common/utils/src/transformations/enumFromStringArray.ts",
        line: 33,
        args: [values],
 ...
 * @since 0.1.0
 */
export type EnumFromStringArray = <T extends A.NonEmptyReadonlyArray<string>>(
  ...values: T
) => { [K in T[number]]: K }
/**
 ...
 * @since 0.1.0
 */
export const enumFromStringArray: EnumFromStringArray = <
  T extends A.NonEmptyReadonlyArray<string>,
>(
  ...values: T
) =>
  A.reduce(
