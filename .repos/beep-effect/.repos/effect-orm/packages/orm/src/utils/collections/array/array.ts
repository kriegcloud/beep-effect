...
export const isNonEmptyReadonlyArrayOfGuard =
  <S extends S.Top & { readonly DecodingServices: never }>(self: S) =>
  (array: unknown): array is A.NonEmptyReadonlyArray<S["Type"]> =>
...
export const spliceRemove =
  ...
  <A>(arr: ReadonlyArray<A>): Array<A> =>
    F.pipe(
      arr,
      A.take(start),
      A.appendAll(F.pipe(arr, A.drop(start + deleteCount))),
    )
