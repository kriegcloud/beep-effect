...
1. Test Bodies - Use Effect.fn(function* () {...})
   - REQUIRED: it.effect("test name", Effect.fn(function* () {...}))
   - FORBIDDEN: it.effect("test name", () => Effect.gen(function* () {...}))
...
3. Layer Composition - Use it.layer() utility for shared layers
   - REQUIRED: it.layer(TestLayer)("suite", (it) => { it.effect(...) })
   - FORBIDDEN: it.effect("test", () => Effect.gen(...).pipe(Effect.provide(TestLayer)))
...
// Unit test
it.effect("validates input", Effect.fn(function* () {
...
it.layer(TestLayer, { timeout: Duration.seconds(60) })("integration", (it) => {
...
| `[TEST_FILE_PATH]` | Path to test file | `packages/orm/test/services/UserService.test.ts` |
...
- Use it.layer() utility with appropriate timeout (30-60 seconds)
...
Target: packages/orm/test/services/UserService.test.ts
...
2. Layer Composition - Use it.layer() utility for shared layers
   - REQUIRED: it.layer(TestLayer)("suite", (it) => { it.effect(...) })
...
it.layer(TestLayer, { timeout: Duration.seconds(60) })("UserService", (it) => {
