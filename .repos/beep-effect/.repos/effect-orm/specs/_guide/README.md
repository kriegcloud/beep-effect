...
## Package-Targeting Specs

If your spec targets `packages/orm/*`, check if the package already exists:

```bash
ls packages/orm/  # Exists? → Bootstrapped. Empty/missing? → Greenfield.
```

**Bootstrapped packages** require "Extend" language (not "Create") and a "Current State" section.

See [patterns/bootstrapped-package-specs.md](patterns/bootstrapped-package-specs.md) for the full pattern.
...
- Path alias usage (@beep/effect-orm)
- Module organization
...
Follow @effect/vitest patterns (it.effect, it.layer, it.scoped helpers).
```
...
| **test-writer**               | Effect-first tests with @effect/vitest | `*.test.ts` files      |
...
Create the spec directory and required files manually. Choose the appropriate structure based on the complexity score:

**Simple (score 0-20):**

```bash
mkdir -p specs/pending/[SPEC_NAME]
...
```bash
mkdir -p specs/pending/[SPEC_NAME]/{outputs,handoffs}
touch specs/pending/[SPEC_NAME]/README.md
touch specs/pending/[SPEC_NAME]/REFLECTION_LOG.md
touch specs/pending/[SPEC_NAME]/QUICK_START.md
```
...
touch specs/pending/[SPEC_NAME]/REFLECTION_LOG.md
touch specs/pending/[SPEC_NAME]/QUICK_START.md
touch specs/pending/[SPEC_NAME]/MASTER_ORCHESTRATION.md
touch specs/pending/[SPEC_NAME]/AGENT_PROMPTS.md
touch specs/pending/[SPEC_NAME]/RUBRICS.md
```
...
### Step 2: Scaffold with doc-writer (Optional)
...
- [patterns/bootstrapped-package-specs.md](patterns/bootstrapped-package-specs.md) - Extending existing packages
