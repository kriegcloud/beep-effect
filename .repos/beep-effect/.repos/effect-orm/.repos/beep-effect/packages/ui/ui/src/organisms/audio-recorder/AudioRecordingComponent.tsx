...
const LiveAudioVisualizer = React.lazy(async () => {
  const { LiveAudioVisualizer } = await import("../visualize-audio");
 ...
 */
export const AudioRecorder: (props: Props) => React.ReactElement = ({
  onRecordingComplete,
  onNotAllowedOrFound,
  recorderControls,
  ...
  const stopAudioRecorder: (save?: undefined | boolean) => void = (save = true) => {
    setShouldSave(save);
    stopRecording();
  };
  ...
  const convertToDownloadFileExtension = async (webmBlob: Blob): Promise<Blob> => {
    ...
    // Use .slice() to copy data to a new ArrayBuffer (ffmpeg may use SharedArrayBuffer)
    const binaryData = outputData instanceof Uint8Array ? outputData.slice() : new TextEncoder().encode(outputData);
  ...
  return (
    ...
    >
      <img
        src={isRecording ? assetPaths.icons.save : assetPaths.icons.mic}
      ...
      {showVisualizer ? (
        ...
        >
          <span className="audio-recorder-status-dot" />
          Recording
        </span>
      )}
      <img
        src={isPaused ? assetPaths.icons.play : assetPaths.icons.pause}
