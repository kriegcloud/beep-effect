...
import { suggestionPlugin } from "@beep/ui/components/editor/plugins/suggestion-kit";
...
const TYPE_TEXT_MAP: Record<string, (node?: undefined | TElement) => string> = {
  [KEYS.audio]: () => "Audio",
      ...
      entries.forEach(([node]) => {
        if (TextApi.isText(node)) {
          ...
          if (lineBreakData?.id === keyId2SuggestionId(id)) {
            const textMapper = TYPE_TEXT_MAP[node.type];
            if (lineBreakData.type === "insert") {
              newText += lineBreakData.isLineBreak ? BLOCK_SUGGESTION : BLOCK_SUGGESTION + (textMapper?.(node) ?? "");
            } else if (lineBreakData.type === "remove") {
              text += lineBreakData.isLineBreak ? BLOCK_SUGGESTION : BLOCK_SUGGESTION + (textMapper?.(node) ?? "");
            }
          }
        }
      });
