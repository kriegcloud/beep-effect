...
import type { ColumnType, GeneratedColumnConfig, GeneratedIdentityConfig } from "../../column-builder";
...
import { iife } from "../../tracing-utils";
import type { Update } from "../../utils";
import type { PgIndexOpClass } from "../indexes";
import type { PgSequenceOptions } from "../sequence";
import { makePgArray, parsePgArray } from "../utils/array";
...
type ArrayDimensionStringToNumber<T extends PgArrayDimensionString> = T extends "[]"
  ? 1
  : T extends "[][]"
    ? 2
    : T extends "[][][]"
...
export interface PgColumnBuilderConfig {
  ...
  isAutoincrement?: undefined | boolean;
  hasRuntimeDefault?: undefined | boolean;
  enumValues?: undefined | string[];
  identity?: undefined | "always" | "byDefault";
  generated?: undefined | unknown;
  dimensions?: undefined | PgArrayDimension;
...
export interface PgColumnBaseConfig<out TDataType extends ColumnType.Type = ColumnType.Type> {
  name: string;
  dataType: TDataType;
  tableName: string;
  notNull: boolean;
...
type WrapArray<T, N extends number> = N extends 1
  ? T[]
  : N extends 2
    ? T[][]
    : N extends 3
...
export type ResolvePgColumnConfig<
  ...
  out TData = T["dimensions"] extends 1 | 2 | 3 | 4 | 5 ? WrapArray<GetBaseData<T>, T["dimensions"]> : GetBaseData<T>,
> = {
  name: string;
  tableName: TTableName;
  dataType: T["dataType"];
  data: TData;
  driverParam: T["dimensions"] extends 1 | 2 | 3 | 4 | 5
  ...
  insertType: T["generated"] extends true
    ? never
    : T["identity"] extends "always"
      ? never
...
export type PgBuildColumn<
  ...
  TBuiltConfig extends PgColumnBaseConfig<ColumnType.Type> = ResolvePgColumnConfig<
    TBuilder[PgColumnBuilderBrand],
    TTableName
  >,
> = PgColumn<ColumnType.Type, TBuiltConfig, {}>;

export type PgBuildColumns<out TTableName extends string, out TConfigMap extends Record<string, AnyPgColumnBuilder>> = {
  [Key in keyof TConfigMap]: PgBuildColumn<TTableName, TConfigMap[Key]>;
} & {};

export type PgBuildExtraConfigColumns<out TConfigMap extends Record<string, AnyPgColumnBuilder>> = {
...
> {
  ...
  protected constructor(name: string, dataType: ColumnType.Type, columnType: string) {
    this.config = {
      name,
      keyAsName: name === "",
      notNull: false,
  ...
  unique(name?: undefined | string, config?: undefined | { nulls: "distinct" | "not distinct" }): this {
  ...
  generatedAlwaysAsIdentity(
    sequence?: undefined | (PgSequenceOptions & { name?: undefined | string })
  ...
  /** @internal */
  buildExtraConfigColumn<TTableName extends string>(table: AnyPgTable<{ name: TTableName }>): ExtraConfigColumn {
...
export abstract class PgColumn<
  out TColumnType extends ColumnType.Type = ColumnType.Type,
...
export type IndexedExtraConfigType = {
  ...
  nulls?: undefined | "first" | "last";
  opClass?: undefined | string;
};

export class ExtraConfigColumn<
  out T extends PgColumnBaseConfig<ColumnType.Type> = PgColumnBaseConfig<ColumnType.Type>,
> extends PgColumn<ColumnType.Type, T, IndexedExtraConfigType> {
...
export type AnyPgColumn<TPartial extends Partial<PgColumnBaseConfig<ColumnType.Type>> = {}> = PgColumn<
  any,
  Required<Update<PgColumnBaseConfig<ColumnType.Type>, TPartial>>
>;
