import type { Column, Columns, GetColumnData } from "./column";
...
export function getTableUniqueName<
  ...
  TResult extends string = T extends Table
    ? T["_"]["schema"] extends undefined
      ? `public.${T["_"]["name"]}`
      : `${T["_"]["schema"]}.${T["_"]["name"]}`
...
export type InferModelFromColumns<
  ...
  TConfig extends { override?: undefined | boolean } = { dbColumnNames: false; override: false },
> = Simplify<
  TInferMode extends "insert"
    ? {
        [Key in keyof TColumns & string as RequiredKeyOnly<Key, TColumns[Key]>]: GetColumnData<TColumns[Key], "query">;
      } & {
        [Key in keyof TColumns & string as OptionalKeyOnly<Key, TColumns[Key], TConfig["override"]>]?:
          | GetColumnData<TColumns[Key], "query">
          | undefined;
      }
    : {
        [Key in keyof TColumns & string]: GetColumnData<TColumns[Key], "query">;
      }
>;

/** @deprecated Use one of the alternatives: {@link InferSelectModel} / {@link InferInsertModel}, or `table.$inferSelect` / `table.$inferInsert`
 */
export type InferModel<TTable extends Table, TInferMode extends "select" | "insert" = "select"> = InferModelFromColumns<
  TTable["_"]["columns"],
  TInferMode
>;

export type InferSelectModel<TTable extends Table> = InferModelFromColumns<TTable["_"]["columns"], "select">;

export type InferInsertModel<
  TTable extends Table,
  TOverride extends { override?: undefined | boolean } = { override: false },
