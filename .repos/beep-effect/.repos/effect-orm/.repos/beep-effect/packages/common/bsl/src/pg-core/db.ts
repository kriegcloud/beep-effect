...
import { type ColumnsSelection, type SQL, type SQLWrapper, sql } from "../sql/sql";
import { WithSubquery } from "../subquery";
import type { DrizzleTypeError, NeonAuthToken } from "../utils";
import type { PgColumn } from "./columns/index";
import { _RelationalQueryBuilder } from "./query-builders/_query";
import { PgCountBuilder } from "./query-builders/count";
...
import type { WithBuilder } from "./subquery";
...
> {
  ...
  constructor(
    ...
    if (this._.schema) {
      for (const [tableName, columns] of Struct.entries(this._.schema)) {
    ...
    for (const [tableName, relation] of Struct.entries(relations)) {
      (this.query as PgDatabase<TQueryResult, TSchema, AnyRelations, V1.TablesRelationalConfig>["query"])[tableName] =
   ...
   * ```
   */
  $with: WithBuilder = (alias: string, selection?: undefined | ColumnsSelection) => {
    const self = this;
    const as = (
      qb:
  ...
  $count(source: PgTable | PgViewBase | SQL | SQLWrapper, filters?: undefined | SQL<unknown>) {
  ...
  select<TSelection extends SelectedFields>(fields?: undefined | TSelection): PgSelectBuilder<TSelection | undefined> {
    return new PgSelectBuilder({
      fields: fields ?? undefined,
  ...
  execute<TRow extends Record<string, unknown> = Record<string, unknown>>(
    ...
    >(builtQuery, undefined, undefined, false);
    return new PgRaw(
  ...
  transaction<T>(
    ...
    config?: undefined | PgTransactionConfig
  ): Promise<T> {
