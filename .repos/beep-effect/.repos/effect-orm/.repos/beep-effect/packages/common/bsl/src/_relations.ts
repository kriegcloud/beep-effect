...
import { PrimaryKeyBuilder } from "./pg-core/primary-keys";
import {
  and,
  asc,
  between,
...
import { type Placeholder, SQL, sql } from "./sql/sql";
import { type AnyTable, getTableUniqueName, type InferModelFromColumns, Table } from "./table";
import type { Assume, ColumnsWithTable, Equal, Simplify, ValueOrArray } from "./utils";
export abstract class Relation<TTableName extends string = string> {
  ...
  readonly referencedTable: AnyTable<{ readonly name: TTableName }>;
  readonly relationName: string | undefined;

  protected constructor(
    sourceTable: Table,
    referencedTable: AnyTable<{ readonly name: TTableName }>,
...
> {
  ...
  readonly table: AnyTable<{ readonly name: TTableName }>;
  readonly config: (helpers: TableRelationsHelpers<TTableName>) => TConfig;

  constructor(
    table: AnyTable<{ readonly name: TTableName }>,
...
> extends Relation<TTableName> {
  ...
  protected declare $relationBrand: "One";
  readonly config: RelationConfig<TTableName, string, AnyColumn<{ readonly tableName: TTableName }>[]> | undefined;
  readonly isNullable: TIsNullable;
  constructor(
    sourceTable: Table,
    referencedTable: AnyTable<{ readonly name: TTableName }>,
    config: RelationConfig<TTableName, string, AnyColumn<{ readonly tableName: TTableName }>[]> | undefined,
...
export class Many<TTableName extends string> extends Relation<TTableName> {
  ...
  protected declare $relationBrand: "Many";
  readonly config: { readonly relationName: string } | undefined;

  constructor(
    sourceTable: Table,
    referencedTable: AnyTable<{ name: TTableName }>,
    config: { readonly relationName: string } | undefined
  ) {
    super(sourceTable, referencedTable, config?.relationName);
    this.config = config;
  }
...
export type ExtractTableRelationsFromSchema<
  ...
  TTableName extends string,
> = ExtractObjectValues<{
  [K in keyof TSchema as TableRelationsKeysOnly<TSchema, TTableName, K>]: TSchema[K] extends Relations<
    TTableName,
    infer TConfig
  >
    ? TConfig
...
export type ExtractRelationsFromTableExtraConfigSchema<TConfig extends unknown[]> = ExtractObjectValues<{
  [K in keyof TConfig as TConfig[K] extends Relations<any> ? K : never]: TConfig[K] extends Relations<
    infer TRelationConfig
  >
    ? TRelationConfig
    : never;
}>;
...
> = {
  columns?:
    | {
        [K in keyof TTableConfig["columns"]]?: undefined | boolean;
      }
    | undefined;
  with?:
    | {
        ...
        (TFullSelection["columns"] extends Record<string, unknown>
          ? InferModelFromColumns<{
              [K in Equal<
                ...
                false
              > extends true
                ? Exclude<keyof TTableConfig["columns"], NonUndefinedKeysOnly<TFullSelection["columns"]>>
                : {
                    [K in keyof TFullSelection["columns"]]: Equal<TFullSelection["columns"][K], true> extends true
                      ? K
                      : never;
...
export function extractTablesRelationalConfig<TTables extends TablesRelationalConfig>(
  ...
  if (Struct.keys(schema).length === 1 && "default" in schema && !is(schema["default" as const], Table)) {
    schema = schema["default" as const] as Record<string, unknown>;
  }

  // table DB name -> schema table key
  const tableNamesMap: Record<string, string> = {};
  // Table relations found before their tables - need to buffer them until we know the schema table key
  const relationsBuffer: Record<string, { relations: Record<string, Relation>; primaryKey?: undefined | AnyColumn[] }> =
    {};
  const tablesConfig: TablesRelationalConfig = {};
  for (const [key, value] of Struct.entries(schema)) {
    if (is(value, Table)) {
      ...
      for (const [relationName, relation] of Struct.entries(relations)) {
        if (tableName) {
...
export function relations<TTableName extends string, TRelations extends Record<string, Relation<any>>>(
  ...
  return new Relations<TTableName, TRelations>(
    ...
    (helpers: TableRelationsHelpers<TTableName>) =>
      Object.fromEntries(
        Struct.entries(relations(helpers)).map(([key, value]) => [key, value.withFieldName(key)])
      ) as TRelations
  );
}
...
export function createOne<TTableName extends string>(sourceTable: Table) {
  return function one<
    ...
    TColumns extends [AnyColumn<{ tableName: TTableName }>, ...AnyColumn<{ tableName: TTableName }>[]],
  >(
    table: TForeignTable,
    config?: undefined | RelationConfig<TTableName, TForeignTable["_"]["name"], TColumns>
...
export function createMany(sourceTable: Table) {
  return function many<TForeignTable extends Table>(
    ...
    config?: undefined | { relationName: string }
...
export function createTableRelationsHelpers<TTableName extends string>(sourceTable: AnyTable<{ name: TTableName }>) {
  return {
