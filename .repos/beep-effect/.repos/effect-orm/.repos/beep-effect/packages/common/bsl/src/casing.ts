...
import type { View } from "./sql/index";
import { Table } from "./table";
import type { Casing } from "./utils";
...
export class CasingCache {
  ...
  private readonly convert: (input: string) => string;

  constructor(casing?: undefined | Casing) {
  ...
  private cacheTable(table: Table | View) {
    ...
    if (!this.cachedTables[tableKey]) {
      for (const column of R.values(table[Table.Symbol.Columns])) {
