...
import type { InferModelFromColumns } from "../table";
...
import { getPgColumnBuilders, type PgColumnsBuilders } from "./columns/all";
import type {
  AnyPgColumnBuilder,
  ExtraConfigColumn,
  PgBuildColumns,
...
import type { PgPolicy } from "./policies";
...
export type AnyPgTable<TPartial extends Partial<TableConfig> = NonNullable<unknown>> = PgTable<UpdateTableConfig<TableConfig, TPartial>>;
...
export type InfeInsertColumns<TColumns extends PgColumns> = Simplify<
  {
    ...
    [Key in keyof TColumns & string as TColumns[Key]["_"]["insertType"] extends never
      ? never
      : undefined extends TColumns[Key]["_"]["insertType"]
        ? never
        : Key]: TColumns[Key]["_"]["insertType"];
  } & {
    ...
    [Key in keyof TColumns & string as TColumns[Key]["_"]["insertType"] extends never
      ...
      : undefined extends TColumns[Key]["_"]["insertType"]
        ? Key
        : never]?: undefined | TColumns[Key]["_"]["insertType"];
  }
>;
...
export interface PgTableFnInternal<TSchema extends string | undefined = undefined> {
  <TTableName extends string, TColumnsMap extends Record<string, AnyPgColumnBuilder>>(
    name: TTableName,
    columns: TColumnsMap,
    extraConfig?: undefined | ((self: PgBuildExtraConfigColumns<TColumnsMap>) => PgTableExtraConfigValue[])
  ): PgTableWithColumns<{
    name: TTableName;
    schema: TSchema;
    columns: PgBuildColumns<TTableName, TColumnsMap>;
