...
import { SelectionProxyHandler } from "../../selection-proxy";
import { type ColumnsSelection, type Query, SQL, type SQLWrapper } from "../../sql/sql";
import { Subquery } from "../../subquery";
import { getTableName, type InferInsertModel, Table } from "../../table";
import {
  type Assume,
...
import type { PgColumn } from "../columns/common";
import { extractUsedTable } from "../utils";
...
export interface PgUpdateConfig {
  ...
  set: UpdateSet;
  table: PgTable;
  from?: undefined | PgTable | Subquery | PgViewBase | SQL;
  joins: PgSelectJoinConfig[];
  returningFields?: undefined | SelectedFields;
  returning?: undefined | SelectedFieldsOrdered;
  withList?: undefined | Subquery[];
}

export type PgUpdateSetSource<TTable extends PgTable, TModel extends Record<string, any> = InferInsertModel<TTable>> = {
...
export type PgUpdateJoinFn<T extends AnyPgUpdate, TDynamic extends boolean, TJoinType extends JoinType> = <
...
> = TJoins extends [infer TJoin extends Join, ...infer TRest extends Join[]]
  ? AccumulateToResult<
      ...
      AppendToResult<
        ...
        TSelectedFields,
        TJoin["name"],
        TJoin["table"] extends Table
          ? TJoin["table"]["_"]["columns"]
    ...
    SQLWrapper
{
  ...
  private createJoin<TJoinType extends JoinType>(joinType: TJoinType): PgUpdateJoinFn<this, TDynamic, TJoinType> {
    return ((
      table: PgTable | Subquery | PgViewBase | SQL,
      ...
      if (typeof on === "function") {
        const from =
          this.config.from && !is(this.config.from, SQL) ? this.getTableLikeFields(this.config.from) : undefined;
        on = on(
          new Proxy(
            this.config.table[Table.Symbol.Columns],
            new SelectionProxyHandler({ sqlAliasedBehavior: "sql", sqlBehavior: "sql" })
          ) as any,
          from && (new Proxy(from, new SelectionProxyHandler({ sqlAliasedBehavior: "sql", sqlBehavior: "sql" })) as any)
        );
      }
        ...
        switch (joinType) {
          ...
          case "right": {
            this.joinsNotNullableMap = Object.fromEntries(
              Struct.entries(this.joinsNotNullableMap).map(([key]) => [key, false])
            );
            this.joinsNotNullableMap[tableName] = true;
            break;
          }
  ...
  returning(fields?: SelectedFields): PgUpdateWithout<AnyPgUpdate, TDynamic, "returning"> {
    if (!fields) {
  ...
  _prepare(name?: string): PgUpdatePrepare<this> {
    const query = this.session.prepareQuery<PreparedQueryConfig & { execute: TReturning[] }>(
