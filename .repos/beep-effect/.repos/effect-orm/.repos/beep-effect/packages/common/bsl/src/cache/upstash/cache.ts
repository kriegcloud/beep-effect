...
import type { MutationOption } from "../../cache/core/index";
import { Cache } from "../../cache/core/index";
import { entityKind, is } from "../../entity";
import { OriginalName, Table } from "../../table";
...
export class UpstashCache extends Cache {
  ...
  private internalConfig: { seconds: number; hexOptions?: undefined | ExpireOptions };

  constructor(
    public redis: Redis,
    config?: undefined | CacheConfig,
    protected useGlobally?: undefined | boolean
  ) {
    super();
  ...
  private toInternalConfig(config?: undefined | CacheConfig): { seconds: number; hexOptions?: undefined | ExpireOptions } {
    return config
      ? {
          seconds: config.ex!,
  ...
  override async put(key: string, response: any, tables: string[], isTag = false, config?: CacheConfig): Promise<void> {
    const isAutoInvalidate = tables.length !== 0;

    const pipeline = this.redis.pipeline();
    const ttlSeconds = config?.ex ? config.ex : this.internalConfig.seconds;
    const hexOptions = config?.hexOptions ? config.hexOptions : this.internalConfig?.hexOptions;
