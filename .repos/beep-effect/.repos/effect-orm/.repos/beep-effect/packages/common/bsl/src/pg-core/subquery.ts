import type { TypedQueryBuilder } from "../query-builders/query-builder";
import type { AddAliasToSelection } from "../query-builders/select.types";
import type { ColumnsSelection, SQL } from "../sql/sql";
import type { Subquery, WithSubquery, WithSubqueryWithoutSelection } from "../subquery";
import type { QueryBuilder } from "./query-builders/query-builder";

export type SubqueryWithSelection<TSelection extends ColumnsSelection, TAlias extends string> = Subquery<
  TAlias,
  AddAliasToSelection<TSelection, TAlias, "pg">
> &
  AddAliasToSelection<TSelection, TAlias, "pg">;

export type WithSubqueryWithSelection<TSelection extends ColumnsSelection, TAlias extends string> = WithSubquery<
  TAlias,
  AddAliasToSelection<TSelection, TAlias, "pg">
> &
  AddAliasToSelection<TSelection, TAlias, "pg">;

export interface WithBuilder {
  <TAlias extends string>(
    alias: TAlias
  ): {
    as: {
      <TSelection extends ColumnsSelection>(
        qb: TypedQueryBuilder<TSelection> | ((qb: QueryBuilder) => TypedQueryBuilder<TSelection>)
      ): WithSubqueryWithSelection<TSelection, TAlias>;
      (
        qb: TypedQueryBuilder<undefined> | ((qb: QueryBuilder) => TypedQueryBuilder<undefined>)
      ): WithSubqueryWithoutSelection<TAlias>;
    };
  };
  <TAlias extends string, TSelection extends ColumnsSelection>(
    alias: TAlias,
    selection: TSelection
  ): {
    as: (qb: SQL | ((qb: QueryBuilder) => SQL)) => WithSubqueryWithSelection<TSelection, TAlias>;
  };
}
