...
export class Compression extends Effect.Service<Compression>()("app/Compression", {
  effect: Effect.gen(function* () {
    ...
    const decompress = (buffer: Uint8Array) =>
      Effect.tryPromise({
        try: async () => {
          const blob = new Blob([buffer as Uint8Array<ArrayBuffer>], { type: "application/gzip" });
...
const decodeWorkspace = flow(Schema.decode(Schema.parseJson(Workspace)), Effect.orDie);
const encodeWorkspace = flow(Schema.encode(Schema.parseJson(Workspace)), Effect.orDie);

export class WorkspaceCompression extends Effect.Service<WorkspaceCompression>()("app/Compression/Workspace", {
