...
export function DialogClose({ children, className, ...props }: React.ComponentProps<typeof DialogPrimitive.Close>) {
  return (
      ...
      <DialogOverlay className={cn("md:py-10")}>
        <Component
          className={cn(
            ...
            fixed ? "gap-0 p-0 sm:max-h-[min(640px,80vh)]" : "gap-4 overflow-y-auto p-6",
            className
          )}
          onOpenAutoFocus={(e) => {
            if (disableAutoFocus) {
...
export function DialogModalTitle({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) {
  return (
    <DialogPrimitive.Title className={cn("text-lg font-semibold md:text-xl md:tracking-tight", className)} {...props} />
  );
}

export function DialogDescription({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Description>) {
  return <DialogPrimitive.Description className={cn("text-sm text-subtle-foreground", className)} {...props} />;
}

export const { DialogProvider, useDialogSet, useDialogStore, useDialogValue } = createAtomStore(
  {
...
export function DialogTrigger(props: React.ComponentProps<typeof DialogModalTrigger>) {
  const variant = useDialogValue("variant");
  const ResponsiveTrigger = variant === "drawer" ? DrawerTrigger : DialogModalTrigger;
...
export function DialogTitle(props: React.ComponentProps<typeof DialogModalTitle>) {
  const variant = useDialogValue("variant");

  return variant === "drawer" ? <DrawerTitle {...props} /> : <DialogModalTitle {...props} />;
}
...
export function DrawerTitle({ className, ...props }: React.ComponentProps<typeof DrawerPrimitive.Title>) {
  return (
    <DrawerPrimitive.Title className={cn("text-lg font-semibold md:text-xl md:tracking-tight", className)} {...props} />
  );
}

export function DrawerDescription({ className, ...props }: React.ComponentProps<typeof DrawerPrimitive.Description>) {
  return <DrawerPrimitive.Description className={cn("text-sm text-subtle-foreground", className)} {...props} />;
}
