import { createNoopenHeader, createXDownloadOptionsHeaderValue } from "@beep/build-utils/secure-headers/no-open";
...
describe("createNoopenHeader", () => {
  let headerValueCreatorMock: ReturnType<typeof mock<typeof createXDownloadOptionsHeaderValue>>;
  beforeEach(() => {
  ...
  it("should return Some with X-Download-Options header when value is provided", async () => {
    ...
    const result = await runEffect(createNoopenHeader(undefined, headerValueCreatorMock));
...
describe("createXDownloadOptionsHeaderValue", () => {
  describe("when giving undefined", () => {
    it('should return "noopen"', async () => {
      expect(await runEffect(createXDownloadOptionsHeaderValue())).toBe("noopen");
      expect(await runEffect(createXDownloadOptionsHeaderValue(undefined))).toBe("noopen");
    });
  });

  describe("when giving false", () => {
    it("should return undefined", async () => {
      expect(await runEffect(createXDownloadOptionsHeaderValue(false))).toBeUndefined();
    });
  });

  describe('when giving "noopen"', () => {
    it('should return "noopen"', async () => {
      expect(await runEffect(createXDownloadOptionsHeaderValue("noopen"))).toBe("noopen");
    });
  });

  describe("when giving invalid value", () => {
    it("should raise error", () => {
      expect(Exit.isFailure(runEffectExit(createXDownloadOptionsHeaderValue(true as never)))).toBe(true);
      expect(Exit.isFailure(runEffectExit(createXDownloadOptionsHeaderValue("foo" as never)))).toBe(true);
      expect(Exit.isFailure(runEffectExit(createXDownloadOptionsHeaderValue([] as never)))).toBe(true);
    });
  });
});
