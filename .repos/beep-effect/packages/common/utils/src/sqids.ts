...
interface SqidsOptions {
  ...
  readonly blocklist?: HashSet.HashSet<string> | Iterable<string> | undefined;
}
...
const codePointAtUnsafe = (s: string, index: number): number => s.codePointAt(index)!;

const sliceString = (s: string, start: number, end?: number): string => s.slice(start, end);

const hasMultibyteChars = (s: string): boolean => new Blob([s]).size !== s.length;

const hasUniqueChars = (s: string): boolean =>
  pipe(toChars(s), HashSet.fromIterable, HashSet.size, (size) => size === s.length);

const rotateAlphabet = (alphabet: string, offset: number): string =>
  sliceString(alphabet, offset) + sliceString(alphabet, 0, offset);

const reverseString = (s: string): string => pipe(toChars(s), A.reverse, fromChars);

const shuffle = (alphabet: string): string => {
  const chars = [...toChars(alphabet)];
  const len = chars.length;

  for (let i = 0, j = len - 1; j > 0; i++, j--) {
    const r = (i * j + codePointAtUnsafe(chars[i]!, 0) + codePointAtUnsafe(chars[j]!, 0)) % len;
    const temp = chars[i]!;
    chars[i] = chars[r]!;
    chars[r] = temp;
  }
...
const toId = (num: number, alphabet: string): string => {
  const chars = toChars(alphabet);
  const len = chars.length;

  const buildId = (result: number, acc: ReadonlyArray<string>): ReadonlyArray<string> => {
    const newAcc = pipe(
...
const toNumber = (id: string, alphabet: string): number => {
  ...
  return pipe(
    toChars(id),
    A.reduce(0, (acc, char) => acc * len + chars.indexOf(char))
  );
};

const filterBlocklist = (
  blocklist: HashSet.HashSet<string>,
  alphabetChars: HashSet.HashSet<string>
): HashSet.HashSet<string> =>
  pipe(
    blocklist,
    HashSet.filter((word) => word.length >= 3),
    HashSet.map((word) => word.toLowerCase()),
    HashSet.filter((word) => {
      const wordChars = HashSet.fromIterable(toChars(word));
      return pipe(
        wordChars,
        HashSet.every((c) => HashSet.has(alphabetChars, c))
      );
    })
  );
...
interface SqidsState {
  readonly alphabet: string;
  readonly minLength: number;
  readonly blocklist: HashSet.HashSet<string>;
}

const isHashSet = <A>(value: HashSet.HashSet<A> | Iterable<A>): value is HashSet.HashSet<A> => HashSet.isHashSet(value);

const toHashSet = <A>(value: HashSet.HashSet<A> | Iterable<A>): HashSet.HashSet<A> =>
  isHashSet(value) ? value : HashSet.fromIterable(value);

const createState = (options?: SqidsOptions): SqidsState => {
  const alphabet = options?.alphabet ?? defaultOptions.alphabet;
  const minLength = options?.minLength ?? defaultOptions.minLength;
  const blocklistInput = options?.blocklist ?? defaultOptions.blocklist;
  const blocklist = toHashSet(blocklistInput);

  if (hasMultibyteChars(alphabet)) {
    throw new Error("Alphabet cannot contain multibyte characters");
  }

  if (alphabet.length < MIN_ALPHABET_LENGTH) {
    throw new Error(`Alphabet length must be at least ${MIN_ALPHABET_LENGTH}`);
  }
  ...
  const alphabetChars = HashSet.fromIterable(toChars(alphabet.toLowerCase()));
  const filteredBlocklist = filterBlocklist(blocklist, alphabetChars);
...
const encodeNumbersWithState = (state: SqidsState, numbers: ReadonlyArray<number>, increment: number): string => {
  if (increment > state.alphabet.length) {
    throw new Error("Reached max attempts to re-generate the ID");
  }

  const alphabetLen = state.alphabet.length;

  const offset = pipe(
    numbers,
    A.reduce(numbers.length, (acc, num, idx) => acc + codePointAtUnsafe(state.alphabet, num % alphabetLen) + idx),
    (sum) => (sum + increment) % alphabetLen
  );

  const rotated = rotateAlphabet(state.alphabet, offset);
  ...
  for (let i = 0; i < numbers.length; i++) {
    const num = numbers[i]!;
    parts.push(toId(num, sliceString(alphabet, 1)));

    if (i < numbers.length - 1) {
      parts.push(charAtUnsafe(alphabet, 0));
      alphabet = shuffle(alphabet);
    }
  }
  ...
  if (state.minLength > id.length) {
    ...
    while (state.minLength - id.length > 0) {
      alphabet = shuffle(alphabet);
      id += sliceString(alphabet, 0, Math.min(state.minLength - id.length, alphabet.length));
    }
  }

  if (isBlockedId(id, state.blocklist)) {
    return encodeNumbersWithState(state, numbers, increment + 1);
  }
...
const decodeWithState = (state: SqidsState, id: string): ReadonlyArray<number> => {
  ...
  const idChars = toChars(id);

  const allCharsValid = pipe(
    idChars,
    A.every((c) => HashSet.has(alphabetChars, c))
  );
  ...
  while (slicedId.length > 0) {
    ...
    if (chunks.length > 0) {
      ...
      result.push(toNumber(firstChunk, sliceString(alphabet, 1)));
...
export default class Sqids {
  ...
  encode(numbers: ReadonlyArray<number>): string {
    ...
    const allInRange = pipe(
      numbers,
      A.every((n) => n >= 0 && n <= MAX_VALUE)
    );

    if (!allInRange) {
      throw new Error(`Encoding supports numbers between 0 and ${MAX_VALUE}`);
    }
