...
import dedent from "@beep/utils/dedent/dedent";
...
describe("dedent", () => {
  ...
  describe("function character escapes", () => {
    ...
    describe.each([undefined, false, true])("with escapeSpecialCharacters %s", (escapeSpecialCharacters) => {
      test("backticks", () => {
        expect(dedent.withOptions({ escapeSpecialCharacters })(`\``)).toMatchSnapshot();
      });
  ...
  describe.each([undefined, false, true])("with trimWhitespace %s", (trimWhitespace) => {
  ...
  describe.each([undefined, false, true])("with alignValues %s", (alignValues) => {
