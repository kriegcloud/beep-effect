...
export const DefaultGraphEncoded = S.TemplateLiteral("<", S.String, ">").annotations(
  $I.annotations("DefaultGraphEncoded", {
...
export class DefaultGraph extends S.Class<DefaultGraph>($I`DefaultGraph`)(
  ModelType.makeKind.DefaultGraph({
    value: S.optionalWith(S.String, {
      default: thunkEmptyStr,
    }),
  }),
  $I.annotations("DefaultGraph", {
    description: "A named node in the RDF graph",
  })
) {
  static readonly Equivalence = S.equivalence(DefaultGraph);
  static readonly new = (value?: undefined | string) => new DefaultGraph(value ? { value } : {});
...
export class DefaultGraphFromString extends S.transform(DefaultGraphEncoded, DefaultGraph, {
  strict: true,
  decode: (string) => pipe(string, Str.replace(">", ""), Str.replace("<", ""), DefaultGraph.new),
