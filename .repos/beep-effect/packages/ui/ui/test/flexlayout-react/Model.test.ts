  ...
  describe("Actions", () => {
    describe("Add", () => {
      it("empty tabset", () => {
        ...
        doAction(
          Actions.addNode(
            JsonTabNode.make({ id: "2", name: "newtab1", component: "grid" }),
            "1",
            DockLocation.CENTER,
            -1
          )
        ...
        expect(tabs).toEqual("/ts0/t0[newtab1]*");
        expect(tab("/ts0/t0").getId()).toEqual("2");
        expect(tab("/ts0/t0").getComponent()).toEqual("grid");
      });
      ...
      describe("tabsets", () => {
        beforeEach(() => {
          ...
          expect(tabs).toEqual("/ts0/t0[One]*,/ts1/t0[Two]*");
        });

        it("add to tabset center", () => {
          const id0 = tabset("/ts0").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab1", component: "grid" }), id0, DockLocation.CENTER, -1)
          );

          expect(tabs).toEqual("/ts0/t0[One],/ts0/t1[newtab1]*,/ts1/t0[Two]*");

          const id1 = tabset("/ts1").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab2", component: "grid" }), id1, DockLocation.CENTER, -1)
          );

          expect(tabs).toEqual("/ts0/t0[One],/ts0/t1[newtab1]*,/ts1/t0[Two],/ts1/t1[newtab2]*");
        });

        it("add to tabset at position", () => {
          const id0 = tabset("/ts0").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab1", component: "grid" }), id0, DockLocation.CENTER, 0)
          );

          expect(tabs).toEqual("/ts0/t0[newtab1]*,/ts0/t1[One],/ts1/t0[Two]*");

          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab2", component: "grid" }), id0, DockLocation.CENTER, 1)
          );

          expect(tabs).toEqual("/ts0/t0[newtab1],/ts0/t1[newtab2]*,/ts0/t2[One],/ts1/t0[Two]*");

          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab3", component: "grid" }), id0, DockLocation.CENTER, 3)
          );

          expect(tabs).toEqual("/ts0/t0[newtab1],/ts0/t1[newtab2],/ts0/t2[One],/ts0/t3[newtab3]*,/ts1/t0[Two]*");
        });

        it("add to tabset top", () => {
          const id0 = tabset("/ts0").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab1", component: "grid" }), id0, DockLocation.TOP, -1)
          );

          expect(tabs).toEqual("/r0/ts0/t0[newtab1]*,/r0/ts1/t0[One]*,/ts1/t0[Two]*");

          const id1 = tabset("/ts1").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab2", component: "grid" }), id1, DockLocation.TOP, -1)
          );

          expect(tabs).toEqual("/r0/ts0/t0[newtab1]*,/r0/ts1/t0[One]*,/r1/ts0/t0[newtab2]*,/r1/ts1/t0[Two]*");
        });

        it("add to tabset bottom", () => {
          const id0 = tabset("/ts0").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab1", component: "grid" }), id0, DockLocation.BOTTOM, -1)
          );

          expect(tabs).toEqual("/r0/ts0/t0[One]*,/r0/ts1/t0[newtab1]*,/ts1/t0[Two]*");

          const id1 = tabset("/ts1").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab2", component: "grid" }), id1, DockLocation.BOTTOM, -1)
          );

          expect(tabs).toEqual("/r0/ts0/t0[One]*,/r0/ts1/t0[newtab1]*,/r1/ts0/t0[Two]*,/r1/ts1/t0[newtab2]*");
        });

        it("add to tabset left", () => {
          const id0 = tabset("/ts0").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab1", component: "grid" }), id0, DockLocation.LEFT, -1)
          );

          expect(tabs).toEqual("/ts0/t0[newtab1]*,/ts1/t0[One]*,/ts2/t0[Two]*");

          const id1 = tabset("/ts2").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab2", component: "grid" }), id1, DockLocation.LEFT, -1)
          );

          expect(tabs).toEqual("/ts0/t0[newtab1]*,/ts1/t0[One]*,/ts2/t0[newtab2]*,/ts3/t0[Two]*");
        });

        it("add to tabset right", () => {
          const id0 = tabset("/ts0").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab1", component: "grid" }), id0, DockLocation.RIGHT, -1)
          );

          expect(tabs).toEqual("/ts0/t0[One]*,/ts1/t0[newtab1]*,/ts2/t0[Two]*");

          const id1 = tabset("/ts2").getId();
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab2", component: "grid" }), id1, DockLocation.RIGHT, -1)
          );

          expect(tabs).toEqual("/ts0/t0[One]*,/ts1/t0[newtab1]*,/ts2/t0[Two]*,/ts3/t0[newtab2]*");
        });
      });
      ...
      describe("borders", () => {
        ...
        it("add to top border", () => {
          ...
          expect(tabsMatch(path)).toEqual("/b/top/t0[top1],/b/top/t1[newtab1]");
          expect(tabsDontMatch(path)).toEqual(others);

          // add tab at position 0
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab2", component: "grid" }), id0, DockLocation.CENTER, 0)
          );

          expect(tabsMatch(path)).toEqual("/b/top/t0[newtab2],/b/top/t1[top1],/b/top/t2[newtab1]");
          expect(tabsDontMatch(path)).toEqual(others);

          // add tab at position 1
          doAction(
            Actions.addNode(JsonTabNode.make({ name: "newtab3", component: "grid" }), id0, DockLocation.CENTER, 1)
          );

          expect(tabsMatch(path)).toEqual("/b/top/t0[newtab2],/b/top/t1[newtab3],/b/top/t2[top1],/b/top/t3[newtab1]");
          expect(tabsDontMatch(path)).toEqual(others);
        });
        ...
        it("add to bottom border", () => {
          ...
          expect(tabsMatch(path)).toEqual("/b/bottom/t0[bottom1],/b/bottom/t1[bottom2],/b/bottom/t2[newtab1]");
          expect(tabsDontMatch(path)).toEqual(others);
        ...
        it("add to left border", () => {
          ...
          expect(tabsMatch(path)).toEqual("/b/left/t0[left1],/b/left/t1[newtab1]");
          expect(tabsDontMatch(path)).toEqual(others);
          ...
          expect(tabsMatch(path)).toEqual("/b/left/t0[newtab2],/b/left/t1[left1],/b/left/t2[newtab1]");
          expect(tabsDontMatch(path)).toEqual(others);
        ...
        it("add to right border", () => {
          ...
          expect(tabsMatch(path)).toEqual("/b/right/t0[right1],/b/right/t1[newtab1]");
          expect(tabsDontMatch(path)).toEqual(others);
          ...
          expect(tabsMatch(path)).toEqual("/b/right/t0[newtab2],/b/right/t1[right1],/b/right/t2[newtab1]");
          expect(tabsDontMatch(path)).toEqual(others);
    ...
    describe("Move", () => {
      beforeEach(() => {
        ...
        textRender(model);
        expect(tabs).toEqual("/ts0/t0[One]*,/ts1/t0[Two]*,/ts2/t0[Three]*");
      });
      ...
      it("move to center", () => {
        ...
        expect(tabs).toEqual("/ts0/t0[Two],/ts0/t1[One]*,/ts1/t0[Three]*");
      });
      ...
      it("move to center position", () => {
        ...
        expect(tabs).toEqual("/ts0/t0[One]*,/ts0/t1[Two],/ts1/t0[Three]*");

        fromId = tab("/ts1/t0").getId();
        doAction(Actions.moveNode(fromId, toId, DockLocation.CENTER, 1));
        expect(tabs).toEqual("/ts0/t0[One],/ts0/t1[Three]*,/ts0/t2[Two]");
      });
      ...
      it("move to top", () => {
        ...
        expect(tabs).toEqual("/r0/ts0/t0[One]*,/r0/ts1/t0[Two]*,/ts1/t0[Three]*");
      });
      ...
      it("move to bottom", () => {
        ...
        expect(tabs).toEqual("/r0/ts0/t0[Two]*,/r0/ts1/t0[One]*,/ts1/t0[Three]*");
      });
      ...
      it("move to right", () => {
        ...
        expect(tabs).toEqual("/ts0/t0[Two]*,/ts1/t0[One]*,/ts2/t0[Three]*");
      });
    });
    ...
    describe("Delete", () => {
      ...
      it("delete from tabset with 1 tab", () => {
        ...
        expect(tabs).toEqual("/ts0/t0[Two]*,/ts1/t0[Three]*");
      });
      ...
      it("delete tab from tabset with 3 tabs", () => {
        ...
        toId = tab("/ts0").getId();
        doAction(Actions.moveNode(fromId, toId, DockLocation.CENTER, -1));
        expect(tabs).toEqual("/ts0/t0[Two],/ts0/t1[One],/ts0/t2[Three]*");
        // now had three tabs in /ts0

        doAction(Actions.deleteTab(tab("/ts0/t1").getId()));
        expect(tabs).toEqual("/ts0/t0[Two],/ts0/t1[Three]*");

        doAction(Actions.deleteTab(tab("/ts0/t1").getId()));
        expect(tabs).toEqual("/ts0/t0[Two]*");
      });
      ...
      it("delete tabset", () => {
        ...
        expect(tabs).toEqual("/ts0/t0[Three]*");
      });
      ...
      it("delete tab from borders", () => {
        ...
        expect(tabs).toEqual("/b/bottom/t0[bottom2],/b/left/t0[left1],/b/right/t0[right1],/ts0/t0[One]*");

        doAction(Actions.deleteTab(tab("/b/bottom/t0").getId()));
        expect(tabs).toEqual("/b/left/t0[left1],/b/right/t0[right1],/ts0/t0[One]*");

        doAction(Actions.deleteTab(tab("/b/left/t0").getId()));
        expect(tabs).toEqual("/b/right/t0[right1],/ts0/t0[One]*");

        doAction(Actions.deleteTab(tab("/b/right/t0").getId()));
        expect(tabs).toEqual("/ts0/t0[One]*");
      });
    });
    ...
    describe("Other Actions", () => {
      ...
      it("rename tab", () => {
        ...
        expect(tabs).toEqual("/ts0/t0[renamed]*,/ts1/t0[Two]*");
      });

      it("select tab", () => {
        doAction(
          Actions.addNode(
            JsonTabNode.make({ name: "newtab1", component: "grid" }),
            tabset("/ts0").getId(),
            DockLocation.CENTER,
            -1
          )
      ...
      it("set active tabset", () => {
        ...
        expect(ts0.isActive()).toEqual(false);
        expect(ts1.isActive()).toEqual(false);
        doAction(Actions.selectTab(tab("/ts0/t0").getId()));
        expect(ts0.isActive()).toEqual(true);
        expect(ts1.isActive()).toEqual(false);
        doAction(Actions.selectTab(tab("/ts1/t0").getId()));
        expect(ts0.isActive()).toEqual(false);
        expect(ts1.isActive()).toEqual(true);
      ...
      //     expect(tab("/ts1/t0").isFloating()).toBe(false);
      //     doAction(Actions.floatTab(tab("/ts1/t0").getId()));
      //     expect(tab("/ts1/t0").isFloating()).toBe(true);
