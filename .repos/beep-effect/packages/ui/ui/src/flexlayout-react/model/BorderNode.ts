...
export class BorderNode extends Node implements IDropTarget {
  ...
  /** @internal */
  isAutoSelectTab(whenOpen?: undefined | boolean) {
    if (whenOpen == null) {
  ...
  /** @internal */
  getSplitterBounds(_index: number, useMinSize = false) {
    const pBounds = [0, 0];
  ...
  calculateSplit(_splitter: BorderNode, splitterPos: number) {
    ...
    if (this.location === DockLocation.BOTTOM || this.location === DockLocation.RIGHT) {
      return Math.max(0, O.getOrElse(O.fromNullable(pBounds[1]), () => 0) - splitterPos);
    }
    return O.match(O.fromNullable(pBounds[0]), {
      onNone: () => 0,
      onSome: (bound) => Math.max(0, splitterPos - bound),
    });
  }
