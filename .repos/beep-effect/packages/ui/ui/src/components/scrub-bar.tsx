...
import { type ComponentProps, createContext, type HTMLAttributes, useCallback, useContext, useRef } from "react";

export class ScrubBarHookOutSideContextError extends S.TaggedError<ScrubBarHookOutSideContextError>()(
  "ScrubBarHookOutSideContextError",
  {}
) {
  override get message() {
    return "useScrubBarContext must be used within a ScrubBar.Root";
  }
}
...
function formatTimestamp(value: number) {
  ...
  return `${minutes}:${F.pipe(String(seconds), Str.padStart(2, "0"))}`;
}
...
interface ScrubBarContextValue {
  ...
  readonly onScrub?: undefined | ((time: number) => void);
  readonly onScrubStart?: undefined | (() => void);
  readonly onScrubEnd?: undefined | (() => void);
}
...
function useScrubBarContext() {
  ...
  if (!context) {
    throw new ScrubBarHookOutSideContextError();
  }
  return context;
}
...
function ScrubBarContainer({
  ...
  return (
    <ScrubBarContext.Provider value={contextValue}>
      <div data-slot="scrub-bar-root" className={cn("flex w-full items-center", className)} {...props}>
        {children}
      </div>
...
interface ScrubBarTimeLabelProps extends HTMLAttributes<HTMLSpanElement> {
  ...
  readonly format?: undefined | ((time: number) => string);
}

function ScrubBarTimeLabel({ className, time, format = formatTimestamp, ...props }: ScrubBarTimeLabelProps) {
  return (
