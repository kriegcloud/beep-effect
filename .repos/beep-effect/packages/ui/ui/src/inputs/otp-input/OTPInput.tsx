...
import { KEYBOARD_KEY } from "./event";
import { getFilledArray, joinArrayStrings, mergeArrayStringFromIndex, mergeRefs, split, updateIndex } from "./helpers";
import TextFieldBox from "./TextFieldBox";
import type { MuiOtpInputProps } from "./types";
...
const MuiOtpInput = React.forwardRef((props: MuiOtpInputProps, propRef: MuiOtpInputProps["ref"]) => {
  ...
  const matchIsCompletedEvent = useEvent((filledStrings: string) => {
    const finalValue = Str.slice(0, length)(filledStrings);
  ...
  const getCharactersSplitted = () => {
    return A.map(valueSplitted, ({ character }) => {
      return character;
    });
  };
  ...
  const handleOneInputPaste = (event: React.ClipboardEvent<HTMLDivElement>) => {
    ...
    const characters = A.map(
      mergeArrayStringFromIndex(currentCharacter, split(content), currentInputIndex),
      (character, index) => {
  ...
  const handleBlur = (event: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement, Element>) => {
    const anInputIsFocused = A.some(valueSplitted, ({ inputRef }) => {
  ...
  return (
    <Box
      ...
      {...restBoxProps}
    >
      {A.map(valueSplitted, ({ character, inputRef }, index) => {
        const {
          onPaste,
          onFocus,
          onKeyDown,
