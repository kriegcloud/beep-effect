...
import type { SharedEntityIds } from "../../entity-ids";
 ...
 * ```
 */
export const matchesRoles = (roles: readonly BaseRole.Type[], userRole: BaseRole.Type): boolean => {
 ...
 * @example
 * ```ts
 * matchesUserIds(["user-1", "user-2"], "user-1" as SharedEntityIds.UserId.Type) // true
 * matchesUserIds(["user-1", "user-2"], "user-3" as SharedEntityIds.UserId.Type) // false
 * ```
 */
export const matchesUserIds = (
  userIds: readonly SharedEntityIds.UserId.Type[],
 ...
 * ```
 */
export const matchesPlatformAdmin = (required: boolean, actual: boolean): boolean => {
 ...
 *   userId: "user-1" as SharedEntityIds.UserId.Type,
 *   role: "admin",
 *   functionalRoles: [],
 ...
 *   userId: "user-2" as SharedEntityIds.UserId.Type,
 *   role: "member",
 ...
 * ```
 */
export const matchesSubjectCondition = (condition: SubjectCondition, subject: SubjectContext): boolean => {
 ...
 */
export const getSubjectMismatchReason = (condition: SubjectCondition, subject: SubjectContext): string | null => {
  ...
  if (condition.functionalRoles !== undefined && condition.functionalRoles.length > 0) {
    if (!matchesFunctionalRoles(condition.functionalRoles, subject.functionalRoles)) {
      const userRoles = subject.functionalRoles.length > 0 ? subject.functionalRoles.join(", ") : "none";
