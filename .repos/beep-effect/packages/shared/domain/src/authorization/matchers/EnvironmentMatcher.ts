 ...
 * ```
 */
export const createEnvironmentContext = (date: Date = new Date(), ipAddress?: string): EnvironmentContext => {
 ...
 * ```
 */
export const matchesTimeOfDay = (timeRange: typeof TimeRange.Type, currentTime: string): boolean => {
 ...
 * ```
 */
export const matchesDayOfWeek = (allowedDays: readonly number[], currentDay: number): boolean => {
...
const parseCIDR = (cidr: string): { ip: string; prefixLength: number } => {
  ...
  const ip = parts[0] ?? "";
  const prefixLength = parts[1] !== undefined ? Number.parseInt(parts[1], 10) : 32;
 ...
 * ```
 */
export const matchesIPAllowList = (allowList: readonly string[], ipAddress: string): boolean => {
 ...
 * ```
 */
export const matchesIPDenyList = (denyList: readonly string[], ipAddress: string): boolean => {
 ...
 * ```
 */
export const matchesEnvironmentCondition = (condition: EnvironmentCondition, context: EnvironmentContext): boolean => {
