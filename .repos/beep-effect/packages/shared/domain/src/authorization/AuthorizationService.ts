...
import type { Action } from "../entity-ids";
import type { CurrentOrganizationMembership } from "../membership/CurrentOrganizationMembership.ts";
import type { AuthorizationAuditError, PermissionDeniedError, PolicyLoadError } from "./AuthorizationErrors.ts";
...
export interface AuthorizationServiceShape {
  ...
  readonly checkPermission: (
    ...
    void,
    PermissionDeniedError | PolicyLoadError | AuthorizationAuditError,
  ...
  readonly checkPermissions: (
    ...
    Partial<Record<Action.Type, boolean>>,
    PolicyLoadError,
    CurrentOrganizationMembership | CurrentEnvironmentContext
  >;
   ...
   */
  readonly hasRole: (role: BaseRole.Type) => Effect.Effect<boolean, never, CurrentOrganizationMembership>;
   ...
   * @returns Effect containing boolean indicating if user has the role
   */
  readonly hasFunctionalRole: (role: FunctionalRole) => Effect.Effect<boolean, never, CurrentOrganizationMembership>;
