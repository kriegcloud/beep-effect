 ...
 */
export const hasPermission = (permissions: ReadonlySet<Action.Type>, action: Action.Type): boolean => {
  // Direct match
 ...
 */
export const getResourceType = (action: Action.Type): string => {
  if (action === "*") return "*";
 ...
 */
export const permissionSetToArray = (permissions: ReadonlySet<Action>): ReadonlyArray<Action> => {
  const result: Action[] = [];
