...
const makeMentionRecordExtensions = Effect.gen(function* () {
  ...
  const hardDelete = (mentionRecordId: KnowledgeEntityIds.MentionRecordId.Type): Effect.Effect<void, DatabaseError> =>
    sql`DELETE FROM ${sql(tableName)} WHERE id = ${mentionRecordId}`.pipe(
      Effect.asVoid,
      Effect.mapError(DatabaseError.$match),
      Effect.withSpan("MentionRecordRepo.hardDelete", {
        captureStackTrace: false,
        attributes: { mentionRecordId },
      })
    );
...
export const RepoLive: Layer.Layer<Entities.MentionRecord.Repo, never, DbClient.SliceDbRequirements> = Layer.effect(
