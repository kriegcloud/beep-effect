...
const buildResolvedGraph = (
  ...
  canonicalEntities: readonly AssembledEntityModel[],
  ...
  const canonicalById = MutableHashMap.empty<string, AssembledEntityModel>();
  ...
  const resolvedRelations = A.empty<AssembledRelationModel>();
...
> = Effect.gen(function* () {
  ...
  const resolve = Effect.fn(
    (graphs: readonly KnowledgeGraphModel[], ontologyId: string, config: ResolutionConfig = {}) =>
      Effect.gen(function* () {
        ...
        if (originalCount === 0) {
          ...
          return {
            graph: {
              entities: A.empty<AssembledEntityModel>(),
              relations: A.empty<AssembledRelationModel>(),
              entityIndex: {},
              stats: {
                entityCount: 0,
                relationCount: 0,
        ...
        const canonicalEntities = A.empty<AssembledEntityModel>();
