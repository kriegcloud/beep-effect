import { EventBus, EventBusDurable, EventBusLive } from "@beep/knowledge-server/Service/EventBus";
...
const EventBusDurableTestLive = EventBusDurable.pipe(
  Layer.provide(
    Layer.mergeAll(EventJournal.layerMemory, PersistedQueue.layer.pipe(Layer.provide(PersistedQueue.layerStoreMemory)))
  )
);
...
describe("Service/EventBus", () => {
  ...
  scoped(
    "durable impl publishes events to topic subscribers",
    Effect.fn(function* () {
      const bus = yield* EventBus;
    ...
    }, Effect.provide(EventBusDurableTestLive))
  );

  effect(
    "durable impl supports minimal FIFO job queue operations",
    Effect.fn(function* () {
      const bus = yield* EventBus;
