# AGENTS.md — `@beep/workspaces-client`

## Purpose & Fit
- Host Effect-first client helpers for the documents slice so apps and CLIs do not talk to server endpoints directly.
- Provide a thin layer over `@beep/workspaces-domain` entities and HTTP contracts exported by server runtimes.
...
- Once HTTP API implementation is complete in `@beep/workspaces-server`, this package will expose client-side wrappers.

## Planned Architecture
When the CLIENT is implemented, it should provide:
- Client wrappers for domain entities and RPC contracts from `@beep/workspaces-domain`.
- Type-safe HTTP clients for document CRUD, knowledge page operations, discussions, and comments.
- Effect-based error handling aligned with domain error types from `@beep/workspaces-domain/errors`.
...
- ALWAYS validate all external data with schemas from `@beep/schema` or `@beep/workspaces-domain` before exposing to callers.
...
import { Document, Comment, Discussion } from "@beep/workspaces-domain/entities";

export class DocumentsClient extends Effect.Service<DocumentsClient>()("@beep/workspaces-client/DocumentsClient", {
...
## Verifications
- `bun run check --filter=@beep/workspaces-client`
- `bun run lint --filter=@beep/workspaces-client`
- `bun run test --filter=@beep/workspaces-client` (add focused tests as soon as real code lands).
...
- **Root Cause**: Client contract names diverge from server-side handler names in `@beep/workspaces-server`.
...
- **Root Cause**: `@beep/workspaces-domain` schema was updated but client contracts still reference old schema versions.
- **Solution**: Client contracts MUST import schemas from `@beep/workspaces-domain` directly rather than duplicating definitions. When domain schemas change, client contracts automatically align.
...
- **Solution**: Use `Effect.mapError` or `Effect.catchTag` to map server error responses to `@beep/workspaces-domain/errors` types. Keep error mapping in sync with server error taxonomy and use tagged error unions for precise error handling.
...
- [ ] Added or updated tests under `packages/workspaces/client/test/`.
