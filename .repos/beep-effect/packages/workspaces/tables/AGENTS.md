# AGENTS.md — `@beep/workspaces-tables`

## Purpose & Fit
- Define Drizzle tables and relations for the documents slice, exposing a schema namespace (`DocumentsDbSchema`) consumed by `@beep/workspaces-server` and shared runtimes.
- Bridge `@beep/workspaces-domain` entities to PostgreSQL storage while honoring shared multi-tenant defaults from `@beep/shared-tables`.
...
## Usage Patterns
- Tables are consumed by `@beep/workspaces-server` repos via the `DocumentsDb` service.
...
## Verifications
- `bun run check --filter=@beep/workspaces-tables`
- `bun run lint --filter=@beep/workspaces-tables`
- `bun run test --filter=@beep/workspaces-tables` (add coverage as schemas grow).
