# AGENTS.md — `@beep/workspaces-server`

## Purpose & Fit
- Provides the infrastructure layer for the documents slice, wiring domain models from `@beep/workspaces-domain` to database services via repositories.
...
## Surface Map
- **`DocumentsDb.Db`** — Context tag to access the scoped database client with documents tables schema (`packages/workspaces/server/src/db/Db/Db.ts`).
- **`DocumentsDb.Db.layer`** — Layer providing the scoped database connection built on `DbClient.make` with documents tables schema.
- **`DocumentsRepos.layer`** — Aggregated layer exporting all document repositories (`packages/workspaces/server/src/db/repositories.ts`).
...
- **`SignedUrlService`** — Effect service for S3-based file storage operations (`packages/workspaces/server/src/SignedUrlService.ts`).
- **`ExifToolService`** — Effect service for extracting and processing EXIF metadata from image files, available via `@beep/workspaces-server/files` export (`packages/workspaces/server/src/files/ExifToolService.ts`).
- **`PdfMetadataService`** — Effect service for extracting metadata from PDF files, available via `@beep/workspaces-server/files` export (`packages/workspaces/server/src/files/PdfMetadataService.ts`).
...
- **Root barrel exports** — `src/index.ts` and `src/db.ts` forward all public pieces for `@beep/workspaces-server` consumers. Additional export path `files` provides file processing utilities.
...
- When new tables land in `@beep/workspaces-tables`, update `DocumentsDb.Db` consumers and refresh migrations via `packages/_internal/db-admin` before trusting runtime layers.
...
- **Extend DocumentRepo with custom query methods**
  ...
  When adding new query methods to an existing repository, follow this pattern from `packages/workspaces/server/src/db/repos/Document.repo.ts`:

  ```ts
  import { DbRepo } from "@beep/shared-server/factories";
  import { DocumentsDb } from "@beep/workspaces-server/db";
  import { DocumentsEntityIds, SharedEntityIds } from "@beep/shared-domain";
  import { Document, Entities } from "@beep/workspaces-domain";
  ...
  export class DocumentRepo extends Effect.Service<DocumentRepo>()("@beep/workspaces-server/db/repos/DocumentRepo", {
    dependencies,
    accessors: true,
...
## Verifications
- `bun run check --filter=@beep/workspaces-server`
- `bun run lint --filter=@beep/workspaces-server`
- `bun run test --filter=@beep/workspaces-server`
