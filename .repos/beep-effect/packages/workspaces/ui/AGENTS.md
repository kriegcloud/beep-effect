# AGENTS.md — `@beep/workspaces-ui`

## Purpose & Fit
- House reusable React components for the documents slice so apps do not reimplement UI flows.
- Build atop `@beep/ui` components and `@beep/ui-core` design tokens while consuming `@beep/workspaces-domain` entities and future `@beep/workspaces-client` clients.
...
- ALWAYS validate data with `@beep/workspaces-domain` schemas; NEVER trust external data blindly.
- Separate data fetching from presentation: inject `@beep/workspaces-client` clients or handlers via props instead of hardcoding fetch calls.
...
## Verifications
- `bun run check --filter=@beep/workspaces-ui`
- `bun run lint --filter=@beep/workspaces-ui`
- `bun run test --filter=@beep/workspaces-ui` (add component tests as features land).
...
- [ ] Validation uses `@beep/workspaces-domain`; networking is injected (or uses `@beep/workspaces-client` when available).
...
- [ ] Tests and examples added under `packages/workspaces/ui/test/` (or in apps) for new surfaces.
