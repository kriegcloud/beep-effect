...
## Purpose & Fit
- Aggregates every Drizzle schema used across slices by re-exporting IAM, Documents, Comms, Customization, and shared tables so migrations have a single source of truth (`packages/_internal/db-admin/src/schema.ts:3`).
...
- Hosts generated SQL migrations and Drizzle metadata consumed by CI, Docker-backed tests, and local seed/reset workflows (`packages/_internal/db-admin/drizzle/`).
...
- `src/tables.ts` — Barrel re-export that aggregates all slice tables: IAM, Documents, Comms, Customization, and shared Drizzle tables.
- `src/schema.ts` — Root schema barrel that re-exports tables, relations, and custom columns; referenced by `drizzle.config.ts`.
- `src/relations.ts` — Unified Drizzle relations definitions merging cross-table relationships from all slices (user, organization, team).
- `src/slice-relations.ts` — Re-exports slice-specific relation definitions from IAM, Documents, Customization, and Comms packages.
...
- `test/container.ts` — Testcontainers-backed Layer that provisions Postgres 15 + `pg_uuidv7`, applies migrations, and wires repos for integration testing.
- `test/AccountRepo.test.ts` — Active integration tests for AccountRepo demonstrating Layer composition with IAM repos.
...
- `test/container.ts` — Testcontainers implementation that provisions Postgres with migrations for integration testing.
- `test/AccountRepo.test.ts` — Active IAM AccountRepo integration tests that merge the admin container Layer with slice repositories to cover insert/update/delete flows.
...
- Integration tests in `test/AccountRepo.test.ts` use the PgTest container from `test/container.ts` to exercise repositories against real Postgres.
...
- **`AdminDb` is for tooling only**: Production applications should use slice-specific `*Db` Layers (e.g., `IamDb.Db`, `DocumentsDb.Db`), not `AdminDb`. Using `AdminDb` in production bypasses slice-specific configuration.
...
- **Testcontainers port conflicts**: The `test/container.ts` uses dynamic port allocation. Ensure tests do not conflict with local PostgreSQL instances on port 5432.
