...
const $I = $TodoxId.create("app/lexical/plugins/KeywordsPlugin");
...
export class KeywordsPluginNotRegisteredError extends S.TaggedError<KeywordsPluginNotRegisteredError>(
  $I`KeywordsPluginNotRegisteredError`
)(
  "KeywordsPluginNotRegisteredError",
  {
    message: S.String,
  },
  $I.annotations("KeywordsPluginNotRegisteredError", {
    description: "an Error which occured because the KeywordNode was not registered on editor",
  })
) {
  static readonly new = (message: string) => new KeywordsPluginNotRegisteredError({ message });
}
...
export default function KeywordsPlugin(): JSX.Element | null {
  ...
  useEffect(() => {
    if (!editor.hasNodes([KeywordNode])) {
      throw KeywordsPluginNotRegisteredError.new("KeywordsPlugin: KeywordNode not registered on editor");
    }
  }, [editor]);
