...
- Verification command: `node --check initiatives/stack-installer/ops/start-proof-watch-window.mjs`
- Changed files:
  - initiatives/stack-installer/ops/start-proof-watch-window.mjs
- Verification notes:
...
Although the PID-negation bug is real and validated, it is in a local initiatives/stack-installer/ops proof helper rather than an exposed product/runtime service. The attack path requires local write/tamper access to the selected output root and operator interaction with --replace-existing. Impact is availability-only for signalable local process groups and usually same-user/self-impact; severe effects depend on atypical elevated execution or shared writable output directories. Therefore the original medium rating overstates threat-model relevance, and this should be ignored for main product criticality, with remediation still recommended as defensive hardening.
Path
...
operator/user runs initiatives/stack-installer/ops/start-proof-watch-window.mjs
...
Blindspots
Static review did not prove every possible distribution path for initiatives/stack-installer/ops, but package files and Tauri config reviewed do not show it as a packaged application entry point.
