...
# TLDR
The repo already models a real multi-language stack: Bun/Effect packages, Vite apps, and Tauri/Rust wrappers with Bun sidecars. Turbo can handle this setup, but the native build surfaces are only partially represented in Turbo today. The main gap is cacheable native outputs, not package discovery.

# Score
0.69 / 1.00

# Current repo evidence
- `apps/desktop/package.json` and `apps/V2T/package.json` both expose `dev:native`, `build:native`, and `build:sidecar` scripts.
...
- `apps/desktop/src-tauri/tauri.conf.json` and `apps/V2T/src-tauri/tauri.conf.json` use `beforeBuildCommand: "bun run build && bun run build:sidecar"`, so native packaging depends on JS builds plus sidecar compilation.
- `apps/desktop/turbo.json` and `apps/V2T/turbo.json` only add `dev:desktop` `with` wiring; no native build outputs are modeled there.
- `bunx turbo query ls @beep/desktop --output json` and `bunx turbo query ls @beep/v2t --output json` show the native scripts exist in the workspace, but Turbo treats them as ordinary scripts rather than cached native build artifacts.
...
- Consider package-level `turbo.json` files for `apps/desktop` and `apps/V2T` so Rust-adjacent task behavior lives next to the owning package instead of only in root config and package scripts.
...
- Repo: `/home/elpresidank/YeeBois/projects/beep-effect/apps/V2T/src-tauri/tauri.conf.json`
