...
const CATEGORY_BY_TAG = new Map<string, TSCategory>(CATEGORY_TAXONOMY.map((category) => [category._tag, category]));
 ...
 * @category Utility
 */
export function getCategoriesByArchLayer(layer: ArchitecturalLayer): ReadonlyArray<TSCategory> {
  return CATEGORY_TAXONOMY.filter((category) => category.architecturalLayers.some((archLayer) => archLayer === layer));
}
...
export function getCandidateCategories(
  ...
  const combined: { category: TSCategory; combinedConfidence: number }[] = [];
