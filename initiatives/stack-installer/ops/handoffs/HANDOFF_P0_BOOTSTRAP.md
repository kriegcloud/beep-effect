...
packet for `stack-installer` — a new sibling initiative under `initiatives/` —
that captures the locked design decisions below in the same full ceremony used
by `initiatives/ai-metrics-stack`. Pre-fill P0 as completed; leave later phases
...
2. Read `initiatives/README.md` for vocabulary and lifecycle rules.
3. Read `initiatives/ai-metrics-stack/` end-to-end as the shape reference:
...
4. Skim `initiatives/agentic-professional-runtime/` for cross-reference framing
...
6. Read `initiatives/canonical-slice-factory/SPEC.md` and `README.md` because
...
Create exactly this tree under `initiatives/stack-installer/`:
...
Mirror `initiatives/ai-metrics-stack/README.md` shape. Required sections:
- **Status:** Active
...
Mirror `initiatives/ai-metrics-stack/SPEC.md` shape. Required sections:
...
Mirror `initiatives/ai-metrics-stack/PLAN.md`. One section per phase
(P0–P5), each with:
...
Mirror `initiatives/ai-metrics-stack/ops/manifest.json`. Required keys:
...
`initiatives/ai-metrics-stack/history/outputs/p0-current-state.md` shape.
Required sections:
...
Mirror `initiatives/ai-metrics-stack/ops/handoffs/README.md`. List each
...
1. `git status --short` — confirm only `initiatives/stack-installer/**`
   files are added.
2. `cat initiatives/stack-installer/ops/manifest.json | jq .` — confirm
   valid JSON.
3. `rg -n "P0|P1|P2|P3|P4|P5" initiatives/stack-installer/` — confirm phase
