...
- The current inventory generator renders a compliance inventory over the current `bun run topo-sort` universe and checks gaps that package docgen does not fully validate yet: required export tags, summaries, TSDoc grammar, forbidden legacy tags, example import aliases, unsafe examples, root custom tag registration, and schema annotation/type-alias gaps. See [scripts/generate-jsdoc-documentation-inventory.mjs](../../../scripts/generate-jsdoc-documentation-inventory.mjs).
...
- `@beep/repo-cli` already depends on `@openai/codex-sdk` through the catalog in [packages/tooling/tool/cli/package.json](../../../packages/tooling/tool/cli/package.json), and the root catalog pins `@openai/codex-sdk` to `~0.129.0` in [package.json](../../../package.json).
...
- The installed local SDK package is `@openai/codex-sdk` version `0.129.0`. Its README says the TypeScript SDK wraps the `codex` CLI, spawns it, exchanges JSONL events over stdin/stdout, supports `run`, `runStreamed`, structured output schemas, resume, working directory controls, CLI environment control, and `--config` overrides. Its type declarations expose `ThreadOptions` for model, sandbox, working directory, reasoning effort, network access, web search, approval policy, and additional directories.
- The repo already has a local automation wrapper, [scripts/codex-quality-review-fix-loop.sh](../../../scripts/codex-quality-review-fix-loop.sh), which runs `codex exec --cd "$ROOT_DIR"` with the repo-local `quality-review-fix-loop` skill and explicitly avoids publishing or PR actions unless requested.
