# JSDoc Worker Eval Research

Curated provider-backed evaluation reports live here.

- [2026-05-12 Ollama Qwen3-Coder 30B host eval](./2026-05-12-ollama-qwen3-coder-30b-host-eval.md)
- [2026-05-12 Codex GPT-5.4 Mini low worker eval](./2026-05-12-codex-gpt-5.4-mini-low-worker-eval.md)

Reports should record:

- provider and model id
- host/runtime assumptions
- source quality report commands
- eval command
- packet count and package scope
- runtime and failure modes
- worker score and draft quality observations
- recommendation for whether workers remain experimental
