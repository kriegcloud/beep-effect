...
They were technically viable, but they are not the right next mutation because the repo is now moving toward the `cluster-first`, `workflow-backed` direction described in [cluster-first-repo-expert-memory-plan.md](./cluster-first-repo-expert-memory-plan.md).
...
- `packages/ai/sdk/src/core/service/AgentHttpApi.ts` (historical path from the superseded exploration; not present in this checkout)
- `packages/ai/sdk/src/core/service/AgentHttpHandlers.ts` (historical path from the superseded exploration; not present in this checkout)
- `packages/ai/sdk/src/core/service/AgentHttpServer.ts` (historical path from the superseded exploration; not present in this checkout)
