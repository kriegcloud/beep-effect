...
## 🎯 OVERVIEW
Comprehensive JSDoc documentation patterns used throughout the beep-effect codebase, ensuring consistent, practical, and compilable examples for all APIs.
...
### Core beep-effect codebase Imports
```typescript
/**
 * @example
 * ```ts
 ...
 * 
 * const program = Effect.gen(function* () {
 *   const person = yield* S.decodeUnknown(PersonSchema)({
 *     name: "Alice",
 *     age: 30
 *   })
 *   return person
 ...
 *     )
 *     
 *     const user = yield* S.decodeUnknown(UserSchema)(response.json)
...
This comprehensive JSDoc approach ensures that beep-effect codebase documentation provides practical, reliable examples that help developers understand and correctly use the APIs.