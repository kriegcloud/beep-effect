...
const inferExportKindFromDeclarationType = (declaration: unknown): ExportKind => {
  ...
  switch (declaration.type) {
    case "VariableDeclaration":
      return kindFromVariableDeclarationKind(
        typeof declaration.kind === "string" ? declaration.kind : undefined
      )
      ...
      if (declaration !== null && declaration !== undefined) {
        if (n.VariableDeclaration.check(declaration)) {
          ...
          if (n.Identifier.check(identifier)) {
            output.set(
              identifier.name,
              inferExportKindFromDeclarationType(declaration)
            )
          }
        }
      }
...
const inferExportKindFromTsMorphNode = (declaration: Node): ExportKind => {
  ...
  if (Node.isVariableDeclaration(declaration)) {
    const declarationList = declaration.getFirstAncestorByKind(
...
const collectExportsWithTsMorph = (
  sourceFile: SourceFile,
  repoRoot: string
): Map<string, TsMorphExportDetails> => {
  ...
  for (const [exportName, declarations] of exportedDeclarations) {
    ...
    const declaration = declarations.find(
      (item) => item.getSourceFile().getFilePath() === sourceFile.getFilePath()
    ) ?? declarations[0]
...
export const parseModuleExports = (options: {
  ...
  const sourceFile = options.project.addSourceFileAtPathIfExists(options.moduleSourcePath)
  ...
  const missingInTsMorph = [...astNames]
    .filter((name) => !tsMorphNames.has(name))
    .sort((a, b) => a.localeCompare(b))

  const missingInAst = [...tsMorphNames]
