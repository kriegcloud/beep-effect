 ...
 *
 * Overview:
 * Creates a `Combiner` for `UndefinedOr<A>` that only combines values when both operands are not `undefined`, failing fast if either is `undefined`.
 *
...
const sourceSummary = "Creates a `Combiner` for `UndefinedOr<A>` that only combines values when both operands are not `undefined`, failing fast if either is `undefined`.";
const sourceExample = "";
