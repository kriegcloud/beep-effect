 ...
 * Export Coordinates
 * ========================================================================== */
const exportName = "mapArrayEffect";
const exportKind = "const";
const moduleImportPath = "effect/Stream";
const sourceSummary = "Effectfully maps over non-empty array chunks emitted by the stream.";
const sourceExample = "import { Array, Console, Effect, Stream } from \"effect\"\n\nconst program = Effect.gen(function*() {\n  const result = yield* Stream.fromArray([1, 2, 3, 4]).pipe(\n    Stream.rechunk(2),\n    Stream.mapArrayEffect((chunk, index) =>\n      Effect.succeed(Array.map(chunk, (n) => n + index * 10))\n    ),\n    Stream.runCollect\n  )\n  yield* Console.log(result)\n})\n\nEffect.runPromise(program)\n// Output: [1, 2, 13, 14]";
