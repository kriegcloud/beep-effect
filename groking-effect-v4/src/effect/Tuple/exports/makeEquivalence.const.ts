...
const sourceSummary = "Creates an `Equivalence` for tuples by comparing corresponding elements using the provided per-position `Equivalence`s. Two tuples are equivalent when all their corresponding el...";
const sourceExample = "import { Equivalence, Tuple } from \"effect\"\n\nconst eq = Tuple.makeEquivalence([\n  Equivalence.strictEqual<string>(),\n  Equivalence.strictEqual<number>()\n])\n\nconsole.log(eq([\"Alice\", 30], [\"Alice\", 30])) // true\nconsole.log(eq([\"Alice\", 30], [\"Bob\", 30]))   // false";
