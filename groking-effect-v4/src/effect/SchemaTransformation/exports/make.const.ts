...
const sourceSummary = "Constructs a `Transformation` from an object with `decode` and `encode` `Getter`s. If the input is already a `Transformation`, returns it as-is.";
const sourceExample = "import { SchemaGetter, SchemaTransformation } from \"effect\"\n\nconst t = SchemaTransformation.make({\n  decode: SchemaGetter.transform<number, string>((s) => Number(s)),\n  encode: SchemaGetter.transform<string, number>((n) => String(n))\n})";
