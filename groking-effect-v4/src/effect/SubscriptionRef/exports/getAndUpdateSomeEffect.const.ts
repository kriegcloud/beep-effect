...
const sourceSummary =
  "Atomically retrieves the current value and optionally updates it with the result of applying an effectful function that returns an `Option`, notifying subscribers only if the va...";
const sourceExample =
  'import { Effect, Option, SubscriptionRef } from "effect"\n\nconst program = Effect.gen(function*() {\n  const ref = yield* SubscriptionRef.make(10)\n\n  const oldValue = yield* SubscriptionRef.getAndUpdateSomeEffect(\n    ref,\n    (n) => Effect.succeed(n > 5 ? Option.some(n + 3) : Option.none())\n  )\n  console.log("Old value:", oldValue)\n\n  const newValue = yield* SubscriptionRef.get(ref)\n  console.log("New value:", newValue)\n})';
