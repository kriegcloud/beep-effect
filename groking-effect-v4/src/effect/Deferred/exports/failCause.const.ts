...
const sourceSummary = "Fails the `Deferred` with the specified `Cause`, which will be propagated to all fibers waiting on the value of the `Deferred`.";
const sourceExample = "import { Cause, Deferred, Effect } from \"effect\"\n\nconst program = Effect.gen(function*() {\n  const deferred = yield* Deferred.make<number, string>()\n  const success = yield* Deferred.failCause(\n    deferred,\n    Cause.fail(\"Operation failed\")\n  )\n  console.log(success) // true\n})";
