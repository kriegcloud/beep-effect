...
const sourceSummary = "Create an `Equivalence` for records using the provided `Equivalence` for values. Two records are considered equivalent if they have the same keys and their corresponding values ...";
const sourceExample = "import { Equal, Record } from \"effect\"\nimport * as assert from \"node:assert\"\n\nconst recordEquivalence = Record.makeEquivalence(Equal.asEquivalence<number>())\n\nassert.deepStrictEqual(recordEquivalence({ a: 1, b: 2 }, { a: 1, b: 2 }), true)\nassert.deepStrictEqual(recordEquivalence({ a: 1, b: 2 }, { a: 1, b: 3 }), false)";
