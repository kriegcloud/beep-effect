...
const sourceSummary = "A shortcut for `MyStruct.mapFields(Struct.assign(fields))`. This is useful when you want to add new fields to an existing struct or a union of structs.";
const sourceExample = "import { Schema, Tuple } from \"effect\"\n\n// Add a new field to all members of a union of structs\nconst schema = Schema.Union([\n  Schema.Struct({ a: Schema.String }),\n  Schema.Struct({ b: Schema.Number })\n]).mapMembers(Tuple.map(Schema.fieldsAssign({ c: Schema.Number })))";
