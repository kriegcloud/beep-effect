...
const sourceSummary = "Performs left-to-right function composition. The first argument may have any arity, the remaining arguments must be unary.";
const sourceExample = "import { flow } from \"effect/Function\"\nimport * as assert from \"node:assert\"\n\nconst len = (s: string): number => s.length\nconst double = (n: number): number => n * 2\n\nconst f = flow(len, double)\n\nassert.strictEqual(f(\"aaa\"), 6)";
