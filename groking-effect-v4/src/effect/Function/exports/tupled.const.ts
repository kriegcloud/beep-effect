...
const sourceSummary = "Creates a tupled version of this function: instead of `n` arguments, it accepts a single tuple argument.";
const sourceExample = "import { tupled } from \"effect/Function\"\nimport * as assert from \"node:assert\"\n\nconst sumTupled = tupled((x: number, y: number): number => x + y)\n\nassert.deepStrictEqual(sumTupled([1, 2]), 3)";
