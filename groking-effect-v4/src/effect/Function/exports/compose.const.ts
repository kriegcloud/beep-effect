...
const sourceSummary = "Composes two functions, `ab` and `bc` into a single function that takes in an argument `a` of type `A` and returns a result of type `C`. The result is obtained by first applying...";
const sourceExample = "import { compose } from \"effect/Function\"\nimport * as assert from \"node:assert\"\n\nconst increment = (n: number) => n + 1\nconst square = (n: number) => n * n\n\nassert.strictEqual(compose(increment, square)(2), 9)";
