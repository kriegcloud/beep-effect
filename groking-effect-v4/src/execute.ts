...
import {
  generateModuleSurface,
  generatePackageSurface,
  generateRepositorySurface
...
const resolveRoots = (flags: CliFlags): {
  repoRoot: string
  packageRoot: string
  ...
  const outputRoot =
    flags.outputRoot === undefined
      ? path.join(packageRoot, "src")
      : path.resolve(process.cwd(), flags.outputRoot)

  const manifestPath =
    flags.manifestPath === undefined
      ? undefined
      : path.resolve(process.cwd(), flags.manifestPath)
...
const detectCommand = (args: ReadonlyArray<string>): {
  command: Command
...
const verifyRepositoryOutput = (outputRoot: string): {
  moduleCount: number
...
const runGenerateCommand = (flags: CliFlags): void => {
  ...
  const exportCount = packageResult.moduleResults.reduce(
    (total, result) => total + result.exportFiles.length,
    0
  )
