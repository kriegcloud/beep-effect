...
export const readNamedExport = (moduleRecord: Record<string, unknown>, exportName: string): unknown =>
  moduleRecord[exportName];

export const logHeader = Effect.fn(function* ({ icon, moduleImportPath, exportName, exportKind }: PlaygroundHeader) {
  yield* Console.log(`\n${separator}`);
...
export const logSummary = (summary: string): Effect.Effect<void> => Console.log(`\n📝 Overview\n${summary}`);
...
export const logExampleHeader = Effect.fn(function* (index: number, title: string, description: string) {
  yield* Console.log(`\n${separator}`);
...
export const runExamples = Effect.fn(function* (examples: ReadonlyArray<PlaygroundExample>) {
...
export const inspectNamedExport = Effect.fn(function* (params: {
...
export const probeNamedExportFunction = Effect.fn(function* (params: {
  readonly moduleRecord: Record<string, unknown>;
  readonly exportName: string;
}) {
  const target = readNamedExport(params.moduleRecord, params.exportName);
...
export const probeNamedExportConstructor = Effect.fn(function* (params: {
  readonly moduleRecord: Record<string, unknown>;
  readonly exportName: string;
}) {
  const target = readNamedExport(params.moduleRecord, params.exportName);
...
export const inspectTypeLikeExport = Effect.fn(function* (params: {
  readonly moduleRecord: Record<string, unknown>;
  readonly exportName: string;
}) {
...
const createPlaygroundProgramBase = Effect.fn(function* (options: PlaygroundProgramOptions) {
...
export const createPlaygroundProgram = (options: PlaygroundProgramOptions): Effect.Effect<void> =>
  createPlaygroundProgramBase(options).pipe(reportProgramError);
