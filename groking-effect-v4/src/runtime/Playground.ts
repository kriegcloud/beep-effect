...
export const logHeader = ({ icon, moduleImportPath, exportName, exportKind }: PlaygroundHeader): Effect.Effect<void> =>
  Effect.gen(function* () {
    yield* Console.log(`\n${separator}`);
...
export const logExampleHeader = (index: number, title: string, description: string): Effect.Effect<void> =>
  Effect.gen(function* () {
    yield* Console.log(`\n${separator}`);
...
export const runExamples = (examples: ReadonlyArray<PlaygroundExample>): Effect.Effect<void> =>
  Effect.gen(function* () {
