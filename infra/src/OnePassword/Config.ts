...
export class OnePasswordConfigError extends TaggedErrorClass<OnePasswordConfigError>($I`OnePasswordConfigError`)(
  ...
  $I.annote("OnePasswordConfigError", {
    description: "",
  })
) {
  static readonly newCause: {
    (cause: unknown, message: string): OnePasswordConfigError;
    (message: string): (cause: unknown) => OnePasswordConfigError;
  } = dual(
    2,
    (cause: unknown, message: string): OnePasswordConfigError =>
      new OnePasswordConfigError({
        message,
        cause,
      })
  );
...
class ServiceShape extends S.Class<ServiceShape>($I`ServiceShape`)(
  {
    ...
    keepAlive: S.Boolean.pipe(S.optionalKey, SchemaUtils.withKeyDefaults(false)),
  },
  $I.annote("ServiceShape", {
    description: "",
  })
) {}
 ...
 * @since 0.0.0
 */
export class Service extends Context.Service<Service, ServiceShape>()($I`Service`) {
  static readonly config = Config.all({
    serverURL: Config.schema(ServiceShape.fields.serverURL, "1PASSWORD_CONNECT_SERVER_URL"),
    token: Config.schema(ServiceShape.fields.token, "1PASSWORD_CONNECT_TOKEN"),
    keepAlive: Config.schema(ServiceShape.fields.keepAlive, "1PASSWORD_CONNECT_KEEP_ALIVE"),
  });
  ...
  static readonly layerConfig = (overrides?: undefined | Partial<ServiceShape>) => {
    ...
    const serviceEffect = Effect.gen(function* () {
      ...
      return O.fromNullishOr(overrides).pipe(
        O.map((overrides) => ({
      ...
      Effect.withSpan($I`Service.Effect`, {
        attributes: {
          overrides,
        },
      })
    ...
    return Layer.effect(Service, serviceEffect);
  };
}
