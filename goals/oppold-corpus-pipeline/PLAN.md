...
| P2 Extraction | pending | Harden `drivers/libpff` (pffexport subprocess engine) and `drivers/tika` (real invocation); add the `beep corpus` command family; run extraction over `raw/` into `staging/`. | Per-message artifacts with attachments + text/metadata manifests emitted; coverage summary recorded; drivers pass repo quality gates. |
| P3 Organization | pending | Build the `organized/` client/matter/patent-family taxonomy from restored names + extracted metadata; group multi-version drafts. | `organized/` tree + catalog views exist; user sampling review recorded. |
| P4 Enrichment | pending | New `drivers/uspto` (Open Data Portal); resolve patent/application numbers from the corpus; fetch official metadata + published PDFs; anchor version groups to patent families. | Enrichment manifest exists; family anchors present in catalog. |
