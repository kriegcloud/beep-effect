...
Move the OIP solo-practice website into `@beep/oip-web` as a build-ready,
...
- `apps/oip-web` owns v1 marketing-site copy, section data, SEO inputs,
  JSON-LD inputs, and launch review notes.
- `@beep/ui` remains the product-agnostic UI foundation. OIP-specific
...
- `@beep/oip-web` renders the OIP home page instead of the starter page.
...
- Runtime assets are copied into `apps/oip-web/public/oip/*`.
- Package checks pass:
