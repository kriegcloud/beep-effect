...
| P4 Dupes Cleanup | seeded | Remove low-risk repeated config/test-helper clones and classify generated-code clones. | Clone cleanup does not grow `beep reuse clones --check` and Fallow dupes examples shrink where targeted. |
...
- Dupes work starts with obvious shared helpers/config. Generated clones are
  classified rather than force-refactored.
