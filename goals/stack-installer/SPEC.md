...
`goals/canonical-slice-factory` is the creation path for new installer
...
1. Sibling initiative. `stack-installer` lives at
   `goals/stack-installer/` and is not a child of another packet.
...
6. [research](./research)
7. `goals/ai-metrics-stack` only as packet-shape reference
