...
- Verification command: `node --check goals/stack-installer/ops/proof-upload-status.mjs`
- Changed files:
...
operator runs goals/stack-installer/ops/proof-upload-status.mjs
...
The status helper is in an goals/ops path rather than packaged app files, so production distribution exposure may be lower than active proof-runbook exposure.
