...
Execute packages in this order (by violation count and business criticality):
...
2. **P2 - Medium Priority** (Supporting packages)
   ...
   - `@beep/documents-server` (~6 violations)
...
- **Test files with `@ts-expect-error`**: These are intentionally testing invalid inputs
- **Type definition files**: May use native JS in documentation examples
- **Generated files**: Will be overwritten

### External Library Boundaries

Some violations may be intentional at library boundaries:
- `new Date()` when calling external APIs that require native Date
- `instanceof` checks for framework-specific types (e.g., Lexical nodes)
- `typeof` in type guard functions that return valid Effect predicates

Mark these with `[INTENTIONAL]` in PLAN.md rather than fixing.
...
1. Save current progress in PLAN.md (mark completed items)
2. Note the current package and file being worked on
3. Create a `HANDOFF.md` with:
