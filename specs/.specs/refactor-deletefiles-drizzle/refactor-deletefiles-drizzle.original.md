...
2. **After refactoring, check if `SqlClient` and `SqlSchema` imports are still needed**:
   - `SqlClient` may still be used by `deleteFiles` if it's the only remaining raw SQL query
   - If no other queries use raw SQL, these imports can be removed
...
- [ ] Uses `A.map` instead of native `.map()`
- [ ] `Effect.withSpan("FileRepo.deleteFiles")` preserved
- [ ] Returns `ReadonlyArray<File.UploadKey.Type>`
