...
**The following will result in AUTOMATIC FAILURE of the entire implementation:**

1. Using `withThemeByClassName` instead of `withThemeByDataAttribute`
2. Missing `attributeName: "data-color-scheme"` in theme decorator
3. Using `class="dark"` instead of `data-color-scheme` attribute

| Score | Criteria |
|-------|----------|
| 0 | **AUTOMATIC FAIL**: Wrong decorator or attribute (see above) |
| 1-4 | No theme integration; components unstyled |
...
**MUST verify before any other checks:**

- [ ] Uses `withThemeByDataAttribute` (NOT `withThemeByClassName`)
- [ ] Sets `attributeName: "data-color-scheme"` (NOT `class` or `data-theme`)
- [ ] Decorator order correct: attribute OUTER, provider INNER
...
- [ ] Emotion cache set up with `prepend: true`
...
# 2. Click theme toggle in toolbar
# 3. Open DevTools -> Elements -> Inspect <html> tag
# 4. VERIFY: data-color-scheme="light" or data-color-scheme="dark"
# 5. FAIL if: class="dark" or no attribute at all
```
