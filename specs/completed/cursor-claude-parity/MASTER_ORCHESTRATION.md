# cursor-claude-parity: Master Orchestration

> End-to-end runbook for achieving operational parity between `.claude`/`.codex` and `.cursor`.
...
- P1 Gap Analysis
- P2 Cursor Config Implementation
- P3 Validation
...
- `.claude/` is reference source of truth; `.codex/` is secondary reference (post-parity)
...
- Extend `sync-cursor-rules` where applicable; document any new sync or copy strategy
...
Build a measurable baseline of `.cursor`, `.claude`, and `.codex` capability inventory and parity targets.
...
1. Count and categorize `.cursor` capability sources (rules, skills)
2. Compare against `.claude` and `.codex` inventories
3. Enumerate parity domains
4. Classify requirements as `required` or `optional`
5. Build initial strategy labels
...
- [ ] Baseline facts include timestamped counts for all three config dirs
...
Translate baseline capability inventory into a concrete implementation decision set for `.cursor`.
...
1. Map each required capability to Cursor path
...
5. Define prerequisites for P2 implementation (including sync-cursor-rules extension criteria if needed)
...
## 6. P2: Cursor Config Implementation

### P2 Objective

Create/extend `.cursor/` implementation that satisfies approved P1 decisions.
...
1. Validate/extend `.cursor/rules/` (sync-cursor-rules)
2. Add or adapt skills to `.cursor/skills/`
3. Add workflow/command mappings (rules, skills, or docs as appropriate)
4. Add context discoverability surfaces
...
- [ ] `.cursor/` structure meets parity targets
- [ ] Required capabilities implemented or approved adaptation applied
- [ ] No `.claude` or `.codex` regression introduced
...
Demonstrate operational parity through scenario-based verification in Cursor IDE.
...
find specs/cursor-claude-parity -maxdepth 3 -type f | sort
find .cursor -type f | sort

# rules sync (if extending)
bun run repo-cli sync-cursor-rules --dry-run
