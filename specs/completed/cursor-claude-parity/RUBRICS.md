# cursor-claude-parity: Evaluation Rubrics

> Quantitative grading model for operational parity between `.claude`/`.codex` and `.cursor`.
...
|----------|--------|---------|
| Capability Coverage | 35% | Required source capabilities are available in Cursor configuration |
...
| Score | Criteria |
|-------|----------|
| 5 | Cursor guidance aligns with all critical repo guardrails |
...
- `.claude/**` and `.codex/**` source guidance
...
| S5: Rules sync + skill discoverability | Yes |
...
- `.cursor` guidance conflicts with project safety constraints
