...
const buildUserPrompt = (content: string, url?: string): string => {
  const truncated =
    Str.length(content) > MAX_CONTENT_SIZE
      ? `${Str.slice(0, MAX_CONTENT_SIZE)(content)}\n\n[truncated]`
      : content;
...
const estimateWordCount = (content: string): number => {
  ...
  const normalized = Str.replace(/\s+/g, " ")(trimmed);
  const parts = Str.split(" ")(normalized);
  return A.length(A.filter(parts, Str.isNonEmpty));
};
