...
You are creating a file-level implementation plan for the IP law knowledge graph package. Read `outputs/p1-schema-design.md` and the README Assumptions and Defaults. Define the package scaffold order with dependency relationships. List every source file with purpose, exports, and dependencies. Define seed data plan with at least 1 patent, 1 trademark, and 1 copyright scenario. List quality gates.

Write: `outputs/p2-implementation-plan.md`
...
You are performing final verification of the IP law knowledge graph package. Run `pnpm check`, `pnpm lint-fix`, `pnpm test`, and `pnpm build` for the package. Record full command output. Classify any failures as pre-existing or new. Resolve new failures. Write a final readiness statement confirming the package meets all success criteria from the README.
