...
- DNS domains: `TBD (e.g. api.<domain>)`
- Owner: `TBD`
...
- Known mismatches called out in `outputs/R5_IAC_OPS_PRODUCTION_PLAN.md`:
  ...
  - `KV_REDIS_PASSWORD` must be non-empty; prod must use a real password (empty string will fail schema validation).
...
- [ ] Prod never uses `bun run db:push` (schema drift); only `bun run db:migrate` from versioned migrations.
...
- OTLP export targets must be HTTPS and reachable from Cloud Run; dashboards/alerts must exist for the `/knowledge` critical path.
