...
const serviceEffect: Effect.Effect<ReconciliationServiceShape, never, Storage | WikidataClient> = Effect.gen(function* () {
  ...
  const decodeStoredTask = (entityIri: string, stored: StoredValue): Effect.Effect<VerificationTask, ReconciliationError> =>
    ...
    storage.get(taskKey(taskId)).pipe(
      Effect.flatMap(
        O.match({
          onNone: () => Effect.succeed(O.none()),
          onSome: (stored) =>
            decodeStoredTask(stored.key, stored).pipe(Effect.map(O.some)),
        })
      )
    );
  ...
  const listPendingTasks: ReconciliationServiceShape["listPendingTasks"] = () =>
    storage.list(QUEUE_PREFIX).pipe(
      Effect.flatMap((stored) =>
        Effect.forEach(stored, (entry) => decodeStoredTask(entry.key, entry), { concurrency: 10 })
      ),
      Effect.map((tasks) => tasks.filter((t) => t.status === "pending"))
    );
