...
Read `handoffs/HANDOFF_P4.md`, all prior phase artifacts, the root plus workspace `package.json` and `turbo.json` files, `infra/package.json`, the repo-law inputs named in `README.md`, the relevant command surfaces in `apps/V2T`, `packages/v2t-sidecar`, and `infra`, and the delegation assets under `prompts/`.
