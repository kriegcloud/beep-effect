...
const run = async () => {
  ...
  assert(shaclResult.conforms, `Seed dataset does not conform to structural SHACL shapes: ${stableStringify(shaclResult)}`);

  const scaffoldCounts = (scaffold.counts as Record<string, number | string | boolean | null | undefined>) ?? {};
  assert(scaffoldCounts.jsDocTags === 113, `Expected 113 mechanically scaffolded tags, received ${String(scaffoldCounts.jsDocTags)}.`);
