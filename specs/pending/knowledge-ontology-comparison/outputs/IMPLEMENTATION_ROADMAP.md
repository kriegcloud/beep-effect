...
This roadmap is reconciled to the current repository state on 2026-02-08.
...
Phase 7 capability parity acceleration also closed the key P1/P2 service gaps (classifier, enrichment, reconciliation, cross-batch resolver, and LLM fallback chains).
...
| EventBus + Ontology Registry | Complete | `packages/knowledge/server/src/Service/{EventBus,OntologyRegistry}.ts` |
| Storage abstraction | Implemented with scoped divergence | `packages/knowledge/server/src/Service/Storage.ts` |
...
| LLM fallback chain | `P6-09` | `FULL` | LanguageModel + EmbeddingModel fallback chains implemented with tests |
...
- [ ] `P6-02` and `P6-04` divergence rationale locked (or implemented).
