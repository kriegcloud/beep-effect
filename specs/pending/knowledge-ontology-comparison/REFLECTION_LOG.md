...
### Addendum: Phase 6 Closure Updates (2026-02-07)

This Phase 6 entry was superseded by additional closure work in the same day:

1. **P6-02: Durable engine mode added (still diverges from sharded cluster parity)**
   - Added opt-in workflow runtime mode `engine-durable-sql` in `packages/knowledge/server/src/Runtime/WorkflowRuntime.ts`.
   - Verified single-node semantics remain correct even if persistence writes fail:
     - `packages/knowledge/server/test/Workflow/WorkflowPersistence.singleNodeSemantics.test.ts`
   - Remaining divergence: multi-runner sharding / runner registration semantics are not implemented.

2. **P6-03: EventBus parity closed**
   - Added durable EventBus implementation (`EventJournal` + `PersistedQueue`) + SQL layer wiring:
     - `packages/knowledge/server/src/Service/EventBus.ts`
     - `packages/knowledge/server/test/Service/EventBus.test.ts`

3. **P6-04: Storage abstraction improved; signed URL/cloud parity intentionally deferred**
   - Added backends:
     - Memory (existing): `StorageMemoryLive`
     - Local FS (atomic writes + generation preconditions): `StorageLocalLive`
     - Durable SQL (sqlite via `bun:sqlite`): `StorageSqlLive`
   - Added signed URL extension seam (`signedUrl`), returning `Option.none()` in all current backends.
   - Evidence:
     - `packages/knowledge/server/src/Service/Storage.ts`
     - `packages/knowledge/server/test/Service/{Storage,StorageLocal,StorageSql}.test.ts`

4. **P6-09: Embedding fallback chain parity closed**
   - Added embedding resilience wrapper (retry + central circuit feedback + optional fallback):
...
- `bun test packages/knowledge/server/test/Workflow/`: pass (16 pass, 0 fail)
- `bun test packages/knowledge/server/test/Resilience/`: pass (14 pass, 0 fail)

### Follow-up Risks
- `P6-02` remains divergence from effect-ontology’s multi-runner sharded cluster persistence (single-node durable mode exists).
- `P6-04` remains divergence until a cloud backend with signed URL support is implemented.
...
## Entry 5: Phase 8 Repository Law Remediation (2026-02-08)

### Phase
Phase 8 - Repository Law Remediation (In progress; enforcement and remediation applied)

### What Triggered This Phase
- `packages/knowledge/server/src/Service/Storage.ts` was authored in a prior session in a style that violated repo laws:
  - Promise-first IO and Node fs/path usage in service code
  - Unsafe assertions / weak schema boundaries
  - Ad-hoc JSON encoding/decoding (stringify/parse) in application surfaces
  - Native collection/date usage where Effect utilities are preferred
- Several of the written “laws” existed as guidance, but were not enforced automatically (manual-only rule triggers and limited verification wiring).

### Root Cause (Most Likely)
1. **Manual-only rule application**
   - The most important Effect pattern rules were marked `trigger: manual` (and Cursor had `alwaysApply: false`), so an agent could produce code without seeing the constraints.
2. **Enforcement not wired into default workflows**
   - Repo-law verification exists (`repo-cli verify patterns` / `verify:patterns:ci`), but it was not part of the default “check” loop or commit loop for the knowledge slice.
3. **Documentation contained copyable anti-pattern code**
   - The code standards docs included example snippets using `JSON.parse(...) as Type`, which is incompatible with the repo’s schema-safe boundary rules.
4. **Missing “stop-the-line” signal**
   - Without pre-commit/CI enforcement, the first time the regression is observed is often during later parity work or runtime failures, after it has already spread.

### What Was Done
1. **Remediated the Storage service**
   - `packages/knowledge/server/src/Service/Storage.ts` is now Effect-first: `@effect/platform` FileSystem/Path, `Effect.fn`, `Option`/`HashMap`/`Match`, `effect/String`/`effect/Array`, `DateTime`, and `@effect/sql` + `SqlSchema` + `@effect/sql-sqlite-bun` for the durable backend.
   - Optimistic concurrency conflicts fail explicitly via `Effect.fail(new StorageGenerationConflictError(...))` rather than implicit/unsafe flows.

2. **Fixed a durable EventBus correctness defect**
   - `packages/knowledge/server/src/Service/EventBus.ts` durable decoder no longer drops every journal entry (it previously ended with an unconditional `Effect.as(O.none())`), which caused test timeouts.

3. **Turned repo-law rules into defaults**
   - `.claude/rules/effect-patterns.md` now applies always with TS globs.
   - `.cursor/rules/effect-patterns.mdc` now has `alwaysApply: true` with TS globs.

4. **Removed misleading examples from standards**
   - `.claude/rules/code-standards.md` and `.cursor/rules/code-standards.mdc` no longer include copyable `JSON.parse(...) as T` patterns and instead show `S.decode(S.parseJson(...))`.

5. **Added scoped enforcement**
   - `.husky/pre-commit` now runs repo-law checks for knowledge slice changes (pattern verification for `@beep/knowledge-server`, entity id verification for knowledge-domain sources).
   - `.github/workflows/repo-law.yml` adds CI enforcement for `agents:validate:strict`, `verify:patterns:ci`, and `verify:entityids`.

### What Worked Well
- The EventBus timeout failure was a useful indicator that the “pass” claims in parity artifacts must be backed by runnable evidence; fixing the defect restored determinism.
- Converting “rules” from guidance to always-on defaults reduced the probability of reintroducing the same class of regression.

### Remaining Loopholes / Risks
- Pattern verification cannot practically detect every “native method” usage without risking false positives; it should be treated as a backstop, not the primary design constraint.
- Pre-commit enforcement is intentionally scoped (knowledge slice only) to keep commits fast; repo-wide enforcement is still a policy decision.
- Runtime differences between Bun and node-shared platform adapters can cause surprising behavior; prefer relying on `@effect/platform-bun` layers and verify under Bun with tests.
