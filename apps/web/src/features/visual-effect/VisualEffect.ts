...
export interface VisualEffectService {
  ...
  readonly notify: (message: string, options?: { duration?: number; icon?: string }) => Effect.Effect<void>;
}
...
export class VisualEffect<A, E = never> {
  ...
  get effect(): Effect.Effect<A, E> {
    ...
    return Effect.gen(
      function* (this: VisualEffect<A, E>) {
        ...
        const wrappedEffect = Option.isSome(maybeParentService) ? this._effect : effectWithRootService;
  ...
  private setState(newState: EffectState<A, E>) {
    ...
    if (!validTransitions?.has(newState.type)) {
      if (process.env.NODE_ENV === "development") {
        console.warn(`Invalid state transition from ${previousState.type} to ${newState.type} for task ${this.name}`);
      }
      return;
    }
...
export const notify = (message: string, options?: { duration?: number; icon?: string }): Effect.Effect<void, never> =>
  Effect.serviceOption(VisualEffectService).pipe(
    Effect.flatMap((option) => (Option.isSome(option) ? option.value.notify(message, options) : Effect.void))
  );
...
export function useVisualEffectNotification<A, E>(effect: VisualEffect<A, E>) {
  return useSyncExternalStore(effect.subscribeToNotifications.bind(effect), () => effect.getCurrentNotification());
}
...
export const visualEffect = <A, E = never>(name: string, effect: Effect.Effect<A, E>, showTimer = false) =>
