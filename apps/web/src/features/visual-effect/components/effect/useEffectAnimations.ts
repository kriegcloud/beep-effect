...
export function useTaskAnimations(): AnimationValues {
  const nodeWidth = useSpring(animationTokens.dimensions.nodeDefaults.width, animationTokens.springs.nodeWidth);
...
export function useRunningAnimation(
  isRunning: boolean,
  animations: Pick<AnimationValues, "rotation" | "shakeX" | "shakeY" | "borderOpacity" | "glowIntensity">
) {
  ...
  useEffect(() => {
    ...
    // border pulse
    animControls.border = animate(animations.borderOpacity, [...animationTokens.timing.borderPulse.values], {
    ...
    // glow pulse
    animControls.glow = animate(animations.glowIntensity, [...animationTokens.timing.glowPulse.values], {
    ...
    const jitter = () => {
      ...
      const angle =
        (Math.random() * animationTokens.shake.running.angleRange + animationTokens.shake.running.angleBase) *
        (Math.random() < 0.5 ? 1 : -1);

      const offset =
        (Math.random() * animationTokens.shake.running.offsetRange + animationTokens.shake.running.offsetBase) *
        (Math.random() < 0.5 ? -1 : 1);

      const offsetY =
        (Math.random() * animationTokens.shake.running.offsetYRange + animationTokens.shake.running.offsetYBase) *
        (Math.random() < 0.5 ? -1 : 1);
...
export function useEffectAnimations(
  ...
  }, [transition.justCompleted, transition.justStarted, animations.flashOpacity, prefersReducedMotion]);
  ...
  useEffect(() => {
    ...
    const shakeSequence = async () => {
      const { intensity, duration, count, rotationRange, returnDuration } = animationTokens.shake.failure;
    ...
    const glitchSequence = async () => {
      ...
      for (let i = 0; i < t.initialCount && !cancelled; i++) {
        ...
        await new Promise<void>((resolve) => {
          scheduleIdle(resolve, t.initialDelayMin + Math.random() * Math.max(0, t.initialDelayMax - t.initialDelayMin));
        });
      ...
      const subtle = () => {
        ...
        scheduleIdle(subtle, t.subtleDelayMin + Math.random() * Math.max(0, t.subtleDelayMax - t.subtleDelayMin));
      };
      subtle();
    };
