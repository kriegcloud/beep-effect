...
import type { ChatMessage, ToolName } from "@beep/ui/components/editor/use-chat";
import { markdownJoinerTransform } from "@beep/ui/lib/markdown-joiner-transform";
import {
    ...
    const stream = createUIMessageStream<ChatMessage>({
      execute: async ({ writer }) => {
        ...
        if (!toolName) {
          ...
          writer.write({
            data: AIToolName as ToolName.Type,
            type: "data-toolName",
          });
...
const getCommentTool = (
  editor: SlateEditor,
  {
    ...
    readonly messagesRaw: ChatMessage[];
    readonly model: LanguageModel;
    readonly writer: UIMessageStreamWriter<ChatMessage>;
  }
) =>
  tool({
