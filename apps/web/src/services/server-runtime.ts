...
type AppError = ConfigError | SqlError;
...
type DatabaseLive = Layer.Layer<PgClient | SqlClient | DbPool, AppError, never>;
...
type SliceDbsLive = Layer.Layer<SliceDbs, AppError, never>;
...
// Provide IamDatabase and AuthEmail into Auth (providers first)
type AuthLive = Layer.Layer<AuthService, AppError, never>;
const AuthLive: AuthLive = AuthService.DefaultWithoutDependencies.pipe(Layer.provide([SliceDbsLive, AuthEmailLive]));

// Auth includes all deps; merge Base for platform/logging
type AppLive = Layer.Layer<Resource.Resource | HttpClient.HttpClient | AuthService, AppError, never>;
