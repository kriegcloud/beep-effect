 ...
 */
export const ExchangeRateId = S.NonEmptyString.check(S.isUUID(7)).pipe(
  S.brand("ExchangeRateId"),
  $I.annoteSchema("ExchangeRateId", {
 ...
 */
export const RateType = LiteralKit(["Spot", "Average", "Historical", "Closing"]).pipe(
  $I.annoteSchema("RateType", {
 ...
 */
export const RateSource = LiteralKit(["Manual", "Import", "API"]).pipe(
  $I.annoteSchema("RateSource", {
 ...
 */
export const Rate = S.BigDecimal.check(S.makeFilter(BigDecimal.isPositive)).pipe(
  S.brand("Rate"),
  $I.annoteSchema("Rate", {
    description: "Rate - A positive BigDecimal representing an exchange rate",
    documentation:
      "Exchange rates must be positive (greater than 0). Uses BigDecimal for high precision decimal arithmetic.",
  })
);
