 ...
 */
export const DecimalPlaces = LiteralKit([
  0,
  2,
  3,
  4
...
export class Currency extends S.Class<Currency>($I`Currency`)(
  ...
  $I.annote(
    "Currency",
    {
      ...
      documentation: "Contains the ISO 4217 code, display name, symbol, decimal places,\nand active status for a currency."
    }
  )
) {
  /**
 ...
 */
export const CURRENCIES_BY_CODE: HashMap.HashMap<string & Brand.Brand<"CurrencyCode">, Currency> = HashMap.fromIterable(A.map(
  COMMON_CURRENCIES,
  (currency) => [
    currency.code,
    currency
 ...
 */
export const getCurrencyByCode = (code: CurrencyCode): O.Option<Currency> => HashMap.get(
  CURRENCIES_BY_CODE,
  code
);
