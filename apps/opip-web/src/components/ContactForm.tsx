...
export function ContactForm({
  ...
  return (
    <form
      action="/api/contact"
      className="grid gap-4 rounded-lg border border-[color-mix(in_oklab,var(--opip-on-soil)_22%,transparent)] bg-[color-mix(in_oklab,var(--opip-soil)_30%,transparent)] p-6"
      method="post"
    >
      ...
      <p
        className="text-sm text-[var(--opip-gold-bright)] empty:hidden"
      ...
      {status === "rejected" && (
        <p className="text-sm text-[color-mix(in_oklab,var(--opip-on-soil)_88%,transparent)]" role="status">
