...
const defaultRetryOn: ReadonlyArray<CronRetryOn> = A.make<CronRetryOn>(
  "rate_limit",
  "overloaded",
  "network",
  "timeout",
...
const defaultSessionRetentionEncoded = { hours: 24 };
...
export class CronRetryConfig extends S.Class<CronRetryConfig>($I`CronRetryConfig`)(
  ...
  $I.annote("CronRetryConfig", {
    description: "Retry policy overrides for one-shot cron jobs, including attempts, backoff schedule, and retryable error kinds."
  })
) {}
 ...
 * @since 0.0.0
 */
export class CronFailureDestinationConfig extends S.Class<CronFailureDestinationConfig>($I`CronFailureDestinationConfig`)(
  {
...
export class CronRunLogConfig extends S.Class<CronRunLogConfig>($I`CronRunLogConfig`)(
  {
    ...
    keepLines: SchemaUtils.withKeyDefaults(
...
export class CronConfig extends S.Class<CronConfig>($I`CronConfig`)(
  {
    ...
    maxConcurrentRuns: S.OptionFromOptionalKey(
      S.Int.check(S.isGreaterThanOrEqualTo(1))
    ).annotateKey({
  ...
  $I.annote("CronConfig", {
    description: "Top-level cron scheduler configuration, including retries, retention, webhook fallback, and failure-notification settings."
  })
) {}
