...
const serviceEffect = Effect.gen(function* () {
  ...
  const isPathInsideWithRealpath = (
    ...
    return yield* pipe(
      ...
      Result.match({
        ...
        onSuccess: ([baseReal, candidateReal]) => isPathInside(
          baseReal,
          candidateReal
        )
      })
  ...
  const safeRealpathSync = (filePath: string) => pipe(
    filePath,
    fs.realPath,
    Effect.result,
    Effect.withSpan(
      "ScanPaths.safeRealpathSycn",
      {
        attributes: {
          filePath
        }
    ...
    (
      ...
      return pipe(
        ...
        P.or(
          Eq.equals(""),
          P.every([
            P.not(Str.startsWith(`..${path.sep}`)),
            P.not(Eq.equals("..")),
            P.not(path.isAbsolute)
          ])
        )
      );
    })
  ...
  const extensionUsesSkippedScannerPath = (entry: string): boolean => pipe(
    entry,
    Str.split(/[\\/]+/),
    A.filter(Boolean),
    A.some(
      P.or(
        Eq.equals("node_modules"),
        P.every([
          Str.startsWith("."),
          P.not(Eq.equals(".")),
          P.not(Eq.equals(".."))
