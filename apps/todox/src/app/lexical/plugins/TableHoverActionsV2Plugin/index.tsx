...
function getTableFromMouseEvent(
  ...
  readonly tableElement: HTMLTableElement | null;
} {
...
function TableHoverActionsV2({ anchorElem }: { readonly anchorElem: HTMLElement }): JSX.Element | null {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      if (
        (floatingElemRef.current && event.target instanceof Node && floatingElemRef.current.contains(event.target)) ||
      ...
      const cellSelector = `td${getThemeSelector(getTheme, "tableCell")}, th${getThemeSelector(getTheme, "tableCell")}`;
      const hoveredCell = isHTMLElement(event.target) ? event.target.closest<HTMLTableCellElement>(cellSelector) : null;
  ...
  useEffect(() => {
    const handleMouseLeave = (event: MouseEvent) => {
      ...
      if (nextTarget && leftFloatingElemRef.current && leftFloatingElemRef.current.contains(nextTarget as Node)) {
        return;
      }
      setIsVisible(false);
      setIsLeftVisible(false);
...
export default function TableHoverActionsV2Plugin({
  ...
  return isEditable ? createPortal(<TableHoverActionsV2 anchorElem={anchorElem} />, anchorElem) : null;
}
