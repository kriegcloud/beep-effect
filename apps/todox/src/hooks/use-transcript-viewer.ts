...
function useTranscriptViewer({
  ...
  const guessedDuration = useMemo((): number => {
  ...
  const handleTimeUpdate = useCallback(
    (currentTime: number) => {
      ...
      const currentWord =
        currentWordIndex >= 0 && currentWordIndex < words.length ? words[currentWordIndex] : undefined;
      ...
      if (currentTime >= currentWord.endTime && currentWordIndex + 1 < words.length) {
        while (next + 1 < words.length && currentTime >= (words[next + 1]?.startTime ?? 0)) {
          next++;
        }
        // If we're inside the next word's window, pick it.
        if (currentTime < (words[next]?.endTime ?? 0)) {
  ...
  const currentWord =
    currentWordIndex >= 0 && currentWordIndex < words.length ? (words[currentWordIndex] ?? null) : null;
