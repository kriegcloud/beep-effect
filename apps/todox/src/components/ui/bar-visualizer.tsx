...
export interface AudioAnalyserOptions {
  ...
  readonly minDecibels?: undefined | number;
  readonly maxDecibels?: undefined | number;
}
...
export function useAudioVolume(
  mediaStream?: undefined | MediaStream | null,
...
export interface MultiBandVolumeOptions {
  readonly bands?: undefined | number;
  readonly loPass?: undefined | number; // Low frequency cutoff
  readonly hiPass?: undefined | number; // High frequency cutoff
  readonly updateInterval?: undefined | number; // Update interval in ms
  readonly analyserOptions?: undefined | AudioAnalyserOptions;
}
 ...
 */
export function useMultibandVolume(mediaStream?: undefined | MediaStream | null, options: MultiBandVolumeOptions = {}) {
  const opts = useMemo(
  ...
  const [frequencyBands, setFrequencyBands] = useState<number[]>(() => A.replicate(0, opts.bands ?? 5));
  const bandsRef = useRef<number[]>(A.replicate(0, opts.bands ?? 5));
  const frameId = useRef<number | undefined>(undefined);

  useEffect(() => {
    if (!mediaStream) {
      const emptyBands = A.replicate(0, opts.bands ?? 5);
    ...
    const updateVolume = (timestamp: number) => {
      if (timestamp - lastUpdate >= updateInterval) {
        ...
        const chunks: number[] = A.makeBy(opts.bands!, (i) => {
          let sum = 0;
          let count = 0;
        ...
        for (let i = 0; i < A.length(chunks); i++) {
...
const generateConnectingSequenceBar = (columns: number): number[][] => {
  return A.makeBy(columns, (x) => [x, columns - 1 - x]);
};
...
export interface BarVisualizerProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  readonly state?: undefined | AgentState;
  /** Number of bars to display */
  readonly barCount?: undefined | number;
  /** Audio source */
  readonly mediaStream?: undefined | MediaStream | null;
  ...
  readonly demo?: undefined | boolean;
  /** Align bars from center instead of bottom */
  readonly centerAlign?: undefined | boolean;
}
...
const BarVisualizerComponent = React.forwardRef<HTMLDivElement, BarVisualizerProps>(
  (
    ...
    const fakeVolumeBandsRef = useRef<number[]>(A.replicate(0.2, barCount));
    const [fakeVolumeBands, setFakeVolumeBands] = useState<number[]>(() => A.replicate(0.2, barCount));
    ...
    return (
      <div
        ...
        {...props}
      >
        {A.map(volumeBands, (volume, index) => {
          const heightPct = Math.min(maxHeight, Math.max(minHeight, volume * 100 + 5));
          const isHighlighted = highlightedIndices ? A.contains(highlightedIndices, index) : false;
