...
export function createPushModal<T>({ modals }: CreatePushModalOptions<T>) {
    ...
    useEffect(() => {
      ...
      const popHandler: Handler<EventHandlers["pop"]> = ({ name }) => {
        setState((items) => {
          ...
          return items.map((item) => (match?.key === item.key ? { ...item, closedAt: Date.now(), open: false } : item));
        });
      };
    ...
    return (
      <>
        {state.map((item) => {
          ...
          const Component = "Component" in modal ? modal.Component : (modal as React.ComponentType<unknown>);
  ...
  type GetComponentProps<T> = T extends
    ...
    ? P
    : never;
  type IsObject<T> = Prettify<T> extends Record<number | string | symbol, unknown> ? Prettify<T> : never;
