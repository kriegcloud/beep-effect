...
async function fetchLatestVersions(packages, packageJson) {
  ...
  const versionPromises = packages.map(async (pkg) => {
    ...
    if (version) {
      const currentVersion = packageJson.dependencies[pkg]?.replace(/^\D*/, "") ?? "Not installed";
...
async function main() {
  try {
    ...
    const INCLUDE_PACKAGES = Object.keys(packageJson.dependencies).filter((pkg) => pkg.startsWith("@udecode"));

    // Fetch latest versions in parallel
    const versionMap = await fetchLatestVersions(INCLUDE_PACKAGES, packageJson);

    // Update package.json with new versions
    const updatedPackages = await updatePackageVersions(packageJson, versionMap);
