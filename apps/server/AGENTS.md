...
- Runtime dependencies from `@beep/runtime-server`:
  ...
  - `AppLive` — main layer combining all services (logging, tracing, slices, devtools).
  - Layer composition: `LoggingLive`, `TracingLive`, `DevToolsLive`, `SlicesLive`.
  - Slice infrastructure: `CoreServicesLive`, `SliceDatabaseClientsLive`, `SliceReposLive` for IAM/documents.
