...
const InlineCombobox = ({
  ...
  const { props: inputProps, removeInput } = useComboboxInput({
    ...
    onCancelInput: (cause) => {
      if (cause !== "backspace") {
        const atValue = insertPoint?.current;
        if (atValue !== null && atValue !== undefined) {
          editor.tf.insertText(trigger + value, { at: atValue });
        } else {
...
const InlineComboboxInput = ({
  ...
  ref?: undefined | React.RefObject<HTMLInputElement | null>;
}) => {
  ...
  // Filter out undefined values from inputProps and props to satisfy exactOptionalPropertyTypes
  const filteredInputProps = Object.fromEntries(Object.entries(inputProps).filter(([, value]) => value !== undefined));
  const filteredProps = Object.fromEntries(Object.entries(props).filter(([, value]) => value !== undefined));
