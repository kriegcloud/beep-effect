      ...
      <div className="relative">
        <svg width={config.circle} height={config.circle} className="-rotate-90">
          ...
          <motion.circle
            ...
            className={cn(
              "stroke-current",
              progressColor === "usage" ? getUsageClasses(percentage, "circle") : "text-primary"
            )}
  ...
  return (
    <div className="bg-muted/20 space-y-2 rounded-xl p-4">
      <div className="flex items-center justify-between">
        ...
        <motion.span className="text-muted-foreground text-xs">{display}</motion.span>
      </div>
      <div className={cn("bg-muted w-full overflow-hidden rounded-full", config.bar)}>
        <motion.div
          className={cn(
            "rounded-full bg-gradient-to-r",
            config.bar,
            progressColor === "usage" ? getUsageClasses(percentage, "linear") : "from-primary to-primary/70"
          )}
          initial={{ width: 0 }}
      ...
      <Card className={cn("w-auto", className)}>
        {(title || description) && (
          <CardHeader className="space-y-1">
            {title && <CardTitle className="truncate text-base leading-tight font-medium">{title}</CardTitle>}
