  ...
  const getPercentageBar = (percentage: number) => {
    ...
    return (
      <div className="flex min-w-[120px] items-center gap-2">
        ...
        <span className="w-10 text-right text-xs font-medium tabular-nums">{Math.round(percentage)}%</span>
      </div>
    );
  };
            ...
            <TableBody>
              {resources.length === 0 ? (
                ...
                resources.map((resource, index) => {
                  const percentage =
                    resource.percentage ?? (resource.limit > 0 ? (resource.used / resource.limit) * 100 : 0);
