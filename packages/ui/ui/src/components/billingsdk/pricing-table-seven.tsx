...
export function PricingTableSeven({
  ...
  const renderFeatureValue = (value: boolean | string | undefined) => {
    if (typeof value === "boolean") {
      return value ? <Check className="text-primary h-5 w-5" /> : <span className="text-muted-foreground">—</span>;
    }
    ...
    <section className={cn(sectionVariants({ size, theme }), className)}>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="pb-1 text-center">
          <h1 className={cn(titleVariants({ size, theme }), "leading-[1.12]")}>{title}</h1>
        ...
        <div className="mt-16">
          ...
          <div
            className={cn(
              ...
              theme === "classic" ? "bg-card/50 border-border/50 shadow-md backdrop-blur-sm" : "bg-card"
            )}
          >
            <AnimatePresence>
              {features.map((category, categoryIndex) => (
                ...
                >
                  ...
                  {/* Category Header */}
                  <div className={cn("px-4 py-4 sm:px-6", theme === "classic" ? "bg-muted/30" : "bg-muted/50")}>
                    <h3 className="text-foreground text-sm font-semibold">{category.category}</h3>
                  </div>
                  ...
                  {category.items.map((feature, featureIndex) => (
                    <div
                      key={feature.name}
                      className={cn("px-4 py-4 sm:px-6", featureIndex > 0 && "border-border/50 border-t")}
                    >
                      {/* Mobile Layout */}
                      <div className="block sm:hidden">
                        <div className="mb-3 flex items-center space-x-2">
                          <span className="text-foreground text-sm font-medium">{feature.name}</span>
                          {feature.tooltip && <Info className="text-muted-foreground h-4 w-4" />}
                        </div>
                        <div className="space-y-2">
                          {plans.map((plan) => (
                            <div key={plan.id} className="flex items-center justify-between">
                              <span className="text-muted-foreground text-sm">{plan.name}</span>
                              <div className="flex items-center">{renderFeatureValue(feature[plan.id])}</div>
                            </div>
                          ))}
                        </div>
                      </div>
