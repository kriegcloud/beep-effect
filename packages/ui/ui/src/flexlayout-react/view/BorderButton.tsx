...
export const BorderButton = (props: IBorderButtonProps) => {
  ...
  const isClosable = () => {
    ...
    if (selected || closeType === ICloseType.DecodedEnum.Always) {
      return true;
    }
    if (closeType === ICloseType.DecodedEnum.Visible) {
  ...
  let iconAngle = 0;
  if (!node.getModel().isEnableRotateBorderIcons()) {
    if (border === "left") {
      iconAngle = 90;
  ...
  if (node.isEnableClose()) {
    const closeTitle = layout.i18nName(I18nLabel.DecodedEnum.Close_Tab);
