...
export const BorderTabSet = (props: IBorderTabSetProps) => {
  ...
  const onOverflowClick = (event: React.MouseEvent<HTMLElement, MouseEvent>) => {
    ...
    if (callback !== undefined) {
      ...
      showPopup(element, border, items, onOverflowItemSelect, layout);
    }
    event.stopPropagation();
  };
  ...
  const tabButtons: UnsafeTypes.UnsafeAny = [];
  ...
  const layoutTab = (i: number) => {
    ...
    if (i < border.getChildren().length - 1) {
      tabButtons.push(<div key={`divider${i}`} className={cm(CLASSES.FLEXLAYOUT__BORDER_TAB_DIVIDER)} />);
    }
  };
  ...
  let buttons: UnsafeTypes.UnsafeAny[] = [];
  let stickyButtons: UnsafeTypes.UnsafeAny[] = [];
  ...
  if (border.isEnableTabScrollbar()) {
    miniScrollbar = (
      <div ref={miniScrollRef} className={cm(CLASSES.FLEXLAYOUT__MINI_SCROLLBAR)} onPointerDown={onScrollPointerDown} />
    );
  }
