...
export interface ILayoutProps {
  ...
  popoutClassName?: undefined | string;
  ...
  onAction?: undefined | ((action: Action) => Action | undefined);
  ...
  onModelChange?: undefined | ((model: Model, action: Action) => void);
  ...
  classNameMapper?: undefined | ((defaultClassName: string) => string);
  ...
  i18nMapper?: undefined | ((id: I18nLabel.Type, param?: undefined | string) => string | undefined);
...
interface ILayoutInternalProps extends ILayoutProps {
  ...
  windowId?: undefined | string;
  mainLayout?: undefined | LayoutInternal;
}

/** @internal */
interface ILayoutInternalState {
  rect: Rect;
  editingTab?: undefined | TabNode;
  portal?: undefined | React.ReactPortal;
  showEdges: boolean;
  showOverlay: boolean;
...
export class LayoutInternal extends React.Component<ILayoutInternalProps, ILayoutInternalState> {
  ...
  private readonly findBorderBarSizeRef: React.RefObject<HTMLDivElement | null>;
  private readonly mainRef: React.RefObject<HTMLDivElement | null>;
  private previousModel?: undefined | Model;
  private readonly orderedTabIds: string[];
  private readonly orderedTabMoveableIds: string[];
  private moveableElementMap = new Map<string, HTMLElement>();
  private dropInfo: DropInfo | undefined;
  private outlineDiv?: undefined | HTMLElement;
  private currentDocument?: undefined | Document;
  private currentWindow?: undefined | Window;
  private readonly supportsPopout: boolean;
  private readonly popoutURL: string;
  private readonly icons: IIcons;
  ...
  private readonly mainLayout: LayoutInternal;
  ...
  private readonly popoutWindowName: string;
      ...
      for (const [_, location] of DockLocation.values) {
        ...
        if (showBorder && border) {
          borderSetComponents.set(
            location,
            <BorderTabSet layout={this} border={border} size={this.state.calculatedBorderBarSize} />
          );
  ...
  setEditingTab(tabNode?: undefined | TabNode) {
  ...
  getScreenRect(inRect: Rect) {
    ...
    rect.y =
      this.currentWindow!.screenY + this.currentWindow!.scrollY + (navHeight - navWidth / 2) + layoutRect.y + rect.y;
  ...
  i18nName(id: I18nLabel.Type, param?: undefined | string) {
    let message: string | undefined = undefined;
    if (this.props.i18nMapper) {
      message = this.props.i18nMapper(id, param) ?? "";
    }
    if (P.isUndefined(message)) {
      message = id + (P.isUndefined(param) ? "" : param);
    }
    return message;
  }
  ...
  addTabWithDragAndDrop(
    ...
    LayoutInternal.dragState = new DragState(this.mainLayout, DragSource.DecodedEnum.Add, tempNode, json, onDrop);
  }
  ...
  public setDragNode = (event: DragEvent, node: Node & IDraggable) => {
    LayoutInternal.dragState = new DragState(
      this.mainLayout,
      DragSource.DecodedEnum.Internal,
      node,
      undefined,
      undefined
    );
    ...
    if (node instanceof TabSetNode) {
      let rendered = false;
      let content = this.i18nName(I18nLabel.DecodedEnum.Move_Tabset);
      if (node.getChildren().length > 0) {
        content = this.i18nName(I18nLabel.DecodedEnum.Move_Tabs).replace("?", String(node.getChildren().length));
      }
      ...
      const isInVerticalBorder =
        parentNode instanceof BorderNode && (parentNode as BorderNode).getOrientation() === Orientation.HORZ;
  ...
  onDragEnter = (event: React.DragEvent<HTMLElement>) => {
    ...
    if (!LayoutInternal.dragState && this.props.onExternalDrag) {
      ...
      if (externalDrag) {
        ...
        LayoutInternal.dragState = new DragState(
          this.mainLayout,
          DragSource.DecodedEnum.External,
          tempNode,
          externalDrag.json,
          externalDrag.onDrop
        );
  ...
  onDragOver = (event: React.DragEvent<HTMLElement>) => {
    if (this.dragging && !this.isDraggingOverWindow) {
      ...
      const dropInfo = this.props.model.findDropTargetNode(
        this.windowId,
        LayoutInternal.dragState!.dragNode!,
        pos.x,
        pos.y
      );
      if (dropInfo) {
      ...
      if (this.dropInfo) {
        if (dragState.dragJson !== undefined) {
          const newNode = this.doAction(
            Actions.addNode(dragState.dragJson, this.dropInfo.node.getId(), this.dropInfo.location, this.dropInfo.index)
          );
...
export declare namespace DragSource {
  export type Type = typeof DragSource.Type;
  export type Encoded = typeof DragSource.Encoded;
}

export class DragSource extends BS.MappedLiteralKit(
  ["Internal", "internal"],
  ["External", "external"],
  ["Add", "add"]
) {}
