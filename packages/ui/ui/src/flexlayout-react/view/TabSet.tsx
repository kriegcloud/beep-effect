...
export const TabSet = (props: ITabSetProps) => {
  ...
  React.useLayoutEffect(() => {
    ...
    if (!node.getContentRect().equals(newContentRect) && !Number.isNaN(newContentRect.x)) {
      node.setContentRect(newContentRect);
      if (splitterDragging) {
        // next movement will draw tabs again, only redraw after pause/end
        if (timer.current) {
  ...
  const onOverflowClick = (event: React.MouseEvent<HTMLElement, MouseEvent>) => {
    ...
    const items = hiddenTabs.map((h) => {
      return { index: h, node: node.getChildren()[h] as TabNode };
    });
  ...
  const onCloseTab = (event: React.MouseEvent<HTMLElement, MouseEvent>) => {
    F.pipe(
      node.getChildren(),
      A.get(0),
      O.map((child) => (child as TabNode).getId()),
      O.map((id) => layout.doAction(Actions.deleteTab(id)))
    );
    event.stopPropagation();
  };
  ...
  if (node.isEnableTabStrip()) {
    for (let i = 0; i < node.getChildren().length; i++) {
      ...
      tabs.push(
        <TabButton layout={layout} node={child} path={`${path}/tb${i}`} key={child.getId()} selected={isSelected} />
      );
      if (i < node.getChildren().length - 1) {
        tabs.push(<div key={`divider${i}`} className={cm(CLASSES.FLEXLAYOUT__TABSET_TAB_DIVIDER)} />);
      }
    }
  }
  ...
  if (!node.isEnableTabWrap()) {
    if (isShowHiddenTabs) {
      const overflowTitle = layout.i18nName(I18nLabel.DecodedEnum.Overflow_Menu_Tooltip);
      let overflowContent: React.ReactNode;
      if (typeof icons.more === "function") {
        ...
        overflowContent = (
          <>
            {icons.more}
            <div className={cm(CLASSES.FLEXLAYOUT__TAB_BUTTON_OVERFLOW_COUNT)}>
  ...
  if (
    ...
    const popoutTitle = layout.i18nName(I18nLabel.DecodedEnum.Popout_Tab);
    buttons.push(
      <button
        type={"button"}
        key="popout"
  ...
  if (node.canMaximize()) {
    const minTitle = layout.i18nName(I18nLabel.DecodedEnum.Restore);
    const maxTitle = layout.i18nName(I18nLabel.DecodedEnum.Maximize);
    buttons.push(
      <button
        key="max"
        type={"button"}
  ...
  if (!node.isMaximized() && showClose) {
    const title = isTabStretch
      ? layout.i18nName(I18nLabel.DecodedEnum.Close_Tab)
      : layout.i18nName(I18nLabel.DecodedEnum.Close_Tabset);
    buttons.push(
      <button
        key="close"
        type={"button"}
  ...
  if (node.isActive() && node.isEnableActiveIcon()) {
    const title = layout.i18nName(I18nLabel.DecodedEnum.Active_Tabset);
    buttons.push(
      <div
        key="active"
  ...
  let tabStrip: React.ReactElement | undefined = undefined;
  ...
  const style: Record<string, UnsafeTypes.UnsafeAny> = {
