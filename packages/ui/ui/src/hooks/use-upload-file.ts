import type { OurFileRouter } from "@beep/ui/lib/uploadthing";
...
interface UseUploadFileProps
  ...
  > {
  readonly onUploadComplete?: undefined | ((file: UploadedFile) => void);
  readonly onUploadError?: undefined | ((error: unknown) => void);
}
...
export function getErrorMessage(err: unknown) {
  ...
  if (err instanceof z.ZodError) {
    const errors = A.map(err.issues, (issue) => issue.message);
