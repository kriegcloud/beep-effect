...
const formatPeerMessage = (pm: PeerMessage): string =>
  Match.value(pm).pipe(
    Match.when({ type: "created" }, (p) =>
      `${p.userId} created: ${p.node.$.syncId}|previousId: ${p.previousId}|parentId: ${p.parentId}|streamId: ${p.streamId}`
    ),
    Match.when({ type: "destroyed" }, (p) =>
      `${p.userId} destroyed: ${p.node.$.syncId}|streamId: ${p.streamId}`
    ),
    Match.when({ type: "updated" }, (p) =>
      `${p.userId} updated: ${p.node.$.syncId}|streamId: ${p.streamId}`
    ),
    Match.when({ type: "cursor" }, (p) =>
