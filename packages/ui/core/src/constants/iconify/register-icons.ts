...
export const iconSets = A.reduce(Struct.entries(allIcons), [] as IconifyJSON[], (acc, [k, v]) => {
  const parts = Str.split(":")(k);
  const prefixOption = O.fromNullable(parts[0]);
  const iconNameOption = O.fromNullable(parts[1]);
  const existingPrefixOption = A.findFirst(acc, (item) => O.isSome(prefixOption) && item.prefix === prefixOption.value);
