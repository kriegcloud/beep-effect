...
/**
 * Typed runtime error emitted during VT2 bootstrap and request handling.
 *
 * @since 0.0.0
 * @category DomainModel
 */
export class Vt2RuntimeError extends TaggedErrorClass<Vt2RuntimeError>($I`Vt2RuntimeError`)(
  ...
  $I.annote("Vt2RuntimeError", {
    description: "Typed error for VT2 runtime bootstrap and control-plane boundaries.",
  })
) {}
...
const buildTranscriptProjection = (
  ...
  if (discardedCount > 0) {
    return new Vt2Transcript({
      sessionId,
      status: "failed",
      excerpt: O.some(NonEmptyTrimmedStr.make("Captured media was discarded before downstream intake.")),
      language: O.none(),
      wordCount: NonNegativeInt.make(0),
    });
  }
...
const buildTranscriptFailureProjection = (sessionId: UUID, message: string): Vt2Transcript =>
  new Vt2Transcript({
    sessionId,
    status: "failed",
    excerpt: O.some(NonEmptyTrimmedStr.make(normalizeTranscriptText(message))),
    language: O.none(),
    wordCount: NonNegativeInt.make(0),
  });
...
const localTranscriptProviderLayer = (config: Vt2RuntimeConfig) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const resolvePythonCommand = Effect.fn("Vt2TranscriptProvider.resolvePythonCommand")(function* () {
        if (O.isSome(config.transcriptPythonBin)) {
          return config.transcriptPythonBin.value;
        }
        ...
        return bundledPythonExists ? bundledPython : "python3";
      });
      const transcribe = Effect.fn("Vt2TranscriptProvider.transcribe")(function* (session: Vt2SessionResource) {
        ...
        const pythonCommand = yield* resolvePythonCommand();
        ...
        return new Vt2Transcript({
          sessionId: session.session.id,
          status: "ready",
          excerpt: O.some(NonEmptyTrimmedStr.make(normalizedText)),
          language: normalizeTranscriptLanguage(decoded.language),
          wordCount: countTranscriptWords(normalizedText),
        });
      });

      return Vt2TranscriptProvider.of({ transcribe });
    })
  );
...
const makeVt2Store = Effect.fn("Vt2Store.make")(function* (config: Vt2RuntimeConfig, startedAt: DateTime.Utc) {
  ...
  const initializeTables = Effect.fn("Vt2Store.initializeTables")(function* (): Effect.fn.Return<void, Vt2RuntimeError> {
    yield* sql`PRAGMA busy_timeout = 5000`.pipe(
      Effect.mapError(toRuntimeError("Failed to configure the VT2 sqlite busy timeout.", 500))
    );
    ...
    `.pipe(Effect.mapError(toRuntimeError("Failed to create the VT2 sessions table.", 500)));

    const sessionColumns = yield* sql<{ readonly name: string }>`PRAGMA table_info(${sessionsTable})`.pipe(
      Effect.mapError(toRuntimeError("Failed to inspect the VT2 sessions table columns.", 500))
    );
    ...
    if (!pipe(sessionColumnNames, A.contains("memory_context_packets_json"))) {
      yield* sql`ALTER TABLE ${sessionsTable} ADD COLUMN memory_context_packets_json TEXT NOT NULL DEFAULT '[]'`.pipe(
        Effect.mapError(toRuntimeError("Failed to migrate the VT2 memory context packets column.", 500))
      );
    }
    ...
    if (!pipe(sessionColumnNames, A.contains("composition_runs_json"))) {
      yield* sql`ALTER TABLE ${sessionsTable} ADD COLUMN composition_runs_json TEXT NOT NULL DEFAULT '[]'`.pipe(
        Effect.mapError(toRuntimeError("Failed to migrate the VT2 composition runs column.", 500))
      );
    }
    ...
    `.pipe(Effect.mapError(toRuntimeError("Failed to create the VT2 desktop preferences table.", 500)));

    const encodedPreferences = yield* encodePreferencesJson(defaultVt2DesktopPreferences()).pipe(
      Effect.mapError(toRuntimeError("Failed to encode the default VT2 desktop preferences.", 500))
    );
    ...
    `.pipe(Effect.mapError(toRuntimeError("Failed to seed the VT2 desktop preferences row.", 500)));
  });
  ...
  const loadPreferences = Effect.gen(function* () {
    const rows = yield* sql<Vt2DesktopPreferencesRow>`
      ...
      WHERE id = 1
      LIMIT 1
    `.pipe(Effect.mapError(toRuntimeError("Failed to load the VT2 desktop preferences.", 500)));

    const row = yield* O.match(A.head(rows), {
      onNone: () => Effect.fail(new Vt2RuntimeError({ message: "VT2 desktop preferences were not found.", status: 404, cause: O.none() })),
      onSome: (found) =>
        decodePreferencesRow(found).pipe(
          Effect.mapError(toRuntimeError("Failed to decode the persisted VT2 desktop preferences row.", 500))
        ),
    });

    return yield* decodePreferencesJson(row.preferences_json).pipe(
      Effect.mapError(toRuntimeError("Failed to decode the persisted VT2 desktop preferences.", 500))
    );
  });
  ...
  const savePreferences = Effect.fn("Vt2Store.savePreferences")(function* (
    ...
    const encodedPreferences = yield* encodePreferencesJson(nextPreferences).pipe(
      Effect.mapError(toRuntimeError("Failed to encode the updated VT2 desktop preferences.", 500))
    );
    ...
    yield* sql`
      ...
      WHERE id = 1
    `.pipe(Effect.mapError(toRuntimeError("Failed to persist the VT2 desktop preferences.", 500)));
...
const emitBootstrapStdoutLine = Effect.fn("Vt2Runtime.emitBootstrapStdoutLine")(function* (
  ...
  ).pipe(Effect.mapError(toRuntimeError("Failed to encode the VT2 bootstrap line.", 500)));
...
const launchVt2Sidecar = (config: Vt2RuntimeConfig, startedAt: DateTime.Utc) =>
  Layer.launch(Layer.fresh(sidecarLayer(config, startedAt))).pipe(
    Effect.mapError(toRuntimeError("Failed to launch the VT2 sidecar.", 500))
  );
