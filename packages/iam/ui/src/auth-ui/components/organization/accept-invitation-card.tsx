...
function AcceptInvitationContent({
  ...
  useEffect(() => {
    ...
    if (invitation.status !== "pending" || DateTime.toDate(invitation.expiresAt) < new Date()) {
      toast({
        variant: "error",
        message:
          DateTime.toDate(invitation.expiresAt) < new Date()
      ...
      <CardContent className={cn("flex flex-col gap-6 truncate", classNames?.content)}>
        <Card className={cn("flex-row items-center p-4")}>
          <OrganizationCellView
            organization={{
              id: O.isSome(invitation.organizationId) ? invitation.organizationId.value : "",
...
const AcceptInvitationSkeleton = ({ className, classNames, localization }: AcceptInvitationCardProps) => {
  return (
    <Card className={cn("w-full max-w-sm", className, classNames?.base)}>
      ...
      <CardContent className={cn("flex flex-col gap-6 truncate", classNames?.content)}>
        <Card className={cn("flex-row items-center p-4")}>
          <OrganizationCellView isPending localization={localization} />
