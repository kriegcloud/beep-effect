          ...
          {trigger ||
            (size === "icon" ? (
              <Button
                ...
                {...props}
              >
                {isPending || activeOrganization || !sessionData || (user as User)?.isAnonymous || hidePersonal ? (
                  <OrganizationLogo
              ...
              <Button className={cn("!p-2 h-fit", className, classNames?.trigger?.base)} size={size} {...props}>
                {isPending || activeOrganization || !sessionData || (user as User)?.isAnonymous || hidePersonal ? (
        ...
        >
          ...
          {organizations?.map(
            (organization) =>
              ...
              (pathMode === "slug" ? (
                <Link key={organization.id} href={`${organizationOptions?.basePath}/${organization.slug}`}>
                  <DropdownMenuItem>
                    <OrganizationCellView
                      classNames={classNames?.content?.organization}
                      isPending={isPending}
                ...
                <DropdownMenuItem key={organization.id} onClick={() => switchOrganization(organization)}>
                  <OrganizationCellView
                    classNames={classNames?.content?.organization}
                    isPending={isPending}
                    localization={localization}
