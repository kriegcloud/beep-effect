...
export function AccountView({
  ...
  return (
    <div className={cn("flex w-full grow flex-col gap-4 md:flex-row md:gap-12", className, classNames?.base)}>
          ...
          <Drawer>
            ...
            <DrawerContent>
              ...
              <div className="flex flex-col px-4 pb-4">
                {navItems.map((item) => (
                  <Link key={item.view} href={`${accountOptions?.basePath}/${accountOptions?.viewPaths[item.view]}`}>
                    <Button
                      size="lg"
                      className={cn(
      ...
      {view === "API_KEYS" && <ApiKeysCard classNames={classNames?.card} localization={localization} />}
