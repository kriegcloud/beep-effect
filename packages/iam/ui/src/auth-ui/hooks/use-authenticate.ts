...
interface AuthenticateOptions<TAuthClient extends AnyAuthClient> {
  readonly authClient?: TAuthClient | undefined;
  readonly authView?: AuthViewPath | undefined;
...
export function useAuthenticate<TAuthClient extends AnyAuthClient>(options?: AuthenticateOptions<TAuthClient>) {
  ...
  useEffect(() => {
    ...
    const redirectTo =
      currentUrl.searchParams.get("redirectTo") || window.location.href.replace(window.location.origin, "");
