import { callBetterAuth } from "@beep/iam-sdk/better-auth/handler";
...
describe("callBetterAuth", () => {
  it("returns response data when executor succeeds", () =>
    Effect.gen(function* () {
      const effect = callBetterAuth({ plugin: "test", method: "demo" }, async (_signal) => ({ data: "ok" }));
  ...
  it("normalizes Better Auth error payloads", () =>
    Effect.gen(function* () {
      const effect = callBetterAuth(
        { plugin: "test", method: "demo", defaultErrorMessage: "failed" },
        async (_signal) => ({
          error: {
            message: "nope",
            code: "BAD",
  ...
  it("records metrics when configured", () =>
    Effect.gen(function* () {
      const successCounter = Metric.counter("callBetterAuth_success");
      const errorCounter = Metric.counter("callBetterAuth_error");
      const latencyHistogram = Metric.histogram(
        "callBetterAuth_latency",
  ...
  it("wraps thrown executor errors", () =>
    Effect.gen(function* () {
      const effect = callBetterAuth({ plugin: "test", method: "throws" }, async (_signal) => {
  ...
  it("guards concurrent submissions when semaphoreKey is provided", () =>
    Effect.gen(function* () {
      ...
      const guarded = callBetterAuth({ plugin: "test", method: "guarded", semaphoreKey: "same" }, async (_signal) => {
        running++;
  ...
  it("allows concurrent submissions for different semaphore keys", () =>
    Effect.gen(function* () {
      ...
      const makeEffect = (key: string) =>
        callBetterAuth({ plugin: "test", method: key, semaphoreKey: key }, async (_signal) => {
          running++;
          if (running > maxRunning) {
            maxRunning = running;
          }
