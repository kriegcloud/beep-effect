import {
  ...
  withBetterAuthSpan,
  withSpanAndMetrics,
} from "@beep/iam-sdk/better-auth/instrumentation";
...
describe("better-auth instrumentation", () => {
  it("annotates logs with provided values", () =>
    Effect.gen(function* () {
      const annotations = yield* annotateBetterAuthLogs({ values: { foo: "bar" } })(Effect.logAnnotations);
  ...
  it("creates a span with the expected name", () =>
    withBetterAuthSpan({ spanName: "better-auth:test" })(
