...
import {Contract, FailureContinuation, FailureContinuationHandlers, makeFailureContinuation} from "../../contract-kit";
import * as P from "effect/Predicate";
import * as AST from "effect/SchemaAST";
import {Any} from "../../contract-kit/Contract";
import {invariant} from "@beep/invariant";
import * as ParseResult from "effect/ParseResult";
import * as Either from "effect/Either";
import Bool from "effect/Boolean";
import * as Effect from "effect/Effect";
import * as S from "effect/Schema";
import * as Data from "effect/Data";
import {IamError} from "../../errors";
import {withFetchOptions} from "./client-method-helpers";

type MakeHandlerParams<
  ContractSchema extends Contract.Any
> = {
  readonly contract: ContractSchema;
  readonly effect: <R>(payload: ContractSchema["parametersSchema"]["Type"]) =>
...
export type All =
  | Any
  | S.Schema<any, never, unknown>
  | S.Schema<never, any, unknown>
  | S.Schema<never, never, unknown>

export type Refine<T> = T extends S.Schema<infer A, infer E, infer R> ? R extends never ?
  A extends unknown ?
    E extends unknown ?
      R extends never ?
...
export const make = <
  const ContractSchema extends Contract.Any
>(
  {
    contract,
    effect,
  ...
  const mapSchemaToValidator = <Schema extends S.Schema.All>(schema: NoInfer<Schema>) => {
    ...
    type Validator =
      ...
      readonly validator: (input: unknown, overrideOptions?: undefined | AST.ParseOptions) =>
        Effect.Effect<ContractSchema["failureSchema"]["Type"], ParseResult.ParseError, never>
    }, MakeHandlerError, never>
      | Effect.Effect<{
      ...
      readonly validator: (input: unknown, overrideOptions?: undefined | AST.ParseOptions) =>
        Effect.Effect<
          Either.Either<
            ...
            ContractSchema["successSchema"]["Type"]
          >,
    ...
    return Bool.match(S.isSchema(schema), {
      onFalse: () => Effect.fail(new MakeHandlerError({
        cause: schema,
        message: "Schema is not a schema without context",
      })),
      onTrue: () => Effect.succeed(Match.value(contract.failureMode).pipe(
        Match.withReturnType<Validator>(),
        Match.when("error", () => Effect.succeed({
          failureMode: "error" as const,
          validator: (input: unknown, overrideOptions?: undefined | AST.ParseOptions) => F.pipe(
            ((input: unknown, overrideOptions?: undefined | AST.ParseOptions): Effect.Effect<ContractSchema["failureSchema"]["Type"], ParseResult.ParseError, never> =>
              S.decodeUnknown(schema)(input, overrideOptions) as Effect.Effect<ContractSchema["failureSchema"]["Type"], ParseResult.ParseError, never>),
            (decodeFn) => decodeFn(input, overrideOptions)
          ),
        } as const)),
        Match.when("return", () => Effect.succeed({
          failureMode: "return" as const,
          validator: (
            input: unknown,
            overrideOptions?: undefined | AST.ParseOptions
          ) => Effect.succeed(
            S.decodeUnknownEither(schema)(input, overrideOptions)),
        } as const)),
        Match.orElse(() => Effect.fail(new MakeHandlerError({
          cause: schema,
  ...
  const handleError = (e: unknown) =>
    S.isSchema(contract.failureSchema) && P.not(
      P.and(
        AST.isNeverKeyword,
        AST.isUndefinedKeyword,
      )
    )
      ? Effect.fail(new MakeHandlerError({
        cause: e,
  ...
  const validatorFn =
    F.flow(
      (payload: ContractSchema["parametersSchema"]["Type"]) =>
        S.decode(contract.parametersSchema)(payload).pipe(
      ...
      Effect.flatMap((resultEffect) => resultEffect.pipe(
        Effect.catchTags({
  ...
  return <R>(
    ...
    return F.pipe(
      payload,
      S.decode(contract.parametersSchema),
      validatorFn,
      Effect.match({
