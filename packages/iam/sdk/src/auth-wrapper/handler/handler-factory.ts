...
import type { RetryPolicy, TimeoutPolicy } from "./call-auth";
import { callAuth } from "./call-auth";
...
export interface AuthToastConfig<Output, ExtraArgs extends Array<unknown>> {
...
type AuthToastInput<Output, ExtraArgs extends Array<unknown>> =
  | AuthToastConfig<Output, ExtraArgs>
  | ToastMessages<Output, IamError, ExtraArgs>;

interface CommonAuthHandlerConfig<Input, Encoded, Output, ExtraArgs extends Array<unknown>, OnSuccessR> {
  readonly name: string;
  readonly plugin: string;
  readonly method: string;
  readonly run: (encoded: Encoded, signal: AbortSignal) => Promise<AuthErrorPayload | { readonly data: Output }>;
  readonly toast?: AuthToastInput<Output, ExtraArgs>;
...
interface HandlerWithSchema<Input, Encoded, SchemaR, Output, ExtraArgs extends Array<unknown>, OnSuccessR>
  extends CommonAuthHandlerConfig<Input, Encoded, Output, ExtraArgs, OnSuccessR> {
...
interface IdentityHandler<Input, Output, ExtraArgs extends Array<unknown>, OnSuccessR>
  extends CommonAuthHandlerConfig<Input, Input, Output, ExtraArgs, OnSuccessR> {
...
export type AuthHandler<Input, ExtraArgs extends Array<unknown> = [], R = never> = (
  input: Input,
  ...extra: ExtraArgs
...
type AnyConfig<
  ...
  PrepareR,
  OnSuccessR,
> = AuthHandlerConfig<Input, Encoded, SchemaR, Output, ExtraArgs, PrepareR, OnSuccessR>;
...
const toIamSchemaError = (error: unknown, plugin: string, method: string): IamError =>
  new IamErrorConstructor(error, `Failed to prepare Auth handler input for ${plugin}.${method}`, {
    plugin,
    method,
  });

const isToastConfig = <Output, ExtraArgs extends Array<unknown>>(
  toast: AuthToastInput<Output, ExtraArgs>
): toast is AuthToastConfig<Output, ExtraArgs> => "defaults" in toast;

const normalizeToastConfig = <Output, ExtraArgs extends Array<unknown>>(
  toast: AuthToastInput<Output, ExtraArgs> | undefined
): AuthToastConfig<Output, ExtraArgs> | undefined => {
  if (!toast) {
    return undefined;
  }
...
const createHandlerEffect =
  <Input, Encoded, Output, ExtraArgs extends Array<unknown>, EncodeR, OnSuccessR>(
    config: CommonAuthHandlerConfig<Input, Encoded, Output, ExtraArgs, OnSuccessR>,
    encode: (input: Input) => Effect.Effect<Encoded, IamError, EncodeR>
  ) =>
  (callContext: Parameters<typeof callAuth>[0]) =>
    function* (input: Input, ...extra: ExtraArgs) {
      const baseEffect: Effect.Effect<Output, IamError, EncodeR | OnSuccessR> = encode(input).pipe(
        Effect.flatMap((encoded) => callAuth<Output>(callContext, (signal) => config.run(encoded, signal))),
...
export namespace AuthHandler {
  export const make = <
    ...
    OnSuccessR = never,
  >(
    config: AuthHandlerConfig<Input, Encoded, SchemaR, Output, ExtraArgs, PrepareR, OnSuccessR>
  ): AuthHandler<Input, ExtraArgs, SchemaR | PrepareR | OnSuccessR> => {
    ...
    const buildHandler = <TargetEncoded, EncodeR>(
      targetConfig: CommonAuthHandlerConfig<Input, TargetEncoded, Output, ExtraArgs, OnSuccessR>,
      encode: (input: Input) => Effect.Effect<TargetEncoded, IamError, EncodeR>
    ): AuthHandler<Input, ExtraArgs, EncodeR | OnSuccessR> => {
