...
export interface Contractkit<in out Contracts extends Record<string, Contract.Any>> extends
  Effect.Effect<
    WithHandler<Contracts>,
    never,
    Contract.HandlersFor<Contracts>
  >,
  Inspectable,
  Pipeable
{
...
export type ContractsByName<Contracts> = Contracts extends Record<string, Contract.Any> ?
  ...
  : Contracts extends ReadonlyArray<Contract.Any> ? { readonly [Contract in Contracts[number] as Contract["name"]]: Contract }
  : never
...
const Proto = {
  ...
  toContext(
    ...
    build: Record<string, (params: UnsafeTypes.UnsafeAny) => UnsafeTypes.UnsafeAny> | Effect.Effect<Record<string, (params: UnsafeTypes.UnsafeAny) => UnsafeTypes.UnsafeAny>>
  ) {
  ...
  commit(this: Contractkit<Record<string, Contract.Any>>) {
    return Effect.gen(this, function*() {
      ...
      const context = yield* Effect.context<never>()
      const schemasCache = new WeakMap<UnsafeTypes.UnsafeAny, {
        readonly context: Context.Context<never>
        readonly handler: (params: UnsafeTypes.UnsafeAny) => Effect.Effect<UnsafeTypes.UnsafeAny, UnsafeTypes.UnsafeAny>
        readonly decodeParameters: (u: unknown) => Effect.Effect<Contract.Parameters<UnsafeTypes.UnsafeAny>, ParseError>
      ...
      const handle = Effect.fn("Contractkit.handle", { captureStackTrace: false })(
...
const makeProto = <Contracts extends Record<string, Contract.Any>>(contracts: Contracts): Contractkit<Contracts> =>
  Object.assign(function() {}, Proto, { contracts }) as UnsafeTypes.UnsafeAny
...
const resolveInput = <Contracts extends ReadonlyArray<Contract.Any>>(
  ...
  for (const tool of contracts) {
    output[tool.name] = (Schema.isSchema(tool) ? Contract.fromTaggedRequest(tool as UnsafeTypes.UnsafeAny) : tool) as UnsafeTypes.UnsafeAny
  }
  return output
}
