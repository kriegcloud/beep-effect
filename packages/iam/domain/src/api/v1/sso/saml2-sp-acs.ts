/**
 * @fileoverview SAML2 Service Provider Assertion Consumer Service endpoint contract.
 *
 * Handles SAML2 assertions from identity providers at the SP ACS endpoint.
...
const $I = $IamDomainId.create("api/v1/sso/saml2-sp-acs");

/**
 * Path parameters for SAML2 SP ACS.
 *
 * @since 1.0.0
 */
export class PathParams extends S.Class<PathParams>($I`PathParams`)(
  ...
  $I.annotations("SAML2SpAcsPathParams", {
    description: "Path parameters for SAML2 SP ACS endpoint.",
  })
) {}

/**
 * Request payload for SAML2 SP ACS.
 *
...
export class Payload extends S.Class<Payload>($I`Payload`)(
  ...
  $I.annotations("SAML2SpAcsPayload", {
    description: "Payload for SAML2 SP ACS endpoint.",
  })
) {}

/**
 * Success response for SAML2 SP ACS.
 *
 * @since 1.0.0
 */
export class Success extends S.Class<Success>($I`Success`)(
  {
    /** Redirect URL after successful SAML2 authentication */
    url: S.optionalWith(BS.URLString, { nullable: true }).annotations({
      description: "Redirect URL after successful SAML2 authentication",
    }),
  },
  $I.annotations("SAML2SpAcsSuccess", {
    description: "SAML2 SP ACS success response.",
  })
) {}

/**
 * SAML2 SP ACS endpoint contract.
 *
 ...
 * @since 1.0.0
 */
export const Contract = HttpApiEndpoint.post("saml2-sp-acs", "/saml2/sp/acs/:providerId")
  .setPath(PathParams)
  .setPayload(Payload)
  .addSuccess(Success)
  .addError(
    IamAuthError.annotations(
      $I.annotations("IamAuthError", {
        description: "An error during SAML2 SP ACS processing.",
      })
    )
  );
