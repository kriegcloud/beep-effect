 ...
 * @module update
 *
 * Domain contract for updating an API key.
 *
...
const $I = $IamDomainId.create("api/v1/api-key/update");
...
export class Payload extends S.Class<Payload>($I`Payload`)(
  {
    ...
    enabled: S.optionalWith(S.Boolean, { nullable: true }).annotations({
  ...
  $I.annotations("ApiKeyUpdatePayload", {
    description: "Payload for updating an API key.",
  })
) {}

/**
 * Success response after updating an API key.
 *
 * @since 1.0.0
 * @category Schema
 */
export class Success extends S.Class<Success>($I`Success`)(
  ...
  $I.annotations("ApiKeyUpdateSuccess", {
    description: "Success response after updating an API key.",
  })
) {}

/**
 * Update API key endpoint contract.
 *
 * POST /api-key/update
 *
  ...
  .addError(
    IamAuthError.annotations(
      $I.annotations("IamAuthError", {
        description: "An error indicating a failure to update API key.",
      })
    )
  )
  .addSuccess(Success);
