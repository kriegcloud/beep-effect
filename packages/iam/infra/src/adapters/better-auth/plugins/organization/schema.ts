...
import type { InferAdditionalFieldsFromPluginOptions } from "../../internal";
import { generateId } from "../../internal";
import type { OrganizationOptions } from "./plugin-options";

export const role = z.string();
export const invitationStatus = z.enum(["pending", "accepted", "rejected", "canceled"]).default("pending");
...
export const defaultRolesSchema = z.union([z.enum(defaultRoles), z.array(z.enum(defaultRoles))]);
...
export type InferOrganizationRolesFromOption<O extends OrganizationOptions | undefined> = O extends { roles: any }
  ? keyof O["roles"]
  : "admin" | "member" | "owner";
...
export type InferOrganization<O extends OrganizationOptions, isClientSide extends boolean = true> = Prettify<
  Organization & InferAdditionalFieldsFromPluginOptions<"organization", O, isClientSide>
>;
...
export type InferInvitation<O extends OrganizationOptions> = (O["teams"] extends {
  enabled: true;
}
  ? {
      id: string;
