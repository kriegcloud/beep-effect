...
export const createFieldAttribute = <T extends FieldType, C extends Omit<FieldAttributeConfig<T>, "type">>(
  type: T,
  config?: C
) => {
  return {
...
export type InferFieldsOutput<Field> = Field extends Record<infer Key, FieldAttribute>
  ? {
...
export type InferFieldsInput<Field> = Field extends Record<infer Key, FieldAttribute>
  ? {
      [key in Key as Field[key]["required"] extends false
        ? never
 ...
 */
export type InferFieldsInputClient<Field> = Field extends Record<infer Key, FieldAttribute>
  ? {
      [key in Key as Field[key]["required"] extends false
        ? never
        : Field[key]["defaultValue"] extends string | number | boolean | Date
 ...
 */
export type FieldAttributeToObject<Fields extends Record<string, FieldAttribute>> = AddOptionalFields<
  {
