...
function parseISODate(value: string): Date | null {
  ...
  const date = new Date(
    Date.UTC(
      ...
      ms ? Number.parseInt(ms.padEnd(3, "0"), 10) : 0
    )
  );

  if (offsetSign) {
    const offset =
      (Number.parseInt(offsetHour!, 10) * 60 + Number.parseInt(offsetMinute!, 10)) * (offsetSign === "+" ? -1 : 1);
...
export function parseJSON<T = unknown>(value: unknown, options: ParseOptions = { strict: true }): T {
