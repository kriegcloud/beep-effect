...
describe("SparqlParser", () => {
  describe("parse - SELECT queries", () => {
    ...
    effect("should parse SELECT with multiple variables", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should parse SELECT * (wildcard)", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should parse SELECT with FILTER", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should parse SELECT with OPTIONAL", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse SELECT DISTINCT", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should parse SELECT with LIMIT and OFFSET", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should parse SELECT with ORDER BY", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should parse SELECT with UNION", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          SELECT ?s WHERE {
  ...
  describe("parse - CONSTRUCT queries", () => {
    effect("should parse basic CONSTRUCT query", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse CONSTRUCT with multiple triples in template", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          CONSTRUCT {
    ...
    effect("should parse CONSTRUCT WHERE (shorthand)", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
  ...
  describe("parse - ASK queries", () => {
    effect("should parse basic ASK query", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should parse ASK with specific subject", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse ASK with FILTER", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          ASK {
  ...
  describe("parse - PREFIX declarations", () => {
    effect("should extract single PREFIX", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should extract multiple PREFIXes", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          PREFIX ex: <http://example.org/>
    ...
    effect("should handle empty prefix (base IRI)", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should handle query without PREFIXes", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should handle common vocabulary prefixes", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
  ...
  describe("parse - error cases", () => {
    effect("should return SparqlSyntaxError for missing closing brace", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should return SparqlSyntaxError for invalid keyword", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should return error for empty query", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should return SparqlSyntaxError for malformed triple", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should return SparqlUnsupportedFeatureError for DESCRIBE", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should return SparqlUnsupportedFeatureError for INSERT DATA", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should return SparqlUnsupportedFeatureError for DELETE DATA", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const error = yield* Effect.flip(
    ...
    effect("should return SparqlUnsupportedFeatureError for DELETE WHERE", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const error = yield* Effect.flip(
  ...
  describe("parse - queryString preservation", () => {
    effect("should preserve original query string", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should preserve whitespace in query string", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const originalQuery = `
          PREFIX ex: <http://example.org/>
  ...
  describe("parse - AST structure", () => {
    effect("should return valid AST for SELECT", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should return valid AST for CONSTRUCT", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
    ...
    effect("should return valid AST for ASK", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
  ...
  describe("parse - complex queries", () => {
    effect("should parse query with multiple graph patterns", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          PREFIX foaf: <http://xmlns.com/foaf/0.1/>
    ...
    effect("should parse query with nested OPTIONAL", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse query with BIND", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          SELECT ?s ?fullName
    ...
    effect("should parse query with VALUES", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          SELECT ?s ?name
    ...
    effect("should parse query with GROUP BY and HAVING", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse query with subquery", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
          SELECT ?s ?name
  ...
  describe("parse - literal handling", () => {
    effect("should parse query with string literal", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse query with language-tagged literal", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse query with typed literal", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse query with boolean literal", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
    ...
    effect("should parse query with numeric literal", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
        const { query } = yield* parser.parse(`
  ...
  describe("parse - service isolation", () => {
    effect("should provide fresh parser per test", () =>
      Effect.gen(function* () {
        const parser = yield* SparqlParser;
