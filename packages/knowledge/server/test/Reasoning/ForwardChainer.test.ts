...
describe("ForwardChainer", () => {
  describe("fixed-point convergence", () => {
    live("reaches fixed point with simple rdfs9 ontology", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
    ...
    live("reaches fixed point with multi-level subclass chain", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
            subject: fixtures.alice,
    ...
    live("combines domain inference with subclass entailment", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
    ...
    live("returns empty result for empty input", () =>
      Effect.gen(function* () {
        const config = new ReasoningConfig({ maxDepth: 10, maxInferences: 1000 });
    ...
    live("returns empty result when no rules apply", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
            subject: fixtures.alice,
  ...
  describe("deduplication", () => {
    live("deduplicates initial quads", () =>
      Effect.gen(function* () {
        const quad = new Quad({
          subject: fixtures.alice,
          predicate: RDF_TYPE,
    ...
    live("does not duplicate inferred triples", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
            subject: fixtures.alice,
  ...
  describe("provenance tracking", () => {
    live("tracks provenance for inferred triples", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
            subject: fixtures.alice,
    ...
    live("records rdfs9 rule in provenance", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
            subject: fixtures.alice,
        ...
        const provenanceEntries = Object.values(
          result.provenance
        ) as InferenceProvenance[];
        const hasRdfs9 = A.some(
          provenanceEntries,
          (e: InferenceProvenance) => e.ruleId === "rdfs9"
        );
        assertTrue(hasRdfs9);
      }) as Effect.Effect<void>
    );
  ...
  describe("statistics", () => {
    ...
    live("records triples inferred count", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
            subject: fixtures.alice,
        ...
        strictEqual(
          result.stats.triplesInferred,
          A.length(result.derivedTriples)
        );
      }) as Effect.Effect<void>
    );

    live("records duration in milliseconds", () =>
      Effect.gen(function* () {
        const quads = [
          new Quad({
            subject: fixtures.alice,
  ...
  describe("depth limit", () => {
    live("respects max depth and fails when exceeded mid-inference", () =>
      Effect.gen(function* () {
        ...
        const classes = A.append(classChain, new Quad({
          subject: fixtures.alice,
          predicate: RDF_TYPE,
  ...
  describe("inference limit", () => {
    live("fails when inference limit exceeded", () =>
      Effect.gen(function* () {
        const instanceQuads = A.map(A.range(0, 9), (i) =>
          new Quad({
