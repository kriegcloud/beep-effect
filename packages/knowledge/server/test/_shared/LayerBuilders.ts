...
import { ExtractionWorkflow, type ExtractionWorkflowShape, type ExtractionWorkflowParams } from "@beep/knowledge-server/Workflow/ExtractionWorkflow";
import { WorkflowPersistence, type WorkflowPersistenceShape } from "@beep/knowledge-server/Workflow/WorkflowPersistence";
...
export function makeExtractionWorkflowTestLayer(deps: {
  ...
  const impl: Effect.Effect<ExtractionWorkflowShape, never, ExtractionPipeline | WorkflowPersistence> = Effect.gen(function* () {
  ...
  return Layer.effect(ExtractionWorkflow, impl).pipe(Layer.provide(deps.persistenceLayer), Layer.provide(deps.pipelineLayer));
}
