...
const astToSparqlQuery: (
	...
	Effect.fn("SparqlParser.astToSparqlQuery")(function* (
	ast: sparqljs.SparqlQuery,
	queryString: string
) {
...
const serviceEffect: Effect.Effect<SparqlParserShape> = Effect.gen(function* () {
	...
	const parse: (queryString: string) => Effect.Effect<ParseResult, SparqlSyntaxError | SparqlUnsupportedFeatureError> = Effect.fn("SparqlParser.parse")(function* (queryString: string) {
		...
		return {query, ast};
	}, (effect, queryString) => effect.pipe(
