...
export interface BatchOrchestratorShape {
	readonly run: (
		...
		BatchResult,
		BatchAlreadyRunningError | InvalidStateTransitionError | BatchNotFoundError
	>;
}
...
const serviceEffect = Effect.gen(function* () {
	...
	const _durableActivities = yield* DurableActivities;

	const emitEvent = (event: Omit<BatchEvent, "timestamp"> & { readonly timestamp?: unknown }) =>
		Effect.gen(function* () {
			const now = yield* DateTime.now;
			yield* emitter.emit({ ...event, timestamp: now } as BatchEvent);
		}).pipe(
			Effect.catchAllCause((cause) =>
				Effect.logDebug("BatchOrchestrator: failed to emit event").pipe(
				...
				(doc, index) =>
					Effect.gen(function* () {
						...
						const either = yield* Effect.either(
							workflow.run(makeWorkflowParams(doc, params))
						);
	...
	const processDocumentsAbortAll = (
		...
		return Effect.iterate(initial, {
			...
			body: (state) =>
				Effect.gen(function* () {
					const doc = F.pipe(
						A.get(params.documents, state.index),
						O.getOrThrow
					);
			...
			const finalState = yield* Effect.iterate(initial, {
				...
				body: (state) =>
					Effect.gen(function* () {
						...
						const retryDocs = A.filter(params.documents, (d) =>
							F.pipe(failedDocIds, A.contains(d.documentId))
						);
						...
						const retryMap = HashMap.fromIterable(
							A.map(retryResults, (r) => [r.documentId, r] as const)
						);
	...
	const aggregateResults = (
		...
		const successExtractions: ReadonlyArray<ExtractionResult> = A.filterMap(
			documentResults,
			(r) =>
	...
	const run: BatchOrchestratorShape["run"] = Effect.fn("BatchOrchestrator.run")(
		function* (params: BatchOrchestratorParams) {
