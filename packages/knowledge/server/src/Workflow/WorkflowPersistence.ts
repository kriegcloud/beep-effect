...
const serviceEffect = Effect.gen(function* () {
	...
	const updateExecutionStatus: WorkflowPersistenceShape["updateExecutionStatus"] =
		Effect.fn("WorkflowPersistence.updateExecutionStatus")(function* (id, status, updates) {
	...
	const getExecution: WorkflowPersistenceShape["getExecution"] =
		Effect.fn("WorkflowPersistence.getExecution")(function* (id) {
	...
	const findLatestBatchExecutionByBatchId: WorkflowPersistenceShape["findLatestBatchExecutionByBatchId"] =
		Effect.fn("WorkflowPersistence.findLatestBatchExecutionByBatchId")(function* (batchId) {
			const rows = yield* sql<WorkflowExecutionRecord>`
          SELECT *
          FROM ${sql(executionTable)}
	...
	const cancelExecution: WorkflowPersistenceShape["cancelExecution"] =
		Effect.fn("WorkflowPersistence.cancelExecution")(function* (id, error) {
	...
	const requireBatchExecutionByBatchId: WorkflowPersistenceShape["requireBatchExecutionByBatchId"] = F.flow(
		(batchId) => F.pipe(batchId, findLatestBatchExecutionByBatchId, Effect.flatMap(
			O.match({
