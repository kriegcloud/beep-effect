...
const serviceEffect: Effect.Effect<BatchEventEmitterShape> = Effect.gen(function* () {
  ...
  const subscribe: BatchEventEmitterShape["subscribe"] = (batchId) =>
    Stream.fromPubSub(pubsub).pipe(Stream.filter((event) => event.batchId === batchId));

  const subscribeAll: BatchEventEmitterShape["subscribeAll"] = () => Stream.fromPubSub(pubsub);
