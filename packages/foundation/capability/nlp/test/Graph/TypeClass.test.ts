...
import * as TC from "@beep/nlp/Graph/TypeClass";
import { describe, expect, it } from "@effect/vitest";
import * as Effect from "effect/Effect";

describe("TypeClass pure operations", () => {
  it.effect("pureOperation mints a child per produced value", Effect.fn(function* () {
  ...
  it.effect("mapOperation transforms data 1:1", Effect.fn(function* () {
  ...
  it.effect("filterOperation keeps or drops by predicate", Effect.fn(function* () {
...
describe("TypeClass functor laws", () => {
  it.effect("map identity: map(op, id) = op", Effect.fn(function* () {
  ...
  it.effect("map composition: map(op, f∘g) = map(map(op, g), f)", Effect.fn(function* () {
...
describe("TypeClass monad & applicative", () => {
  it.effect("chain sequences dependent operations", Effect.fn(function* () {
  ...
  it.effect("ap forms the Cartesian product of functions and values", Effect.fn(function* () {
  ...
  it.effect("pure lifts a constant value", Effect.fn(function* () {
...
describe("TypeClass alt & combinators", () => {
  it.effect("alt collects results from both operations", Effect.fn(function* () {
    const node = yield* EG.makeNode("x");
  ...
  it.effect("empty is the identity for alt", Effect.fn(function* () {
    const node = yield* EG.makeNode("x");
  ...
  it.effect("when / unless gate on the predicate", Effect.fn(function* () {
  ...
  it.effect("replicate applies an operation n times", Effect.fn(function* () {
    const node = yield* EG.makeNode("x");
    const op = TC.pureOperation("id", (s: string) => [s]);
...
describe("TypeClass graph execution & folding", () => {
  it.effect("executeOperation adds a child layer to leaves", Effect.fn(function* () {
  ...
  it.effect("foldableGraph folds over all node data", Effect.fn(function* () {
