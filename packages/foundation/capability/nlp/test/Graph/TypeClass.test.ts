...
describe("TypeClass pure operations", () => {
  it.effect("pureOperation mints a child per produced value", () =>
    Effect.gen(function* () {
  ...
  it.effect("mapOperation transforms data 1:1", () =>
    Effect.gen(function* () {
  ...
  it.effect("filterOperation keeps or drops by predicate", () =>
    Effect.gen(function* () {
...
describe("TypeClass functor laws", () => {
  it.effect("map identity: map(op, id) = op", () =>
    Effect.gen(function* () {
  ...
  it.effect("map composition: map(op, f∘g) = map(map(op, g), f)", () =>
    Effect.gen(function* () {
...
describe("TypeClass monad & applicative", () => {
  it.effect("chain sequences dependent operations", () =>
    Effect.gen(function* () {
  ...
  it.effect("ap forms the Cartesian product of functions and values", () =>
    Effect.gen(function* () {
  ...
  it.effect("pure lifts a constant value", () =>
    Effect.gen(function* () {
...
describe("TypeClass alt & combinators", () => {
  it.effect("alt collects results from both operations", () =>
    Effect.gen(function* () {
      const node = yield* EG.makeNode("x");
  ...
  it.effect("empty is the identity for alt", () =>
    Effect.gen(function* () {
      const node = yield* EG.makeNode("x");
  ...
  it.effect("when / unless gate on the predicate", () =>
    Effect.gen(function* () {
  ...
  it.effect("replicate applies an operation n times", () =>
    Effect.gen(function* () {
      const node = yield* EG.makeNode("x");
      const op = TC.pureOperation("id", (s: string) => [s]);
...
describe("TypeClass graph execution & folding", () => {
  it.effect("executeOperation adds a child layer to leaves", () =>
    Effect.gen(function* () {
      const g0 = yield* EG.singleton("root text");
      const split = TC.pureOperation("split", (s: string) => s.split(" "));
      const g1 = yield* TC.executeOperation(g0, split);
      // root + 2 word children
      expect(EG.size(g1)).toBe(3);
    })
  );

  it.effect("foldableGraph folds over all node data", () =>
    Effect.gen(function* () {
