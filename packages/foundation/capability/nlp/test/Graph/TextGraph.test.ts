...
describe("TextGraph construction", () => {
  it.effect("singleton builds a one-node graph", Effect.fn(function* () {
  ...
  it.effect("empty graph has no nodes", Effect.fn(function* () {
...
describe("TextGraph acyclicity", () => {
  it.effect("addChildren keeps the graph acyclic", Effect.fn(function* () {
...
describe("TextGraph from document (service-backed)", () => {
  it.effect("fromDocument creates a doc root with sentence children", Effect.fn(function* () {
  ...
  it.effect("tokenizeNodes is idempotent", Effect.fn(function* () {
...
describe("TextGraph traversal & queries", () => {
  it.effect("getRoots / getLeaves reflect structure", Effect.fn(function* () {
    const g0 = yield* TG.singleton("doc", "document");
  ...
  it.effect("topo walker yields all nodes", Effect.fn(function* () {
    const g0 = yield* TG.singleton("doc", "document");
    const root = TG.getRoots(g0)[0]!;
