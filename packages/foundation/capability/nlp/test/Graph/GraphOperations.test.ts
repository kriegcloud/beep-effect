...
describe("ExecutionId", () => {
  it.effect("generates distinct ids", Effect.fn(function* () {
...
describe("Operation constructors", () => {
  it.effect("pure mints one child node per produced value", Effect.fn(function* () {
  ...
  it.effect("transform produces a single mapped child", Effect.fn(function* () {
  ...
  it.effect("filter keeps or drops based on the predicate", Effect.fn(function* () {
  ...
  it.effect("identity re-emits the node under a fresh id", Effect.fn(function* () {
...
describe("ResultStore", () => {
  ...
  it.effect("stores and retrieves a result, incrementing hits", Effect.fn(function* () {
  ...
  it.effect("delete and clear remove entries", Effect.fn(function* () {
    const store = yield* ResultStore.ResultStore;
...
describe("GraphExecutor", () => {
  ...
  it.effect("applies an operation to leaf nodes, producing new nodes", Effect.fn(function* () {
  ...
  it.effect("reports a cache miss then a cache hit for the same node", Effect.fn(function* () {
  ...
  it.effect("validate warns when there are no leaf nodes", Effect.fn(function* () {
    const executor = yield* Executor.GraphExecutor;
  ...
  it.effect("estimateCost scales by the number of leaf nodes", Effect.fn(function* () {
  ...
  it.effect("surfaces a per-node error without failing the run", Effect.fn(function* () {
    const boom = Operation.make<string, string, never, Errors.OperationError>({
        name: "boom",
        description: "",
        category: "transformation",
        apply: node => Effect.fail(Errors.OperationError.make({ cause: new Error("boom"), nodeId: node.id, operationName: "boom" })),
    });
    const graph = yield* EG.singleton("x");
    const executor = yield* Executor.GraphExecutor;
