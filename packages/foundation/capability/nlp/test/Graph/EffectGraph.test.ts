...
describe("EffectGraph construction", () => {
  it.effect("singleton has one root node", Effect.fn(function* () {
  ...
  it.effect("addNode links a child under its parent", Effect.fn(function* () {
  ...
  it.effect("generates distinct node ids", Effect.fn(function* () {
...
describe("Catamorphism", () => {
  it.effect("counts nodes via a bottom-up fold (1 + sum of children)", Effect.fn(function* () {
    ...
    const counts = yield* EG.cata<string, number>(g3, (_node, children) => 1 + A.reduce(children, 0, (s, n) => s + n));
    expect(counts).toHaveLength(1);
    expect(counts[0]).toBe(4);
})
  );

  it.effect("collects data bottom-up", Effect.fn(function* () {
    const g0 = yield* EG.singleton("root");
    const root = EG.getRoots(g0)[0]!;
    const c1 = yield* EG.makeNode("a", O.some(root.id));
    const g1 = EG.addNode(g0, c1);
    const collected = yield* EG.cata(g1, (node, children): ReadonlyArray<string> => A.prepend(A.flatten(children), node.data));
...
describe("Anamorphism", () => {
  it.effect("unfolds a bounded tree from a numeric seed", Effect.fn(function* () {
...
describe("Functor map", () => {
  it.effect("map preserves structure and transforms data", Effect.fn(function* () {
    const g0 = yield* EG.singleton("root");
    const root = EG.getRoots(g0)[0]!;
