...
describe("GraphOps effectful operations", () => {
  it.effect("traverseNodesCollect runs an effect per node", () =>
    Effect.gen(function* () {
      const g = sample();
  ...
  it.effect("mapNodesEffect rebuilds the graph with transformed nodes", () =>
    Effect.gen(function* () {
      const g = sample();
...
describe("GraphOps streaming & merge", () => {
  it.effect("streamNodes emits every node", () =>
    Effect.gen(function* () {
      const g = sample();
  ...
  it.effect("batchNodes groups nodes into fixed-size batches", () =>
    Effect.gen(function* () {
      const g = sample();
      const batches = yield* Stream.runCollect(GraphOps.batchNodes(g, GraphOps.getRoots(g), "dfs", 2));
      // 4 nodes / batch size 2 = 2 batches
