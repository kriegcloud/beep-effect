...
import * as Composition from "@beep/nlp/Backend/Composition";
...
describe("withFallback", () => {
  it.effect("uses the secondary backend when the primary fails", () =>
    Effect.gen(function* () {
...
describe("withCaching", () => {
  it.effect("memoizes a lookup so the backend runs once per key", () =>
    Effect.gen(function* () {
      let calls = 0;
