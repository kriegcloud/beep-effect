...
describe("Functor laws", () => {
  ...
  it.effect("composition: map(g ∘ f) = map(g) ∘ map(f)", () =>
    Effect.gen(function* () {
...
describe("Monad laws", () => {
  it.effect("left identity: pure(a).flatMap(f) = f(a)", () =>
    Effect.gen(function* () {
  ...
  it.effect("right identity: m.flatMap(pure) = m", () =>
    Effect.gen(function* () {
  ...
  it.effect("associativity: (m.flatMap(f)).flatMap(g) = m.flatMap(x => f(x).flatMap(g))", () =>
    Effect.gen(function* () {
...
describe("Applicative", () => {
  it.effect("product runs both operations on the same input", () =>
    Effect.gen(function* () {
  ...
  it.effect("zipWith combines both results with a function", () =>
    Effect.gen(function* () {
      const lenA = Composable.makeOperation("lenA", S.String, S.Number, (s) => Effect.succeed(s.length));
      const lenB = Composable.makeOperation("lenB", S.String, S.Number, (s) => Effect.succeed(s.length * 10));
...
describe("compose + traverse + aggregate", () => {
  it.effect("compose threads output into the next operation's input", () =>
    Effect.gen(function* () {
  ...
  it.effect("traverse maps an operation over an array of inputs", () =>
    Effect.gen(function* () {
