...
describe("Functor laws", () => {
  it.effect("identity: map(id) = id", Effect.fn(function* () {
  ...
  it.effect("composition: map(g ∘ f) = map(g) ∘ map(f)", Effect.fn(function* () {
...
describe("Monad laws", () => {
  it.effect("left identity: pure(a).flatMap(f) = f(a)", Effect.fn(function* () {
  ...
  it.effect("right identity: m.flatMap(pure) = m", Effect.fn(function* () {
  ...
  it.effect("associativity: (m.flatMap(f)).flatMap(g) = m.flatMap(x => f(x).flatMap(g))", Effect.fn(function* () {
...
describe("Applicative", () => {
  it.effect("product runs both operations on the same input", Effect.fn(function* () {
  ...
  it.effect("zipWith combines both results with a function", Effect.fn(function* () {
    const lenA = Composable.makeOperation("lenA", S.String, S.Number, s => Effect.succeed(s.length));
    const lenB = Composable.makeOperation("lenB", S.String, S.Number, s => Effect.succeed(s.length * 10));
...
describe("compose + traverse + aggregate", () => {
  it.effect("compose threads output into the next operation's input", Effect.fn(function* () {
    const pipeline = Composable.compose(len, inc);
    expect(yield* pipeline.run("hello")).toBe(6);
})
  );

  it.effect("traverse maps an operation over an array of inputs", Effect.fn(function* () {
