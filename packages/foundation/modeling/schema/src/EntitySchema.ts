...
import {SchemaAST as AST, Match, Tuple, pipe} from "effect";
...
export const StorageKind = LiteralKit([
  ...
  "text",
  "timestampMillis",
]).pipe($I.annoteSchema("StorageKind", {
...
export const ValueStrategy = LiteralKit([
  ...
  "providedByContext",
  "updatedOnWrite",
]).pipe($I.annoteSchema("ValueStrategy", {
 ...
 * @category schemas
 */
export const IndexHintKind = LiteralKit([
  "btree",
  "gin",
  "hash",
  "lookup",
  "unique",
]).pipe($I.annoteSchema("IndexHintKind", {
...
export const EncodedAbsenceKind = LiteralKit([
  ...
  "optionalNullish",
  "ambiguous",
]).pipe($I.annoteSchema("EncodedAbsenceKind", {
 ...
 * @category models
 */
export type PersistOptions<Strategy extends PersistStrategy = "provided", ColumnName extends string | undefined = string | undefined, IndexHints extends ReadonlyArray<IndexHint> | undefined = undefined, > = {
  readonly columnName?: ColumnName;
  readonly indexHints?: IndexHints;
  readonly valueStrategy?: Strategy;
};

type PersistDescriptorShape<TStorageKind extends StorageKind = StorageKind, TValueStrategy extends PersistStrategy = PersistStrategy, TColumnName extends string | undefined = string | undefined, TIndexHints extends ReadonlyArray<IndexHint> | undefined = undefined, > =
  {
 ...
 * @category models
 */
export type PersistDescriptor<TStorageKind extends StorageKind = StorageKind, TValueStrategy extends PersistStrategy = PersistStrategy, TColumnName extends string | undefined = string | undefined, TIndexHints extends ReadonlyArray<IndexHint> | undefined = ReadonlyArray<IndexHint> | undefined, > = TStorageKind extends StorageKind
...
const PersistDescriptorStorageBase = StorageKind.toTaggedUnion("storageKind")(
  PersistDescriptorCases);
type PersistDescriptorStatics = Pick<typeof PersistDescriptorStorageBase, "cases" | "guards" | "isAnyOf" | "match">;

const attachPersistDescriptorStatics = <Schema extends S.Decoder<PersistDescriptor.Any>>(schema: Schema): Schema & PersistDescriptorStatics => SchemaUtils.withStatics(schema,
  () => Struct.pick(PersistDescriptorStorageBase, [
    "cases",
    "guards",
    "isAnyOf",
    "match",
 ...
 * @category schemas
 */
export const PersistDescriptor: S.Decoder<PersistDescriptor.Any> & PersistDescriptorStatics = attachPersistDescriptorStatics(
  PersistDescriptorStorageBase.pipe($I.annoteSchema("PersistDescriptor", {
 ...
 * @category models
 */
export type PersistDescriptorByValueStrategy<Descriptor extends PersistDescriptor.Any = PersistDescriptor> = Descriptor extends unknown
  ? {
 ...
 * @category models
 */
export type EntityIdSchema<Entity extends EntityIdLike> =
  S.Codec<Entity["Type"], number>
  & Entity;
...
type TextStorage<Encoded> = NonNullish<Encoded> extends string
  ? PersistDescriptor<"literal" | "text">
  : never;

type BoolStorage<Encoded> = NonNullish<Encoded> extends boolean
  ? PersistDescriptor<"bool">
  : never;

type BlobStorage<Encoded> = NonNullish<Encoded> extends Uint8Array
  ? PersistDescriptor<"blob">
  : never;

type JsonStorage<Encoded> = NonNullish<Encoded> extends JsonContainer
...
type ExtraPersistedKeys<FieldMap extends Fields, Persisted extends PersistedMap> = Exclude<keyof Persisted, keyof FieldMap>;
 ...
 * @category models
 */
export type CheckedPersistedFor<FieldMap extends Fields, Persisted extends PersistedFor<FieldMap>> =
  Persisted
 ...
 * @category models
 */
export type Definition<FieldMap extends Fields = Fields, Persisted extends PersistedMap = PersistedMap, TableName extends string = string, EntityId extends EntityIdLike | undefined = EntityIdLike | undefined, > =
  {
 ...
 * @category models
 */
export type ClassInput<FieldMap extends Fields, Persisted extends PersistedFor<FieldMap>, TableName extends string = string, EntityId extends EntityIdLike | undefined = undefined, > = {
  readonly entityId?: EntityId;
  readonly fields: FieldMap;
 ...
 */
export const defineClassInput = <const FieldMap extends Fields, const Persisted extends PersistedFor<FieldMap>, const TableName extends string = string, const EntityId extends EntityIdLike | undefined = undefined, >(input: ClassInput<FieldMap, Persisted, TableName, EntityId>): ClassInput<FieldMap, Persisted, TableName, EntityId> => input;

type VariantFieldFor<Field extends S.Top, Descriptor extends PersistDescriptor.Any, > = Descriptor["valueStrategy"] extends "generatedOnInsert"
...
type VariantFieldsFor<FieldMap extends Fields, Persisted extends PersistedMap> = {
  readonly [K in keyof FieldMap]: K extends keyof Persisted
    ? VariantFieldFor<FieldMap[K], Persisted[K]>
    : FieldMap[K];
};
 ...
 * @category models
 */
export type EntityClass<Self, FieldMap extends Fields, Persisted extends PersistedFor<FieldMap>, Inherited = {}, TableName extends string = string, EntityId extends EntityIdLike | undefined = EntityIdLike | undefined, > =
 ...
 * @category models
 */
export type AssignPersisted<BasePersisted extends PersistedMap, ExtensionPersisted extends PersistedMap> = Simplify<StructAssign<BasePersisted, ExtensionPersisted>>;
 ...
 * @category models
 */
type AssignedEntityParts<BaseFields extends Fields, BasePersisted extends PersistedFor<BaseFields>, ExtensionFields extends Fields, ExtensionPersisted extends PersistedFor<ExtensionFields>, > = {
 ...
 * @category models
 */
export type AssignedPersisted<BaseFields extends Fields, BasePersisted extends PersistedFor<BaseFields>, ExtensionFields extends Fields, ExtensionPersisted extends PersistedFor<ExtensionFields>, > = CheckedPersistedFor<Assign<BaseFields, ExtensionFields>, AssignPersisted<BasePersisted, ExtensionPersisted> & PersistedFor<Assign<BaseFields, ExtensionFields>>>;
 ...
 * @category constructors
 */
const assignEntityParts = <const BaseFields extends Fields, const BasePersisted extends PersistedFor<BaseFields>, const ExtensionFields extends Fields, const ExtensionPersisted extends PersistedFor<ExtensionFields>, >(
 ...
 * @category constructors
 */
export type ClassFactory<Self, FieldMap extends Fields, Persisted extends PersistedFor<FieldMap>, TableName extends string = string, EntityId extends EntityIdLike | undefined = EntityIdLike | undefined, > =
  EntityClass<Self, FieldMap, Persisted, {}, TableName, EntityId>
  & {
  readonly Class: <Child = never>(identifier: string) => <const ChildFields extends Fields, const ChildPersisted extends PersistedFor<ChildFields>, const ChildTableName extends string = string, const ChildEntityId extends EntityIdLike | undefined = undefined, >(
    ...
    : EntityClass<Child, Assign<FieldMap, ChildFields>, AssignedPersisted<FieldMap, Persisted, ChildFields, ChildPersisted>, Self, ChildTableName, ChildEntityId>;
};

const descriptor = <const TStorageKind extends StorageKind>(storageKind: TStorageKind) => <const Strategy extends PersistStrategy = "provided", const ColumnName extends string | undefined = undefined, const IndexHints extends ReadonlyArray<IndexHint> | undefined = undefined, >(options?: PersistOptions<Strategy, ColumnName, IndexHints>): PersistDescriptor<TStorageKind, Strategy, ColumnName, IndexHints> => {
  const base = {
    storageKind,
  ...
  return Struct.assign(
    Struct.assign(
      base,
      options?.columnName === undefined
        ? {}
        : {columnName: options.columnName},
    ),
    options?.indexHints === undefined
      ? {}
 ...
 * @category constructors
 */
export const entityId = <const Entity extends EntityIdLike>(schema: Entity): EntityIdSchema<Entity> => schema as EntityIdSchema<Entity>;
 ...
 * @category constructors
 */
export const literal = <const Value extends string | number | boolean | bigint>(value: Value): S.Literal<Value> => S.Literal(
  value);

const titleSegment = <const Identifier extends string>(identifier: Identifier): LastPathSegment<Identifier> => A.lastNonEmpty(
  Str.split(identifier, "/")) as LastPathSegment<Identifier>;
 ...
 * @category naming
 */
export const tableNameFromIdentifier = <const Identifier extends string>(identifier: Identifier): TableNameFromIdentifier<Identifier> => Str.snakeCase(
  titleSegment(identifier)) as TableNameFromIdentifier<Identifier>;
...
export const columnNameFor: {
  ...
  <const Descriptor extends PersistDescriptor>(descriptor: Descriptor): <const Key extends string>(key: Key) => ColumnNameFor<Key, Descriptor>;
} = dual(
  2,
  <const Key extends string, const Descriptor extends PersistDescriptor>(
    key: Key,
    descriptor: Descriptor,
  ): ColumnNameFor<Key, Descriptor> => (descriptor.columnName ?? Str.snakeCase(
    key)) as ColumnNameFor<Key, Descriptor>,
);
...
class AstAbsence extends S.Class<AstAbsence>($I`AstAbsence`)({
  ...
  description: "Represents the absence of a value in an AST declaration, with options for null, undefined, and ambiguity.",
})) {
}
...
const astAbsence: (input: AST.AST) => AstAbsence = Match.type<AST.AST>()
  .pipe(
    ...
    Match.tags({
      Declaration: (ast) => isJsonDeclaration(ast)
      ...
      Union: (ast) => A.reduce(
        ...
        (accumulator, member) => combineAstAbsence(
          accumulator,
          astAbsence(member),
        ),
      ),
...
export const EncodedFieldShape = EncodedAbsenceKind.mapMembers((members) => {
  const make = <T extends EncodedAbsenceKind>(literal: S.Literal<T>) => S.Struct(
    {
  ...
  return pipe(members, Tuple.evolve([
    make,
    make,
    make,
    make,
...
}).pipe($I.annoteSchema("EncodedFieldShape", {
...
export const selectedRowFieldShape: {
  ...
  if (shape.isAmbiguous || shape.isOptional || shape.allowsUndefined) {
    throw new Error(`Persisted selected-row field '${key}' must encode SQL absence as null, not undefined, a missing key, or an ambiguous declared schema.`);
  }
  return shape;
});
 ...
 * @category ast
 */
export const isEncodedNullable = (field: S.Top): boolean => encodedFieldShape(
  field).allowsNull;
 ...
 * @category ast
 */
export const isEncodedOptional = (field: S.Top): boolean => encodedFieldShape(
  field).isOptional;

const withDefinitionAnnotation = <FieldMap extends Fields, Persisted extends PersistedFor<FieldMap>, TableName extends string, EntityId extends EntityIdLike | undefined, >(
  definition: Definition<FieldMap, Persisted, TableName, EntityId>,
  annotations?: SchemaAnnotations,
): SchemaAnnotations => Struct.assign(
  annotations ?? {},
  {[DefinitionAnnotationKey]: definition},
);

const normalizeDefinition = <const FieldMap extends Fields, const Persisted extends PersistedFor<FieldMap>, const TableName extends string, const EntityId extends EntityIdLike | undefined, >(
  identifier: string,
  ...
  tableName: (input.tableName ?? input.entityId?.tableName ?? tableNameFromIdentifier(
    identifier)) as TableName,
}) as unknown as Definition<FieldMap, Persisted, TableName, EntityId>;

const attachDefinition = <Class extends object, FieldMap extends Fields, Persisted extends PersistedFor<FieldMap>, TableName extends string, EntityId extends EntityIdLike | undefined, >(
  entityClass: Class,
  definition: Definition<FieldMap, Persisted, TableName, EntityId>,
): Class & {
  readonly definition: Definition<FieldMap, Persisted, TableName, EntityId>;
...
const attachEntityClassDefinition = <Self, const FieldMap extends Fields, const Persisted extends PersistedFor<FieldMap>, Inherited, const TableName extends string, const EntityId extends EntityIdLike | undefined, >(
  entityClass: Model.ClassShape<Self, VariantFieldsFor<FieldMap, Persisted>, {}, Inherited>,
  definition: Definition<FieldMap, Persisted, TableName, EntityId>,
): EntityClass<Self, FieldMap, Persisted, Inherited, TableName, EntityId> => attachDefinition(
  entityClass,
  definition,
) as EntityClass<Self, FieldMap, Persisted, Inherited, TableName, EntityId>;

const variantFieldFor = <const Field extends S.Top, const Descriptor extends PersistDescriptor.Any>(
  field: Field,
  descriptor: Descriptor,
): VariantFieldFor<Field, Descriptor> => Match.value(descriptor)
  .pipe(Match.withReturnType<unknown>(), Match.when(
    ...
    (self) => self.valueStrategy === "defaultedOnInsert" || self.valueStrategy === "updatedOnWrite" || self.valueStrategy === "providedByContext" || self.valueStrategy === "derived" || self.valueStrategy === "computedByService",
    () => Model.GeneratedByApp(field),
  ), Match.orElse(() => field)) as VariantFieldFor<Field, Descriptor>;
...
const extendEntityModelClass = <Self, Child, const FieldMap extends Fields, const Persisted extends PersistedFor<FieldMap>, const ChildFields extends Fields, const ChildPersisted extends PersistedFor<ChildFields>, const TableName extends string, const EntityId extends EntityIdLike | undefined, >(
  baseClass: EntityClass<Self, FieldMap, Persisted, {}, TableName, EntityId>,
  identifier: string,
  fields: VariantFieldsFor<ChildFields, ChildPersisted>,
  annotations?: SchemaAnnotations,
): Model.ClassShape<Child, VariantFieldsFor<Assign<FieldMap, ChildFields>, AssignedPersisted<FieldMap, Persisted, ChildFields, ChildPersisted>>, {}, Self> => {
...
const createClass = <Self, const FieldMap extends Fields, const Persisted extends PersistedFor<FieldMap>, const TableName extends string, const EntityId extends EntityIdLike | undefined, >(
  identifier: string,
  input: ClassInput<FieldMap, Persisted, TableName, EntityId>,
  annotations?: SchemaAnnotations,
): EntityClass<Self, FieldMap, Persisted, {}, TableName, EntityId> => {
...
const withClassFactory = <Self, const FieldMap extends Fields, const Persisted extends PersistedFor<FieldMap>, const TableName extends string, const EntityId extends EntityIdLike | undefined, >(baseClass: EntityClass<Self, FieldMap, Persisted, {}, TableName, EntityId>): ClassFactory<Self, FieldMap, Persisted, TableName, EntityId> => {
  const Class = <Child = never>(identifier: string) => <const ChildFields extends Fields, const ChildPersisted extends PersistedFor<ChildFields>, const ChildTableName extends string = string, const ChildEntityId extends EntityIdLike | undefined = undefined, >(
    input: ClassInput<ChildFields, ChildPersisted, ChildTableName, ChildEntityId>,
    annotations?: SchemaAnnotations,
  ) => {
    ...
    const childInput = defineClassInput({
      ...
      persisted: childParts.persisted, ...(input.tableName === undefined
        ? {}
        : {tableName: input.tableName}),
    });
    const childDefinition = normalizeDefinition(identifier, childInput);
    const childClass = extendEntityModelClass<Self, Child, FieldMap, Persisted, ChildFields, ChildPersisted, TableName, EntityId>(
      baseClass,
      identifier,
      variantFieldsFor<ChildFields, ChildPersisted>(
        input.fields,
        input.persisted,
      ),
 ...
 * @category constructors
 */
export const ClassFactory = (identifier: string) => <const FieldMap extends Fields, const Persisted extends PersistedFor<FieldMap>, const TableName extends string = string, const EntityId extends EntityIdLike | undefined = undefined, >(
  input: ClassInput<FieldMap, Persisted, TableName, EntityId>,
  annotations?: SchemaAnnotations,
): ClassFactory<TypeShape<FieldMap>, FieldMap, Persisted, TableName, EntityId> => withClassFactory(
  createClass<TypeShape<FieldMap>, FieldMap, Persisted, TableName, EntityId>(
    identifier,
    input,
    annotations,
  ));
