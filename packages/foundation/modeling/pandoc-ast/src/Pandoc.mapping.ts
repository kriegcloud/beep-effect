...
import {A, dual, Eq, N, O} from "@beep/utils";
...
const MdHeadingLevel = LiteralKit([
	1,
	2,
	3,
	4,
	5,
	6,
]).pipe($I.annoteSchema("MdHeadingLevel", {
	description: "Markdown Heading level",
}));

export type MdHeadingLevel = typeof MdHeadingLevel.Type;

const emptyProjection = <Value>(value: Value): Projection<Value> => ({
	issues: [],
	value,
});

const mergeIssues = <Value>(values: ReadonlyArray<Projection<Value>>): ReadonlyArray<PandocMappingIssue.Type> => A.flatten(
	A.map(values, (value) => value.issues));
...
}): PandocMappingIssue => PandocMappingIssue.fromPath({
...
const hasPandocAttr = (attr: PandocAttr.Type): boolean => attr.id.length > 0 || attr.classes.length > 0 || attr.keyValues.length > 0;

const hasCodeBlockDroppedAttr = (attr: PandocAttr.Type): boolean => attr.id.length > 0 || attr.classes.length > 1 || attr.keyValues.length > 0;

const hasOrderedListMarkerLoss = (node: OrderedList.Type): boolean => node.start !== 1 || node.style !== "DefaultStyle" || node.delimiter !== "DefaultDelim";

const hasTargetTitle = (target: PandocTarget.Type): boolean => target.title.length > 0;

const appendIndex: {
	(path: JsonPath, key: string, index: number): JsonPath;
	(key: string, index: number): (path: JsonPath) => JsonPath;
} = dual(
	3,
	(path: JsonPath, key: string, index: number): JsonPath => [
		...path,
...
): Effect.Effect<Projection<ReadonlyArray<Output>>, S.SchemaError> => Effect.map(Effect.forEach(
	values,
	(value, index) => project(value, appendIndex(path, "children", index)),
), (projected) => ({
...
const mdInlineText: (inline: Md.Inline) => string = Match.type<Md.Inline>().pipe(Match.tagsExhaustive({
...
const pandocInlineText: (inline: PandocInline.Type) => string = Match.type<PandocInline.Type>()
...
const pandocBlockText: (block: PandocBlock.Type) => string = Match.type<PandocBlock.Type>().pipe(Match.tagsExhaustive({
	...
	bulletlist: (block) => A.join(A.map(block.items, (item) => A.join(A.map(item, pandocBlockText), "\n")), "\n"),
	orderedlist: (block) => A.join(A.map(block.items, (item) => A.join(A.map(item, pandocBlockText), "\n")), "\n"),
	horizontalrule: () => "",
...
const pandocInlineToMd = (
	...
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<Md.Inline>>, S.SchemaError> => Match.value(inline).pipe(Match.tagsExhaustive({
	str: (node) => Effect.succeed(emptyProjection([mdText(node.text)])),
	space: () => Effect.succeed(emptyProjection([mdText(" ")])),
	softbreak: () => Effect.succeed({
		issues: [
			issue({
				construct: "SoftBreak",
	...
	emph: (node) => Effect.map(
		pandocInlinesToMd(node.children, path),
		({
			issues,
			value,
		}) => ({
	...
	unknownInline: (node) => Effect.succeed({
		issues: [
			issue({
...
const pandocInlinesToMd: {
	...
	(path: JsonPath): (inlines: ReadonlyArray<PandocInline.Type>) => Effect.Effect<Projection<ReadonlyArray<Md.Inline>>, S.SchemaError>;
} = dual(2, (
	inlines: ReadonlyArray<PandocInline.Type>,
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<Md.Inline>>, S.SchemaError> => projectChildValues(
	inlines,
	path,
	pandocInlineToMd,
));
const mdHeadingLevelFromPandoc = Match.type<number>().pipe(
	Match.withReturnType<MdHeadingLevel>(),
	Match.when(N.isLessThanOrEqualTo(1), MdHeadingLevel.thunk.number1),
	Match.when(Eq.equals(2), MdHeadingLevel.thunk.number2),
	Match.when(Eq.equals(3), MdHeadingLevel.thunk.number3),
	Match.when(Eq.equals(4), MdHeadingLevel.thunk.number4),
	Match.when(Eq.equals(5), MdHeadingLevel.thunk.number5),
	Match.orElse(MdHeadingLevel.thunk.number6),
);

const pandocHeadingLevelProjection = (level: number, path: JsonPath): Projection<MdHeadingLevel> => ({
	issues: level < 1 || level > 6
		? [
			issue({
				construct: "Header",
				direction: "pandoc-to-md",
...
const headingToMd: {
	(level: MdHeadingLevel, children: ReadonlyArray<Md.Inline>): Md.Block,
	(children: ReadonlyArray<Md.Inline>): (level: MdHeadingLevel) => Md.Block
} = dual(2, (level: MdHeadingLevel, children: ReadonlyArray<Md.Inline>): Md.Block => MdHeadingLevel.$match(level, {
	number1: () => Md.H1.make({children}),
	number2: () => Md.H2.make({children}),
	number3: () => Md.H3.make({children}),
	number4: () => Md.H4.make({children}),
	number5: () => Md.H5.make({children}),
	number6: () => Md.H6.make({children}),
}))

const isPlainOrPara = (block: PandocBlock.Type): block is Plain.Type | Para.Type => block._tag === "plain" || block._tag === "para";
...
const pandocListItemBlockToMdInlines: {
	...
	(
		path: JsonPath,
	): (block: PandocBlock.Type) => Effect.Effect<Projection<ReadonlyArray<Md.ListItemChild>>, S.SchemaError>
} = dual(2, (
	block: PandocBlock.Type,
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<Md.ListItemChild>>, S.SchemaError> => Match.value(block).pipe(
	Match.tags({
		plain: (node) => pandocInlinesToMd(node.children, path),
		para: (node) => pandocInlinesToMd(node.children, path),
	}),
	Match.orElse((node) => Effect.map(
		pandocBlockToMd(node, path),
		({
			issues,
			value,
		}) => ({
...
const pandocListItemBlockToMdChildBlocks = (
	block: PandocBlock.Type,
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<Md.ListItemChild>>, S.SchemaError> => Effect.map(
	pandocBlockToMd(block, path),
	({
		issues,
		value,
	}) => ({
...
const pandocListItemBlocksToMdChildren = (
	...
	return item.length === 1 && first !== undefined && isPlainOrPara(first)
		...
		: Effect.map(Effect.forEach(
			item,
			(block, index) => pandocListItemBlockToMdChildBlocks(block, appendIndex(path, "blocks", index)),
		), (blocks) => ({
...
const mdInlinesToPandocPlain = (
	...
	path: JsonPath,
): Effect.Effect<Projection<PandocBlock.Type>, S.SchemaError> => Effect.map(
	mdInlinesToPandoc(inlines, path),
	({
		issues,
		value,
	}) => ({
...
const mdListItemChildrenToPandocBlocks = Effect.fn("mdListItemChildrenToPandocBlocks")(
	function* (
...
const pandocListItemToMd = (
	item: ReadonlyArray<PandocBlock.Type>,
	path: JsonPath,
): Effect.Effect<Projection<Md.Li>, S.SchemaError> => Effect.map(
	pandocListItemBlocksToMdChildren(item, path),
	({
		issues,
		value,
	}) => ({
...
const mdListText = (items: ReadonlyArray<Md.Li | Md.TaskItem>): string => A.join(A.map(
	items,
	(item) => mdListItemChildrenText(item.children),
), "\n");

const youtubeWatchUrl = (videoId: string): string => `https://www.youtube.com/watch?v=${encodeURIComponent(videoId)}`;

const mdTableText = (block: Md.Table): string => A.join(A.map(
	block.children,
	(row) => A.join(A.map(row.children, (cell) => mdInlinesText(cell.children)), " | "),
), "\n");

const mdBlockText: (block: Md.Block) => string = Match.type<Md.Block>().pipe(Match.tagsExhaustive({
...
const pandocChildBlocksToMd = (
	...
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<Md.Block>>, S.SchemaError> => Effect.map(
	Effect.forEach(
		children,
		(child, index) => pandocBlockToMd(child, appendIndex(path, "children", index)),
	),
	(values) => ({
...
const pandocListItemsToMd = (
	...
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<Md.Li>>, S.SchemaError> => Effect.map(Effect.forEach(
	items,
	(item, index) => pandocListItemToMd(item, appendIndex(path, "items", index)),
), (values) => ({
	issues: mergeIssues(values),
...
const pandocBlockToMd: {
	...
	(
		path: JsonPath,
	): (block: PandocBlock.Type) =>Effect.Effect<Projection<Md.Block>, S.SchemaError>
} = dual(2, (
	block: PandocBlock.Type,
	path: JsonPath,
): Effect.Effect<Projection<Md.Block>, S.SchemaError> => Match.value(block).pipe(Match.tagsExhaustive({
	plain: (node) => Effect.map(
		pandocInlinesToMd(node.children, path),
		({
			issues,
			value,
	...
	blockquote: (node) => Effect.map(
		pandocChildBlocksToMd(node.children, path),
		({
			issues,
			value,
		}) => ({
	...
	codeblock: (node) => Effect.succeed({
		issues: hasCodeBlockDroppedAttr(node.attr)
			...
			: [],
		value: Md.Pre.make({
			language: O.fromUndefinedOr(node.attr.classes[0]),
			value: node.text,
		}),
	}),
	bulletlist: (node) => Effect.map(
		pandocListItemsToMd(node.items, path),
		({
			issues,
			value,
		}) => ({
		...
		({
			...
			issues: [
				...
				issue({
					...
					path,
					severity: hasPandocAttr(node.attr)
						? "unsupported"
						: "lossy",
				}),
			],
	...
	unknownBlock: (node) => Effect.succeed({
		issues: [
			issue({
				construct: node.constructor,
				direction: "pandoc-to-md",
...
const mdInlineToPandoc = (
	inline: Md.Inline,
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<PandocInline.Type>>, S.SchemaError> => Match.value(inline)
	.pipe(Match.tagsExhaustive({
		text: (node) => Effect.succeed(emptyProjection([Str.make({text: node.value})])),
		rawMarkdown: (node) => Effect.succeed({
			issues: [
				issue({
					construct: node._tag,
		...
		strong: (node) => Effect.map(
			mdInlinesToPandoc(node.children, path),
			({
				issues,
				value,
			}) => ({
		...
		code: (node) => Effect.succeed(emptyProjection([
			Code.make({
		...
		img: (node) => Effect.succeed(emptyProjection([
			Image.make({
				attr: PandocAttr.empty,
...
const mdInlinesToPandoc = (
	inlines: ReadonlyArray<Md.Inline>,
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<PandocInline.Type>>, S.SchemaError> => projectChildValues(
	inlines,
	path,
	mdInlineToPandoc,
);

const headingLevel = (tag: "h1" | "h2" | "h3" | "h4" | "h5" | "h6"): number => Match.value(tag)
	.pipe(
		Match.when("h1", () => 1),
		Match.when("h2", () => 2),
		Match.when("h3", () => 3),
...
const mdListItemToPandocBlocks = (
	...
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<PandocBlock.Type>>, S.SchemaError> => mdListItemChildrenToPandocBlocks(
	item.children,
	path,
);
...
const mdListItemsToPandocBlocks = (
	...
	path: JsonPath,
): Effect.Effect<Projection<ReadonlyArray<ReadonlyArray<PandocBlock.Type>>>, S.SchemaError> => Effect.map(
	Effect.forEach(
		items,
		(item, index) => mdListItemToPandocBlocks(item, appendIndex(path, "children", index)),
	),
	(values) => ({
		issues: mergeIssues(values),
		value: A.map(values, (item) => item.value),
...
const mdBlockToPandoc = (
	block: Md.Block,
	path: JsonPath,
): Effect.Effect<Projection<PandocBlock.Type>, S.SchemaError> => Match.value(block).pipe(Match.tagsExhaustive({
	...
	blockquote: (node) => Effect.map(
		Effect.forEach(
			node.children,
			(child, index) => mdBlockToPandoc(child, appendIndex(path, "children", index)),
		),
		(children) => ({
	...
	pre: (node) => Effect.succeed(emptyProjection(CodeBlock.make({
		attr: PandocAttr.make({
			classes: O.match({
				onNone: () => [],
				onSome: (language: string) => [language],
			})(node.language),
			id: "",
			keyValues: [],
		}),
	...
	ul: (node) => Effect.map(
		mdListItemsToPandocBlocks(node.children, path),
		({
			issues,
			value,
		}) => ({
 ...
 * @since 0.0.0
 */
export const pandocToDocument = (pandoc: PandocDocument.Type): Effect.Effect<PandocToDocumentResult, S.SchemaError> => Effect.map(
	Effect.forEach(
		pandoc.blocks,
		(block, index) => pandocBlockToMd(
			block,
			[
				"blocks",
				index,
	...
	(blocks) => PandocToDocumentResult.make({
 ...
 * @since 0.0.0
 */
export const documentToPandoc = (document: Md.Document.Type): Effect.Effect<DocumentToPandocResult, S.SchemaError> => Effect.map(
	Effect.forEach(
		document.children,
		(block, index) => mdBlockToPandoc(
			block,
			[
				"children",
				index,
	...
	(blocks) => DocumentToPandocResult.make({
