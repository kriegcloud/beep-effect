...
export const errorMessageWithFormattedIssue = F.flow(
  (message: string) => (issue: ParseResult.ParseIssue) =>
    F.pipe(message, Str.concat(`: ${ParseResult.TreeFormatter.formatIssueSync(issue)}`))
);
...
const makeEntityIdTitleAnnotation = <const Brand extends `${Capitalize<NonEmptyString>}Id`>(brand: Brand) =>
  F.pipe(
...
export const makeEntityId = <const Brand extends `${Capitalize<string>}Id`>(
  brand: Brand,
