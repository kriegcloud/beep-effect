        ...
        get: (key) =>
          _get(key).pipe(
            Effect.map((value) => {
              ...
              return O.some(Encoding.encodeBase64(value as Uint8Array));
            })
          ),
        getUint8Array: (key) =>
          _get(key).pipe(
            Effect.map((value) => {
              if (value === undefined) {
                return O.none<Uint8Array>();
              }
              if (value instanceof Uint8Array) {
                return O.some(value);
              }
              return O.some(encoder.encode(value as string));
            })
          ),
        set: (key, value) => withStore("readwrite", (store) => store.put(value, key)).pipe(Effect.asVoid),
