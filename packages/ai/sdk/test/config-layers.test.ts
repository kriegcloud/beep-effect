...
test("AgentSdkConfig reads sandbox settings from config provider", async () => {
  const layer = AgentSdkConfig.layer.pipe(
    Layer.provide(
      configLayer({
        ...
        SANDBOX_IGNORE_VIOLATIONS: '{"rg":["line-too-long"],"bash":["unsafe-flag"]}',
      })
    )
  );
  ...
  expect(options.sandbox?.network?.allowedDomains).toEqual(["example.com", "api.example.com"]);
  expect(options.sandbox?.network?.allowUnixSockets).toEqual(["/tmp/socket1", "/tmp/socket2"]);
  ...
  expect(options.sandbox?.ripgrep?.args).toEqual(["--hidden", "--glob", "!node_modules"]);
