...
test("filterAssistant keeps only assistant messages", async () => {
  const result = await Effect.runPromise(
    MessageFilters.filterAssistant(Stream.fromIterable(allMessages)).pipe(Stream.runCollect)
  );
...
test("filterResultSuccess keeps only success results", async () => {
  const result = await Effect.runPromise(
    MessageFilters.filterResultSuccess(Stream.fromIterable(allMessages)).pipe(Stream.runCollect)
  );
  const items = Array.from(result);
  expect(items).toHaveLength(1);
...
test("filterResultError keeps only error results", async () => {
  const result = await Effect.runPromise(
    MessageFilters.filterResultError(Stream.fromIterable(allMessages)).pipe(Stream.runCollect)
  );
  const items = Array.from(result);
  expect(items).toHaveLength(1);
...
test("filterUser keeps user and replay messages", async () => {
  const result = await Effect.runPromise(
    MessageFilters.filterUser(Stream.fromIterable(allMessages)).pipe(Stream.runCollect)
  );
...
test("filterStreamEvents keeps only stream_event messages", async () => {
  const result = await Effect.runPromise(
    MessageFilters.filterStreamEvents(Stream.fromIterable(allMessages)).pipe(Stream.runCollect)
  );
  const items = Array.from(result);
  expect(items).toHaveLength(1);
...
test("filterToolProgress keeps only tool_progress messages", async () => {
  const result = await Effect.runPromise(
    MessageFilters.filterToolProgress(Stream.fromIterable(allMessages)).pipe(Stream.runCollect)
  );
  const items = Array.from(result);
  expect(items).toHaveLength(1);
...
test("toTextStream extracts text chunks from assistant and stream messages", async () => {
  ...
  const result = await Effect.runPromise(MessageFilters.toTextStream(stream).pipe(Stream.runCollect));
