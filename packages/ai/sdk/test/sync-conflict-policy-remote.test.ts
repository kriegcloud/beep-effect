...
const writeConflict = (layer: Layer.Layer<Sync.ConflictPolicy>, order: "ascending" | "descending" = "ascending") =>
  runEffect(
    Effect.gen(function* () {
      ...
      const ordered = order === "ascending" ? [first, second] : [second, first];
      ...
      Effect.provide(Layer.mergeAll(Storage.layerKeyValueStore().pipe(Layer.provide(KeyValueStore.layerMemory)), layer))
    )
  );
...
test("ConflictPolicy.merge applies during remote writes", async () => {
  const entries = await writeConflict(Sync.ConflictPolicy.layerMerge((_entry, conflicts) => conflicts[0] ?? _entry));
  expect(entries).toHaveLength(1);
  expect(new TextDecoder().decode(entries[0]!.payload)).toBe("first");
});
