...
test("ChatHistoryStore layerFileSystemBun persists events", async () => {
  const program = withTempDir("chat-history-", (dir) =>
    Effect.gen(function* () {
      const layerA = Storage.ChatHistoryStore.layerFileSystemBun({ directory: dir }).pipe(Layer.orDie);
      const layerB = Storage.ChatHistoryStore.layerFileSystemBun({ directory: dir }).pipe(Layer.orDie);
...
test("ArtifactStore layerFileSystemBun persists records", async () => {
  const program = withTempDir("artifact-store-", (dir) =>
    Effect.gen(function* () {
      const layerA = Storage.ArtifactStore.layerFileSystemBun({ directory: dir }).pipe(Layer.orDie);
      const layerB = Storage.ArtifactStore.layerFileSystemBun({ directory: dir }).pipe(Layer.orDie);
