...
test("SyncAuditEventStore records conflict events", async () => {
  ...
  const syncAuditLayer = Sync.layerAuditEventStore.pipe(Layer.provide(auditLayer));
  const journalLayer = Storage.layerKeyValueStore().pipe(Layer.provide(KeyValueStore.layerMemory));

  const layer = Layer.mergeAll(journalLayer, Sync.ConflictPolicy.layerReject("reject"), syncAuditLayer, auditLayer);
