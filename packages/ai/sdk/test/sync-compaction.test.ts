...
test("Compaction.byAge keeps entries within max age", async () => {
  const entries = [makeRemoteEntry(1, 1000), makeRemoteEntry(2, 6000), makeRemoteEntry(3, 9000)];
...
test("Compaction.byCount keeps the latest entries", async () => {
  ...
  const kept = await runEffect(Sync.Compaction.byCount(2)(entries).pipe(Effect.map(extractEntries)));
...
test("Compaction.bySize trims to the most recent entries within the limit", async () => {
  const entries = [makeRemoteEntry(1, 1000, 4), makeRemoteEntry(2, 2000, 8), makeRemoteEntry(3, 3000, 12)];

  const last = entries[2]!.entry;
  const secondLast = entries[1]!.entry;
  const maxBytes = entrySize(last) + entrySize(secondLast) - 1;

  const kept = await runEffect(Sync.Compaction.bySize(maxBytes)(entries).pipe(Effect.map(extractEntries)));
...
test("Compaction.composite applies strategies in order", async () => {
  ...
  const kept = await runEffect(
    ...
    )(entries).pipe(Effect.map(extractEntries))
  );
