...
test("SessionManager.create merges defaults and overrides", async () => {
  ...
  expect((createOptions as { pathToClaudeCodeExecutable?: string })?.pathToClaudeCodeExecutable).toBe("/tmp/claude");
  expect((createOptions as { executableArgs?: string[] })?.executableArgs).toEqual(["--inspect", "--no-warnings"]);
...
test("SessionManager.resume merges defaults and overrides", async () => {
  ...
  expect((resumeOptions as { disallowedTools?: string[] })?.disallowedTools).toEqual(["Override"]);
});
