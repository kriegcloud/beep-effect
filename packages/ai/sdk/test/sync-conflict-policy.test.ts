...
test("ConflictPolicy.lastWriteWins chooses the newest entry", async () => {
  ...
  const result = await resolveWith(Sync.ConflictPolicy.layerLastWriteWins, older, [newer]);
...
test("ConflictPolicy.firstWriteWins chooses the earliest entry", async () => {
  ...
  const result = await resolveWith(Sync.ConflictPolicy.layerFirstWriteWins, newer, [older]);
...
test("ConflictPolicy.reject returns rejection with reason", async () => {
  ...
  const result = await resolveWith(Sync.ConflictPolicy.layerReject("conflict"), entry, [conflict]);
...
test("EventJournalKeyValueStore applies ConflictPolicy during remote writes", async () => {
  ...
  const journalLayer = Storage.layerKeyValueStore().pipe(Layer.provide(KeyValueStore.layerMemory));
  const layer = Layer.mergeAll(journalLayer, Sync.ConflictPolicy.layerReject("reject"), auditLayer);
