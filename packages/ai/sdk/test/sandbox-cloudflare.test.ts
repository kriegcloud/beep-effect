...
const makeReadable = (chunks: ReadonlyArray<Uint8Array>, options?: { readonly close?: boolean }) =>
...
const cloudflareSandboxModule = {
  ...
  getSandbox: (binding: unknown, id: string, options?: { sleepAfter?: string }) => {
...
const makeOptions = (overrides?: Partial<CloudflareSandboxOptions>): CloudflareSandboxOptions => ({
...
const makeSuccessResultMessage = (result: string, uuid: string, sessionId: string): SDKMessage =>
  ({
    type: "result",
    subtype: "success",
    result,
...
describe.sequential("SandboxCloudflare", () => {
  ...
  test("SandboxCloudflare.runAgent parses split NDJSON and strips empty lines", async () => {
    ...
    state.execStreamFactory = () => makeReadable([textEncoder.encode(chunkA), textEncoder.encode(chunkB)]);
    ...
    expect(state.execCalls.some((command) => command.startsWith("rm -f '/tmp/.claude-prompt-"))).toBe(true);
  });
  ...
  test("SandboxCloudflare.runAgent passes --session-id when resume is set", async () => {
    ...
    state.execStreamFactory = () => makeReadable([textEncoder.encode(`${JSON.stringify(resultMessage)}\n`)]);
    ...
    expect(state.execStreamCalls[0]).toMatch(/--model 'haiku'/);
    // Should NOT have --dangerously-skip-permissions since not requested
    expect(state.execStreamCalls[0]).not.toMatch(/--dangerously-skip-permissions/);
  });
  ...
  test("SandboxCloudflare.runAgent handles UTF-8 chunk boundaries", async () => {
    ...
    state.execStreamFactory = () => makeReadable([bytes.slice(0, splitIndex), bytes.slice(splitIndex)]);
  ...
  test("SandboxCloudflare.runAgent share and broadcast fan out output", async () => {
    ...
    const sharedCollected = await runInSandbox(
      Effect.gen(function* () {
        ...
        const [first, second] = yield* Effect.all([Stream.runCollect(shared), Stream.runCollect(shared)], {
