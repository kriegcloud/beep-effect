...
const debugEnabled = Bun.env.SYNC_TEST_DEBUG === "1" || Bun.env.SYNC_TEST_DEBUG === "true";
const debug = (...args: Array<unknown>) => {
  if (debugEnabled) console.log("[sync-test]", ...args);
};
const debugEffect = (...args: Array<unknown>) => (debugEnabled ? Effect.sync(() => debug(...args)) : Effect.void);
...
const makeReplicaLayer = (url: string, kv: KeyValueStore.KeyValueStore, options: { readonly prefix: string }) => {
  const baseLayer = Storage.ChatHistoryStore.layerJournaledWithEventLog({
    prefix: options.prefix,
    identityKey: sharedIdentityKey,
  });
  const syncLayer = Sync.SyncService.layerWebSocket(url, { disablePing: true }).pipe(Layer.provide(baseLayer));
