...
test("ArtifactStore.put compensates record write when index save fails", async () => {
  ...
  const layer = Storage.ArtifactStore.layerKeyValueStore({ prefix }).pipe(Layer.provide(control.layer));
...
test("ChatHistoryStore.appendMessage compensates event write when meta save fails", async () => {
  ...
  const layer = Storage.ChatHistoryStore.layerKeyValueStore({ prefix }).pipe(Layer.provide(control.layer));
...
test("ChatHistoryStore.cleanup repairs trailing meta gaps", async () => {
  ...
  const chatLayer = Storage.ChatHistoryStore.layerKeyValueStore({ prefix }).pipe(Layer.provide(control.layer));
  const sessionIndexLayer = Storage.SessionIndexStore.layerKeyValueStore({
    prefix: indexPrefix,
  }).pipe(Layer.provide(control.layer));

  const layer = Layer.mergeAll(chatLayer, sessionIndexLayer, Storage.StorageConfig.layer);
