...
test("QueryHandle.share multicasts within scope", async () => {
  ...
  const program = Effect.scoped(
    Effect.gen(function* () {
      ...
      const [left, right] = yield* Effect.all([Stream.runCollect(shared), Stream.runCollect(shared)], {
        concurrency: "unbounded",
      });

      return [Chunk.toReadonlyArray(left), Chunk.toReadonlyArray(right)];
    })
  );
...
test("QueryHandle.broadcast fans out streams within scope", async () => {
  ...
  const program = Effect.scoped(
    Effect.gen(function* () {
      ...
      const [left, right] = yield* Effect.all([Stream.runCollect(first), Stream.runCollect(second)], {
        concurrency: "unbounded",
      });
...
test("QueryHandle.share replays to late subscribers while upstream is active", async () => {
  ...
  const program = Effect.scoped(
    Effect.gen(function* () {
      ...
      const [firstChunk, secondChunk] = yield* Effect.all([Fiber.join(firstFiber), Fiber.join(secondFiber)], {
        concurrency: "unbounded",
      });
...
test("QueryHandle.stream is single-use across runs", async () => {
  ...
  const program = Effect.gen(function* () {
    ...
    return [Chunk.toReadonlyArray(first), Chunk.toReadonlyArray(second)];
  });
...
test("QueryHandle.sendAll fails after closeInput", async () => {
  const program = Effect.scoped(
    Effect.gen(function* () {
      ...
      yield* handle.closeInput;
      return yield* Effect.either(handle.sendAll([makeUserMessage("first"), makeUserMessage("second")]));
    })
  );
