  ...
  Stream.fromQueue(request.output).pipe(
    ...
    Stream.ensuring(
      Ref.set(request.consumerDetached, true).pipe(
        Effect.andThen(Queue.shutdown(request.output).pipe(Effect.ignore))
      )
    )
  )
...
export const makeSessionTurnDriver = ({
  ...
  Effect.gen(function*() {
    ...
    const processTurnRequest = (request: TurnRequest) =>
      Effect.gen(function*() {
        ...
        yield* markActiveTurn(request.id).pipe(
          Effect.andThen(logState(`turn-active:${request.id}`))
        )
        yield* runTurn(request).pipe(
          Effect.catch((error) => failTurn(request, error))
        )
      }).pipe(
        Effect.ensuring(
          finishTurn.pipe(
            Effect.andThen(logState(`turn-finished:${request.id}`))
          )
        )
      )
    ...
    yield* Effect.addFinalizer(() =>
      shutdown.pipe(
        Effect.andThen(Queue.shutdown(turnQueue).pipe(Effect.ignore))
      )
    )
    ...
    const sendRaw = Effect.fn("SessionTurnDriver.sendRaw")(
      (message: string | SDKUserMessage) =>
        Effect.acquireUseRelease(
          beginRaw.pipe(
            Effect.andThen(logState("raw-send-begin"))
          ),
          () => send(message),
          () =>
            endRaw.pipe(
              Effect.andThen(logState("raw-send-end"))
            )
        )
    )
    ...
    const streamRawBase = Stream.unwrapScoped(
      Effect.gen(function*() {
        ...
        yield* Effect.addFinalizer(() =>
          endRaw.pipe(
            Effect.andThen(logState("raw-stream-end"))
          )
        )
        return stream
      })
