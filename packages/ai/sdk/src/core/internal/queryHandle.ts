...
export type SdkQueryLike = AsyncIterable<SDKMessage> & {
  ...
  rewindFiles: (userMessageUuid: string, options?: undefined | {
...
const inputUnavailable = (label: string) =>
  Effect.fail(
    TransportError.make(
      label
    )
...
export const makeQueryHandle = (
  ...
  const broadcast = <N extends number>(n: N, maximumLag?: undefined | StreamBroadcastConfig) =>	Stream.broadcast(
				stream,
	    {
        capacity: n,
				replay: maximumLag ?? defaultBroadcastLag
	    }
			)
  ...
  const rewindFiles = Effect.fn("QueryHandle.rewindFiles")(
    (userMessageUuid: string, options?: undefined | {readonly dryRun?: undefined | boolean }) =>
