...
const makeAgentSdkConfig = Effect.gen(function*() {
  ...
  const executable = yield* Config.option(
    Config.schema( Schema.Literals(["bun", "deno", "node"]), "EXECUTABLE")
  )
  ...
  const permissionMode = yield* Config.option(
    Config.schema( PermissionMode, "PERMISSION_MODE")
  )
  ...
  const sandboxNetworkHttpProxyPort = yield* Config.option(
    Config.int("SANDBOX_NETWORK_HTTP_PROXY_PORT")
  )
  const sandboxNetworkSocksProxyPort = yield* Config.option(
    Config.int("SANDBOX_NETWORK_SOCKS_PROXY_PORT")
  )
  ...
  const sandboxProviderValue = yield* Config.option(
    Config.schema( SandboxProviderSchema, "SANDBOX_PROVIDER")
  )
  ...
  const storageBackendValue = yield* Config.option(
    Config.schema(StorageBackendSchema, "STORAGE_BACKEND")
  )
  const storageModeValue = yield* Config.option(
    Config.schema(StorageModeSchema, "STORAGE_MODE" )
  )
...
export class AgentSdkConfig extends ServiceMap.Service<AgentSdkConfig, Effect.Success<typeof makeAgentSdkConfig>>()(
  ...
  static readonly layerWithOverrides = (overrides: {
    ...
    const provider = ConfigProvider.orElse(
      ConfigProvider.fromUnknown(new Map(entries)),
      ConfigProvider.fromEnv()
    )
    return AgentSdkConfig.layer.pipe(
      Layer.provide(ConfigProvider.layerAdd(provider))
    )
  }
  /**
