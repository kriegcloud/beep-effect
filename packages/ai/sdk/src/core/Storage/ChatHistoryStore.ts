...
const normalizeRange = (lastSequence: number, options?: ChatHistoryListOptions) => {
  if (lastSequence <= 0) {
    return {
      start: 1,
      end: 0,
...
const resolveListLimit = (options: ChatHistoryListOptions | undefined, fallback?: number) => options?.limit ?? fallback;
...
const resolveEnabled = Effect.gen(function* () {
  const config = yield* Effect.serviceOption(StorageConfig);
  return O.isNone(config) ? true : config.value.settings.enabled.chatHistory;
});
...
const logIndexWarning = (operation: string, sessionId: string, cause: unknown) =>
  Effect.logWarning(`[ChatHistoryStore] session index ${operation} failed for session=${sessionId}: ${String(cause)}`);
...
const touchSessionIndex = (sessionId: string, timestamp: number) =>
  ...
  }).pipe(Effect.catch((cause) => logIndexWarning("touch", sessionId, cause).pipe(Effect.asVoid)));
...
const layerChatJournalHandlers = (options?: { readonly prefix?: string }) =>
  EventLogModule.group(ChatEventGroup, (handlers) =>
    handlers.handle(ChatEventTag, ({ payload }) =>
      Effect.gen(function* () {
        ...
        const eventKey = (sessionId: string, sequence: number) => `${prefix}/${sessionId}/events/${sequence}`;
          ...
          Effect.gen(function* () {
            ...
            if (retention.maxAgeMs !== undefined) {
              ...
              const events = yield* Effect.forEach(
                ...
                (sequence) =>
                  eventStore
                    .get(eventKey(sessionId, sequence))
                    .pipe(Effect.mapError((cause) => mapError("retention", cause))),
                { discard: false }
              );
...
const layerChatJournalCompaction = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const strategy = strategies.length === 1 ? strategies[0]! : Compaction.composite(...strategies);
...
const journaledEventLogLayer = <R = never>(
  ...
  const baseLayer = EventLogModule.layerEventLog.pipe(
    Layer.provide(layerEventJournalKeyValueStore({ key: keys.journalKey })),
...
const makeJournaledStore = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const repairTrailingMetaGap = (sessionId: string, meta: ChatMeta) =>
      Effect.gen(function* () {
        ...
        while (nextLastSequence > 0) {
          const eventOption = yield* eventStore
            .get(eventKey(sessionId, nextLastSequence))
            .pipe(Effect.mapError((cause) => toStorageError(storeName, "cleanup", cause)));
          if (O.isSome(eventOption)) {
            break;
          }
          nextLastSequence -= 1;
        }
    ...
    const appendMessages = Effect.fn("ChatHistoryStore.appendMessages")(function* (
      sessionId: string,
      ...
      yield* Effect.forEach(
        ...
        (event) =>
          log
            ...
            .pipe(Effect.mapError((cause) => toStorageError(storeName, "appendMessages", cause))),
        { discard: true }
      );
      return events;
    });

    const list = Effect.fn("ChatHistoryStore.list")(function* (sessionId: string, options?: ChatHistoryListOptions) {
      const config = yield* Effect.serviceOption(StorageConfig);
      const defaultLimit = O.getOrUndefined(O.map(config, (value) => value.settings.pagination.chatPageSize));
      const metaOption = yield* metaStore
        .get(metaKey(sessionId))
        .pipe(Effect.mapError((cause) => toStorageError(storeName, "list", cause)));
      if (O.isNone(metaOption)) return [];
      ...
      const { start, end, limit, reverse } = normalizeRange(meta.lastSequence, rangeOptions);
      if (limit <= 0) return [];
      ...
      return events.flatMap((eventOption) => (O.isSome(eventOption) ? [eventOption.value] : []));
    });
    ...
    const purge = Effect.fn("ChatHistoryStore.purge")((sessionId: string) =>
      Effect.gen(function* () {
        const metaOption = yield* metaStore
          .get(metaKey(sessionId))
          .pipe(Effect.mapError((cause) => toStorageError(storeName, "purge", cause)));
        if (O.isNone(metaOption)) return;
    ...
    const cleanup = Effect.fn("ChatHistoryStore.cleanup")(function* () {
      ...
      yield* Effect.forEach(
        ...
        (sessionId) =>
          metaStore.get(metaKey(sessionId)).pipe(
            Effect.mapError((cause) => toStorageError(storeName, "cleanup", cause)),
                    ...
                    if (retentionValue.maxAgeMs !== undefined) {
                      ...
                      const events = yield* Effect.forEach(
                        ...
                        (sequence) =>
                          eventStore
                            .get(eventKey(sessionId, sequence))
                            .pipe(Effect.mapError((cause) => toStorageError(storeName, "retention", cause))),
                        { discard: false }
                      );
...
export class ChatHistoryStore extends ServiceMap.Service<ChatHistoryStore, ChatHistoryStoreShape>()(
      ...
      Effect.gen(function* () {
        ...
        const appendMessages = Effect.fn("ChatHistoryStore.appendMessages")(function* (
          ...
          yield* Effect.forEach(
            ...
            (event) =>
              eventStore
                .set(eventKey(sessionId, event.sequence), event)
                .pipe(Effect.mapError((cause) => toStorageError(storeName, "appendMessages", cause))),
            { discard: true }
          );
  ...
  static readonly layerJournaledWithSyncWebSocket: <R = never>(
    ...
    const syncOptions =
      options?.disablePing !== undefined || options?.protocols !== undefined
        ...
        : undefined;
    let syncLayer = SyncService.layerWebSocket(url, syncOptions).pipe(Layer.provide(baseLayer));
    ...
    return Layer.effect(ChatHistoryStore, Effect.service(ChatHistoryStore)).pipe(Layer.provide(combined));
  };
  ...
  static readonly layerJournaledFileSystem = (options?: {
    ...
    ChatHistoryStore.layerJournaled(resolveJournaledOptions(options)).pipe(
      Layer.provide(KeyValueStore.layerFileSystem(options?.directory ?? defaultStorageDirectory))
    );
