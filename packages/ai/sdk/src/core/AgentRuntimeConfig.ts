...
const makeAgentRuntimeConfig = Effect.gen(function* () {
	...
	const settings: AgentRuntimeSettings = {
		...
		queryTimeout: Option.getOrElse(
			queryTimeout,
			() => defaultSettings.queryTimeout
		),
		...
		retryMaxRetries: Math.max(
			...
			Option.getOrElse(
				retryMaxRetries,
				() => defaultSettings.retryMaxRetries
			)
		),
		retryBaseDelay: Option.getOrElse(
			retryBaseDelay,
			() => defaultSettings.retryBaseDelay
		)
	};
...
export class AgentRuntimeConfig extends ServiceMap.Service<AgentRuntimeConfig, {
	 ...
	 */
	static readonly layerFromEnv = (prefix = "AGENTSDK") => AgentRuntimeConfig.layer.pipe(Layer.provide(layerConfigFromEnv(prefix)));
	...
	static readonly layer = Layer.effect(
		AgentRuntimeConfig,
		makeAgentRuntimeConfig
	);

	/**
	 * Build AgentRuntimeConfig with explicit overrides applied to defaults.
	 */
	static readonly layerWith = (overrides?: undefined | Partial<AgentRuntimeSettings>) => Layer.effect(
		AgentRuntimeConfig,
		Effect.succeed({
