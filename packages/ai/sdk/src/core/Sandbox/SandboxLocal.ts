...
const make = Effect.gen(function* () {
  ...
  const writeFile = Effect.fn("SandboxLocal.writeFile")((path: string, content: string) =>
    Effect.tryPromise({
  ...
  const readFile = Effect.fn("SandboxLocal.readFile")((path: string) =>
    Effect.tryPromise({
  ...
  const runAgent = Effect.fn("SandboxLocal.runAgent")(
    (prompt: string, options?: import("../Schema/Options.js").Options) =>
      supervisor.submit(prompt, options).pipe(Effect.mapError((cause) => mapError("runAgent", cause)))
  );
