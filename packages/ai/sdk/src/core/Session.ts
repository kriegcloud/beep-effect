 ...
 */
export class SessionClosedError extends Schema.TaggedErrorClass<SessionClosedError>()(
  "SessionClosedError",
  {
...
const sessionClosed = (message: string) =>
  SessionClosedError.make(message)
...
export const fromSdkSession = Effect.fn("Session.fromSdkSession")(function*(
  ...
  const stream = Stream.scoped(Stream.fromEffect(
    Effect.uninterruptibleMask((restore) =>
      Effect.gen(function*() {
        ...
        yield* Effect.addFinalizer(() =>
          streamSemaphore.release(1).pipe(Effect.andThen(endStream))
        )
