...
export const resolveTurnTimeouts = (
  ...
  runtime.turnSendTimeout !== undefined || runtime.turnResultTimeout !== undefined
    ? {
        ...(runtime.turnSendTimeout !== undefined
          ? { turnSendTimeout: runtime.turnSendTimeout }
          : {}),
        ...(runtime.turnResultTimeout !== undefined
          ? { turnResultTimeout: runtime.turnResultTimeout }
          : {})
      }
    : undefined
...
const makeSessionConfig = Effect.gen(function*() {
  ...
  const executable = yield* Config.option(
    Config.schema(Schema.Literals(["bun", "node"]), "EXECUTABLE")
  )
  ...
  const permissionMode = yield* Config.option(
    Config.schema(SessionPermissionMode, "PERMISSION_MODE")
  )
  ...
  const runtime: SessionRuntimeSettings = {
    ...
    ...(Option.isSome(turnSendTimeout)
      ? { turnSendTimeout: turnSendTimeout.value }
      : {}),
    ...(Option.isSome(turnResultTimeout)
      ? { turnResultTimeout: turnResultTimeout.value }
      : {})
  }
