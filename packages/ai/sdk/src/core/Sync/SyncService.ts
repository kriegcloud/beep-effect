...
const remoteIdToString = (remoteId: Uint8Array) => Array.from(remoteId)
...
export class SyncConfig extends ServiceMap.Service<SyncConfig, SyncConfigOptions>()(
	...
	static readonly layer = (options: SyncConfigOptions) => Layer.succeed(
		SyncConfig,
		SyncConfig.of(options)
	);
}
...
export class SyncService extends ServiceMap.Service<SyncService, {
	...
	static readonly layerMemory: (options?: SyncServiceWebSocketOptions) => Layer.Layer<SyncService, unknown, EventLogModule.EventLog> = (options) => Layer.unwrap(Effect.gen(function* () {
...
function makeWithWebSocket(
	...
	return Effect.gen(function* () {
		...
		yield* service.connectWebSocket(
			url,
			options
		);
		return service;
...
function makeService() {
	return Effect.gen(function* () {
		...
		const buildStatus = (input: {
			...
			connected: input.connected, ...(input.url !== undefined
			                                ? {url: input.url}
		...
		const mergeStatus = (
			...
			return buildStatus({
				...
				connected: next.connected ?? previous?.connected ?? false, ...(url !== undefined
				                                                               ? {url}
			...
			(map) => {
				...
				next.set(
					...
					mergeStatus(
						...
						{
							key,
							connected: false, ...(error !== undefined
							                      ? {lastError: error}
							                      : {})
						}
					)
		...
		const markSynced = (key: string) => Effect.gen(function* () {
			const now = yield* Clock.currentTimeMillis;
			yield* SubscriptionRef.update(
				statusRef,
				(map) => {
		...
		const runTracked = <R>(
			...
			Scope.provide(scope)(
				effect.pipe(
					Effect.catchCause((cause) => markDisconnected(
						key,
						Cause.pretty(cause)
					)),
		...
		const removeConnector = (key: string) => Ref.update(
			connectorsRef,
			(map) => {
				const next = new Map(map);
				next.delete(key);
		...
		const connectInternal = Effect.fn("SyncService.connectInternal")(
			function* (
				key: string,
				kind: RemoteKind,
				effect: Effect.Effect<void, never, Scope.Scope>,
				...
				const hasFiber = yield* FiberMap.has(
					fibers,
					key
				);
				if (hasFiber) {
				...
				yield* runTracked(
					key,
					effect,
					{onlyIfMissing: !hasFiber}
				);
			});

		const wrapRemote = (
			remote: EventLogRemoteService,
			key: string
		): EventLogRemoteService => ({
			id: remote.id,
			write: (
				identity,
				entries
			...
			.pipe(Effect.tap(() => markSynced(key))),
			changes: (
				identity,
				startSequence
			) => remote.changes(
		...
		const trackedEventLog = (key: string) => EventLogModule.EventLog.of({
			...log,
			registerRemote: (remote) => {
				const remoteId = remoteIdToString(remote.id);
				const trackedRemote = wrapRemote(
					remote,
					key
				);
				return log.registerRemote(trackedRemote)
				.pipe(
					Effect.tap(() => updateRemoteId(
						key,
						remoteId
					)),
					Effect.tap(() => markConnected(
						key,
						"url",
						key
					))
		...
		const connectSocket = Effect.fn("SyncService.connectSocket")(
			function* (
				key: string,
				options?: SyncServiceWebSocketOptions
			) {
				const socket = yield* Option.match(
					socketOption,
					{
						onNone: () => Effect.die(new Error("SyncService.connectSocket requires Socket.Socket. Provide BunSocket.layerNet.")),
						onSome: (service) => Effect.succeed(service)
					}
				);
				const effect = EventLogRemote.fromSocket(options)
				.pipe(
		...
		const connect = Effect.fn("SyncService.connect")(
			function* (remote: EventLogRemoteService) {
				...
				yield* connectInternal(
					key,
					"remoteId",
					log.registerRemote(trackedRemote)
					.pipe(
						Effect.tap(() => updateRemoteId(
							key,
							key
		...
		const connectWebSocket = Effect.fn("SyncService.connectWebSocket")(
			function* (
				url: string,
				options?: SyncServiceWebSocketOptions
			) {
				const webSocketConstructor = yield* Option.match(
					webSocketConstructorOption,
					{
						onNone: () => Effect.die(new Error("SyncService.connectWebSocket requires Socket.WebSocketConstructor. Provide BunSocket.layerWebSocketConstructor.")),
		...
		const toKey = (input: RemoteKey | string) => typeof input === "string"
		                                             ? input
		                                             : input.key;

		const disconnect = Effect.fn("SyncService.disconnect")(
		...
		const disconnectWebSocket = Effect.fn("SyncService.disconnectWebSocket")(
			(url: string) => disconnect(url));
			...
			() => syncNowSemaphore.withPermits(1)(
				Effect.gen(function* () {
					...
					yield* Effect.forEach(
						connectors,
						([key, connector]) => FiberMap.remove(
							fibers,
							key
						)
						.pipe(Effect.andThen(runTracked(
							key,
							connector.effect,
							{onlyIfMissing: false}
						))),
		...
		const status = Effect.fn("SyncService.status")(
			...
			.pipe(Effect.map((map) => Array.from(map.values()))));

		const statusStream = () => SubscriptionRef.changes(statusRef)
		.pipe(Stream.map((map) => Array.from(map.values())));
