...
export const makeScopedDb = <TFullSchema extends Record<string, unknown> = Record<string, never>>(schema: TFullSchema) => {
  const makeService = (
  config: Config,
) =>
  Effect.gen(function* () {
          ...
          Effect.flatMap((runPromiseExit) =>
            Effect.async<T, DbErrors.DbError | E, R>((resume) => {
              db.transaction(async (tx: TransactionClient<TFullSchema>) => {
                const txWrapper = (
                  fn: (client: TransactionClient<TFullSchema>) => Promise<any>,
                ) =>
                  Effect.tryPromise({
                    try: () => fn(tx),
                    catch: (cause) => {
