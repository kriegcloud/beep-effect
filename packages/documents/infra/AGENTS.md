...
- **`DocumentsDb.DocumentsDb`** — Context tag to access the shared database connection and migrations (`packages/documents/infra/src/db/Db.ts`).
- **`DocumentsRepos.layer`** — Aggregated layer exporting all files repositories for vertical slices (`packages/documents/infra/src/adapters/repositories.ts`).
- **`FileRepo` service** — Repo constructed via `Repo.make`, ready for custom queries inside the `effect` block (`packages/documents/infra/src/adapters/repos/File.repo.ts`).
- **`FilesConfig` tag + helpers** — Projects `serverEnv` into an S3-focused config, with `Live` and `layerFrom` entry points (`packages/documents/infra/src/config.ts`).
- **`StorageService`** — Effect service that wraps `S3Service.putObject` to mint pre-signed upload URLs (`packages/documents/infra/src/SignedUrlService.ts`).
...
- When new tables land in `@beep/shared-tables/schema`, update `DocumentsDb` consumers and refresh migrations via `packages/_internal/db-admin` before trusting runtime layers.
...
- **Hydrate FileRepo with a temporary Postgres client (integration tests)**
  ```ts
  import { FileRepo } from "@beep/documents-infra/adapters/repos";
...
- Update `packages/documents/infra/AGENTS_MD_PLAN.md` whenever exports or dependencies change.
