# AGENTS — `@beep/files-tables`

## Purpose & Fit
- Define Drizzle tables and relations for the files slice, exposing a schema namespace (`FilesDbSchema`) consumed by `@beep/files-infra` and shared runtimes.
- Bridge `@beep/files-domain` entities/value objects to storage while honoring shared multi-tenant defaults from `@beep/shared-tables`.
...
- Keep exports centralized via `src/schema.ts` and `src/index.ts` so infra layers can import `FilesDbSchema` without reaching into `tables/`.
...
## Verifications
- `bun run check --filter=@beep/files-tables`
- `bun run lint --filter=@beep/files-tables`
- `bun run test --filter=@beep/files-tables` (add coverage as schemas arrive).
