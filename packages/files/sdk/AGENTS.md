# AGENTS — `@beep/files-sdk`

## Purpose & Fit
- Host Effect-first client helpers for the files slice (upload flows, signed URL retrieval, metadata decoding) so apps and CLIs do not talk to infra endpoints directly.
- Provide a thin layer over `@beep/files-domain` value objects and any RPC/HTTP contracts exported by server runtimes.
...
- Validate all external data with schemas from `@beep/common/schema` or `@beep/files-domain` before exposing to callers.
...
- `bun run test --filter=@beep/files-sdk` (add focused tests as soon as real code lands).
...
- [ ] Added or updated tests under `packages/files/sdk/test/`.
