# @beep/data Agent Guide

## Purpose & Fit
- A library containing data values & utilities
...
## Quick Recipes
```ts
import { VERSION } from "@beep/data"
```

## Verifications
- `bunx turbo run test --filter=@beep/data`
- `bunx turbo run lint --filter=@beep/data`
- `bunx turbo run check --filter=@beep/data`
