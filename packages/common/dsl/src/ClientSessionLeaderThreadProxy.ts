...
import type { BeepEvent } from "./schema/mod.ts";
...
export interface ClientSessionLeaderThreadProxy {
  events: {
    ...
    push(batch: ReadonlyArray<BeepEvent.Client.Encoded>): Effect.Effect<void, UnknownError | LeaderAheadError>;
    /** Stream events with filtering */
    stream(options: StreamEventsOptions): Stream.Stream<BeepEvent.Client.Encoded, UnknownError>;
  };
