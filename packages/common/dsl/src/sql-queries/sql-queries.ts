...
export const findManyRows = <TColumns extends PgDSL.Columns>({
  columns,
  tableName,
  where,
  limit,
...
export const countRows = <TColumns extends PgDSL.Columns>({
  columns,
  tableName,
  where,
}: {
...
export const insertRow = <TColumns extends PgDSL.Columns>({
  tableName,
  columns,
  values,
...
export const insertRowPrepared = <TColumns extends PgDSL.Columns>({
  tableName,
  columns,
  options = { orReplace: false },
}: {
...
export const insertRows = <TColumns extends PgDSL.Columns>({
  columns,
  tableName,
  valuesArray,
}: {
  ...
  // NOTE consider batching for large arrays (https://pg.org/forum/info/f832398c19d30a4a)
...
export const insertOrIgnoreRow = <TColumns extends PgDSL.Columns>({
  columns,
  tableName,
  values: values_,
  returnRow,
...
export const updateRows = <TColumns extends PgDSL.Columns>({
  columns,
  tableName,
...
export const deleteRows = <TColumns extends PgDSL.Columns>({
  columns,
  tableName,
  where,
}: {
...
export const upsertRow = <TColumns extends PgDSL.Columns>({
  tableName,
  columns,
...
export const createTable = ({
  ...
  table: PgDSL.TableDefinition<any, PgDSL.Columns>;
  tableName: string;
}): string => {
...
export const makeBindValues = <TColumns extends PgDSL.Columns, TKeys extends keyof TColumns>({
  columns,
  values,
  variablePrefix = "",
  skipNil,
  ...
  const codecMap = pipe(
    columns,
    objectEntries,
    ArrayUtils.Readonly.mapReadonly(([columnName, columnDef]) => [
      columnName,
      (value: any) => {
...
const buildWhereSql = <TColumns extends PgDSL.Columns>({
  where,
}: {
  where: ClientTypes.WhereValuesForColumns<TColumns>;
}) => {
  const getWhereOp = (columnName: string, value: ClientTypes.WhereValueForDecoded<any>) => {
    ...
    if (typeof value === "object" && typeof value.op === "string" && ClientTypes.isValidWhereOp(value.op)) {
      return `${value.op} $where_${columnName}`;
    }
    if (typeof value === "object" && typeof value.op === "string" && value.op === "in") {
  ...
  return pipe(
    where,
    objectEntries,
    ArrayUtils.Readonly.mapReadonly(([columnName, value]) => `${columnName} ${getWhereOp(columnName, value)}`),
    A.join(" AND ")
  );
};
