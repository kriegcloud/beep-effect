import { isDevEnv, isNil, isReadonlyArray } from "@beep/utils";
import { Hash, Option, Schema } from "effect";
...
import { isQueryBuilder } from "./schema/state/pg/query-builder/api.ts";
import { getResultSchema } from "./schema/state/pg/query-builder/impl.ts";
...
export const getExecStatementsFromMaterializer = ({
  ...
  event:
    | { decoded: BeepEvent.Client.Decoded; encoded: undefined }
    | { decoded: undefined; encoded: BeepEvent.Client.Encoded };
}): ReadonlyArray<{
  statementSql: string;
...
export const makeMaterializerHash =
  ({ schema, dbState }: { schema: LiveStoreSchema; dbState: PgDb }) =>
  (event: BeepEvent.Client.Encoded): Option.Option<number> => {
    if (isDevEnv()) {
...
const fromMaterializerResult = (
  ...
  if (typeof materializerResult === "string") {
