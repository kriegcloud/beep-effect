 ...
 *
 * How it works:
 * 1. Each table's schema is hashed using PgAst.hash()
...
import { makeColumnSpec } from '../schema/state/pg/column-spec.ts'
import { PgAst } from '../schema/state/pg/db-schema/mod.ts'
import type { SchemaEventDefsMetaRow, SchemaMetaRow } from '../schema/state/pg/system-tables/state-tables.ts'
import {
  isStateSystemTable,
  ...
  stateSystemTables,
} from '../schema/state/pg/system-tables/state-tables.ts'
...
export const makeSchemaManager = (db: PgDb): Effect.Effect<SchemaManager> =>
  Effect.gen(function* () {
    yield* migrateTable({
      db,
      tableAst: schemaEventDefsMetaTable.pgDef.ast,
...
export const migrateDb = ({
  ...
  Effect.gen(function* () {
    ...
    const tableDefs = [
      ...
      ...stateSystemTables,
      ...Array.from(schema.state.pg.tables.values()).filter((_) => !isStateSystemTable(_.pgDef.name)),
    ]

    const tablesToMigrate = new Set<{ tableAst: PgAst.Table; schemaHash: number }>()

    const migrationsReportEntries: MigrationsReportEntry[] = []
    for (const tableDef of tableDefs) {
      const tableAst = tableDef.pgDef.ast
      const tableName = tableAst.name
      const dbSchemaHash = dbSchemaHashByTable[tableName]
      const schemaHash = PgAst.hash(tableAst)
...
export const migrateTable = ({
  db,
  tableAst,
  schemaHash = PgAst.hash(tableAst),
  behaviour,
  skipMetaTable = false,
}: {
  db: PgDb
  tableAst: PgAst.Table
  schemaHash?: number
...
const createIndexFromDefinition = (tableName: string, index: PgAst.Index) => {
