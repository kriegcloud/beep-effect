 ...
 * @example
 * ```ts
 * import { Events } from '@beep/livestore'
 * import { Schema } from 'effect'
 * import { deprecated } from '@beep/common/schema'
 *
...
export const logDeprecationWarnings = (
  ...
  Effect.gen(function* () {
    ...
    if (eventDeprecation !== undefined && !warnedDeprecatedEvents.has(eventName)) {
      ...
      yield* Effect.logWarning("@beep/schema:deprecated-event", {
        event: eventName,
    ...
    for (const { field, reason } of deprecatedFields) {
      ...
      if (!warnedDeprecatedFields.has(key)) {
        ...
        yield* Effect.logWarning("@beep/schema:deprecated-field", {
          event: eventName,
          field,
          reason,
        });
