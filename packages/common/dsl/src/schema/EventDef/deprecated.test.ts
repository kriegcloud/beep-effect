...
describe("deprecated annotations", () => {
  ...
  test("works with optional fields in Struct", () => {
    ...
    expect(findDeprecatedFieldsWithValues(struct, { oldField: "x" })).toEqual([
      { field: "oldField", reason: "Legacy" },
    ]);
  });
...
describe("logDeprecationWarnings", () => {
  ...
  test("logs event deprecation warning", () => {
    ...
    expect(logs).toEqual([["@beep/schema:deprecated-event", { event: "Old", reason: "Use New" }]]);
  });
  ...
  test("logs field deprecation warning", () => {
    ...
    expect(logs).toEqual([["@beep/schema:deprecated-field", { event: "Ev", field: "old", reason: "Use new" }]]);
  });
