...
import type * as BeepEvent from "./BeepEvent/mod.ts";
...
export type UnknownEventContext = {
  readonly event: Pick<BeepEvent.Client.Encoded, "name" | "args" | "seqNum" | "clientId" | "sessionId">;
...
const handleUnknownEvent = ({
  ...
  Effect.gen(function* () {
    ...
    switch (config.strategy) {
      ...
      case "warn": {
        yield* Effect.logWarning("@beep/common:schema:unknown-event", context);
        return;
      }
      case "ignore": {
        return;
      ...
      case "callback": {
        ...
        yield* Effect.tryAll<void>(() => callback(context, error)).pipe(
          Effect.catchAll((cause) =>
            Effect.logWarning("@beep/common:schema:unknown-event:callback-error", {
              event: context.event,
