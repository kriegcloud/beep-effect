 ...
 * const decoded = Schema.decodeUnknownSync(BeepEvent.Input.Encoded)(rawEvent)
 * ```
 */
export const Encoded = Schema.Struct({
  name: Schema.String,
  args: Schema.Any,
}).annotations({ title: "BeepEvent.Input.Encoded" });
 ...
 * @example
 * ```ts
 * const inputSchema = BeepEvent.Input.makeSchema(schema)
...
export const makeSchema = <TSchema extends LiveStoreSchema>(schema: TSchema): ForRecord<TSchema["_EventDefMapType"]> =>
  ...
  ).annotations({ title: "BeepEvent.Input" }) as any;
