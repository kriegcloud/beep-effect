import { memoizeByRef } from "@beep/dsl/util";
...
}).annotations({ title: "BeepEvent.Client.Decoded" });
 ...
 * const event: BeepEvent.Client.Encoded = {
 *   name: 'todoCreated-v1',
 ...
 * const pending: BeepEvent.Client.Encoded = {
 *   ...event,
...
export const Encoded = Schema.Struct({
  ...
  clientId: Schema.String,
  sessionId: Schema.String,
}).annotations({ title: "BeepEvent.Client.Encoded" });
 ...
 *
 * Note: This class is exported for internal use. The preferred access is via `BeepEvent.Client.EncodedWithMeta`.
 */
export class EncodedWithMeta extends Schema.Class<EncodedWithMeta>("BeepEvent.Client.EncodedWithMeta")({
  name: Schema.String,
  args: Schema.Any,
  seqNum: EventSequenceNumber.Client.Composite,
  parentSeqNum: EventSequenceNumber.Client.Composite,
  ...
  meta: Schema.Struct({
    ...
    syncMetadata: Schema.Option(BS.Json),
  ...
  static fromGlobal = (
    ...
    meta: {
      syncMetadata: Option.Option<BS.Json.Type>;
 ...
 * @example
 * ```ts
 * const eventSchema = BeepEvent.Client.makeSchema(schema)
...
export const makeSchema = <TSchema extends LiveStoreSchema>(
  ...
  ).annotations({ title: "BeepEvent.Client" }) as any;
