...
> => {
  ...
  const columns = {
    id: PgDSL.text({ primaryKey: true }),
    value: PgDSL.json({ schema: optimisticColumnSchema }),
  };
...
export const deriveEventAndMaterializer = ({
  ...
  const derivedSetMaterializer = defineMaterializer(derivedSetEventDef, ({ id, value }) => {
    ...
    const schemaProps = BS.getResolvedPropertySignatures(valueSchema);
    if (schemaProps.length === 0 || partialSet === false) {
      const valueColJsonSchema = Schema.parseJson(valueSchema);
      const encodedInsertValue = BS.encodeSyncDebug(valueColJsonSchema)(value ?? defaultValue);
      const encodedUpdateValue = BS.encodeSyncDebug(valueColJsonSchema)(value);
    ...
    const encodedInsertValue = BS.encodeSyncDebug(valueColJsonSchema)(mergeDefaultValues(defaultValue, value));
    ...
    const encodedPartialUpdate = BS.encodeSyncDebug(partialUpdateSchema)(value);
    ...
    const onConflictClause =
      setBindValues.length > 0 ? `ON CONFLICT (id) DO UPDATE SET value = ${jsonSetSql}` : "ON CONFLICT (id) DO NOTHING";
...
} & ClientDocumentTableDef.Trait<TTableDef["pgDef"]["name"], any, any, any> =>
  tableDef.options.isClientDocumentTable === true;
...
const makeGetQueryBuilder = <TTableDef extends ClientDocumentTableDef<any, any, any, any>>(
  ...
  return ((...args: any[]) => {
    ...
    const query = sql`SELECT * FROM '${tableDef.pgDef.name}' WHERE id = ?`;
...
> = TableDef<
  ClientDocumentTableDef.PgDef<TName, TType>,
  {
...
export namespace ClientDocumentTableDef {
  ...
  export type PgDef<TName extends string, TType> = PgDSL.TableDefinition<
    TName,
    {
      id: PgDSL.ColumnDefinition<string, string> & { default: Option.Some<string> };
      value: PgDSL.ColumnDefinition<string, TType> & { default: Option.Some<TType> };
    }
  >;
