...
import { casesHandled, shouldNeverHappen } from "@beep/dsl/util";
  ...
  const api = {
    ...
    where: function () {
      ...
      if (arguments.length === 1) {
        ...
        const newOps = Struct.entries(params)
    ...
    orderBy() {
      ...
      const [col, direction] = arguments as any as [keyof TTableDef["pgDef"]["columns"] & string, "asc" | "desc"];
    ...
    //   if (id === undefined) {
    //     invalidQueryBuilder(`Id missing for row query on non-singleton table ${tableDef.pgDef.name}`)
    //   }
...
export const getResultSchema = (qb: QueryBuilder<any, any, any>): Schema.Schema<any> => {
  ...
  switch (queryAst._tag) {
    case "SelectQuery": {
      ...
      if (queryAst.pickFirst.behaviour === "undefined") {
      ...
      if (queryAst.pickFirst.behaviour === "error") {
    ...
    case "RowQuery": {
      return queryAst.tableDef.rowSchema.pipe(
        Schema.pluck("value"),
        Schema.annotations({ title: `${queryAst.tableDef.pgDef.name}.value` }),
        Schema.Array,
        Schema.headOrElse()
      );
    }
