...
const formatWhereClause = (
  ...
  const whereClause = whereConditions
    .map(({ col, op, value }) => {
      ...
      const colDef = tableDef.pgDef.columns[col];
      if (colDef === undefined) {
...
export const astToSql = (ast: QueryBuilderAst): { query: string; bindValues: SqlValue[]; usedTables: Set<string> } => {
  ...
  const usedTables = new Set<string>([ast.tableDef.pgDef.name]);
  ...
  if (ast._tag === "InsertQuery") {
      ...
      if (ast.onConflict.action._tag === "replace") {
        ...
        if (ast.onConflict.action._tag === "ignore") {
          ...
          updateCols.forEach((col) => {
            const value = updateValues[col];
            if (value !== undefined) {
              const colDef = ast.tableDef.pgDef.columns[col];
              if (colDef === undefined) {
                throw new Error(`Column ${col} not found`);
              }
              const encodedValue = Schema.encodeSync(colDef.schema)(value);
    ...
    let query = `${insertVerb} INTO '${ast.tableDef.pgDef.name}' (${quotedColumns.join(", ")}) VALUES (${placeholders})`;
  ...
  if (ast._tag === "UpdateQuery") {
    ...
    if (setColumns.length === 0) {
      console.warn(
        `UPDATE query requires at least one column to set (for table ${ast.tableDef.pgDef.name}). Running no-op query instead to skip this update query.`
      );
    ...
    let query = `UPDATE '${ast.tableDef.pgDef.name}' SET ${setColumns
  ...
  if (ast._tag === "DeleteQuery") {
    let query = `DELETE FROM '${ast.tableDef.pgDef.name}'`;
  ...
  if (ast._tag === "CountQuery") {
    const query = [
      `SELECT COUNT(*) as count FROM '${ast.tableDef.pgDef.name}'`,
  ...
  if (ast._tag === "RowQuery") {
    ...
    const idColDef = ast.tableDef.pgDef.columns.id;
    ...
    return {
      query: `SELECT * FROM '${ast.tableDef.pgDef.name}' WHERE ${quoteIdentifier("id")} = ?`,
  ...
  const fromStmt = `FROM '${ast.tableDef.pgDef.name}'`;
