// import type { Nullable } from '@beep/utils'
import type { NullableProps } from "@beep/types/nullable";
// import { omitUndefineds } from '@beep/utils'
...
import type * as PgAST from "../ast/pg.ts";
...
export type StructSchemaForColumns<TCols extends ConstraintColumns> = S.Schema<
  AnyIfConstained<TCols, FromColumns.RowDecoded<TCols>>,
  AnyIfConstained<TCols, FromColumns.RowEncoded<TCols>>
>;

export type InsertStructSchemaForColumns<TCols extends ConstraintColumns> = S.Schema<
...
export const structSchemaForTable = <TTableDefinition extends TableDefinition<any, any>>(
  ...
  S.Struct(Object.fromEntries(tableDef.ast.columns.map((column) => [column.name, column.schema]))).annotations({
    title: tableDef.name,
  }) as any;
  ...
  S.Struct(
    Object.fromEntries(
      tableDef.ast.columns.map((column) => [
        column.name,
        column.nullable === true || column.default._tag === "Some" ? S.optional(column.schema) : column.schema,
      ])
    )
  ).annotations({
    title: tableDef.name,
...
const columsToAst = (columns: Columns): ReadonlyArray<PgAST.Column> => {
  return Object.entries(columns).map(([name, column]) => {
    ...
    } satisfies PgAST.Column;
  });
};

const indexesToAst = (indexes: ReadonlyArray<Index>): ReadonlyArray<PgAST.Index> => {
  return indexes.map(
    (_) => ({ _tag: "index", columns: _.columns, name: _.name, unique: _.isUnique ?? false }) satisfies PgAST.Index
  );
};
...
export type TableDefinition<TName extends string, TColumns extends Columns> = {
  ...
  readonly indexes?: undefined | ReadonlyArray<Index>;
  readonly ast: PgAST.Table;
};
 ...
 */
export type ConstraintColumns = Record<string, ColumnDefinition<any, any>> & { readonly __constrained?: never };

export type Index = {
  readonly name: string;
  readonly columns: ReadonlyArray<string>;
  /** @default false */
  readonly isUnique?: undefined | boolean;
};
...
export namespace FromTable {
  ...
  export type RowDecoded<TTableDefinition extends TableDefinition<any, any>> = Types.Simplify<
    NullableProps<Pick<RowDecodedAll<TTableDefinition>, NullableColumnNames<TTableDefinition>>> &
  ...
  export type Columns<TTableDefinition extends TableDefinition<any, any>> = {
    readonly [K in keyof TTableDefinition["columns"]]: TTableDefinition["columns"][K]["columnType"];
  };

  export type RowEncodeNonNullableProps<TTableDefinition extends TableDefinition<any, any>> = {
    readonly [K in keyof TTableDefinition["columns"]]: S.Schema.Encoded<TTableDefinition["columns"][K]["schema"]>;
  };

  export type RowEncoded<TTableDefinition extends TableDefinition<any, any>> = Types.Simplify<
    NullableProps<Pick<RowEncodeNonNullableProps<TTableDefinition>, NullableColumnNames<TTableDefinition>>> &
      Omit<RowEncodeNonNullableProps<TTableDefinition>, NullableColumnNames<TTableDefinition>>
  >;
  ...
  //   &
  //     Omit<RowEncodeNonNullableProps<TTableDefinition>, NullableColumnNames<TTableDefinition['columns']>>
  // >

  export type RowDecodedAll<TTableDefinition extends TableDefinition<any, any>> = {
    readonly [K in keyof TTableDefinition["columns"]]: S.Schema.Type<TTableDefinition["columns"][K]["schema"]>;
  };
}
...
export namespace FromColumns {
  ...
  export type RowDecodedAll<TColumns extends Columns> = {
    readonly [K in keyof TColumns]: S.Schema.Type<TColumns[K]["schema"]>;
  };

  export type RowEncodedAll<TColumns extends Columns> = {
    readonly [K in keyof TColumns]: S.Schema.Encoded<TColumns[K]["schema"]>;
  };

  export type RowEncoded<TColumns extends Columns> = Types.Simplify<
    NullableProps<Pick<RowEncodeNonNullableProps<TColumns>, NullableColumnNames<TColumns>>> &
      Omit<RowEncodeNonNullableProps<TColumns>, NullableColumnNames<TColumns>>
  >;

  export type RowEncodeNonNullableProps<TColumns extends Columns> = {
    readonly [K in keyof TColumns]: S.Schema.Encoded<TColumns[K]["schema"]>;
  };

  export type NullableColumnNames<TColumns extends Columns> = keyof {
    // TODO double check why there is a `true` in the type
    readonly [K in keyof TColumns as TColumns[K] extends ColumnDefinition<any, true> ? K : never]: {};
  };

  export type RequiredInsertColumns<TColumns extends Columns> = {
    [K in keyof TColumns as TColumns[K]["nullable"] extends true
      ? never
      : TColumns[K]["default"] extends O.Some<any>
        ? never
        : K]: {};
  };
