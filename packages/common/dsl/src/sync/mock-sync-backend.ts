...
import { Effect, Mailbox, Option, Queue, Stream, SubscriptionRef } from "effect";
...
export interface MockSyncBackend {
  pushedEvents: Stream.Stream<BeepEvent.Global.Encoded>;
  connect: Effect.Effect<void>;
  disconnect: Effect.Effect<void>;
  makeSyncBackend: Effect.Effect<SyncBackend.SyncBackend, UnknownError, Scope.Scope>;
  advance: (...batch: BeepEvent.Global.Encoded[]) => Effect.Effect<void>;
  /** Fail the next N push calls with an InvalidPushError (or custom error) */
  failNextPushes: (
    count: number,
    error?: (batch: ReadonlyArray<BeepEvent.Global.Encoded>) => Effect.Effect<never, InvalidPushError>
  ) => Effect.Effect<void>;
}
...
export const makeMockSyncBackend = (
  ...
  Effect.gen(function* () {
    ...
    const syncPullQueue = yield* Queue.unbounded<BeepEvent.Global.Encoded>();
    const pushedEventsQueue = yield* Mailbox.make<BeepEvent.Global.Encoded>();
    const syncIsConnectedRef = yield* SubscriptionRef.make(options?.startConnected ?? false);
    const allEventsRef: { current: BeepEvent.Global.Encoded[] } = { current: [] };
    ...
    const failEffectRef = yield* SubscriptionRef.make<
      ((batch: ReadonlyArray<BeepEvent.Global.Encoded>) => Effect.Effect<never, InvalidPushError>) | undefined
    >(undefined);
    ...
    const makeSyncBackend = Effect.gen(function* () {
      ...
      return SyncBackend.of<BS.Json.Type>({
          ...
          (options?.live
            ...
            : Stream.fromEffect(
                Effect.sync(() => {
                  ...
                  const chunks: { events: BeepEvent.Global.Encoded[]; remaining: number }[] = [];
    ...
    const advance = (...batch: BeepEvent.Global.Encoded[]) =>
      Effect.gen(function* () {
        syncEventSequenceNumberRef.current = batch.at(-1)!.seqNum;
        allEventsRef.current = allEventsRef.current.concat(batch);
        yield* syncPullQueue.offerAll(batch);
