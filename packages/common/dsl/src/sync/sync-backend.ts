...
import type * as BeepEvent from "../schema/BeepEvent/mod.ts";
 ...
 */
export type MakeBackendArgs<TPayload = BS.Json.Type> = {
  storeId: string;
  clientId: string;
...
export type SyncBackendConstructor<TSyncMetadata = BS.Json.Type, TPayload = BS.Json.Type> = (
...
export type SyncBackend<TSyncMetadata = BS.Json.Type> = {
  /**
   ...
   */
  metadata: { name: string; description: string } & Record<string, BS.Json.Type>;
...
export const pullResItemEmpty = <TSyncMetadata = BS.Json.Type>(): PullResItem<TSyncMetadata> => ({
  batch: [],
  pageInfo: pageInfoNoMore,
});

export interface PullResItem<TSyncMetadata = BS.Json.Type> {
  batch: ReadonlyArray<{
    eventEncoded: BeepEvent.Global.Encoded;
    metadata: Option.Option<TSyncMetadata>;
  }>;
  pageInfo: PullResPageInfo;
}

export const of = <TSyncMetadata = BS.Json.Type>(obj: SyncBackend<TSyncMetadata>) => obj;

/**
 * Useful to continue pulling from the last event in the batch.
 */
export const cursorFromPullResItem = <TSyncMetadata = BS.Json.Type>(
