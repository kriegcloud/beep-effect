/// <reference lib="dom" />
import { LS_DEV, TRACE_VERBOSE } from "@beep/utils/env";
import { shouldNeverHappen} from "@beep/utils/misc";
...
import type * as BucketQueue from "@beep/utils/effect/BucketQueue"
...
export const makeClientSessionSyncProcessor = ({
  ...
  const eventSchema = BeepEvent.Client.makeSchemaMemo(schema);
  ...
  const leaderPushQueue = BucketQueue.make<BeepEvent.Client.EncodedWithMeta>().pipe(Effect.runSync);
  ...
  const push: ClientSessionSyncProcessor["push"] = Effect.fn("client-session-sync-processor:push")(function* (batch) {
    ...
    const encodedEventDefs = batch.map(({ name, args }) => {
      ...
      return new BeepEvent.Client.EncodedWithMeta(
...
export interface ClientSessionSyncProcessor {
  push: (
    batch: ReadonlyArray<BeepEvent.Input.Decoded>
