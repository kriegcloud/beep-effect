import { casesHandled, LS_DEV, shouldNeverHappen } from "@beep/utils";
import { Match, ReadonlyArray, Schema } from "effect";
...
export class SyncState extends Schema.Class<SyncState>("SyncState")({
  pending: Schema.Array(BeepEvent.Client.EncodedWithMeta),
...
export class PayloadUpstreamRebase extends Schema.TaggedStruct("upstream-rebase", {
  ...
  rollbackEvents: Schema.Array(BeepEvent.Client.EncodedWithMeta),
  /** Events which need to be applied after the rollback (already rebased by the upstream node) */
  newEvents: Schema.Array(BeepEvent.Client.EncodedWithMeta),
}) {}
...
export class MergeResultAdvance extends Schema.Class<MergeResultAdvance>("MergeResultAdvance")({
  ...
  confirmedEvents: Schema.Array(BeepEvent.Client.EncodedWithMeta),
...
export const merge = ({
  ...
  isClientEvent: (event: BeepEvent.Client.EncodedWithMeta) => boolean;
  isEqualEvent: (a: BeepEvent.Client.EncodedWithMeta, b: BeepEvent.Client.EncodedWithMeta) => boolean;
...
export const findDivergencePoint = ({
  ...
  existingEvents: ReadonlyArray<BeepEvent.Client.EncodedWithMeta>;
  incomingEvents: ReadonlyArray<BeepEvent.Client.EncodedWithMeta>;
  isEqualEvent: (a: BeepEvent.Client.EncodedWithMeta, b: BeepEvent.Client.EncodedWithMeta) => boolean;
  isClientEvent: (event: BeepEvent.Client.EncodedWithMeta) => boolean;
  ignoreClientEvents: boolean;
}): number => {
...
const rebaseEvents = ({
  ...
  events: ReadonlyArray<BeepEvent.Client.EncodedWithMeta>;
