...
import { withPerformanceMeasure } from "@beep/utils/effect/Effect";
import { Chunk, Effect, Option, Schema, Stream } from "effect";
import { type PgDb, UnknownError } from "./adapter-types.ts";
...
export const rematerializeFromEventlog = ({
  ...
  Effect.gen(function* () {
    ...
    const hashEventDef = memoizeByRef((event: EventDef.AnyWithoutFn) => BS.hash(event.schema));

    const processEvent = (row: SystemTables.EventlogMetaRow) =>
      Effect.gen(function* () {
        const args = JSON.parse(row.argsJson);
        const eventEncoded = BeepEvent.Client.EncodedWithMeta.make({
          name: row.name,
          args,
          seqNum: {
        ...
        const resolution = yield* resolveEventDef(schema, {
          operation: "@beep/common:rematerializeFromEventlog:processEvent",
          event: eventEncoded,
...
`,
      ...
      }).pipe(Effect.withSpan(`@beep/common:rematerializeFromEventlog:processEvent`));
...
`);
    ...
    withPerformanceMeasure("@beep/common:rematerializeFromEventlog"),
    Effect.withSpan("@beep/common:rematerializeFromEventlog")
  );
