import { casesHandled, isNotUndefined, LS_DEV, shouldNeverHappen, TRACE_VERBOSE } from "@beep/utils";
import type * as otel from "@opentelemetry/api";
import type { HttpClient, Runtime, Scope, Tracer } from "effect";
import {
  BucketQueue,
  Cause,
  Deferred,
...
import { type IntentionalShutdownCause, type MaterializeError, type PgDb, UnknownError } from "../adapter-types.ts";
import { makeMaterializerHash } from "../materializer-helper.ts";
import type { LiveStoreSchema } from "../schema/mod.ts";
import { BeepEvent, EventSequenceNumber, resolveEventDef, SystemTables } from "../schema/mod.ts";
import {
...
type LocalPushQueueItem = [
  event: BeepEvent.Client.EncodedWithMeta,
...
export const makeLeaderSyncProcessor = ({
  ...
  Effect.gen(function* () {
    const syncBackendPushQueue = yield* BucketQueue.make<BeepEvent.Client.EncodedWithMeta>();
    const localPushBatchSize = params.localPushBatchSize ?? 10;
    const backendPushBatchSize = params.backendPushBatchSize ?? 50;

    const syncStateSref = yield* SubscriptionRef.make<SyncState.SyncState | undefined>(undefined);

    const isClientEvent = (eventEncoded: BeepEvent.Client.EncodedWithMeta) =>
    ...
    const push: LeaderSyncProcessor["push"] = (newEvents, options) =>
      Effect.gen(function* () {
        ...
        Effect.withSpan("@beep/common:LeaderSyncProcessor:push", {
          attributes: {
    ...
    const pushPartial: LeaderSyncProcessor["pushPartial"] = ({ event: { name, args }, clientId, sessionId }) =>
      Effect.gen(function* () {
        ...
        const resolution = yield* resolveEventDef(schema, {
          operation: "@beep/common:LeaderSyncProcessor:pushPartial",
          event: {
            name,
            args,
            clientId,
        ...
        const eventEncoded = new BeepEvent.Client.EncodedWithMeta({
          name,
          args,
          clientId,
          sessionId,
    ...
    }).pipe(Effect.withSpanScoped("@beep/common:LeaderSyncProcessor:boot"));
...
const backgroundApplyLocalPushes = ({
  ...
  syncBackendPushQueue: BucketQueue.BucketQueue<BeepEvent.Client.EncodedWithMeta>;
  schema: LiveStoreSchema;
  isClientEvent: (eventEncoded: BeepEvent.Client.EncodedWithMeta) => boolean;
  otelSpan: otel.Span | undefined;
    ...
    while (true) {
      ...
      if (droppedItems.length > 0) {
        ...
        yield* Effect.forEach(
          droppedItems.filter(
            (item): item is [BeepEvent.Client.EncodedWithMeta, Deferred.Deferred<void, LeaderAheadError>] =>
              item[1] !== undefined
          ),
      ...
      const mergeResult = SyncState.merge({
        ...
        isClientEvent,
        isEqualEvent: BeepEvent.Client.isEqualEncoded,
      });
...
type MaterializeEventsBatch = (_: {
  batchItems: ReadonlyArray<BeepEvent.Client.EncodedWithMeta>;
  /**
...
const materializeEventsBatch: MaterializeEventsBatch = ({ batchItems, deferreds }) =>
  Effect.gen(function* () {
    ...
    Effect.scoped,
    Effect.withSpan("@beep/common:LeaderSyncProcessor:materializeEventItems", {
...
const backgroundBackendPulling = ({
  ...
  isClientEvent: (eventEncoded: BeepEvent.Client.EncodedWithMeta) => boolean;
  restartBackendPushing: (
    filteredRebasedPending: ReadonlyArray<BeepEvent.Client.EncodedWithMeta>
  ...
  Effect.gen(function* () {
    ...
    const onNewPullChunk = (newEvents: BeepEvent.Client.EncodedWithMeta[], pageInfo: SyncBackend.PullResPageInfo) =>
      Effect.gen(function* () {
        if (newEvents.length === 0) return;
      ...
      Stream.tap(({ batch, pageInfo }) =>
        Effect.gen(function* () {
          ...
          yield* onNewPullChunk(
            batch.map((_) =>
              BeepEvent.Client.EncodedWithMeta.fromGlobal(_.eventEncoded, {
                ...
                materializerHashLeader: hashMaterializerResult(BeepEvent.Global.toClientEncoded(_.eventEncoded)),
  ...
  }).pipe(Effect.withSpan("@beep/common:LeaderSyncProcessor:backend-pulling"));
...
const backgroundBackendPushing = ({
  ...
  }).pipe(Effect.interruptible, Effect.withSpan("@beep/common:LeaderSyncProcessor:backend-pushing"));

const trimChangesetRows = (db: PgDb, newHead: EventSequenceNumber.Client.Composite) => {
...
const validatePushBatch = (
  batch: ReadonlyArray<BeepEvent.Client.EncodedWithMeta>,
