import { omitUndefineds, shouldNeverHappen } from "@beep/utils";
import type { HttpClient, Scope } from "effect";
import {
  Deferred,
  Effect,
  KeyValueStore,
...
import { BeepEvent, EventSequenceNumber, SystemTables } from "../schema/mod.ts";
...
export const makeLeaderThreadLayer = ({
  ...
  Effect.gen(function* () {
    ...
    const ctx = {
      ...
      dbEventlog,
      makePgDb,
      eventSchema: BeepEvent.Client.makeSchema(schema),
    ...
    Effect.withSpan("@beep/common:leader-thread:boot"),
    Effect.withSpanScoped("@beep/common:leader-thread"),
    UnknownError.mapToUnknownError,
    Effect.tapCauseLogPretty,
    Layer.unwrapScoped
  );

const hasEventlogTables = (db: PgDb) => {
  const tableNames = new Set(db.select<{ name: string }>(sql`select name from pg_master`).map((_) => _.name));
  const eventlogTables = new Set(SystemTables.eventlogSystemTables.map((_) => _.pgDef.name));
  return isSubsetOf(eventlogTables, tableNames);
};

const hasStateTables = (db: PgDb) => {
  const tableNames = new Set(db.select<{ name: string }>(sql`select name from pg_master`).map((_) => _.name));
  const stateTables = new Set(SystemTables.stateSystemTables.map((_) => _.pgDef.name));
...
const bootLeaderThread = ({
  ...
  UnknownError | PgError | IsOfflineError | InvalidPullError | MaterializerHashMismatchError,
  ...
  Effect.gen(function* () {
    ...
    if (initialBlockingSyncContext.blockingDeferred !== undefined) {
      ...
      yield* initialBlockingSyncContext.blockingDeferred.pipe(
        Effect.withSpan("@beep/common:leader-thread:initial-sync-blocking")
      );
    }
