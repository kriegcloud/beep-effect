import { nanoid } from "@beep/utils/nanoid";
import { Effect, FiberMap, Option, Stream, SubscriptionRef } from "effect";
...
export const bootDevtools = (options: DevtoolsOptions) =>
        ...
        Effect.gen(function* () {
          ...
          const sendMessage: SendMessageToDevtools = (message) =>
            channel
              .send(message)
              .pipe(
                Effect.withSpan("@beep/common:leader-thread:devtools:sendToDevtools"),
  ...
  }).pipe(Effect.withSpan("@beep/common:leader-thread:devtools:boot"));
    ...
    yield* incomingMessages.pipe(
      Stream.tap((decodedEvent) =>
        Effect.gen(function* () {
          ...
          //   `[@beep/common:leader-thread:devtools] incomingMessage: ${decodedEvent._tag} (${requestId})`,
          //   decodedEvent,
          // )
          ...
          switch (decodedEvent._tag) {
            ...
            case "LSD.Leader.LoadDatabaseFile.Request": {
              ...
              const handleLoadDb = Effect.gen(function* () {
                const tableNames = yield* Effect.acquireRelease(makePgDb({ _tag: "in-memory" }), (db) =>
                  Effect.sync(() => db.close())
                ).pipe(
                  Effect.flatMap((db) =>
                    Effect.try(() => {
                      db.import(data);
                      const rows = db.select<{ name: string }>(`select name from pg_master where type = 'table'`);
            ...
            case "LSD.Leader.DatabaseFileInfoReq": {
              if (persistenceInfo === undefined) {
                console.log("[@beep/common:leader-thread:devtools] persistenceInfo is required for this request");
                return;
              }
        ...
        }).pipe(Effect.withSpan(`@beep/common:leader-thread:onDevtoolsMessage:${decodedEvent._tag}`))
      ),
