import { isDevEnv, LS_DEV, shouldNeverHappen } from "@beep/utils";
import { Effect, Option, ReadonlyArray, Schema } from "effect";

import { MaterializeError, MaterializerHashMismatchError, type PgDb } from "../adapter-types.ts";
    ...
    return (eventEncoded, options) =>
      Effect.gen(function* () {
        ...
        const resolution = yield* resolveEventDef(schema, {
          operation: "@beep/common:leader-thread:materializeEvent",
          event: eventEncoded,
        });
        ...
        // console.group('[@beep/common:leader-thread:materializeEvent]', { eventName })
        ...
        yield* execSql(
          ...
          ...insertRow({
            ...
            columns: SystemTables.sessionChangesetMetaTable.pgDef.columns,
            values: {
        ...
        if (skipEventlog === false) {
          ...
          //   console.debug('[@beep/common:leader-thread] skipping eventlog write', mutation, statementSql, bindValues)
        }
        ...
        Effect.withSpan(`@beep/common:leader-thread:materializeEvent`, {
          attributes: {
        ...
        // Effect.logDuration('@beep/common:leader-thread:materializeEvent'),
      );
  });
...
export const rollback = ({
  ...
  Effect.gen(function* () {
    ...
    Effect.withSpan("@beep/common:LeaderSyncProcessor:rollback", {
