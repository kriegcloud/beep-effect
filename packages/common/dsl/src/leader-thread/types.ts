...
import type { MeshNode } from "@beep/webmesh";
import type { Deferred, Effect, HttpClient, Option, Queue, Scope, Stream, Subscribable, SubscriptionRef } from "effect";
...
import { type BeepEvent, EventSequenceNumber, type LiveStoreSchema } from "../schema/mod.ts";
...
export type InitialSyncInfo = Option.Option<{
  ...
  metadata: Option.Option<BS.Json.Type>;
}>;
...
export type LeaderPgDb = PgDb<{ dbPointer: number; persistenceInfo: PersistenceInfo }>;
...
export class LeaderThreadCtx extends Context.Tag("LeaderThreadCtx")<
  LeaderThreadCtx,
  {
    ...
    eventSchema: BeepEvent.ForEventDef.ForRecord<any>;
