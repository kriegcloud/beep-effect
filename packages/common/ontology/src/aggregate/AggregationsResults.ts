/**
 * Compile-time aggregate result resolver.
 *
 * @since 0.0.0
 * @module @beep/ontology/aggregate/AggregationsResults
 */
    ...
    ? unknown extends AO["$groupBy"]
      ...
      : Exclude<AO["$groupBy"], undefined> extends never
        ...
        : Exclude<keyof AO["$groupBy"], AggregatableKeys<Q>> extends never
          ...
          : `Sorry, the following are not valid groups for an aggregation: ${Exclude<keyof AO["$groupBy"] & string, AggregatableKeys<Q>>}`
    : `Sorry, the following are not valid selectors for an aggregation: ${Exclude<keyof AO["$select"] & string, ValidAggregationKeys<Q>>}`;
