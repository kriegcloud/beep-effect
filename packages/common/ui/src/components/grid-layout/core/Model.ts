 ...
 * @since 0.0.0
 */
export const ResizeHandleAxis = LiteralKit(["s", "w", "e", "n", "sw", "nw", "se", "ne"]).pipe(
  $I.annoteSchema("ResizeHandleAxis", {
    description:
      "Axis identifiers for resize handles. Cardinal directions ('n', 's', 'e', 'w') and diagonal ('ne', 'nw', 'se', 'sw').",
  })
);
...
const LayoutConstraint$ref = S.suspend((): S.Codec<LayoutConstraint, LayoutConstraint.Encoded> => LayoutConstraint);
...
export class LayoutItem extends S.Class<LayoutItem>($I`LayoutItem`)(
  {
    ...
    static: S.OptionFromOptionalKey(S.Boolean).annotateKey({
      description:
        "If true, the item cannot be dragged or resized, and other items will move around it during compaction.",
    }),
    ...
    isDraggable: S.OptionFromOptionalKey(S.Boolean).annotateKey({
      description:
        "If false, the item cannot be dragged (but may still be resizable). Overrides grid-level isDraggable for this item.",
    }),
    ...
    isResizable: S.OptionFromOptionalKey(S.Boolean).annotateKey({
      description:
        "If false, the item cannot be resized (but may still be draggable). Overrides grid-level isResizable for this item.",
    }),
    ...
    isBounded: S.OptionFromOptionalKey(S.Boolean).annotateKey({
      description:
        "If true, the item is constrained to the grid container bounds. Overrides grid-level isBounded for this item.",
    }),
    ...
    moved: S.OptionFromOptionalKey(S.Boolean).annotateKey({
      description:
        "Internal flag set during drag/resize operations to indicate the item has moved from its original position. @internal",
    }),
     ...
     * Applied in addition to grid-level constraints.
     */
    constraints: S.OptionFromOptionalKey(S.Array(LayoutConstraint$ref)).annotateKey({
...
export class ConstraintContext extends S.Class<ConstraintContext>($I`ConstraintContext`)(
  {
    ...
    /** Margin between items [x, y] in pixels */
    margin: S.Tuple([S.Number, S.Number]),

    /** Current layout state */
    layout: Layout,
  },
  $I.annote("ConstraintContext", {
...
export class LayoutConstraint extends S.Class<LayoutConstraint>($I`LayoutConstraint`)(
  {
     ...
     * Called after grid unit conversion, before layout update.
     */
    constrainPosition: S.OptionFromOptionalKey(
      Fn({
        input: S.Struct({
          item: LayoutItem$ref,
          x: S.Number,
     ...
     * Called after grid unit conversion, before layout update.
     */
    constrainSize: S.OptionFromOptionalKey(
      Fn({
        input: S.Struct({
          item: LayoutItem$ref,
          w: S.Number,
...
export class ReactDraggableCallbackData extends S.Class<ReactDraggableCallbackData>($I`ReactDraggableCallbackData`)(
  ...
  $I.annote("ReactDraggableCallbackData", {
...
const isDragOverEvent = (u: unknown): u is DragOverEvent =>
  S.is(DOMMouseEvent)(u) &&
  P.hasProperty(u, "nativeElement") &&
  pipe(
    u,
    P.Struct({
      nativeElement: (u: unknown): u is DragOverEvent["nativeElement"] =>
        S.is(DOMEvent)(u) &&
        P.hasProperties("layerX", "layerY")(u) &&
        pipe(
          u,
          P.Struct({
            layerX: P.isNumber,
 ...
 * @since 0.0.0
 */
export const DragOverEvent = S.declare(isDragOverEvent).pipe(
  $I.annoteSchema("DragOverEvent", {
 ...
 * @since 0.0.0
 */
export const CompactType = S.OptionFromNullOr(LiteralKit(["horizontal", "vertical", "wrap"])).pipe(
  $I.annoteSchema("CompactType", {
...
/**
 * type of {@link CompactType} {@inheritDoc CompactType}
 *
 * @since 0.0.0
 */
...
/**
 * Parameters for layout event callbacks.
 *
 * @since 0.0.0
 */
...
export class Compactor extends S.Class<Compactor>($I`Compactor`)(
  {
    ...
    allowOverlap: S.Boolean.annotateKey({
      ...
      documentation:
        "When true, items can be placed on top of each other, and dragging into another item does not push it away. Compaction is skipped after drag/resize.",
    }),
    /**
     ...
     */
    preventCollision: S.OptionFromOptionalKey(S.Boolean).annotateKey({
      description: "Whether to block movement that would cause collision.",
      documentation:
        "When true, dragging into another item is blocked (item snaps back). Other items are NOT pushed away. Only affects drag/resize, not compaction. Has no effect when allowOverlap is true.",
    }),
...
class PositionStrategyBase extends S.Class<PositionStrategyBase>($I`PositionStrategyBase`)(
  ...
  $I.annote("PositionStrategyBase", {
    description:
      "Base class for position strategies, providing common functionality and interfaces for layout positioning.",
  })
) {}
 ...
 * @since 0.0.0
 */
export class PositionStrategyTransform extends PositionStrategyBase.extend<PositionStrategyTransform>(
  $I`PositionStrategyTransform`
)(
  {
    /** Strategy type identifier */
    type: S.tag("transform").annotateKey({
      description: "Strategy type identifier for transform-based positioning.",
    }),
  },
  $I.annote("PositionStrategyTransform", {
 ...
 * @since 0.0.0
 */
export class PositionStrategyAbsolute extends PositionStrategyBase.extend<PositionStrategyAbsolute>(
  $I`PositionStrategyAbsolute`
)(
  {
    /** Strategy type identifier */
    type: S.tag("absolute").annotateKey({
      description: "Strategy type identifier for absolute positioning.",
    }),
  },
  $I.annote("PositionStrategyAbsolute", {
 ...
 * @since 0.0.0
 */
export const PositionStrategy = S.Union([PositionStrategyTransform, PositionStrategyAbsolute]).pipe(
  S.toTaggedUnion("type"),
  $I.annoteSchema("PositionStrategy", {
...
export class GridConfig extends S.Class<GridConfig>($I`GridConfig`)(
  {
    ...
    margin: S.Tuple([S.Number, S.Number])
      .pipe(
        S.withDecodingDefaultKey(() => [10, 10]),
        S.withConstructorDefault(() => O.some([10, 10] as readonly [number, number]))
      )
      .annotateKey({
    ...
    containerPadding: S.OptionFromNullOr(S.Tuple([S.Number, S.Number]))
      .pipe(
        S.withDecodingDefaultKey(() => [10, 10]),
...
export class DragConfig extends S.Class<DragConfig>($I`DragConfig`)(
  {
     ...
     * @default 3
     */
    threshold: S.Number.pipe(S.optionalKey, S.withConstructorDefault(thunkSome(3)), S.withDecodingDefaultKey(thunk(3))),
  },
  $I.annote("DragConfig", {
...
export class ResizeConfig extends S.Class<ResizeConfig>($I`ResizeConfig`)(
  {
    ...
    handles: S.Array(ResizeHandleAxis).pipe(
      S.optionalKey,
...
export class DropConfig extends S.Class<DropConfig>($I`DropConfig`)(
  {
    ...
    defaultItem: S.Struct({
      w: S.Number.pipe(S.withConstructorDefault(thunkSome(1)), S.withDecodingDefaultKey(thunk(1))),
      h: S.Number.pipe(S.withConstructorDefault(thunkSome(1)), S.withDecodingDefaultKey(thunk(1))),
    })
      .pipe(
        S.optionalKey,
 ...
 * @since 0.0.0
 */
export const Breakpoint = NonEmptyTrimmedStr.pipe(S.brand("Breakpoint"));

/**
 * type of {@link Breakpoint} {@inheritDoc Breakpoint}
 *
 * @since 0.0.0
 */
 ...
 * @since 0.0.0
 */
export const Breakpoints = <B extends ReadonlyArray<Breakpoint>>(schema: S.Literals<B>) =>
  S.ReadonlyMap(schema, S.Number);
 ...
 * @since 0.0.0
 */
export const BreakpointCols = <B extends ReadonlyArray<Breakpoint>>(schema: S.Literals<B>) =>
  S.ReadonlyMap(schema, S.Number);
