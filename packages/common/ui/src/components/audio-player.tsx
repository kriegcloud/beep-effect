...
import { Check, Gear, Pause, Play } from "@phosphor-icons/react";
...
class PlayPromiseRefError extends S.TaggedError<PlayPromiseRefError>($I`PlayPromiseRefError`)(
  ...
  $I.annotations("PlayPromiseRefError", {
    description: "An error which occurred while attempting to play an audio file ref",
  })
) {
...
const PlayButton = ({ playing, onPlayingChange, className, onClick, loading, ...otherProps }: PlayButtonProps) => {
  return (
    ...
    >
      {playing ? (
        <Pause className={cn("size-4", loading && "opacity-0")} aria-hidden="true" />
      ) : (
        <Play className={cn("size-4", loading && "opacity-0")} aria-hidden="true" />
      )}
      {loading && (
