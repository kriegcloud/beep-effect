...
import { CaretUpDown, Check, Microphone, MicrophoneSlash } from "@phosphor-icons/react";
...
const $I = $UiId.create("components/mic-selector");
...
export class MediaDeviceEnumerationError extends S.TaggedError<MediaDeviceEnumerationError>(
  ...
  $I.annotations("MediaDeviceEnumerationError", {
    description: "An error which occurred while attempting to enumerate the clients media devices",
  })
) {
...
export function MicSelector({ value, onValueChange, muted, onMutedChange, disabled, className }: MicSelectorProps) {
  ...
  return (
    <DropdownMenu onOpenChange={handleDropdownOpenChange}>
      ...
      >
        {isMuted ? (
          <MicrophoneSlash className="h-4 w-4 flex-shrink-0" />
        ) : (
          <Microphone className="h-4 w-4 flex-shrink-0" />
        )}
        <span className="flex-1 truncate text-left">{currentDevice.label}</span>
        <CaretUpDown className="h-3 w-3 flex-shrink-0" />
      </DropdownMenuTrigger>
