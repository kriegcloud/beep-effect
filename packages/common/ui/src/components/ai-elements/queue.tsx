...
export interface QueueMessagePart {
  ...
  readonly mediaType?: undefined | string;
...
export interface QueueMessage {
  readonly id: string;
  readonly parts: QueueMessagePart[];
}

export interface QueueTodo {
  readonly description?: undefined | string;
  readonly id: string;
  readonly status?: undefined | "pending" | "completed";
...
export type QueueItemIndicatorProps = ComponentProps<"span"> & {
  readonly completed?: undefined | boolean;
};
...
export const QueueSectionLabel = ({ count, label, icon, className, ...props }: QueueSectionLabelProps) => (
  <span className={cn("flex items-center gap-2", className)} {...props}>
    <CaretDown className="size-4 transition-transform group-data-[state=closed]:-rotate-90" />
    {icon}
    <span>{typeof count === "number" ? `${count} ${label}` : label}</span>
  </span>
);
