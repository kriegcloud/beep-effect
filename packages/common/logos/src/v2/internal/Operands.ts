...
export namespace Operand {
  export const make = <const Tag extends StringTypes.NonEmptyString<string>>(
    ...
    const Schema = <const A, const E, const R, const Extra extends StructTypes.StructFieldsWithStringKeys>(
...
export namespace Gte {
  export const { Schema, label } = Operand.make("gte", "greater than or equal to");
}
...
export namespace NotContains {
  export const { Schema, label } = Operand.make("notContains", "does not contain");
}
...
export const makeCoreOperand = <const Tag extends StringTypes.NonEmptyString<string>>(tag: Tag) =>
  S.Struct(
    {
      _tag: S.Literal(tag),
    },
