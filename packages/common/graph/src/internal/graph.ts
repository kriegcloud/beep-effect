...
/** @internal */
export const TypeId: Graph.TypeId = Symbol.for("@beep/Graph") as UnsafeTypes.UnsafeAny;

/** @internal */
export const NodeTypeId: Graph.NodeTypeId = Symbol.for("@beep/GraphNode") as UnsafeTypes.UnsafeAny;

/** @internal */
export const EdgeTypeId: Graph.EdgeTypeId = Symbol.for("@beep/GraphEdge") as UnsafeTypes.UnsafeAny;
...
/** @internal */
export const MutableTypeId: Graph.MutableTypeId = Symbol.for("effect/GraphMutable") as UnsafeTypes.UnsafeAny;
...
class MutableImpl<in out A extends Graph.Graph.Any> implements Graph.Mutable<A> {
  ...
  addEdge(from: number, to: number, data: Graph.Edge.Data<A>): O.Option<number> {
  ...
  unsafeAddEdge(from: number, to: number, data: Graph.Edge.Data<A>): number {
    return O.getOrThrow(this.addEdge(from, to, data));
  }

  updateEdge(from: number, to: number, data: Graph.Edge.Data<A>): O.Option<number> {
    return O.match(findEdge(this.graph, from, to), {
      onSome: (ix) => {
  ...
  unsafeUpdateEdge(from: number, to: number, data: Graph.Edge.Data<A>): number {
    return O.getOrThrow(this.updateEdge(from, to, data));
  }

  removeEdge(index: number): O.Option<Graph.Edge.Data<A>> {
    ...
    return O.some(remove.data);
  }

  removeNode(index: number): O.Option<Graph.Node.Data<A>> {
...
export const mutate = <A extends Graph.Graph.Any>(
  ...
  fn(mutable as UnsafeTypes.UnsafeAny);
  return mutable.graph;
};

/** @internal */
export const isGraph: (value: unknown) => value is Graph.Graph.Unknown = hasProperty(TypeId) as UnsafeTypes.UnsafeAny;

/** @internal */
export const isDirected: <N, E>(value: Graph.Graph<N, E>) => value is Graph.Graph.Directed<N, E> = P.isTagged(
  "DirectedGraph"
) as UnsafeTypes.UnsafeAny;

/** @internal */
export const isUndirected: <N, E>(value: Graph.Graph<N, E>) => value is Graph.Graph.Undirected<N, E> = P.isTagged(
  "UndirectedGraph"
) as UnsafeTypes.UnsafeAny;
...
const makeNode = <N>(data: N, next: [number, number]): Graph.Node<N> => {
...
export const getNode: {
  <N>(index: number): (self: Graph.Graph<N, UnsafeTypes.UnsafeAny>) => O.Option<N>;
  <N>(self: Graph.Graph<N, UnsafeTypes.UnsafeAny>, index: number): O.Option<N>;
} = dual(
  2,
  <N>(graph: Graph.Graph<N, UnsafeTypes.UnsafeAny>, index: number): O.Option<N> =>
    O.fromNullable(graph.nodes[index]).pipe(O.map((node) => node.data))
);

/** @internal */
export const unsafeGetNode: {
  <N>(index: number): (self: Graph.Graph<N, UnsafeTypes.UnsafeAny>) => N;
  <N>(self: Graph.Graph<N, UnsafeTypes.UnsafeAny>, index: number): N;
} = dual(2, <N>(graph: Graph.Graph<N, UnsafeTypes.UnsafeAny>, index: number): N => O.getOrThrow(getNode(graph, index)));

/** @internal */
export const addNode: {
  <N>(data: N): <A extends Graph.Graph<N, UnsafeTypes.UnsafeAny>>(self: A) => Graph.Graph.Preserve<A>;
  <N, A extends Graph.Graph<N, UnsafeTypes.UnsafeAny>>(self: A, data: N): Graph.Graph.Preserve<A>;
} = dual(
  2,
  <N, E, A extends Graph.Graph<N, E>>(self: A, data: N): Graph.Graph.Preserve<A> =>
    mutate(self, (mutable) => mutable.addNode(data as UnsafeTypes.UnsafeAny))
);

/** @internal */
export const removeNode: {
  (node: number): <A extends Graph.Graph.Any>(self: A) => O.Option<Graph.Graph.Preserve<A>>;
  <A extends Graph.Graph.Any>(self: A, node: number): O.Option<Graph.Graph.Preserve<A>>;
} = dual(2, <A extends Graph.Graph.Any>(graph: A, node: number): O.Option<Graph.Graph.Preserve<A>> => {
  let data: O.Option<Graph.Node.Data<A>>;
  const out = mutate(graph, (mutable) => {
    data = mutable.removeNode(node);
  });

  return data!.pipe(O.as(out));
});

const makeEdge = <E>(data: E, node: [number, number], next: [number, number]): Graph.Edge<E> => {
...
export const getEdge: {
  <E>(index: number): (self: Graph.Graph<UnsafeTypes.UnsafeAny, E>) => O.Option<E>;
  <E>(self: Graph.Graph<UnsafeTypes.UnsafeAny, E>, index: number): O.Option<E>;
} = dual(
  2,
  <E>(graph: Graph.Graph<UnsafeTypes.UnsafeAny, E>, index: number): O.Option<E> =>
    O.fromNullable(graph.edges[index]).pipe(O.map((edge) => edge.data))
);

/** @internal */
export const unsafeGetEdge: {
  <E>(index: number): (self: Graph.Graph<UnsafeTypes.UnsafeAny, E>) => E;
  <E>(self: Graph.Graph<UnsafeTypes.UnsafeAny, E>, index: number): E;
} = dual(2, <E>(graph: Graph.Graph<UnsafeTypes.UnsafeAny, E>, index: number): E => O.getOrThrow(getEdge(graph, index)));
...
export const addEdge: {
  <E>(
    ...
    to: number,
    data: E
  ): <A extends Graph.Graph<UnsafeTypes.UnsafeAny, E>>(graph: A) => O.Option<Graph.Graph.Preserve<A>>;
  <E, A extends Graph.Graph<UnsafeTypes.UnsafeAny, E>>(
    graph: A,
    from: number,
    to: number,
    data: E
  ): O.Option<Graph.Graph.Preserve<A>>;
} = dual(
  4,
  <E, A extends Graph.Graph<UnsafeTypes.UnsafeAny, E>>(
    graph: A,
    ...
    let index: O.Option<number>;
    const out = mutate(graph, (mutable) => {
      index = mutable.addEdge(from, to, data as UnsafeTypes.UnsafeAny);
    });

    return index!.pipe(O.as(out));
  }
);
...
export const unsafeAddEdge: {
  <E>(
    ...
    to: number,
    data: E
  ): <A extends Graph.Graph<UnsafeTypes.UnsafeAny, E>>(graph: A) => Graph.Graph.Preserve<A>;
  <E, A extends Graph.Graph<UnsafeTypes.UnsafeAny, E>>(
    graph: A,
    from: number,
    to: number,
  ...
  <E, A extends Graph.Graph<UnsafeTypes.UnsafeAny, E>>(
    ...
    to: number,
    data: E
  ): Graph.Graph.Preserve<A> => O.getOrThrow(addEdge(graph, from, to, data))
);
...
export const updateEdge: {
  ...
  <E, A extends Graph.Graph<UnsafeTypes.UnsafeAny, E>>(
    ...
    let index: O.Option<number>;
    const out = mutate(graph, (mutable) => {
      index = mutable.updateEdge(from, to, data as UnsafeTypes.UnsafeAny);
    });
...
export const unsafeUpdateEdge: {
  ...
  <E, A extends Graph.Graph<UnsafeTypes.UnsafeAny, E>>(
    ...
    to: number,
    data: E
  ): Graph.Graph.Preserve<A> => O.getOrThrow(updateEdge(graph, from, to, data))
);

/** @internal */
export const removeEdge: {
  (edge: number): <A extends Graph.Graph.Any>(self: A) => O.Option<Graph.Graph.Preserve<A>>;
  <A extends Graph.Graph.Any>(self: A, edge: number): O.Option<Graph.Graph.Preserve<A>>;
} = dual(2, <A extends Graph.Graph.Any>(graph: A, edge: number): O.Option<Graph.Graph.Preserve<A>> => {
  let data: O.Option<Graph.Edge.Data<A>>;
  const out = mutate(graph, (mutable) => {
...
} = dual(3, (graph: Graph.Graph.Any, a: number, b: number): boolean => O.isSome(findEdge(graph, a, b)));

/** @internal */
export const findEdge: {
  (from: number, to: number): (self: Graph.Graph.Any) => O.Option<number>;
  (self: Graph.Graph.Any, from: number, to: number): O.Option<number>;
} = dual(3, (graph: Graph.Graph.Any, a: number, b: number): O.Option<number> => {
  if (isUndirected(graph)) {
    return findEdgeUndirected(graph, a, b).pipe(O.map(([index]) => index));
  }

  const node = graph.nodes[a];
  return node === undefined ? O.none() : findEdgeDirectedFromNode(graph, node, b);
});
...
export const filterMap: {
  <A extends Graph.Graph.Any, N, E>(options: {
    mapNodes: (node: Graph.Node.Data<A>) => O.Option<N>;
    mapEdges: (edge: Graph.Edge.Data<A>) => O.Option<E>;
  }): (self: A) => Graph.Graph.Preserve<A, N, E>;
  <A extends Graph.Graph.Any, N, E>(
    self: A,
    options: {
  ...
  <A extends Graph.Graph.Any, N, E>(
    self: A,
    {
      ...
      mapNodes: (data: Graph.Node.Data<A>) => O.Option<N>;
      mapEdges: (data: Graph.Edge.Data<A>) => O.Option<E>;
    }
  ): Graph.Graph.Preserve<A, N, E> =>
    mutate(self, (mutable) => {
      mutable.graph.nodes = [];
      ...
      for (const [i, node] of self.nodes.entries()) {
        ...
        if (O.isSome(mapped)) {
          nodes[i] = mutable.addNode(mapped.value as UnsafeTypes.UnsafeAny);
        }
      }
      ...
      for (const edge of self.edges.values()) {
        ...
        if (nodes[from] !== -1 && nodes[to] !== -1) {
          ...
          if (O.isSome(mapped)) {
            mutable.addEdge(nodes[from]!, nodes[to]!, mapped.value as UnsafeTypes.UnsafeAny);
          }
        }
      }
    }) as UnsafeTypes.UnsafeAny
...
const findEdgeUndirectedFromNode = <N, E>(
  ...
  node: Graph.Node<N>,
  index: number
): O.Option<[number, Graph.Edge.Direction]> => {
  for (const direction of DIRECTIONS) {
    let edix = node.next[direction];
    let edge: Graph.Edge<E>;
    while ((edge = graph.edges[edix]!) !== undefined) {
      if (edge.node[1 - direction] === index) {
        return O.some([edix, direction]);
      }
...
/** @internal */
export const nodes = <A extends Graph.Graph.Any>(graph: A): IterableIterator<[number, Graph.Node.Data<A>]> =>
  new NodeIterator(graph, (index, data) => [index, data] as const);

/** @internal */
export const edges = <A extends Graph.Graph.Any>(graph: A): IterableIterator<[number, Graph.Edge.Data<A>]> =>
  new EdgeIterator(graph, (index, data) => [index, data] as const);
...
class NodeIterator<in out A extends Graph.Graph.Any, out T> implements IterableIterator<T> {
  ...
  private readonly filter?: ((index: number, data: Graph.Node.Data<A>) => boolean) | undefined;

  constructor(
    graph: A,
    map: (index: number, data: Graph.Node.Data<A>) => T,
    filter?: ((index: number, data: Graph.Node.Data<A>) => boolean) | undefined
  ) {
    this.graph = graph;
    this.map = map;
    this.filter = filter;
...
class EdgeIterator<in out A extends Graph.Graph.Any, out T> implements IterableIterator<T> {
  ...
  private readonly filter?: ((index: number, data: Graph.Edge.Data<A>) => boolean) | undefined;

  constructor(
    graph: A,
    map: (index: number, data: Graph.Edge.Data<A>) => T,
    filter?: ((index: number, data: Graph.Edge.Data<A>) => boolean) | undefined
  ) {
    this.graph = graph;
    this.map = map;
    this.filter = filter;
  ...
  implements Iterator<[number, Graph.Edge<Graph.Edge.Data<A>>]>
{
  ...
  constructor(graph: A, direction: Graph.Edge.Direction, nxt: number) {
    this.graph = graph;
    this.direction = direction;
    this.nxt = nxt;
  }
