...
- `packages/shared/domain/src/values/paths.ts` — uses `PathBuilder` from `@beep/shared-domain/factories` (not from this package) for comprehensive dashboard/auth routes.
