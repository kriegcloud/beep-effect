import * as Arbitrary from "effect/Arbitrary";
import type * as B from "effect/Brand";
import * as FC from "effect/FastCheck";
import * as ParseResult from "effect/ParseResult";
import * as S from "effect/Schema";
import RandExp from "randexp-ts";
import { Id } from "./_id";

/**
 * Branded schema for `RegExp` instances with arbitraries for documentation examples.
 *
 * @example
 * import { Regex } from "@beep/schema-v2/internal/regex/regex";
 *
 * const safeRegex = Regex.make(/[a-z]+/);
 *
 * @category Regex
 * @since 0.1.0
 * @internal
 */
export const Regex = S.instanceOf(RegExp)
  .pipe(S.brand("Regex"))
  .annotations(
    Id.annotations("Regex", {
      description: "A regular expression",
      arbitrary: () => (fc) =>
        fc
          .constant(null)
          .map(() => new RegExp(new RandExp(/<([a-z]\w{0,20})>foo<\1>/).gen()) as B.Branded<RegExp, "Regex">),
    })
  );

/**
 * Namespace exposing helper types for the branded regex schema.
 *
 * @example
 * import type { Regex } from "@beep/schema-v2/internal/regex/regex";
 *
 * const pattern: Regex.Type = Regex.make(/foo/);
 *
 * @category Regex
 * @since 0.1.0
 * @internal
 */
export declare namespace Regex {
  /**
   * Runtime type of the branded regex schema.
   *
   * @example
   * import type { Regex } from "@beep/schema-v2/internal/regex/regex";
   *
   * let pattern: Regex.Type;
   *
   * @category Regex
   * @since 0.1.0
   * @internal
   */
  export type Type = typeof Regex.Type;
  /**
   * Encoded representation of the branded regex schema.
   *
   * @example
   * import type { Regex } from "@beep/schema-v2/internal/regex/regex";
   *
   * const encoded: Regex.Encoded = /foo/;
   *
   * @category Regex
   * @since 0.1.0
   * @internal
   */
  export type Encoded = typeof Regex.Encoded;
}

/**
 * Schema that safely parses strings into regular expressions.
 *
 * @example
 * import { RegexFromString } from "@beep/schema-v2/internal/regex/regex";
 *
 * const compiled = RegexFromString.make("[a-z]+");
 *
 * @category Regex
 * @since 0.1.0
 * @internal
 */
export class RegexFromString extends S.NonEmptyTrimmedString.pipe(
  S.transformOrFail(Regex, {
    strict: true,
    decode: (value, _, ast) =>
      ParseResult.try({
        try: () => Regex.make(new RegExp(value)),
        catch: () => new ParseResult.Type(ast, value, "Invalid regular expression"),
      }),
    encode: (value) => ParseResult.succeed(value.toString()),
  })
).annotations(
  Id.annotations("RegexFromString", {
    description: "A string that is a valid regular expression",
    arbitrary: () => (fc) => fc.constant(null).map(() => FC.sample(Arbitrary.make(Regex), 1)[0] as typeof Regex.Type),
  })
) {
  /**
   * Compiles a string into a branded regular expression, throwing if invalid.
   *
   * @example
   * import { RegexFromString } from "@beep/schema-v2/internal/regex/regex";
   *
   * const regex = RegexFromString.make("\\\\d+");
   *
   * @category Regex
   * @since 0.1.0
   * @internal
   */
  static readonly make = (value: string) => S.decodeSync(RegexFromString)(value);
}

/**
 * Namespace exposing helper types for the parsed regex schema.
 *
 * @example
 * import type { RegexFromString } from "@beep/schema-v2/internal/regex/regex";
 *
 * const compiled: RegexFromString.Type = RegexFromString.make("\\\\d+");
 *
 * @category Regex
 * @since 0.1.0
 * @internal
 */
export declare namespace RegexFromString {
  /**
   * Runtime type of the parsed regex schema.
   *
   * @example
   * import type { RegexFromString } from "@beep/schema-v2/internal/regex/regex";
   *
   * let compiled: RegexFromString.Type;
   *
   * @category Regex
   * @since 0.1.0
   * @internal
   */
  export type Type = typeof RegexFromString.Type;
  /**
   * Encoded representation of the parsed regex schema.
   *
   * @example
   * import type { RegexFromString } from "@beep/schema-v2/internal/regex/regex";
   *
   * const literal: RegexFromString.Encoded = "/foo/";
   *
   * @category Regex
   * @since 0.1.0
   * @internal
   */
  export type Encoded = typeof RegexFromString.Encoded;
}
