# Schema-v2 Milestone 8 – Copy/Paste Agent Prompt
...
5. **Read prompts/checklist before editing.** Align with the dependencies and doc requirements recorded in
   `packages/common/schema-v2/MIGRATION_CHECKLIST.md` and prior milestone prompts to avoid re-work.
...
4. **Documentation requirements.** Follow `packages/common/schema-v2/DOCUMENTATION_STRATEGY.md`. Every export (including
...
5. **Checklist discipline.** After migrating each symbol, tick its checkbox in
   `packages/common/schema-v2/MIGRATION_CHECKLIST.md` (Milestone 8 section).
...
Deliverables:
- Migrated schema-v2 modules (SystemSchema, EntityId runtime, HttpRequestDetails, index) with
...
- Regenerated documentation artifacts (`packages/common/schema-v2/docs/**` and `docs/schema-v2/**`).
