...
import type { Entry } from "./event-journal.ts";
import { RemoteEntry, RemoteId } from "./event-journal.ts";
import type { Identity } from "./event-log.ts";
import { EventLog } from "./event-log.ts";
import { EncryptedEntry, EncryptedRemoteEntry, EventLogEncryption, layerSubtle } from "./event-log-encryption.ts";
...
export class ChunkedMessage extends Schema.TaggedClass<ChunkedMessage>(
  "@effect/experimental/EventLogRemote/ChunkedMessage"
)("ChunkedMessage", {
  id: Schema.Number,
 ...
 * @category protocol
 */
export class WriteEntries extends Schema.TaggedClass<WriteEntries>("@effect/experimental/EventLogRemote/WriteEntries")(
  "WriteEntries",
  {
...
export class RequestChanges extends Schema.TaggedClass<RequestChanges>(
  "@effect/experimental/EventLogRemote/RequestChanges"
)("RequestChanges", {
  publicKey: Schema.String,
 ...
 * @category protocol
 */
export class StopChanges extends Schema.TaggedClass<StopChanges>("@effect/experimental/EventLogRemote/StopChanges")(
  "StopChanges",
  {
    publicKey: Schema.String,
  }
 ...
 * @category change
 */
export class RemoteAdditions extends Schema.TaggedClass<RemoteAdditions>(
  "@effect/experimental/EventLogRemote/RemoveAdditions"
)("RemoveAdditions", { entries: Schema.Array(RemoteEntry) }) {}
    ...
    yield* Effect.gen(function* () {
      ...
      const subscriptions = yield* RcMap.make({
        lookup: (publicKey: string) =>
          Effect.acquireRelease(Mailbox.make<RemoteEntry>(), (mailbox) =>
            Effect.zipRight(mailbox.shutdown, Effect.ignoreLogged(write(new StopChanges({ publicKey }))))
          ),
      });
      ...
      return yield* socket
        .run((data) => handleMessage(decodeResponse(data)))
...
export const layerWebSocketBrowser = (
  ...
  layerWebSocket(url, options).pipe(Layer.provide([layerSubtle, Socket.layerWebSocketConstructorGlobal]));
