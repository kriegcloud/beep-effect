...
import type { RemoteId } from "./event-journal.ts";
import { EntryId, makeRemoteId } from "./event-journal.ts";
import { EncryptedRemoteEntry } from "./event-log-encryption.ts";
import type { ProtocolRequest, ProtocolResponse } from "./event-log-remote.ts";
import { Ack, Changes, ChunkedMessage, decodeRequest, encodeResponse, Hello, Pong } from "./event-log-remote.ts";
...
> = Effect.gen(function* () {
  ...
  return (socket) =>
    Effect.gen(() => handler(socket)).pipe(
      Effect.annotateLogs({
