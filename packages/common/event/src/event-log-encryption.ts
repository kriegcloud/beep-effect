/**
 * @since 0.1.0
 */

import { $EventLogEncryptionId } from "@beep/event/internal";
import * as A from "effect/Array";
import * as Context from "effect/Context";
import * as Effect from "effect/Effect";
import * as F from "effect/Function";
import * as Layer from "effect/Layer";
import * as Redacted from "effect/Redacted";
import * as Schema from "effect/Schema";
import * as Str from "effect/String";
import { Entry, EntryId, RemoteEntry } from "./event-journal.ts";
import type { Identity } from "./event-log.ts";

/**
 * @since 0.1.0
 * @category models
 */
export const EncryptedEntry = Schema.Struct({
  entryId: EntryId,
  encryptedEntry: Schema.Uint8ArrayFromSelf,
}).annotations($EventLogEncryptionId.annotations("EncryptedEntry"));

/**
 * @since 0.1.0
 * @category models
 */
export interface EncryptedRemoteEntry extends Schema.Schema.Type<typeof EncryptedRemoteEntry> {}

/**
 * @since 0.1.0
 * @category models
 */
export const EncryptedRemoteEntry = Schema.Struct({
  sequence: Schema.Number,
  iv: Schema.Uint8ArrayFromSelf,
  entryId: EntryId,
  encryptedEntry: Schema.Uint8ArrayFromSelf,
});

/**
 * @since 0.1.0
 * @category encrytion
 */
export class EventLogEncryption extends Context.Tag("@effect/experimental/EventLogEncryption")<
  EventLogEncryption,
  {
    readonly encrypt: (
      identity: typeof Identity.Service,
      entries: ReadonlyArray<Entry>
    ) => Effect.Effect<{
      readonly iv: Uint8Array;
      readonly encryptedEntries: ReadonlyArray<Uint8Array>;
    }>;
    readonly decrypt: (
      identity: typeof Identity.Service,
      entries: ReadonlyArray<EncryptedRemoteEntry>
    ) => Effect.Effect<Array<RemoteEntry>>;
    readonly sha256String: (data: Uint8Array) => Effect.Effect<string>;
    readonly sha256: (data: Uint8Array) => Effect.Effect<Uint8Array>;
  }
>() {}

/**
 * @since 0.1.0
 * @category encrytion
 */
export const makeEncryptionSubtle = (crypto: Crypto): Effect.Effect<typeof EventLogEncryption.Service> =>
  Effect.sync(() => {
    const keyCache = new WeakMap<typeof Identity.Service, CryptoKey>();
    const getKey = (identity: typeof Identity.Service) =>
      Effect.suspend(() => {
        if (keyCache.has(identity)) {
          return Effect.succeed(keyCache.get(identity)!);
        }
        const privateKey = new Uint8Array(Redacted.value(identity.privateKey));
        return Effect.promise(() =>
          crypto.subtle.importKey("raw", privateKey, "AES-GCM", true, ["encrypt", "decrypt"])
        ).pipe(
          Effect.tap((key) => {
            keyCache.set(identity, key);
          })
        );
      });

    return EventLogEncryption.of({
      encrypt: (identity, entries) =>
        Effect.gen(function* () {
          const data = yield* Effect.orDie(Entry.encodeArray(entries));
          const key = yield* getKey(identity);
          const iv = crypto.getRandomValues(new Uint8Array(12));
          const encodedEntries = F.pipe(
            data,
            A.map((entry) => new Uint8Array(entry))
          );
          const encryptedEntries = yield* Effect.promise(() =>
            Promise.all(
              F.pipe(
                encodedEntries,
                A.map((entry) =>
                  crypto.subtle.encrypt(
                    {
                      name: "AES-GCM",
                      iv,
                      tagLength: 128,
                    },
                    key,
                    entry
                  )
                )
              )
            )
          );
          return {
            iv,
            encryptedEntries: F.pipe(
              encryptedEntries,
              A.map((entry) => new Uint8Array(entry))
            ),
          };
        }),
      decrypt: (identity, entries) =>
        Effect.gen(function* () {
          const key = yield* getKey(identity);
          const decryptedBuffers = yield* Effect.promise(() =>
            Promise.all(
              F.pipe(
                entries,
                A.map((data) =>
                  crypto.subtle.decrypt(
                    {
                      name: "AES-GCM",
                      iv: new Uint8Array(data.iv),
                      tagLength: 128,
                    },
                    key,
                    new Uint8Array(data.encryptedEntry)
                  )
                )
              )
            )
          );
          const decryptedData = F.pipe(
            decryptedBuffers,
            A.map((buffer) => new Uint8Array(buffer))
          );
          const decoded = yield* Effect.orDie(Entry.decodeArray(decryptedData));
          return F.pipe(
            decoded,
            A.zip(entries),
            A.map(
              ([entry, remote]: readonly [Entry, EncryptedRemoteEntry]) =>
                new RemoteEntry({ remoteSequence: remote.sequence, entry })
            )
          );
        }),
      sha256: (data) =>
        Effect.promise(() => crypto.subtle.digest("SHA-256", new Uint8Array(data))).pipe(
          Effect.map((hash) => new Uint8Array(hash))
        ),
      sha256String: (data) =>
        Effect.map(
          Effect.promise(() => crypto.subtle.digest("SHA-256", new Uint8Array(data))),
          (hash) => {
            return F.pipe(
              new Uint8Array(hash),
              A.fromIterable,
              A.map((bytes) => F.pipe(bytes.toString(16), Str.padStart(2, "0"))),
              A.reduce("", (acc, part) => `${acc}${part}`)
            );
          }
        ),
    });
  });

/**
 * @since 0.1.0
 * @category encrytion
 */
export const layerSubtle: Layer.Layer<EventLogEncryption> = Layer.suspend(() =>
  Layer.effect(EventLogEncryption, makeEncryptionSubtle(globalThis.crypto))
);
