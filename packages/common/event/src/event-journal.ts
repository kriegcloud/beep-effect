...
import { $EventJournalId } from "@beep/event/internal";
 ...
 * @category tags
 */
export class EventJournal extends Context.Tag($EventJournalId`EventJournal`)<
  EventJournal,
  {
    /**
 ...
 * @category errors
 */
export class EventJournalError extends S.TaggedClass<EventJournalError>("@effect/experimental/EventJournal/Error")(
  "EventJournalError",
  {
    method: S.String,
    cause: S.Defect,
 ...
 * @category remote
 */
export const RemoteIdSchema = S.Uint8ArrayFromSelf.pipe(S.brand(RemoteIdTypeId));

export const RemoteId = S.declare((i: unknown): i is typeof RemoteIdSchema.Type & IDBValidKey =>
  S.is(RemoteIdSchema)(i)
);
 ...
 * @category entry
 */
export const EntryIdSchema = S.Uint8ArrayFromSelf.pipe(S.brand(EntryIdTypeId));
export const EntryId = S.declare((i: unknown): i is typeof EntryIdSchema.Type & IDBValidKey => S.is(EntryIdSchema)(i));
// IDBKeyRange
/**
 * @since 0.1.0
 * @category entry
 ...
 * @category entry
 */
export class Entry extends S.Class<Entry>("@effect/experimental/EventJournal/Entry")({
  id: EntryId,
  event: S.String,
  primaryKey: S.String,
   ...
   * @since 0.1.0
   */
  static arrayMsgPack = S.Array(MsgPack.schema(Entry));

  /**
   * @since 0.1.0
   */
  static encodeArray = S.encode(Entry.arrayMsgPack);

  /**
   * @since 0.1.0
   */
  static decodeArray = S.decode(Entry.arrayMsgPack);
 ...
 * @category entry
 */
export class RemoteEntry extends S.Class<RemoteEntry>("@effect/experimental/EventJournal/RemoteEntry")({
  remoteSequence: S.Number,
  entry: Entry,
}) {}
            ...
            for (const originEntry of compacted) {
              ...
              yield* Effect.async<void, EventJournalError>((resume) => {
                ...
                const cursorRequest = entries
                  .index("id")
                  .openCursor(IDBKeyRange.lowerBound(originEntry.id, true), "next");
      ...
      nextRemoteSequence: (remoteId) =>
        Effect.async<number, EventJournalError>((resume) => {
          ...
          const cursorRequest = tx
            .objectStore("remotes")
            .index("sequence")
            .openCursor(IDBKeyRange.bound([remoteId, 0], [remoteId, Number.POSITIVE_INFINITY]), "prev");
          cursorRequest.onsuccess = () => {
            const cursor = cursorRequest.result;
            if (!cursor) return;
...
const decodeEntryIdb = S.decodeSync(Entry);
const encodeEntryIdb = S.encodeSync(Entry);
const EntryIdbArray = S.Array(Entry);
const decodeEntryIdbArray = S.decodeUnknown(EntryIdbArray);
...
export const layerIndexedDb = (options?: {
  readonly database?: string;
}): Layer.Layer<EventJournal, EventJournalError> => Layer.scoped(EventJournal, makeIndexedDb(options));
