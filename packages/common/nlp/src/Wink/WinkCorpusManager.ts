...
import { CosineSimilarityRequest, WinkSimilarity, WinkSimilarityLive } from "./WinkSimilarity.ts";
import {
  type BM25Config,
...
const decodeStringArray = (
  ...
  Array.isArray(value) && value.every((item) => typeof item === "string")
    ...
    : Effect.fail(makeCorpusError(`Invalid ${context}: expected string[]`, corpusIdOptions(corpusId)));
...
const decodeNumberArray = (
  ...
  Array.isArray(value) && value.every(isFiniteNumber)
    ? Effect.succeed(value)
    : Effect.fail(makeCorpusError(`Invalid ${context}: expected number[]`, corpusIdOptions(corpusId)));
...
const decodeIdfEntries = (
  ...
  if (!Array.isArray(value)) {
    return Effect.fail(makeCorpusError("Invalid idf output: expected [term, idf][]", corpusIdOptions(corpusId)));
  }

  const decoded: Array<readonly [string, number]> = [];
  for (const entry of value) {
    if (!Array.isArray(entry) || entry.length !== 2 || typeof entry[0] !== "string" || !isFiniteNumber(entry[1])) {
      return Effect.fail(makeCorpusError("Invalid idf output: expected [term, idf][]", corpusIdOptions(corpusId)));
    }
...
const decodeMatrix = (
  ...
  if (!Array.isArray(value)) {
    return Effect.fail(makeCorpusError("Invalid document-term matrix output", corpusIdOptions(corpusId)));
  }
...
const isAccessorFn = (value: unknown): value is (...args: Array<any>) => unknown => typeof value === "function";

const isItsFunction = (value: unknown): value is ItsFunction<unknown> => typeof value === "function";
...
const resolveAccessor = (
  ...
  isRecord(its) && isAccessorFn(its[name])
    ...
    : Effect.fail(makeCorpusError(`Wink accessor its.${name} is unavailable`, corpusIdOptions(corpusId)));
...
const resolveVectorizerAccessor = (
  ...
  name: string,
  corpusId?: string
): Effect.Effect<(...args: Array<any>) => unknown, CorpusManagerError> => resolveAccessor(its, name, corpusId);

const makeDefaultSessionState = (corpusId: string, config: BM25Config, nowMs: number): CorpusSessionState => ({
  corpusId,
  documents: [],
...
export class WinkCorpusManager extends Effect.Service<WinkCorpusManager>()("effect-nlp/WinkCorpusManager", {
  effect: Effect.gen(function* () {
    const engine = yield* WinkEngine;
    const similarity = yield* WinkSimilarity;
    const sessionsRef = yield* Ref.make(new Map<string, CorpusSession>());
    ...
    const withSessionsLock = <A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> =>
      sessionsLock.withPermits(1)(effect);

    const getSession = (corpusId: string): Effect.Effect<CorpusSession, CorpusManagerError> =>
      withSessionsLock(
        Effect.gen(function* () {
    ...
    const withCorpusLock = <A, E>(
      corpusId: string,
      f: (session: CorpusSession) => Effect.Effect<A, E | CorpusManagerError>
    ...
    const readTextTokens = (
      ...
      Effect.gen(function* () {
        const normalAccessor = yield* resolveTokenAccessor(its, "normal", corpusId);
    ...
    const compileSession = (
      ...
      Effect.gen(function* () {
        ...
        const termsAccessor = yield* resolveVectorizerAccessor(its, "terms", state.corpusId);
        const vectorAccessor = yield* resolveVectorizerAccessor(its, "vector", state.corpusId);

        const tokenizedDocuments = yield* Effect.forEach(state.documents, (document) =>
          readDocumentTokens(document, its, state.corpusId)
        );
      ...
      createCorpus: (request?: CreateCorpusRequest) =>
        withSessionsLock(
          Effect.gen(function* () {
            ...
            const corpusId = requestedId !== undefined ? requestedId : yield* makeGeneratedId();
            ...
            const stateRef = yield* Ref.make(makeDefaultSessionState(corpusId, config, nowMs));
        ...
        withCorpusLock(request.corpusId, (session) =>
          Effect.gen(function* () {
            ...
            for (const document of incoming) {
              ...
              const tokens = yield* readDocumentTokens(document, its, request.corpusId);
      ...
      query: (request: QueryCorpusRequest) =>
        withCorpusLock(request.corpusId, (session) =>
          Effect.gen(function* () {
            ...
            const queryTokens = yield* readTextTokens(request.query, its, request.corpusId);
            ...
            }).pipe(Effect.flatMap((raw) => decodeNumberArray(raw, "query vector output", request.corpusId)));

            const termsChunk = Chunk.fromIterable(compiled.terms);
            const queryVector = DocumentVector({
              documentId: DocumentId.make(`${request.corpusId}-query-${Date.now()}`),
              vector: Chunk.fromIterable(queryVectorRaw),
              terms: termsChunk,
            });

            const includeText = request.includeText ?? false;
            const scored = yield* Effect.forEach(state.documents, (document, index) =>
              Effect.gen(function* () {
                ...
                const similarityScore = yield* similarity
                  .vectorCosine(
                ...
                const ranked: RankedCorpusDocument = includeText
                  ? {
                      index,
                      id: document.id,
                      score: Number.isFinite(similarityScore.score) ? similarityScore.score : 0,
                      text: document.text,
                    }
                  : {
                      index,
          ...
          Effect.gen(function* () {
            ...
            const idfValues: ReadonlyArray<CorpusIdfValue> = includeIdf
              ? yield* Effect.gen(function* () {
                  const accessor = yield* resolveVectorizerAccessor(its, "idf", request.corpusId);
                  ...
                  const decoded = yield* decodeIdfEntries(idfRaw, request.corpusId);
                  const sortedByIdf = [...decoded].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
                  const idfLimit = sanitizeTopIdfTerms(request.topIdfTerms, sortedByIdf.length);
            ...
            const matrix: ReadonlyArray<ReadonlyArray<number>> = includeMatrix
              ? yield* Effect.gen(function* () {
                  const accessor = yield* resolveVectorizerAccessor(its, "docTermMatrix", request.corpusId);
                  const matrixRaw = yield* Effect.try({
                    try: () => compiled.vectorizer.out(accessor),
                    catch: (cause) =>
                      makeCorpusError("Failed to compute corpus document-term matrix", {
                        corpusId: request.corpusId,
                        cause,
                      }),
                  });
            ...
            const averageDocumentLength =
              state.documents.length > 0 ? state.totalTokenCount / state.documents.length : 0;
...
export const learnCorpusDocuments = (request: LearnCorpusRequest) =>
  Effect.flatMap(WinkCorpusManager, (service) => service.learnDocuments(request));
