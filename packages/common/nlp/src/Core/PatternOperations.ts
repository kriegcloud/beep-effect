import { Array, Chunk, Data, Differ, Match, Option, Predicate, pipe, Schema, String } from "effect";
import {
...
export const hasAnyValue =
  ...
  (element: PatternElement): boolean => {
    const values = extractElementValues(element);
    return Array.some(targetValues, (target) => Array.some(values, (value) => value === target));
  };
...
export const hasAllValues =
  ...
  (element: PatternElement): boolean => {
    const values = extractElementValues(element);
    return Array.every(targetValues, (target) => Array.some(values, (value) => value === target));
  };
...
export const hasDateLikeValues = (element: PatternElement): boolean => {
  const values = extractElementValues(element);
  return Array.every(values, (value) => /^\d{4}-\d{2}-\d{2}$/.test(value) || /^\d{1,2}\/\d{1,2}\/\d{4}$/.test(value));
};
 ...
 */
export const isOptionalElement = Predicate.compose(isPOSElement, hasEmptyValues);

export const isRequiredElement = Predicate.compose(isPOSElement, hasNonEmptyValues);

export const isNumericLiteral = Predicate.compose(isLiteralElement, hasNumericValues);

export const isDateLiteral = Predicate.compose(isLiteralElement, hasDateLikeValues);
 ...
 */
export const OptionalPatternValue = Schema.Union(PatternValue, Schema.Literal("")).pipe(
  Schema.annotations({
...
export const transformElement = (
  ...
  return pipe(
    element,
    Match.value,
    Match.when({ _tag: "POSPatternElement" }, (posElement) => transformFn(posElement)),
    Match.when({ _tag: "EntityPatternElement" }, (entityElement) => transformFn(entityElement)),
    Match.when({ _tag: "LiteralPatternElement" }, (literalElement) => transformFn(literalElement)),
    Match.exhaustive
  );
};
 ...
 */
export const extractBracketContent = (bracketString: string): Option.Option<string> => {
...
export const splitBracketValues = (content: string): ReadonlyArray<string> => {
  return pipe(content, String.split("|"), Array.map(String.trim), Array.filter(String.isNonEmpty));
};
 ...
 */
export const extractElementValues = (element: PatternElement): ReadonlyArray<string> => {
  return pipe(
    element,
    Match.value,
    Match.when({ _tag: "POSPatternElement" }, (posElement) => posElement.value as unknown as ReadonlyArray<string>),
    Match.when(
    ...
    Match.when(
      ...
      (literalElement) => literalElement.value as unknown as ReadonlyArray<string>
    ),
    Match.exhaustive
  );
};
 ...
 */
export const updateElementValues = (element: PatternElement, newValues: ReadonlyArray<string>): PatternElement => {
  return pipe(
    element,
    Match.value,
 ...
 */
export const patchMapValues = (mapFn: (value: string) => string): ((element: PatternElement) => PatternElement) => {
 ...
 */
export const patchAddValue = (newValue: string): ((element: PatternElement) => PatternElement) => {
 ...
 */
export const patchRemoveValue = (valueToRemove: string): ((element: PatternElement) => PatternElement) => {
...
export const patchWhenElement = (
  ...
  return (element) => (elementPredicate(element) ? elementPatch(element) : element);
};
...
export const patchElementAt = (
  ...
  return (pattern) => {
    ...
    const updatedElements = pipe(
      elements,
      Array.map((element, i) => (i === index ? elementPatch(element) : element)),
      Chunk.fromIterable
    );
...
export const patchElementsByType = (
  ...
  const typePredicate =
    type === "POSPatternElement" ? isPOSElement : type === "EntityPatternElement" ? isEntityElement : isLiteralElement;
...
export const patchElementsWhere = (
  ...
  return (pattern) => {
    const updatedElements = pipe(
      pattern.elements,
      Chunk.map((element) => (predicate(element) ? elementPatch(element) : element))
    );
 ...
 */
export const normalizeElementValues = (element: PatternElement): PatternElement => {
  return pipe(
    element,
    patchElementValues((values) =>
      pipe(values, Array.map(String.trim), Array.filter(String.isNonEmpty), Array.dedupe, Array.sort(String.Order))
    )
  );
};
  ...
  return pipe(
    ...
    Match.when({ _tag: "LiteralPatternElement" }, (literalElement) => {
      ...
      const allDateLike = Array.every(
        values,
        (value) => /^\d{4}-\d{2}-\d{2}$/.test(value) || /^\d{1,2}\/\d{1,2}\/\d{4}$/.test(value)
      );
 ...
 */
export const escapePatternElementValues = (element: PatternElement): PatternElement => {
  return pipe(
    element,
    patchElementValues((values) =>
      Array.map(values, (value) => (needsWinkEscaping(value) ? escapeWinkLiteral(value) : value))
    )
  );
};
 ...
 */
export const unescapePatternElementValues = (element: PatternElement): PatternElement => {
  return pipe(
    element,
    patchElementValues((values) =>
      Array.map(values, (value) => (isEscapedWinkLiteral(value) ? unescapeWinkLiteral(value) : value))
    )
  );
};
...
export const toWinkPattern = (pattern: Pattern): string => {
  ...
  const winkElements = Array.map(elements, (element) => {
    const values = extractElementValues(element);
    const escapedValues = Array.map(values, (value) => (needsWinkEscaping(value) ? escapeWinkLiteral(value) : value));
...
export const toWinkCustomEntities = (
  ...
  return Array.map(entities, ({ name, pattern }) => toWinkCustomEntity(name, pattern));
};
