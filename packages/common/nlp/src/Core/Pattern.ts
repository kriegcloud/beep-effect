...
const WinkPOSTag = Schema.Literal(
  ...
  Schema.annotations({
    ...
    description: "Universal POS tags supported by wink-nlp based on the Universal Dependencies tagset",
...
export const POSPatternOption = Schema.Data(
  Schema.NonEmptyArray(Schema.Union(WinkPOSTag, Schema.Literal(""))).pipe(
    Schema.annotations({
      ...
      jsonSchema: {
        $comment: "Array of POS tags for pattern matching; empty string means optional",
      },
    })
  )
);
...
export const POSPatternOptionToBracketString = Schema.transformOrFail(POSPatternOption, Schema.NonEmptyString, {
  strict: true,
  decode: (input, _options, _ast, _fromI) => {
    // Check if there are any non-empty POS tags
    const hasValidTags = input.some((tag) => tag !== "");
    if (!hasValidTags) {
      return Effect.fail(new ParseResult.Type(_ast, input, "POS pattern must contain at least one valid POS tag"));
    }
  ...
  encode: (input, _options, _ast) => {
    ...
    return Schema.decodeUnknown(POSPatternOption)(parts).pipe(
      ...
      Effect.mapError((error) => new ParseResult.Type(_ast, input, `Invalid POS pattern: ${error.message}`))
    );
  },
});
...
export const EntityPatternOption = Schema.DataFromSelf(
  ...
  Schema.annotations({
    ...
    jsonSchema: {
      $comment: "Array of entity types for pattern matching; empty string means optional",
    },
  })
);

export type EntityPatternOption = Schema.Schema.Type<typeof EntityPatternOption>;

export const EntityPatternOptionToBracketString = Schema.transformOrFail(EntityPatternOption, Schema.NonEmptyString, {
  strict: true,
  decode: (input, _options, _ast, _fromI) => {
    ...
    if (!hasValidTypes) {
      return Effect.fail(
        new ParseResult.Type(_ast, input, "Entity pattern must contain at least one valid entity type")
      );
    }
    return Effect.succeed(`[${input.join("|")}]`);
  },
  encode: (input, _options, _ast) => {
    ...
    return Schema.decodeUnknown(EntityPatternOption)(parts).pipe(
      ...
      Effect.mapError((error) => new ParseResult.Type(_ast, input, `Invalid entity pattern: ${error.message}`))
    );
  },
});
...
export const LiteralPatternOption = Schema.DataFromSelf(
  ...
  Schema.annotations({
    ...
    jsonSchema: {
      $comment: "Array of literal words for exact matching; empty string means optional",
    },
  })
);

export const LiteralPatternOptionToBracketString = Schema.transformOrFail(LiteralPatternOption, Schema.NonEmptyString, {
  strict: true,
  decode: (input, _options, _ast, _fromI) => {
    // Check if there are any non-empty literals
    const hasValidLiterals = input.some((literal) => literal !== "");
    if (!hasValidLiterals) {
      return Effect.fail(new ParseResult.Type(_ast, input, "Literal pattern must contain at least one valid literal"));
    }
    return Effect.succeed(`[${input.join("|")}]`);
  },
  encode: (input, _options, _ast) => {
    ...
    return Schema.decodeUnknown(LiteralPatternOption)(parts).pipe(
      Effect.flatMap((decodedParts) => {
        // Check if there are any non-empty literals
        const hasValidLiterals = decodedParts.some((literal) => literal !== "");
        if (!hasValidLiterals) {
          return Effect.fail(
            new ParseResult.Type(_ast, input, "Literal pattern must contain at least one valid literal")
          );
        }
        return Effect.succeed(decodedParts);
      }),
      Effect.mapError((error) => new ParseResult.Type(_ast, input, `Invalid literal pattern: ${error.message}`))
    );
  },
});
...
export class POSPatternElement extends Schema.TaggedClass<POSPatternElement>("POS")("POSPatternElement", {
  value: POSPatternOption,
}) {}

export class EntityPatternElement extends Schema.TaggedClass<EntityPatternElement>("Entity")("EntityPatternElement", {
  value: EntityPatternOption,
}) {}

export class LiteralPatternElement extends Schema.TaggedClass<LiteralPatternElement>("Literal")(
...
export const MarkRange = Schema.Tuple(Schema.Int, Schema.Int).pipe(
  Schema.annotations({
    ...
    title: "Mark Range",
    description: "Token range specification for selective extraction [start, end]",
    examples: [
      [0, 0],
      [1, 2],
      [-2, -1],
...
export class Pattern extends Schema.TaggedClass<Pattern>("effect-nlp/Pattern/Model")("Pattern", {
  id: PatternId,
...
export namespace Pattern {
  ...
  export namespace POS {
    ...
    export const toBracketString = Schema.decodeSync(POSPatternOptionToBracketString);
  ...
  export namespace Entity {
    ...
    export const toBracketString = Schema.decodeSync(EntityPatternOptionToBracketString);
  ...
  export namespace Literal {
    ...
    export const toBracketString = Schema.decodeSync(LiteralPatternOptionToBracketString);
  }
