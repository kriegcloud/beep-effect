...
import {tapChunk} from "../Stream.ts";
...
import {encodeWithTransferables} from "@beep/utils/effect/Schema";
...
export const messagePortChannelWithAck = <MsgListen, MsgSend, MsgListenEncoded, MsgSendEncoded>({ port, schema: inputSchema, debugId }: {
  port: MessagePort;
  schema: InputSchema<MsgListen, MsgSend, MsgListenEncoded, MsgSendEncoded>;
  debugId?: string | number;
}) =>
  ...
  Effect.gen(function* () {
    ...
    yield* channel.listen.pipe(
      ...
      options?.heartbeat
            ...
            Effect.fn(function* (msg) {
              if (msg._tag === "Right" && Schema.is(WebChannelHeartbeat)(msg.right)) {
                if (msg.right._tag === "WebChannel.Ping") {
                  ...
                  const { deferred, requestId } =
                    pendingPingDeferredRef.current ?? shouldNeverHappen("No pending ping");
        ...
        : identity,
      tapChunk((chunk) => Queue.offerAll(queue, chunk)),
      Stream.runDrain,
      Effect.forkScoped
    );
...
export const sendDebugPing = (channel: WebChannel<any, any>) =>
  channel.send(DebugPingMessage.make({ message: "ping" }));
