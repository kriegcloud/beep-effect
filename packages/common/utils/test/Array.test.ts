...
describe("@beep/utils Array.flatMapNonEmpty", () => {
  ...
  it("data-last: flatMaps elements", () => {
    const result = pipe(
      nonEmpty,
      Array.flatMapNonEmpty((x): Array.NonEmptyReadonlyArray<number> => [x, x * 2])
    );
    expect(result).toEqual([1, 2, 2, 4, 3, 6]);
  });
...
describe("@beep/utils Array.mapNonEmptyReadonly", () => {
  ...
  it("data-last: maps elements", () => {
    const result = pipe(
      nonEmpty,
      Array.mapNonEmptyReadonly((x) => x * 2)
    );
    expect(result).toEqual([2, 4, 6]);
  });
...
describe("@beep/utils Array.flatMapNonEmptyReadonly", () => {
  ...
  it("data-last: flatMaps elements", () => {
    const result = pipe(
      nonEmpty,
      Array.flatMapNonEmptyReadonly((x): Array.NonEmptyReadonlyArray<number> => [x, x * 2])
    );
    expect(result).toEqual([1, 2, 2, 4, 3, 6]);
  });
