...
class DomainError extends CauseTaggedErrorClass<DomainError>("DomainError")(
  "DomainError") {
}

class OperationError extends CauseTaggedErrorClass<OperationError>(
  "OperationError")(
  "OperationError",
  {
    operation: S.String,
...
class OptionalContextError extends CauseTaggedErrorClass<OptionalContextError>(
...
class ExtendedOperationError extends OperationError.extend<ExtendedOperationError>(
  "ExtendedOperationError")(
  {
    resource: S.String,
  }) {
}
...
describe("CauseTaggedErrorClass", () => {
  ...
  it("requires extra fields in constructor helpers", () => {
    ...
    expect(OperationError.new(new Error("boom"), "beep", {
      operation: "load-profile",
      retryable: true,
    })).type.toBe<OperationError>();
    expect(OperationError.new("beep", {
      operation: "load-profile",
      retryable: true,
    })(new Error("boom"))).type.toBe<OperationError>();
  ...
  it(
    "requires the extras object even when every custom property is optional",
    () => {
      expect(OptionalContextError.new(
        new Error("boom"),
        "beep",
        {operation: "load-profile"},
      ...
      expect(OptionalContextError.new(
        ...
        new Error("boom"))).type.toBe<OptionalContextError>();
  ...
  it("infers mapError data-first and pipe-friendly helpers", () => {
    ...
    expect(DomainError.mapError(raw, "beep"))
      .type
      .toBeAssignableTo<Effect.Effect<never, DomainError>>();
    expect(pipe(raw, DomainError.mapError("beep")))
      .type
      .toBeAssignableTo<Effect.Effect<never, DomainError>>();
    expect(OperationError.mapError(raw, "beep", {
      operation: "load-profile",
      retryable: true,
    })).type.toBeAssignableTo<Effect.Effect<never, OperationError>>();
    expect(pipe(raw, OperationError.mapError("beep", {
      operation: "load-profile",
      retryable: true,
    }))).type.toBeAssignableTo<Effect.Effect<never, OperationError>>();
  ...
  it("preserves helper typing on extended classes", () => {
    expect(ExtendedOperationError.new(new Error("boom"), "beep", {
      operation: "load-profile",
      retryable: true,
      resource: "profile",
    ...
    expect(pipe(
      ...
      .type
      .toBeAssignableTo<Effect.Effect<never, ExtendedOperationError>>();
