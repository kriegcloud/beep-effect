...
describe("CauseTaggedErrorClass", () => {
  ...
  it("requires the extras object even when every custom property is optional", () => {
    expect(OptionalContextError.new(new Error("boom"), "beep", { operation: "load-profile" })).type.toBe<
      OptionalContextError
    >();
    expect(OptionalContextError.new("beep", { operation: "load-profile" })(new Error("boom"))).type.toBe<
      OptionalContextError
    >();
  ...
  it("infers mapError data-first and pipe-friendly helpers", () => {
    ...
    // @ts-expect-error!
    pipe(raw, OperationError.mapError("beep"));
  });
