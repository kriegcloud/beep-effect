...
const removeBom = (input: string): string => (input.charAt(0) === BOM
  ? input.slice(1)
  : input);

const getRowDelimiterLength = (
  input: string,
  cursor: number
): number => {
...
const isEmptyRow = (row: ReadonlyArray<string>): boolean => pipe(
  row,
  A.join(""),
  Str.replace(
    /\s+/g,
...
const getQuotedFieldStart = (
  ...
  while (currentCursor < input.length) {
    const rowDelimiterLength = getRowDelimiterLength(
      input,
      currentCursor
    );
...
const parseQuotedField = (
  ...
  if (O.isNone(parserOptions.quote)) {
    return Effect.fail(csvError(
      "Quoted field parsing requires a configured quote character.",
      quoteStart
    ));
  }

  const quote = parserOptions.quote.value;
  const escapeChar = O.getOrElse(() => quote)(
    parserOptions.escapeChar);
  ...
  if (!foundClosingQuote) {
    return Effect.fail(csvError(
      `Parse Error: missing closing quote '${quote}'.`,
      quoteStart
    ));
  }
  ...
  while (cursor < input.length) {
    ...
    return Effect.fail(
      csvError(
        `Parse Error: expected delimiter or newline after closing quote, received '${character ?? ""}'.`,
        cursor
      )
    );
  }
...
const parseField = (
  ...
  return pipe(
    ...
    O.match({
      onNone: () => Effect.succeed(parseUnquotedField(
        input,
        cursor,
        parserOptions
      )),
      onSome: (quoteStart) => parseQuotedField(
        input,
        quoteStart,
        parserOptions
      ),
...
const parseRowAt = (
  ...
  cursor: number,
  parserOptions: ParserOptions
): Effect.Effect<ParsedRow, CsvError> =>
  Effect.gen(function* () {
    let currentCursor = cursor;
    ...
    while (true) {
      ...
      const nextRowDelimiterLength = getRowDelimiterLength(
        input,
        currentCursor
      );
      ...
      if (input.at(currentCursor) === parserOptions.delimiter) {
        ...
        const trailingDelimiterLength = getRowDelimiterLength(
          input,
          currentCursor
        );
      ...
      return yield* csvError(
        "Parse Error: parser cursor did not terminate on a delimiter or newline.",
        currentCursor
      );
    }
  });

const advancePastComment = (
  input: string,
  cursor: number
): number => {
  let currentCursor = cursor;
 ...
 * @since 0.0.0
 */
export const parseCsvRows = Effect.fn(function* (
  input: string,
  parserOptions: ParserOptions
) {
  ...
  while (cursor < source.length) {
    ...
    const parsedRow = yield* parseRowAt(
      source,
      cursor,
      parserOptions
    );
