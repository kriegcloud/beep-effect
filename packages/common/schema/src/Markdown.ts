...
type MarkdownHtmlRender = (
  content: string,
  options?: undefined | MarkdownRenderOptions
) => unknown;
...
const getMarkdownHtmlRender = (): O.Option<MarkdownHtmlRender> => {
  const bunRuntime = Reflect.get(
    globalThis,
    "Bun"
  );
  const markdown = P.isObject(bunRuntime)
    ? Reflect.get(
      bunRuntime,
      "markdown"
    )
    : undefined;
  const html = P.isObject(markdown)
    ? Reflect.get(
      markdown,
      "html"
    )
  ...
  if (P.isFunction(html)) {
    const renderMarkdownHtml: MarkdownHtmlRender = (
      content,
      options
    ) => html(
      content,
...
const makeRenderMarkdownHtml = (options?: MarkdownRenderOptions) =>
  Effect.fn("Markdown.renderMarkdownHtml")(
    function* (content: string) {
      const renderMarkdownHtml = yield* O.match(
        getMarkdownHtmlRender(),
        {
          onNone: () =>
            Effect.fail(invalidMarkdownInput(
              content,
              "Bun.markdown.html is unavailable in the current runtime."
            )),
          onSome: Effect.succeed,
        }
      );
      const rendered = yield* Effect.try({
        try: () => renderMarkdownHtml(
          content,
          options
        ),
        catch: (cause) =>
          invalidMarkdownInput(
            content,
            P.isError(cause)
              ? `Invalid Markdown input (${cause.message}).`
              : "Invalid Markdown input."
          ),
      });

      return yield* S.decodeUnknownEffect(S.String)(
        rendered)
        .pipe(
          Effect.mapError(() => invalidMarkdownInput(
            content,
            "Invalid Markdown input (Expected HTML string output)."
          ))
        );
    });
 ...
 * @since 0.0.0
 */
export const decodeMarkdownTextAs = <Schema extends S.Top>(
  schema: Schema,
