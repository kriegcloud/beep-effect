...
export interface EntityId<TableName extends string, Brand extends string>
  extends S.AnnotableClass<EntityId<TableName, Brand>, S.Schema.Type<EntityId.PublicIdSchema<TableName>>, S.Schema.Type<EntityId.PublicIdSchema<TableName>>> {
  readonly [TypeId]: typeof variance;
  readonly create: PublicIdCreate<TableName>;
  readonly tableName: SnakeTag.Literal<TableName>;
  readonly brand: Brand;
  readonly is: (u: unknown, overrideOptions?: AST.ParseOptions | number) => u is S.Schema.Type<EntityId.PublicIdSchema<TableName>>;
...
export function makeEntityIdSchema<const TableName extends string, const Brand extends string>(
  ...
  const create = thunk(`${tableName}__${UUIDLiteralEncoded.create()}` as const);
  ...
  // type M = S.PropertySignature<":", Exclude<`${AppendType<"", SnakeTag.Literal<TableName>>}__${string}-${string}-${string}-${string}-${string}`, undefined>, never, "?:", `${AppendType<"", SnakeTag.Literal<TableName>>}__${string}-${string}-${string}-${string}-${string}` | undefined, true, never>;
  // const mIdSchema: M = publicSchema.pipe(
  //   S.optional,
  //   S.withDefaults({
  //     constructor: () => create(),
  //     decoding: () => create()
  ...
  class BaseClass extends S.make<S.Schema.Type<EntityId.PublicIdSchema<TableName>>>(defaultAST) {
    ...
    static readonly make = (input: string): EntityId.Type<TableName> => {
      invariant(S.is(publicSchema)(input), `Invalid id for ${tableName}: ${input}`, {
        file: "@beep/schema/identity/entity-id/entity-id.ts",
        line: 231,
        args: [input],
      });
