...
export class URLPath extends S.TemplateLiteral("/", S.String).pipe(
  ...
  S.annotations({
    title: "NextJS Path",
    description:
      "A valid NextJS pathname including static assets, API routes, and pages",
    examples: [
    ...
    arbitrary: () => (fc) => {
      ...
      return fc.tuple(segments, filename).map(([segs, file]) => {
        const path = Str.concat(
          `${A.join("/")(segs)}${segs.length > 0 && file ? "/" : ""}${file || ""}`,
        )(`/`);
        return path === "//" ? "/" : path;
      });
    },
  ...
  static readonly is = (input: unknown): input is URLPath.Type =>
    S.is(URLPath)(input);
}
