...
type ExifToolServiceEffect = Effect.Effect<ExifToolServiceShape, MetadataParseError, never>;

export const exifToolServiceEffect: ExifToolServiceEffect = Effect.gen(function* () {
  const { parseMetadata, writeMetadata } = yield* Effect.tryPromise({
  ...
  const extractMetadata: ExifToolServiceShape["extractMetadata"] = Effect.fn("extractMetadata")(function* (
    file: File | Binaryfile
  ) {
  ...
  const writeMetadataFn: ExifToolServiceShape["writeMetadata"] = Effect.fn("writeMetadataFn")(function* (file, tags) {
    const result = yield* Effect.tryPromise({
      try: () => writeMetadata(file, tags as Record<string, string | number | boolean | (string | number | boolean)[]>),
      catch: (e) =>
        new MetadataParseError({
...
export const parseAudioMetadata = Effect.gen(function* () {
  const musicMetadata = yield* Effect.tryPromise({
    try: () => import("music-metadata"),
    catch: (e) =>
      new MetadataParseError({
        message: "MusicMetadata extraction failed",
        cause: e,
        phase: "load",
      }),
  });

  const parseBlob = Effect.fn("parseBlob")(function* (input: File | Blob, options?: IOptions) {
    const result = yield* Effect.tryPromise({
      try: () => musicMetadata.parseBlob(input, options),
      catch: (e) =>
        new MetadataParseError({
          message: "MusicMetadata extraction failed",
          cause: e,
    ...
    return yield* S.decode(IAudioMetadata)(result);
  });

  const parseBuffer = Effect.fn("ParseBuffer")(function* (
    uint8Array: Uint8Array,
    fileInfo?: IFileInfo | string,
    options?: IOptions
  ) {
    const result = yield* Effect.tryPromise({
      try: () => musicMetadata.parseBuffer(uint8Array, fileInfo, options),
      catch: (e) =>
        new MetadataParseError({
          message: "MusicMetadata extraction failed",
          cause: e,
  ...
  const parseWebStream = Effect.fn("parseWebStream")(function* (
    webStream: ReadableStream<Uint8Array>,
    fileInfo?: IFileInfo | string,
    options?: IOptions
  ) {
    const result = yield* Effect.tryPromise({
      try: () => musicMetadata.parseWebStream(webStream, fileInfo, options),
      catch: (e) =>
        new MetadataParseError({
          message: "MusicMetadata extraction failed",
          cause: e,
...
export class MetadataService extends Effect.Service<MetadataService>()($I`MetadataService`, {
  dependencies: [],
  accessors: true,
  effect: Effect.all([exifToolServiceEffect, parseAudioMetadata], { concurrency: 2 }).pipe(
    Effect.map(([exifToolServiceEffect, parseAudioMetadata]) => ({
      exif: exifToolServiceEffect,
      audio: parseAudioMetadata,
    }))
  ),
}) {}
