 ...
 */
export class CurrencyMismatchError extends S.TaggedError<CurrencyMismatchError>()(
 ...
 */
export class DivisionByZeroError extends S.TaggedError<DivisionByZeroError>()(
  "DivisionByZeroError",
  {},
  HttpApiSchema.annotations({ status: 400 })
) {
...
/**
 * Type guards for error types using S.is()
 */
export const isCurrencyMismatchError = S.is(CurrencyMismatchError);
export const isDivisionByZeroError = S.is(DivisionByZeroError);
...
export class MonetaryAmount extends S.Class<MonetaryAmount>("MonetaryAmount")({
   ...
   */
  static fromString(amountStr: string, currency: CurrencyCode): Effect.Effect<MonetaryAmount, ParseResult.ParseError> {
    return Effect.gen(function* () {
      const bd = yield* S.decodeUnknown(S.BigDecimal)(amountStr);
...
/**
 * Type guard for MonetaryAmount using S.is()
 */
export const isMonetaryAmount = S.is(MonetaryAmount);
 ...
 */
export const add = (a: MonetaryAmount, b: MonetaryAmount): Effect.Effect<MonetaryAmount, CurrencyMismatchError> => {
  if (a.currency !== b.currency) {
    return Effect.fail(new CurrencyMismatchError({ expected: a.currency, actual: b.currency }));
  }
  return Effect.succeed(MonetaryAmount.fromBigDecimal(BigDecimal.sum(a.amount, b.amount), a.currency));
};
...
export const subtract = (
  ...
  return Effect.succeed(MonetaryAmount.fromBigDecimal(BigDecimal.subtract(a.amount, b.amount), a.currency));
};
 ...
 */
export const multiply = (amount: MonetaryAmount, multiplier: BigDecimal.BigDecimal): MonetaryAmount => {
  return MonetaryAmount.fromBigDecimal(BigDecimal.multiply(amount.amount, multiplier), amount.currency);
};
 ...
 */
export const unsafeDivide = (amount: MonetaryAmount, divisor: BigDecimal.BigDecimal): MonetaryAmount => {
  return MonetaryAmount.fromBigDecimal(BigDecimal.unsafeDivide(amount.amount, divisor), amount.currency);
};
 ...
 */
export const compare = (a: MonetaryAmount, b: MonetaryAmount): Effect.Effect<-1 | 0 | 1, CurrencyMismatchError> => {
  if (a.currency !== b.currency) {
    return Effect.fail(new CurrencyMismatchError({ expected: a.currency, actual: b.currency }));
  }
 ...
 * Returns an Effect that fails with CurrencyMismatchError if currencies differ.
 */
export const greaterThan = (a: MonetaryAmount, b: MonetaryAmount): Effect.Effect<boolean, CurrencyMismatchError> => {
  if (a.currency !== b.currency) {
    return Effect.fail(new CurrencyMismatchError({ expected: a.currency, actual: b.currency }));
  }
 ...
 * Returns an Effect that fails with CurrencyMismatchError if currencies differ.
 */
export const lessThan = (a: MonetaryAmount, b: MonetaryAmount): Effect.Effect<boolean, CurrencyMismatchError> => {
  if (a.currency !== b.currency) {
    return Effect.fail(new CurrencyMismatchError({ expected: a.currency, actual: b.currency }));
  }
 ...
 * Returns an Effect that fails with CurrencyMismatchError if currencies differ.
 */
export const equals = (a: MonetaryAmount, b: MonetaryAmount): Effect.Effect<boolean, CurrencyMismatchError> => {
  if (a.currency !== b.currency) {
    return Effect.fail(new CurrencyMismatchError({ expected: a.currency, actual: b.currency }));
  }
 ...
 */
export const round = (amount: MonetaryAmount, scale = 2): MonetaryAmount => {
 ...
 * Returns an Effect that fails with CurrencyMismatchError if currencies differ.
 */
export const max = (a: MonetaryAmount, b: MonetaryAmount): Effect.Effect<MonetaryAmount, CurrencyMismatchError> => {
  if (a.currency !== b.currency) {
    return Effect.fail(new CurrencyMismatchError({ expected: a.currency, actual: b.currency }));
  }
  return Effect.succeed(MonetaryAmount.fromBigDecimal(BigDecimal.max(a.amount, b.amount), a.currency));
};
 ...
 * Returns an Effect that fails with CurrencyMismatchError if currencies differ.
 */
export const min = (a: MonetaryAmount, b: MonetaryAmount): Effect.Effect<MonetaryAmount, CurrencyMismatchError> => {
  if (a.currency !== b.currency) {
    return Effect.fail(new CurrencyMismatchError({ expected: a.currency, actual: b.currency }));
  }
  return Effect.succeed(MonetaryAmount.fromBigDecimal(BigDecimal.min(a.amount, b.amount), a.currency));
};
