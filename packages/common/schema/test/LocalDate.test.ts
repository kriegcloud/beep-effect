...
import { Effect, Exit, Equal, Duration } from "effect";
...
describe("LocalDate", () => {
  describe("validation", () => {
    it.effect("accepts valid dates", () =>
      Effect.gen(function* () {
        const decode = S.decodeUnknownEffect(LocalDate)
  ...
  describe("fromDateTime", () => {
    it("creates LocalDate from DateTime", () => {
      const dt = DateTime.makeUnsafe({ year: 2024, month: 6, day: 15 })
  ...
  describe("Order", () => {
    it("compares dates correctly", () => {
      ...
      expect(Order(date1, date1)).toBe(0)
      expect(Order(date1, date3)).toBe(-1)
      expect(Order(date1, date4)).toBe(-1)
    })
  })

  describe("isBefore", () => {
    it("returns true when first date is before second", () => {
      expect(isBefore(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 16 }))).toBe(true)
      expect(isBefore(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2024, month: 7, day: 1 }))).toBe(true)
      expect(isBefore(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2025, month: 1, day: 1 }))).toBe(true)
    })

    it("returns false when first date is not before second", () => {
      expect(isBefore(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 16 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }))).toBe(false)
      expect(isBefore(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }))).toBe(false)
    })
  })

  describe("isAfter", () => {
    it("returns true when first date is after second", () => {
      expect(isAfter(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 16 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }))).toBe(true)
      expect(isAfter(LocalDate.makeUnsafe({ year: 2024, month: 7, day: 1 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }))).toBe(true)
      expect(isAfter(LocalDate.makeUnsafe({ year: 2025, month: 1, day: 1 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }))).toBe(true)
    })

    it("returns false when first date is not after second", () => {
      expect(isAfter(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 16 }))).toBe(false)
      expect(isAfter(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }))).toBe(false)
    })
  })

  describe("equals", () => {
    it("returns true for equal dates", () => {
      expect(equals(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }))).toBe(true)
    })

    it("returns false for different dates", () => {
      expect(equals(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2024, month: 6, day: 16 }))).toBe(false)
      expect(equals(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2024, month: 7, day: 15 }))).toBe(false)
      expect(equals(LocalDate.makeUnsafe({ year: 2024, month: 6, day: 15 }), LocalDate.makeUnsafe({ year: 2025, month: 6, day: 15 }))).toBe(false)
    })
  })
