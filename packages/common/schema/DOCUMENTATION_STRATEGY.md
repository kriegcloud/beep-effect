# `@beep/schema` Documentation Strategy
...
- `@since` reflects the version introduced in schema (start with `0.1.0`).
...
- Each schema folder contains a co-located `_id.ts` that wraps the new `@beep/identity` builders (`SchemaId`,
  `BeepId`). These files expose a ready-to-use `Id` helper that already encodes the namespace for that folder
  (for example `packages/common/schema/src/builders/form/_id.ts` exports `Id = BeepId.from(...)`).
...
- see packages/common/schema/src/core/annotations/example-annotations.ts for header doc example
## Tooling Plan

### 1. Documentation lint

Adapt `scratchpad/effect-smol-docs/scripts/analyze-jsdoc.mjs` into
`packages/common/schema/scripts/analyze-jsdoc.ts`:

- Scan `packages/common/schema/src/**.ts` (skip `internal/**`).
...
- Ensures comments on call signatures created by helpers (e.g. `dual` or other wrappers)
  are preserved.
- Run via `bunx jscodeshift -t packages/common/schema/scripts/codemods/jsdoc.ts` when
  migrating modules that expose higher-order APIs to backfill missing doc nodes.

### 3. Doc generation

- Add `@effect/docgen` to devDependencies of `@beep/schema`.
- Create `packages/common/schema/docgen.json` modeled after
...
- Implement a `docs-copy` script similar to effect-smol’s `docs.mjs` to copy
  package docs into root `docs/schema/**` with `parent` metadata per package.
...
- Add a GitHub workflow or Turbo task that runs:
  ...
  bun run docgen
  ```
  when files under `packages/common/schema/src/**` change.
...
- `bun run docs:lint` — scans every schema file (skipping `src/internal/**`) and writes `jsdoc-analysis-results.json`
...
- `bun run docgen` — executes `@effect/docgen` inside `packages/common/schema` and emits Markdown into
  `packages/common/schema/docs`.
- `bun run docs:site` — runs `bun run docgen` and then copies the rendered guides into `docs/schema/**` via
...
> schema, pass the appropriate scope flag (for example, `bun run docs:lint:file -- --scope invariant packages/common/invariant/src/invariant.ts`).

## Agent Workflow Summary

For each migrated module:

1. Move the code into its schema folder, replacing **all** string/array/object manipulation with Effect utilities
...
4. Run `bun run docgen` to render docs; inspect output under `packages/common/schema/docs`.
