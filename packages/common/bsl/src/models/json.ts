...
export class JsonPrimitive extends S.Union(
  S.String,
  S.Number,
  S.Boolean,
  S.Null,
...
export class JsonArray extends S.suspend(
  (): S.Schema<JsonArray.Type> => S.Array(JsonValue)
).annotations(
...
export class JsonValue extends S.suspend(
  (): S.Schema<JsonValue.Type> => S.Union(
    JsonPrimitive,
    JsonObject,
    S.Array(JsonValue)
  )
...
export class JsonColumnAccepts extends S.Union(
  S.Object,
  JsonArrayUnknown,
  S.mutable(JsonArrayUnknown)
).annotations(
