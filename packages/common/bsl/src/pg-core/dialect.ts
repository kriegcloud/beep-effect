...
import { CasingCache } from "../casing";
...
import type {
  ...
  PgSelectJoinConfig,
  PgUpdateConfig,
} from "../pg-core/query-builders/index";
...
import { and, eq, isSQLWrapper, type SQLWrapper, View } from "../sql/index";
import {
...
import { getTableName, getTableUniqueName, Table, TableColumns } from "../table";
import { type Casing, orderSelectedFields, type UpdateSet } from "../utils";
import { ViewBaseConfig } from "../view-common";
import type { PgSession } from "./session";
import type { PgMaterializedView, PgView } from "./view";
import { PgViewBase } from "./view-base";
...
export class PgDialect {
  ...
  constructor(config?: undefined | PgDialectConfig) {
  ...
  buildUpdateSet(table: PgTable, set: UpdateSet): SQL {
    ...
    const columnNames = Struct.keys(tableColumns).filter(
  ...
  private buildSelection(
    ...
    { isSingleTable = false }: { isSingleTable?: undefined | boolean } = {}
  ): SQL {
    ...
    const chunks = fields.flatMap(({ field }, i) => {
      ...
      if (is(field, SQL.Aliased) && field.isSelectionField) {
        ...
        const entries = Struct.entries(field._.selectedFields) as [string, SQL.Aliased | Column | SQL][];
  ...
  buildSelectQuery({
    ...
    const limitSql =
      typeof limit === "object" || (typeof limit === "number" && limit >= 0) ? sql` limit ${limit}` : undefined;
  ...
  buildInsertQuery({
    ...
    const colEntries: [string, PgColumn][] = Struct.entries(columns).filter(([_, col]) => !col.shouldDisableInsert());
  ...
  buildRefreshMaterializedViewQuery({
    ...
    concurrently?: undefined | boolean;
    withNoData?: undefined | boolean;
  }): SQL {
  ...
  _buildRelationalQuery({
    ...
    tableAlias: string;
    nestedQueryRelation?: undefined | V1.Relation;
    joinOn?: undefined | SQL;
  }): V1.BuildRelationalQueryResult<PgTable, PgColumn> {
    let selection: V1.BuildRelationalQueryResult<PgTable, PgColumn>["selection"] = [];
    let limit: any;
    let offset: any;
    let orderBy: NonNullable<PgSelectConfig["orderBy"]> = [];
    let where: any;
    const joins: PgSelectJoinConfig[] = [];

    if (config === true) {
      const selectionEntries = Struct.entries(tableConfig.columns);
      ...
      const aliasedColumns = Object.fromEntries(
        Struct.entries(tableConfig.columns).map(([key, value]) => [key, aliasedTableColumn(value, tableAlias)])
      );

      if (config.where) {
        const whereSql =
          typeof config.where === "function" ? config.where(aliasedColumns, V1.getOperators()) : config.where;
      ...
      if (config.columns) {
        ...
        for (const [field, value] of Struct.entries(config.columns)) {
          if (value === undefined) {
            continue;
          }
        ...
        if (selectedColumns.length > 0) {
          selectedColumns = isIncludeMode
            ...
            : Struct.keys(tableConfig.columns).filter((key) => !selectedColumns.includes(key));
        }
      } else {
        // Select all columns if selection is not specified
        selectedColumns = Struct.keys(tableConfig.columns);
      }
      ...
      if (config.with) {
        selectedRelations = Struct.entries(config.with)
      ...
      if (config.extras) {
        ...
        for (const [tsKey, value] of Struct.entries(extras)) {
  ...
  private buildRqbColumn(table: Table | View, column: unknown, key: string) {
    if (is(column, Column)) {
      ...
      switch (targetType) {
        ...
        case "PgCustomColumn": {
          return sql`${(<PgCustomColumn<any>>column).jsonSelectIdentifier(
            name,
            sql,
  ...
  private unwrapAllColumns = (table: Table | View, selection: BuildRelationalQueryResult["selection"]) => {
    return sql.join(
      Struct.entries(table[TableColumns]).map(([k, v]) => {
        selection.push({
          key: k,
  ...
  private buildColumns = (
    ...
    config?: undefined | DBQueryConfig<"many">
  ) =>
    config?.columns
      ? (() => {
          const entries = Struct.entries(config.columns);
          ...
          if (colSelectionMode === false) {
            for (const [k, v] of Struct.entries(columnContainer)) {
  ...
  buildRelationalQuery({
    ...
    queryConfig?: undefined | DBQueryConfig<"many"> | true;
    relationWhere?: undefined | SQL;
    mode: "first" | "many";
    errorPath?: undefined | string;
    depth?: undefined | number;
    throughJoin?: undefined | SQL;
    ...
    const where: SQL | undefined =
      params?.where && relationWhere
        ? and(relationsFilterToSQL(table, params.where, tableConfig.relations, schema, this.casing), relationWhere)
        : params?.where
    ...
    const joins = params
      ? (() => {
          ...
          const withEntries = Struct.entries(joins).filter(([_, v]) => v);
          ...
          return sql.join(
            withEntries.map(([k, join]) => {
              ...
              selection.push({
                ...
                isArray: !isSingle,
                isOptional:
                  ((relation as AnyOne).optional ?? false) ||
    ...
    const query = sql`select ${selectionSet} from ${getTableAsAliasSQL(table)}${throughJoin}${sql` ${joins}`.if(
      joins
    )}${sql` where ${where}`.if(where)}${sql` order by ${order}`.if(order)}${sql` limit ${limit}`.if(
      limit !== undefined
