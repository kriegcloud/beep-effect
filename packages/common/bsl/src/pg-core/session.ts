...
import { type Cache, hashQuery, NoopCache } from "../cache/core/cache";
import type { WithCacheConfig } from "../cache/core/types";
import { entityKind, is } from "../entity";
import { DrizzleQueryError, TransactionRollbackError } from "../errors";
import type { AnyRelations, EmptyRelations } from "../relations";
import type { PreparedQuery } from "../session";
import { type Query, type SQL, sql } from "../sql/index";
...
export abstract class PgPreparedQuery<T extends PreparedQueryConfig> implements PreparedQuery {
  protected constructor(
    ...
    private readonly cacheConfig?: undefined | WithCacheConfig
  ) {
  ...
  mapResult(response: unknown, _isFromBatch?: undefined | boolean): unknown {
    return response;
  }
...
this.authToken = token;
  ...
  /** @internal */
  joinsNotNullableMap?: undefined | Record<string, boolean>;
  ...
  abstract execute(placeholderValues?: undefined | Record<string, unknown>): Promise<T["execute"]>;
  /** @internal */
  abstract execute(
    placeholderValues?: undefined | Record<string, unknown>,
    token?: NeonAuthToken
  ): Promise<T["execute"]>;
  /** @internal */
  abstract execute(
  ...
  /** @internal */
  abstract all(placeholderValues?: undefined | Record<string, unknown>): Promise<T["all"]>;
...
export interface PgTransactionConfig {
  isolationLevel?: undefined | "read uncommitted" | "read committed" | "repeatable read" | "serializable";
...
> {
  ...
  async count(sql: SQL, token?: NeonAuthToken): Promise<number> {
    ...
    return Number(res[0]["count" as const]);
  }
