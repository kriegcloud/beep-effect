...
import type { PgColumnBaseConfig } from "../common";
...
> {
  ...
  override build(table: PgTable<any>) {
    return new PgSparseVector(table, this.config as any);
  }
}

export class PgSparseVector extends PgColumn<
  "string sparsevec",
  PgColumnBaseConfig<"string sparsevec">,
  { vectorDimensions: number | undefined }
> {
...
export function sparsevec(config: PgSparseVectorConfig): PgSparseVectorBuilder;
export function sparsevec(name: string, config: PgSparseVectorConfig): PgSparseVectorBuilder;
