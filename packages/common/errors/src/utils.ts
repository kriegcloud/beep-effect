...
import { EnvValue, LogFormat, LogLevel as LogLevelSchema } from "@beep/constants";
...
const parseLevel = (raw: LogLevelSchema.Type): LogLevel.LogLevel => {
...
const EnvVars = S.Struct({
  ENV: EnvValue,
  LOG_FORMAT: LogFormat,
  LOG_LEVEL: LogLevelSchema,
});

const envVars = F.pipe(
  {
    ENV: process.env.NEXT_PUBLIC_ENV,
    LOG_FORMAT: process.env.NEXT_PUBLIC_LOG_FORMAT,
    LOG_LEVEL: process.env.NEXT_PUBLIC_LOG_LEVEL,
  } satisfies Record<keyof typeof EnvVars.Encoded, unknown>,
  S.decodeUnknownEither(EnvVars),
...
export const readEnvLoggerConfig = Effect.gen(function* () {
  const nodeEnv = envVars.ENV;
  const isProd = nodeEnv === "prod";

  const formatEnv = isProd ? "json" : envVars.LOG_FORMAT;
  const levelEnv = envVars.LOG_LEVEL;
