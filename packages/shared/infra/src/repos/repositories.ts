/**
 * Consolidated repository namespace with all shared repository services and layers.
 *
 * @since 0.1.0
 */
import type { Db } from "@beep/shared-infra/Db";
import * as Layer from "effect/Layer";
import type { SharedDb } from "../db";
import { FileRepo } from "./File.repo.ts";
import { FolderRepo } from "./Folder.repo.ts";

/**
 * Union type of all shared repository services.
 *
 * @example
 * ```typescript
 * import type { SharedRepos } from "@beep/shared-infra"
 * import * as Effect from "effect/Effect"
 *
 * type MyServices = SharedRepos | OtherService
 * ```
 *
 * @category models
 * @since 0.1.0
 */
export type SharedRepos = FileRepo | FolderRepo;

/**
 * Layer type providing all shared repositories with database dependencies.
 *
 * @example
 * ```typescript
 * import type { SharedReposLive } from "@beep/shared-infra"
 *
 * type MyLayer = SharedReposLive | OtherLayer
 * ```
 *
 * @category models
 * @since 0.1.0
 */
export type SharedReposLive = Layer.Layer<SharedRepos, never, Db.PgClientServices | SharedDb.SharedDb>;

/**
 * Combined layer providing all shared repository services.
 *
 * @example
 * ```typescript
 * import { SharedRepos } from "@beep/shared-infra"
 * import * as Effect from "effect/Effect"
 *
 * const program = Effect.gen(function* () {
 *   const fileRepo = yield* FileRepo
 *   const folderRepo = yield* FolderRepo
 *   // Use repositories...
 * }).pipe(Effect.provide(SharedRepos.layer))
 * ```
 *
 * @category layers
 * @since 0.1.0
 */
export const layer: SharedReposLive = Layer.mergeAll(FileRepo.Default, FolderRepo.Default);

/**
 * Re-exports File repository implementation.
 *
 * @example
 * ```typescript
 * import { FileRepo } from "@beep/shared-infra/repos/repositories"
 * ```
 *
 * @category exports
 * @since 0.1.0
 */
export * from "./File.repo.ts";

/**
 * Re-exports Folder repository implementation.
 *
 * @example
 * ```typescript
 * import { FolderRepo } from "@beep/shared-infra/repos/repositories"
 * ```
 *
 * @category exports
 * @since 0.1.0
 */
export * from "./Folder.repo.ts";
