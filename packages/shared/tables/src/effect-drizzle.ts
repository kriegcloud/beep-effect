...
export function createInsertSchema<TTable extends Drizzle.Table, TRefine extends TableRefine<TTable> = {}>(
  ...
  let schemaEntries: Record<string, S.Schema.All | S.PropertySignature.All> = Object.fromEntries(
  ...
  if (refine) {
    const refinedEntries = Object.entries(refine).map(([name, refineColumn]) => [
      name,
      typeof refineColumn === "function" && !S.isSchema(refineColumn) && !S.isPropertySignature(refineColumn)
