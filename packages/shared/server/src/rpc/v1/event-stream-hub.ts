  ...
  scoped: Effect.gen(function* () {
    ...
    const unregisterConnection = (userId: SharedEntityIds.UserId.Type, connectionId: string): Effect.Effect<void> =>
      SynchronizedRef.updateEffect(connections, (map) => {
        ...
        }).pipe(Effect.as(map), Effect.tap(F.constant(Effect.logDebug("Unregistered connection"))));
      });
    ...
    const notifyCurrentUser = (event: EventStreamEvents.Type): Effect.Effect<void, never, Policy.AuthContext> =>
