...
- Maintains clean separation between slice-specific SDKs (`@beep/iam-sdk`, `@beep/documents-sdk`) and shared client infrastructure.
...
**Shared RPC Client**:
```typescript
import { SharedClient } from "@beep/shared-sdk/client";
...
```typescript
import { useSharedQuery } from "@beep/shared-sdk/hooks";
...
- `bun run check --filter @beep/shared-sdk` — Type check
- `bun run lint --filter @beep/shared-sdk` — Biome lint
- `bun run test --filter @beep/shared-sdk` — Bun test suite
- `bun run build --filter @beep/shared-sdk` — Build ESM/CJS artifacts
