import { resolveMaybeUrlArg } from "@beep/shared-ui/utils";
...
describe("resolveMaybeUrlArg", () => {
  ...
  it("should add `window.location.origin` if the url is relative and request is clientside", () => {
    ...
    expect(resolveMaybeUrlArg("/foo/bar").href).toBe("http://example.com/foo/bar");
  ...
  it("should take VERCEL_URL if present and url is relative", () => {
    ...
    expect(resolveMaybeUrlArg("/foo/bar").href).toBe("https://example.com/foo/bar");

    delete process.env.VERCEL_URL;
  });

  it("assumes localhost if no VERCEL_URL and url is relative", () => {
    expect(resolveMaybeUrlArg("/foo/bar").href).toBe("http://localhost:3000/foo/bar");
  });

  it("should use `/api/uploadthing` pathname if no url is provided", () => {
    expect(resolveMaybeUrlArg("http://example.com").href).toBe("http://example.com/api/uploadthing");
  });
