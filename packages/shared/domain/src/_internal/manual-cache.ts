...
class ManualCacheImpl<in out Key, in out Value> implements ManualCache<Key, Value> {
  ...
  private trackAccess(key: MapKey<Key>): void {
    ...
    if (MutableRef.compareAndSet(this.cacheState.updating, false, true)) {
      try {
        let loop = true;
        while (loop) {
          const keyToUpdate = MutableQueue.poll(this.cacheState.accesses, MutableQueue.EmptyMutableQueue);
          if (keyToUpdate === MutableQueue.EmptyMutableQueue) {
            loop = false; // Queue is empty
          } else {
            const currentEntry = Option.getOrUndefined(MutableHashMap.get(this.cacheState.map, keyToUpdate.current));
  ...
  public invalidateAll = Effect.sync(() => {
    ...
    while (
      MutableQueue.poll(this.cacheState.accesses, MutableQueue.EmptyMutableQueue) !== MutableQueue.EmptyMutableQueue
    ) {
      // draining
    }
...
export const make = <Key, Value>(options: {
  ...
  return Effect.sync(() => new ManualCacheImpl<Key, Value>(capacity, timeToLive)).pipe(
    Effect.flatMap((cache) => {
      ...
      const periodicEviction = Effect.repeat(cache.evictExpired(), Schedule.fixed(evictionInterval));
