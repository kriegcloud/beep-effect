...
const $I = $SharedDomainId.create("entities/File/schemas/UploadKey");
...
export class ShardPrefixEncoded extends S.String.annotations(
  $I.annotations("ShardPrefixEncoded", {
    description:
    ...
    examples: A.make("a1", "f3", "00", "ff", "2b"),
  })
) {}
...
export class ShardPrefixDecoded extends S.NonEmptyTrimmedString.pipe(
  ...
  $I.annotations("ShardPrefixDecoded", {
    description:
...
export class ShardPrefix extends S.transformOrFail(ShardPrefixEncoded, ShardPrefixDecoded, {
  ...
  decode: (encoded, _, ast) =>
    F.pipe(
      S.decode(ShardPrefixDecoded)(encoded),
      Effect.mapError(
        () =>
  ...
  encode: (decoded, _, ast) =>
    F.pipe(
      S.decode(ShardPrefixEncoded)(decoded),
      Effect.mapError(
        () => new ParseResult.Type(ast, decoded, "Failed to encode shard prefix to string representation")
      )
    ),
}).annotations(
  $I.annotations("ShardPrefix", {
    description:
  ...
  static readonly fromFileId = (fileId: SharedEntityIds.FileId.Type) => {
    ...
    const hexString = F.pipe(Math.abs(hashValue).toString(16), Str.slice(0, 2), Str.padStart(2, "0"));
 ...
 * pattern defined by UploadKeyParts. It's the format that would be used
 ...
 * "/dev/tenants/a1/individual/org_123/user/user_456/avatar/2024/03/file_789.jpg"
 */
export class UploadKeyEncoded extends S.TemplateLiteral(...UploadKeyParts).annotations(
  $I.annotations("UploadKeyEncoded", {
    description:
...
export declare namespace UploadKeyEncoded {
  export type Type = S.Schema.Type<typeof UploadKeyEncoded>;
  export type Encoded = S.Schema.Encoded<typeof UploadKeyEncoded>;
}
 ...
 * It's used internally by the UploadKey transformation to handle the
 ...
 * const components = S.decode(UploadKeyParser)(
 *   "/dev/tenants/a1/individual/org_123/user/user_456/avatar/2024/03/file_789.jpg"
 * );
 *
 * // Construct a path from components
 * const pathString = S.encode(UploadKeyParser)([
 ...
 * ```
 */
export const UploadKeyParser = S.TemplateLiteralParser(...UploadKeyParts).annotations(
  $I.annotations("UploadKeyParser", {
    description:
...
export declare namespace UploadKeyParser {
  export type Type = S.Schema.Type<typeof UploadKeyParser>;
  export type Encoded = S.Schema.Encoded<typeof UploadKeyParser>;
}
 ...
 * ```
 */
export const UploadKeyDecoded = S.Struct({
  ...
  extension: FileExtension, // File extension (validated against allowed types)
}).annotations(
...
export declare namespace UploadKeyDecoded {
  export type Type = S.Schema.Type<typeof UploadKeyDecoded>;
  export type Encoded = S.Schema.Encoded<typeof UploadKeyDecoded>;
}
 ...
 *
 * **Encoding (Decode operation)**: UploadKeyDecoded -> UploadKeyEncoded
 ...
 *
 * **Decoding (Encode operation)**: UploadKeyEncoded -> UploadKeyDecoded
 ...
 * };
 *
 * const s3Path = yield* S.decode(UploadKey)(uploadData);
 * // Result: "/dev/tenants/a1/individual/org__456.../user/user_789/avatar/2024/03/file__123....jpg"
 *
 * // Decode: S3 path -> structured data
 * const parsedData = yield* S.encode(UploadKey)(s3Path);
 * // Result: Original uploadData structure
 * ```
 */
export class UploadKey extends S.transformOrFail(UploadKeyDecoded, UploadKeyEncoded, {
  strict: true,
  decode: (i, _, ast) =>
    F.pipe(
      DateTime.now,
      Effect.flatMap((now) =>
        F.pipe(
          S.encode(FileExtension)(i.extension),
          Effect.zip(S.decode(BS.MonthNumberFromMonthInt)(DateTime.getPartUtc(now, "month"))),
          Effect.map(([extension, month]) => ({
            shardPrefix: ShardPrefix.fromFileId(i.fileId),
            extension,
            month,
      ...
      Effect.flatMap(({ shardPrefix, extension, month, year }) =>
        S.encode(UploadKeyParser)([
          "/",
          i.env,
          "/",
          "tenants",
      ...
      Effect.flatMap(S.decode(UploadKeyEncoded)),
      Effect.mapError(F.constant(new ParseResult.Type(ast, i, "Failed to encode upload path from structured data")))
    ),
  encode: (i, _, ast) =>
    F.pipe(
      S.decode(UploadKeyParser)(i),
      Effect.flatMap((pathParts) =>
        F.pipe(
          Effect.all(
            {
              env: A.get(pathParts, 1),
              organizationType: A.get(pathParts, 7),
              organizationId: A.get(pathParts, 9),
              entityKind: A.get(pathParts, 11),
              entityIdentifier: A.get(pathParts, 13),
              entityAttribute: A.get(pathParts, 15),
              fileId: A.get(pathParts, 21),
              extension: A.get(pathParts, 23),
            },
            { concurrency: 8 }
          ),
          Effect.flatMap(S.decode(UploadKeyDecoded))
        )
      ),
      Effect.mapError(F.constant(new ParseResult.Type(ast, i, "Failed to decode upload path to structured data")))
    ),
}).annotations(
  $I.annotations("UploadKey", {
    description:
   ...
   * ```
   */
  static readonly shardPrefixFromFileId = ShardPrefix.fromFileId;
}

export declare namespace UploadKey {
  export type Type = S.Schema.Type<typeof UploadKey>;
  export type Encoded = S.Schema.Encoded<typeof UploadKey>;
}
