 ...
 */
export const fromString = (dateString: string): Effect.Effect<LocalDate, ParseResult.ParseError> =>
  Str.match(ISO_DATE_PATTERN)(dateString).pipe(
    O.flatMap(A.tail),
    O.map(A.map((str) => Number.parseInt(str, 10))),
    O.flatMap(([year, month, day]) =>
      O.all({
        year: O.fromNullable(year),
        month: O.fromNullable(month),
        day: O.fromNullable(day),
      })
    ),
    O.match({
      onNone: () =>
      ...
      onSome: S.decodeUnknown(LocalDate),
    })
  );
...
export const fromDate = (date: Date): LocalDate =>
  pipe(date, DateTime.unsafeFromDate, DateTime.toPartsUtc, (parts) =>
    LocalDate.make({ year: parts.year, month: parts.month + 1, day: parts.day })
  );
 ...
 */
export const today = (): LocalDate => DateTime.unsafeNow().pipe(DateTime.toDate, fromDate);
...
//   (millis) => DateTime.unsafeMake(Number(millis)).pipe(
//     DateTime.toDate,
//     fromDate
//   )
// );
export const todayEffect = pipe(
  Effect.clockWith((clock) => clock.currentTimeMillis),
  Effect.map((millis) => DateTime.unsafeMake(Number(millis)).pipe(DateTime.toDate, fromDate))
);
...
export const addDays = (date: LocalDate, days: number): LocalDate =>
  pipe(date.toDate(), DateTime.unsafeFromDate, DateTime.toUtc, DateTime.add({ days }), DateTime.toDate, fromDate);

/**
 * Add months to a LocalDate
 */
export const addMonths = (date: LocalDate, months: number): LocalDate =>
  pipe(date.toDate(), DateTime.unsafeFromDate, DateTime.toUtc, DateTime.add({ months }), DateTime.toDate, fromDate);

/**
 * Add years to a LocalDate
 */
export const addYears = (date: LocalDate, years: number): LocalDate =>
  pipe(date.toDate(), DateTime.unsafeFromDate, DateTime.toUtc, DateTime.add({ years }), DateTime.toDate, fromDate);
 ...
 * ```
 */
export const LocalDateFromString: S.Schema<LocalDate, string> = S.transformOrFail(
  ...
  LocalDate,
  {
    ...
    decode: (dateString, _, ast) =>
      Str.match(ISO_DATE_PATTERN)(dateString).pipe(
        ...
        O.match({
          onNone: () =>
            Effect.fail(
              new ParseResult.Type(ast, dateString, `Expected ISO date format YYYY-MM-DD, but received "${dateString}"`)
            ),
    ...
    encode: (localDate) => {
      // Format as ISO 8601 date string (YYYY-MM-DD)
      const y = Str.padStart(4, "0")(localDate.year.toString());
      const m = Str.padStart(2, "0")(localDate.month.toString());
      const d = Str.padStart(2, "0")(localDate.day.toString());
