...
const makeMockUser = (overrides?: undefined | Partial<{ readonly email: BS.Email.Type; readonly name: string }>) =>
  User.Model.jsonCreate.make({
    ...
    name: overrides?.name ?? "Page Test User",
  });
...
const makeMockPage = (
  ...
  S.decodeUnknownSync(Page.Model.jsonCreate)({
    organizationId,
    userId,
 ...
 * the IDs needed for page operations.
 */
const setupTestContext = Effect.gen(function* () {
  ...
  const { data: user } = yield* userRepo.insert(
    makeMockUser({ email: makeTestEmail("page-content-rich"), name: "ContentRich Test User" })
  );
...
describe("Page contentRich round-trip", () => {
  // ==========================================================================
  // Test 1: Insert and retrieve page with complex contentRich
  // ==========================================================================
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const pageInsert = makeMockPage(organizationId, userId, {
            title: "Complex ContentRich Test",
            contentRich,
          });

          const { data: inserted } = yield* pageRepo.insert(pageInsert);
          assertTrue(S.is(Page.Model)(inserted));

          const found = unwrapSome(yield* pageRepo.findById({ id: inserted.id }), "page findById");
  ...
  // ==========================================================================
  // Test 2: Insert page with null/undefined contentRich
  // ==========================================================================
  layer(PgTest, { timeout: Duration.seconds(120) })("null contentRich", (it) => {
    it.effect(
      "should handle page with omitted contentRich",
      () =>
        Effect.gen(function* () {
          const { userId, organizationId } = yield* setupTestContext;
          const pageRepo = yield* PageRepo;
  ...
  // ==========================================================================
  // Test 3: Update contentRich on existing page
  // ==========================================================================
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const { data: updated } = yield* pageRepo.update({
            ...inserted,
