import { DocEmbeddingsClient, EmbeddingsClient, LlmClient, PromptClient } from "@beep/graph-pipeline/Clients";
import { documentRagQuery } from "@beep/graph-pipeline/DocumentRag";
...
const SAMPLE_CHUNKS = [
  { chunkId: "chunk-1", score: 0.95, content: O.some("RLHF stands for Reinforcement Learning from Human Feedback.") },
  {
    chunkId: "chunk-2",
    score: 0.87,
    content: O.some("Constitutional AI uses a set of principles to guide model behavior."),
  },
  { chunkId: "chunk-3", score: 0.72, content: O.some("Transformers use self-attention mechanisms for sequence modeling.") },
];
  ...
  it.effect("fails with DocumentRagError when no chunks have content", () => {
    const EmptyDocEmbeddings = Layer.succeed(DocEmbeddingsClient)(
      DocEmbeddingsClient.of({
        find: () =>
          Effect.succeed({
            chunks: [{ chunkId: "empty-1", score: 0.5, content: O.none() }],
          }),
      })
    );
