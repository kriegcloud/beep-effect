...
const CacheLifeProfile = S.Struct({
...
const DevIndicatorsConfig = S.Union([
  S.Literal(false),
  S.Struct({
    position: optional(
...
const OnDemandEntriesConfig = S.Struct({
 ...
 * @since 0.0.0
 */
export const NextConfig = S.Struct({
 ...
 * @example
 * ```ts
 * import type { NextConfig } from "@beep/repo-configs/next"
 *
 ...
 * @since 0.0.0
 */
export type NextConfig = typeof NextConfig.Type;
 ...
 * @since 0.0.0
 */
export const defineNextConfig = (config: unknown): NextConfig =>
  Result.getOrThrowWith(decodeNextConfigResult(config), schemaIssueToError);
 ...
 * @since 0.0.0
 */
export const NextConfigBase = NextConfig;
 ...
 * @since 0.0.0
 */
export type NextConfigBase = NextConfig;
