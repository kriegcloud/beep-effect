...
const normalizeBareExtension: (extension: string) => string = flow(Str.replace(/^\./, ""), Str.toLowerCase);

const isVideoFileName = (path: Path.Path, name: string): boolean =>
  videoExtensionGuard(normalizeBareExtension(path.extname(name)));
...
const runExtractFrames = Effect.fn("Image.runExtractFrames")(function* (label: string, request: ExtractFramesRequest) {
  ...
  const result = yield* ffmpeg.extractFrames(request, makeExtractFramesEvents(terminal, label));
  return result;
});
...
const imageExtractFramesDirCommand = Command.make(
  ...
  Effect.fn("imageExtractFramesDirCommand")(function* ({ dir, fps, overwrite, prefix }) {
