...
import {
  Cause, Console, Effect, Exit, FileSystem, Match, Order, Path, pipe, Terminal,
} from "effect";
...
const videoFlag = Flag.file("video", {mustExist: true})
  .pipe(Flag.withDescription("Input video file to sample"));
const dirFlag = Flag.directory("dir", {mustExist: true})
  .pipe(Flag.withDescription("Directory containing direct video files to sample"));
const outDirFlag = Flag.directory("out-dir")
  .pipe(Flag.withDescription("Output directory for extracted PNG frames"));
const fpsFlag = Flag.float("fps")
  .pipe(Flag.withDescription(
...
const manifestFlag = Flag.path("manifest", {pathType: "file"}).pipe(
  Flag.withDescription(
    "Manifest output path; defaults to --out-dir/extract-frames-manifest.json"),
  Flag.optional,
);
const overwriteFlag = Flag.boolean("overwrite")
  .pipe(Flag.withDescription("Overwrite existing frame outputs and manifest"));
 ...
 * @since 0.0.0
 */
export class ImageCommandError extends TaggedErrorClass<ImageCommandError>($I`ImageCommandError`)("ImageCommandError",
  {
    message: S.String,
  },
...
const isVideoFileName: {
  ...
  2,
  (path: Path.Path, name: string): boolean => pipe(
    path.extname(name),
...
const renderProgressBar: {
  ...
  const filled = Math.max(
    0,
    Math.min(BAR_WIDTH, Math.round((event.percent / 100) * BAR_WIDTH)),
  );
  const empty = BAR_WIDTH - filled;
  const bar = `${pipe("#", Str.repeat(filled))}${pipe("-", Str.repeat(empty))}`;
  return `\r${label} [${bar}] ${event.frameCount} frame(s) ${event.percent.toFixed(
    1)}%`;
});
...
const renderExtractFramesEvent: {
  ...
  (
    label: string,
    event: FFmpegEvent,
  ): (terminal: Terminal.Terminal) => Effect.Effect<void, never>;
  ...
  return yield* Match.value(event).pipe(
    Match.discriminators("kind")({
      progress: (event) => terminal.display(renderProgressBar(label, event))
        .pipe(Effect.ignore),
      completed: (event) => terminal
        .display(`\r${label} [${pipe(
          "#",
          repeatBarWidth,
        )}] ${event.frameCount} frame(s) 100.0%\n`)
        .pipe(Effect.ignore),
    }),
    Match.orElse(() => terminal.display(`\r${label} [${pipe(
      "-",
      repeatBarWidth,
    )}] 0 frame(s) 0.0%`).pipe(Effect.ignore)),
  );
}));

const printImageIndex = () => Console.log(
  "image commands: extract-frames, extract-frames-dir");
...
}): ExtractFramesRequest => new ExtractFramesRequest({
  fps: options.fps,
...
const extractFramesSummary = (
  frameCount: number,
  outDir: string,
  manifestPath: string,
): string => `wrote ${frameCount} frame(s) to ${outDir}. manifest: ${manifestPath}`;

const makeExtractFramesEvents = (
  terminal: Terminal.Terminal,
  label: string,
): ((event: FFmpegEvent) => Effect.Effect<void, never>) | undefined => process.stdout.isTTY === true
  ? (event: FFmpegEvent) => renderExtractFramesEvent(terminal, label, event)
  : undefined;

const runExtractFrames = Effect.fn("Image.runExtractFrames")(
  function* (label: string, request: ExtractFramesRequest) {
    const ffmpeg = yield* FFmpeg;
    const terminal = yield* Terminal.Terminal;
    return yield *ffmpeg.extractFrames(
      request,
      makeExtractFramesEvents(terminal, label),
    );
  });
...
const collectExtractFramesDirVideos = Effect.fn(
  "Image.collectExtractFramesDirVideos")(
  function* (dir: string): Effect.fn.Return<ReadonlyArray<ExtractFramesDirVideo>, ImageCommandError, FileSystem.FileSystem | Path.Path> {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const directory = path.resolve(dir);
    const entries = yield* fs.readDirectory(directory)
      .pipe(Effect.mapError(() => new ImageCommandError({
    ...
    for (const entry of entries) {
      ...
      if (O.isNone(stat) || stat.value.type !== "File" || !isVideoFileName(
        path,
        entry,
      )) {
        continue;
      ...
      videos = A.append(videos, new ExtractFramesDirVideo({
    ...
    return A.sort(
      ...
      Order.mapInput(
        Str.orderAsc,
        (video: ExtractFramesDirVideo) => video.sourceName,
      ),
    );
  });

const preflightExtractFramesDirVideos = Effect.fn(
  "Image.preflightExtractFramesDirVideos")(
  function* (videos: ReadonlyArray<ExtractFramesDirVideo>): Effect.fn.Return<void, ImageCommandError> {
...
const runExtractFramesDir = Effect.fn("Image.runExtractFramesDir")(
  function* (options: {
    readonly dir: string;
    readonly fps: number;
    readonly overwrite: boolean;
    ...
    for (const video of videos) {
      ...
      const exit = yield* Effect.exit(runExtractFrames(
        `image extract-frames-dir ${video.sourceName}`,
        request,
      ));

      if (Exit.isSuccess(exit)) {
        completedCount += 1;
        yield* Console.log(`image extract-frames-dir: ${video.sourceName}: ${extractFramesSummary(
        ...
        failed = A.append(failed, new ExtractFramesDirFailure({
          message,
    ...
    yield* Console.log(`image extract-frames-dir: processed ${A.length(videos)} video(s); succeeded ${completedCount}; failed ${A.length(
      failed)}.`);
...
const imageExtractFramesCommand = Command.make("extract-frames", {
  fps: fpsFlag,
  ...
  prefix: prefixFlag,
  video: videoFlag,
}, Effect.fn("imageExtractFramesCommand")(function* ({
  fps,
  manifest,
  outDir,
  overwrite,
  ...
  const result = yield* runExtractFrames(
  ...
  yield* Console.log(`image extract-frames: ${extractFramesSummary(
    result.frameCount,
    result.outDir,
    result.manifestPath,
  )}`);
...
const imageExtractFramesDirCommand = Command.make("extract-frames-dir", {
  dir: dirFlag,
  fps: fpsFlag,
  overwrite: overwriteFlag,
  prefix: prefixFlag,
}, Effect.fn("imageExtractFramesDirCommand")(function* ({
  dir,
  fps,
  overwrite,
  prefix,
}) {
  return yield* runExtractFramesDir({
    dir,
    fps,
    overwrite,
    prefix,
  ...
  Command.withDescription(
    "Extract PNG frames from every direct video file in a directory"),
  Command.provide(FFmpeg.makeLayer()),
);
...
export const imageCommand = Command.make("image", {}, printImageIndex).pipe(
  ...
  Command.withSubcommands([
    imageExtractFramesCommand,
    imageExtractFramesDirCommand,
  ]),
);
