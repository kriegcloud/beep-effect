...
const fpsFlag = Flag.float("fps")
	.pipe(Flag.withDescription("Positive frame extraction rate in frames per second"));
const prefixFlag = Flag.string("prefix")
	.pipe(Flag.withDescription("Generated frame prefix; defaults to <video-stem>_frame"), Flag.optional);
const manifestFlag = Flag.path("manifest", {pathType: "file"})
	.pipe(
...
const isVideoFileName: {
	...
	2,
	(path: Path.Path, name: string): boolean => pipe(path.extname(name), normalizeBareExtension, videoExtensionGuard),
)
...
const renderExtractFramesEvent: {
	...
	Effect.fnUntraced(function* (
		...
		return yield* Match.value(event).pipe(Match.discriminators("kind")({
			...
			completed: (event) => terminal
				...
				.pipe(Effect.ignore),
		}), Match.orElse(() => terminal.display(`\r${label} [${pipe("-", repeatBarWidth)}] 0 frame(s) 0.0%`)
			.pipe(Effect.ignore)));
	}),
);
...
const collectExtractFramesDirVideos = Effect.fn("Image.collectExtractFramesDirVideos")(
	function* (dir: string): Effect.fn.Return<ReadonlyArray<ExtractFramesDirVideo>, ImageCommandError, FileSystem.FileSystem | Path.Path> {
		...
		return A.sort(videos, Order.mapInput(Str.orderAsc, (video: ExtractFramesDirVideo) => video.sourceName));
	});

const preflightExtractFramesDirVideos = Effect.fn("Image.preflightExtractFramesDirVideos")(
...
const imageExtractFramesCommand = Command.make("extract-frames", {
	...
	prefix,
	video,
}) => runExtractFrames("image extract-frames", makeExtractFramesRequest({
	fps,
	manifest,
	outDir,
	overwrite,
	...
	.pipe(Effect.tap((result) => Console.log(`image extract-frames: ${extractFramesSummary(
		result.frameCount,
		result.outDir,
		result.manifestPath,
	)}`))))
 ...
 * @since 0.0.0
 */
export const imageCommand = Command.make("image", {}, printImageIndex)
	.pipe(Command.withDescription("Image and video curation commands"), Command.withSubcommands([
