...
const appendTurboSummaryCommand = Command.make(
  ...
  ({ summaryPath }) =>
    appendTurboSummary(summaryPath).pipe(
      Effect.catchTag(
        "CiCommandError",
        Effect.fn(function* (error) {
          process.exitCode = 1;
          yield* Console.error(`[ci] ${error.message}`);
        })
      )
    )
...
export const ciCommand = Command.make(
  ...
  {},
  Effect.fn(function* () {
    yield* Console.log("CI commands:");
    yield* Console.log("- bun run beep ci append-turbo-summary");
  })
).pipe(
