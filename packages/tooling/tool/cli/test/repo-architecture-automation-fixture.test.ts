...
describe("repo architecture automation golden fixture", () => {
  ...
  it("keeps repository errors server-only and use-case errors public", () => {
    const useCasesPublic = readFileSync(
      join(repoRoot, "packages/fixture-lab/specimen/use-cases/src/public.ts"),
      "utf8"
    );
    const useCasesServer = readFileSync(
      join(repoRoot, "packages/fixture-lab/specimen/use-cases/src/server.ts"),
      "utf8"
    );
    const useCaseService = readFileSync(
      join(repoRoot, "packages/fixture-lab/specimen/use-cases/src/entities/Specimen/Specimen.service.ts"),
      "utf8"
    );
    const clientViolations = sourceFilesUnder("packages/fixture-lab/specimen/client/src")
      .filter((filePath) => readFileSync(filePath, "utf8").includes("SpecimenRepositoryNotFound"))
      .map((filePath) => relative(repoRoot, filePath));

    expect(useCasesPublic).toContain("SpecimenNotFound");
    expect(useCasesPublic).not.toContain("SpecimenRepository");
    expect(useCasesServer).toContain("SpecimenRepositoryNotFound");
    expect(useCaseService).toContain("toSpecimenActionError");
  ...
  it("keeps fixture packages synthetic and out of product code", () => {
    const searchedRoots = ["apps", "packages/_internal", "packages/foundation", "packages/tooling"] as const;
    const files = searchedRoots.flatMap((root) => sourceFilesUnder(root));
    const violations = files.flatMap((filePath) => {
      const relativePath = relative(repoRoot, filePath);

      if (
        relativePath.startsWith("packages/tooling/tool/cli/test/fixtures/") ||
        relativePath === "packages/tooling/tool/cli/test/repo-architecture-automation-fixture.test.ts"
      ) {
        return [];
      }
