...
- **Vertical Slice**: UI layer for the documents slice, consuming `@beep/workspaces-domain` and `@beep/workspaces-client`
...
- **Path Alias**: Import as `@beep/workspaces-ui`
...
**Note**: `@beep/ui-core`, `@beep/workspaces-domain`, and `@beep/workspaces-client` are available packages but not yet added as dependencies. They will be included when UI components are implemented.
...
```typescript
import { DocumentEditor } from "@beep/workspaces-ui";
...
```typescript
import { KnowledgePageViewer } from "@beep/workspaces-ui";
...
```typescript
import { DiscussionThread } from "@beep/workspaces-ui";
...
```typescript
import { FileUploader } from "@beep/workspaces-ui";
...
```typescript
import { useDocument } from "@beep/workspaces-ui";
import * as Effect from "effect/Effect";
...
- **Domain Schemas**: Validate all external data with `@beep/workspaces-domain` schemas
...
import * as Effect from "effect/Effect";
import { DocumentSchema } from "@beep/workspaces-domain";
...
- **Business Logic**: Keep domain rules in `@beep/workspaces-domain`
- **Data Fetching Implementation**: Use `@beep/workspaces-client` contracts; inject as props
- **Database Queries**: Belong in `@beep/workspaces-server`
...
```bash
# Type check
bun run --filter @beep/workspaces-ui check

# Lint
bun run --filter @beep/workspaces-ui lint

# Lint and auto-fix
bun run --filter @beep/workspaces-ui lint:fix

# Run tests
bun run --filter @beep/workspaces-ui test

# Test with coverage
bun run --filter @beep/workspaces-ui coverage

# Build
bun run --filter @beep/workspaces-ui build
```
...
- [ ] Validation uses `@beep/workspaces-domain` schemas
- [ ] Data fetching is injected via props or uses `@beep/workspaces-client`
...
- [ ] Tests added under `packages/workspaces/ui/test/`
- [ ] Documentation includes usage examples

## See Also

- `packages/workspaces/ui/AGENTS.md` — Implementation guardrails and integration notes
...
- `packages/workspaces/domain/AGENTS.md` — Domain entities and validation
- `packages/workspaces/client/AGENTS.md` — Client CLIENT contracts
