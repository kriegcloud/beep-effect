...
Implementation will follow the `@beep/iam-client` pattern once `@beep/workspaces-server` infrastructure is finalized.
...
"@beep/workspaces-client": "workspace:*"
```
...
1. **Domain Contracts**: Finalize schemas and error types in `@beep/workspaces-domain`
2. **Server Infrastructure**: Complete server-side routes and handlers in `@beep/workspaces-server`
...
```typescript
"use client";
import { useDocuments } from "@beep/workspaces-client";
...
import { DocumentService } from "@beep/workspaces-client";
...
```bash
# Type check
bun run --filter @beep/workspaces-client check

# Lint and format
bun run --filter @beep/workspaces-client lint
bun run --filter @beep/workspaces-client lint:fix

# Build (ESM + CJS with annotations)
bun run --filter @beep/workspaces-client build

# Test
bun run --filter @beep/workspaces-client test
bun run --filter @beep/workspaces-client coverage

# Circular dependency check
bun run --filter @beep/workspaces-client lint:circular
```
...
| `@beep/workspaces-domain` | Entity models, value objects, domain errors |
| `@beep/workspaces-server` | Server infrastructure (peer dependency) |
...
- Server infrastructure (`@beep/workspaces-server`)
- UI components (`@beep/workspaces-ui`)
- Business logic (`@beep/workspaces-domain`)
- Database schemas (`@beep/workspaces-tables`)

## Related Packages

- **`@beep/iam-client`** - Reference implementation for SDK layer patterns
- **`@beep/workspaces-domain`** - Entity models, value objects, domain errors
- **`@beep/workspaces-server`** - Server-side infrastructure
- **`@beep/workspaces-ui`** - React UI components
