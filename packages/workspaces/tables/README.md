...
Drizzle schema package for the documents slice. Houses PostgreSQL table definitions and relations that back `@beep/workspaces-server` repositories and power the document management system.

## Purpose

Provides type-safe Drizzle table schemas for the documents feature slice, bridging domain models from `@beep/workspaces-domain` to PostgreSQL storage. All tables follow multi-tenant patterns from `@beep/shared-tables` and integrate with Effect-based repositories in `@beep/workspaces-server`.
...
"@beep/workspaces-tables": "workspace:*"
```
...
**Note**: Knowledge base tables (`knowledgeSpace`, `knowledgePage`, `knowledgeBlock`, `pageLink`) exist as domain models in `@beep/workspaces-domain` but are not yet implemented at the schema layer.
...
- **Domain alignment**: Enums generated from `@beep/workspaces-domain` value objects via `BS.toPgEnum`
...
import { document, documentVersion } from "@beep/workspaces-tables/schema";
...
import { document, documentRelations } from "@beep/workspaces-tables";
```
...
```bash
# Type check
bun run --filter @beep/workspaces-tables check

# Lint
bun run --filter @beep/workspaces-tables lint

# Auto-fix lint issues
bun run --filter @beep/workspaces-tables lint:fix

# Run tests
bun run --filter @beep/workspaces-tables test

# Check for circular dependencies
bun run --filter @beep/workspaces-tables lint:circular
```
...
- Align columns with `@beep/workspaces-domain` entities and value objects
...
| `@beep/workspaces-domain` | Value objects and domain enums (`TextStyle`) |
...
- **Consumed by**: `@beep/workspaces-server` (repositories, database adapters)
- **Depends on**: `@beep/workspaces-domain` (entity models), `@beep/shared-tables` (table factories)
...
- Consumed by `@beep/workspaces-server` repositories via the `DocumentsDb` service
