...
const editorFileSystemLayer = Layer.mergeAll(
  BunFileSystem.layer,
  BunPath.layer
);
...
const makeBootstrap = (
  ...
  status: typeof SidecarHealthStatus.Type
) =>
...
const hasMessage = (input: unknown): input is {
  readonly message: string
} =>
  P.isObject(input) && P.hasProperty(
    input,
    "message"
  ) && P.isString(input.message);
...
type EditorWorkspaceStore = {
  ...
  readonly savePage: (
    slug: string,
    page: PageDocument
  ) => Effect.Effect<EditorPageResource, EditorRuntimeError>;
  readonly exportPage: (
    slug: string,
    format: ExportFormat
  ) => Effect.Effect<PageExport, EditorRuntimeError>;
  readonly searchPages: (query: string) => Effect.Effect<ReadonlyArray<PageSummary>, EditorRuntimeError>;
};

const makeEditorWorkspaceStore = Effect.fn("EditorRuntime.makeWorkspaceStore")(
  function* (
    config: EditorRuntimeConfig,
    startedAt: DateTime.Utc
  ) {
    ...
    const revisionsDirectory = path.resolve(
      workspaceRoot,
      "revisions"
    );

    const readPageFile = Effect.fn("EditorRuntime.readPageFile")(
      function* (filePath: string) {
        const raw = yield* fs
          .readFileString(filePath)
          .pipe(Effect.mapError((cause) => toRuntimeError(
            `Failed to read "${filePath}".`,
            500,
            cause
          )));

        return yield* decodePageDocumentJson(raw)
          .pipe(
            Effect.mapError((cause) => toRuntimeError(
    ...
    const writeManifest = Effect.fn("EditorRuntime.writeManifest")(
      function* (manifest: WorkspaceManifest) {
        yield* fs
          .writeFileString(
            manifestPath,
            encodeWorkspaceManifestJson(manifest)
          )
          .pipe(Effect.mapError((cause) => toRuntimeError(
            `Failed to write "${manifestPath}".`,
            500,
            cause
          )));
      });

    const writePage = Effect.fn("EditorRuntime.writePage")(
      function* (page: PageDocument) {
        yield* fs
          .writeFileString(
            ...
            encodePageDocumentJson(page)
          )
          .pipe(Effect.mapError((cause) => toRuntimeError(
            `Failed to persist page "${page.slug}".`,
            500,
            cause
          )));
      });

    const writeRevision = Effect.fn("EditorRuntime.writeRevision")(
      function* (
        page: PageDocument,
        savedAt: DateTime.Utc,
        reason: string
      ) {
        const revision = makeRevisionRecord(
          page,
          savedAt,
          reason
        );
        yield* fs
          .writeFileString(
            ...
            encodeRevisionRecordJson(revision)
          )
          .pipe(Effect.mapError((cause) => toRuntimeError(
            `Failed to persist revision for "${page.slug}".`,
            500,
            cause
          )));
      });

    const ensureWorkspaceScaffold = Effect.fn("EditorRuntime.ensureWorkspaceScaffold")(
      function* () {
        yield* fs
          .makeDirectory(
            pagesDirectory,
          ...
          .pipe(Effect.mapError((cause) => toRuntimeError(
            `Failed to create "${pagesDirectory}".`,
            500,
            cause
          )));
        yield* fs
          ...
          .pipe(Effect.mapError((cause) => toRuntimeError(
            `Failed to create "${revisionsDirectory}".`,
            500,
            cause
          )));
    ...
    const readManifest = Effect.fn("EditorRuntime.readManifest")(
      function* () {
        ...
        return yield* decodeWorkspaceManifestJson(raw)
          .pipe(
            Effect.mapError((cause) => toRuntimeError(
              `Failed to decode "${manifestPath}".`,
              500,
              cause
            ))
          );
    ...
    const listAllPages = Effect.fn("EditorRuntime.listAllPages")(
      function* () {
        yield* ensureWorkspaceScaffold();
        const entries = yield* fs
          .readDirectory(pagesDirectory)
          .pipe(Effect.mapError((cause) => toRuntimeError(
            `Failed to read "${pagesDirectory}".`,
            500,
            cause
          )));
        ...
        const jsonEntries = pipe(
          entries,
          A.filter(Str.endsWith(".json"))
        );

        return yield* Effect.forEach(
          jsonEntries,
          (entry) => readPageFile(path.resolve(
            pagesDirectory,
            entry
          )),
          {
    ...
    const readPageBySlug = Effect.fn("EditorRuntime.readPageBySlug")(
      function* (slug: string) {
        ...
        if (!fileExists) {
          return yield* toRuntimeError(
            `Page "${normalizedSlug}" was not found.`,
            404
          );
        }
    ...
    const pagesLinkingToSlug = (
      pages: ReadonlyArray<PageDocument>,
      slug: string
      ...
      pipe(
        ...
        A.filter((page) => pipe(
            page.outboundLinks,
            A.some(P.Struct({targetSlug: Eq.equals(slug)}))
          )
        )
      );

    const backlinkCountForPages = (
      pages: ReadonlyArray<PageDocument>,
      slug: string
    ): number =>
      pipe(
    ...
    const listPageSummaries = Effect.fn("EditorRuntime.listPageSummaries")(
      function* () {
    ...
    const getBacklinks = Effect.fn("EditorRuntime.getBacklinks")(
      function* (slug: string) {
        const pages = yield* listAllPages();
        ...
        return pipe(
          ...
          A.map(summaryForPage(
            pages
          ))
        );
      });

    const readExistingPage = Effect.fn("EditorRuntime.readExistingPage")(
      function* (slug: string) {
        return yield* readPageBySlug(slug)
          .pipe(
            Effect.map(O.some),
            Effect.catchTag(
              "EditorRuntimeError",
    ...
    return {
      bootstrap: Effect.succeed(makeBootstrap(
        config,
        startedAt,
        "healthy"
      )),
        ...
        Effect.gen(function* () {
          ...
          const nextManifest = new WorkspaceManifest({
            ...
            rootPageSlug: pipe(
              ...
              O.orElse(thunkSome(nextPage.slug))
            ),
            updatedAt: now,
          });
      ...
      searchPages: (query) =>
        Effect.gen(function* () {
          ...
          return pipe(
            pages,
            A.filter((page) => {
              const titleMatches = pipe(
                page.title,
                Str.toLowerCase,
                Str.includes(trimmedQuery)
              );
              const contentMatches = pipe(
...
const emitBootstrapStdoutLine = Effect.fn("EditorRuntime.emitBootstrapStdoutLine")(
  function* (
    config: EditorRuntimeConfig,
    startedAt: DateTime.Utc
  ) {
    ...
    const encoded = yield* encodeBootstrapStdoutJson(
      ...
      .pipe(Effect.mapError((cause) => toRuntimeError(
        "Failed to encode the editor bootstrap line.",
        500,
        cause
      )));
...
const controlPlaneHandlersLayer = (
  config: EditorRuntimeConfig,
  startedAt: DateTime.Utc
) =>
  Layer.unwrap(
    Effect.gen(function* () {
      const store = yield* makeEditorWorkspaceStore(
        config,
        startedAt
      );
      ...
      return Layer.mergeAll(
        HttpApiBuilder.group(
          ...
          (handlers) =>
            handlers.handle(
              "health",
              () => handleInternalControlPlane(store.bootstrap)
            )
        ),
        HttpApiBuilder.group(
          EditorControlPlaneApi,
          ...
          (handlers) =>
            handlers.handle(
              "getWorkspace",
              () => handleInternalControlPlane(store.getWorkspace)
            )
        ),
        HttpApiBuilder.group(
          EditorControlPlaneApi,
          ...
          (handlers) =>
            handlers
              .handle(
                "listPages",
                () => handleInternalControlPlane(store.listPages)
              )
              .handle(
                "searchPages",
                ({query}) => handleInternalControlPlane(store.searchPages(query.query))
              )
              .handle(
                "getPage",
                ({params}) => handleResourceControlPlane(store.getPage(params.slug))
              )
              .handle(
                "savePage",
                ({
                  ...
                  handleResourceControlPlane(store.savePage(
                    params.slug,
                    payload
                  ))
              )
              .handle(
                "exportPage",
                ({params}) =>
                  handleResourceControlPlane(store.exportPage(
                    params.slug,
                    params.format
                  ))
              )
...
const sidecarLayer = (
  ...
  const apiLayer = HttpApiBuilder.layer(EditorControlPlaneApi)
    .pipe(
      Layer.provide(controlPlaneHandlersLayer(
        config,
        startedAt
      ))
    );
...
const launchEditorSidecar = (
  config: EditorRuntimeConfig,
  startedAt: DateTime.Utc
) =>
  Layer.launch(Layer.fresh(sidecarLayer(
    config,
    startedAt
  )))
    .pipe(
      Effect.mapError((cause) => toRuntimeError(
        "Failed to launch the editor sidecar.",
        500,
        cause
      ))
    );
 ...
 */
export const runEditorRuntime = Effect.fn("EditorRuntime.run")(
  function* (config: EditorRuntimeConfig) {
    const startedAt = yield* DateTime.now;
    const shutdownRequested = yield* Ref.make(false);
    const shutdownDeferred = yield* Deferred.make<void>();

    const requestShutdown = Effect.fn("EditorRuntime.requestShutdown")(
      function* () {
        const alreadyRequested = yield* Ref.getAndSet(
          shutdownRequested,
          true
        );
    ...
    const serverFiber = yield* launchEditorSidecar(
      config,
      startedAt
    )
      .pipe(Effect.forkScoped);

    yield* emitBootstrapStdoutLine(
      config,
      startedAt
    );
    yield* Effect.raceFirst(
      Deferred.await(shutdownDeferred),
      Fiber.await(serverFiber)
        .pipe(Effect.asVoid)
    );
 ...
 */
export const loadEditorRuntimeConfig = Effect.fn("EditorRuntime.loadConfig")(
  function* () {
    const path = yield* Path.Path;
    const host = yield* Config.string("BEEP_EDITOR_HOST")
      .pipe(Config.withDefault(defaultHost));
    const port = yield* Config.number("BEEP_EDITOR_PORT")
      .pipe(
    ...
    const version = yield* Config.string("BEEP_EDITOR_VERSION")
      .pipe(Config.withDefault(defaultVersion));
    const appDataDir = yield* O.match(
      appDataDirInput,
      {
        onNone: () => Effect.succeed(path.resolve(
          process.cwd(),
          defaultAppDataDir
        )),
