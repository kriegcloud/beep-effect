...
const LAYER_PATTERN =
  /(export\s+)?const\s+(\w+)(?:\s*:\s*Layer\.Layer<[\s\S]*?>)?\s*=\s*Layer\.(scoped|effect|succeed|sync)\s*\(\s*([\w.]+)\s*,/gm;
...
export const extractServicesFromContent = (content: string, filePath: string): ReadonlyArray<ServiceDefinition> => {
  const results = A.empty<ServiceDefinition>();
...
export const extractLayerMatches = (content: string): ReadonlyArray<LayerMatch> => {
  const results = A.empty<LayerMatch>();
...
const extractDepsFromType = (type: ts.Type, checker: ts.TypeChecker): ReadonlyArray<string> => {
  ...
  const processType = (t: ts.Type): void => {
    ...
    if ((symbolName === "Identifier" || Str.startsWith("Id<")(typeString)) && isTypeReference) {
...
export const createTsProgram = (filePaths: ReadonlyArray<string>): ts.Program => {
...
) => Effect.Effect<ReadonlyArray<ServiceDefinition>, PlatformError, FileSystem.FileSystem> = Effect.fn(
  function* (files) {
    const fs = yield* FileSystem.FileSystem;

    const processFile = (filePath: string) =>
      fs.readFileString(filePath).pipe(Effect.map((content) => extractServicesFromContent(content, filePath)));
...
export const findLayerDefinitions: (
  files: ReadonlyArray<string>
) => Effect.Effect<ReadonlyArray<LayerDefinition>, PlatformError, FileSystem.FileSystem> = Effect.fn(function* (files) {
  const fs = yield* FileSystem.FileSystem;

  const program = createTsProgram(files);

  const processFile = (filePath: string) =>
    fs.readFileString(filePath).pipe(Effect.map((content) => extractLayersFromContent(content, filePath, program)));

  const results = yield* Effect.forEach(files, processFile);
  return A.flatten(results);
});

export const buildArchitectureGraph = Effect.fn(function* (srcPath: string) {
...
export const formatMermaid = (graph: ArchitectureGraph): string => {
  const serviceLines = A.map(
    graph.services,
    (service) => `  ${service.name}["${service.name}<br/><i>${makeShortPath(service.path)}</i>"]`
  );

  const dependencyLines = pipe(
    graph.layers,
    A.flatMap((layer) => A.map(layer.dependencies, (dep) => `  ${layer.serviceName} --> ${dep}`))
  );

  const vms = pipe(
    graph.services,
    A.filter((s) => Str.endsWith("VM")(s.name)),
    A.map((s) => s.name)
  );
...
const groupServicesByType = (graph: ArchitectureGraph, analysisGraph: AnalysisGraph): GroupedServices => {
  ...
  const root = A.empty<ServiceDefinition>();
  const intermediate = A.empty<ServiceDefinition>();
  const leaf = A.empty<ServiceDefinition>();
  const vm = A.empty<ServiceDefinition>();
  ...
  graph.services.forEach((service) => {
    ...
    if (Str.endsWith("VM")(service.name)) {
      vm.push(service);
      return;
    }
...
export const formatHuman = (graph: ArchitectureGraph): string => {
  ...
  const rootNames = A.join(A.map(grouped.root, getName), ", ");
...
const detectCycles = (analysisGraph: AnalysisGraph): ReadonlyArray<ReadonlyArray<string>> => {
  ...
  return pipe(
    sccs,
    A.filter((component) => component.length > 1),
    A.map((component) =>
      A.map(component, (nodeIndex) => {
...
const getName = <T extends { readonly name: string }>(obj: T) => Struct.get(obj, "name");
const checkInvariants = (graph: ArchitectureGraph, analysisGraph: AnalysisGraph): ReadonlyArray<Violation> => {
  const violations = A.empty<Violation>();
  const layerByService = buildLayerMap(graph.layers);
  const grouped = groupServicesByType(graph, analysisGraph);
...
export const computeCommonAncestors = (
  ...
  if (depSetArray.length === 0) {
    return {
      ...
      commonDependencies: A.empty(),
      rootCauseCandidates: A.empty(),
    };
  }
...
const detectCutVerticesAndDomains = (analysisGraph: AnalysisGraph): DomainAnalysis => {
  const nodeCount = Graph.nodeCount(analysisGraph.graph);
  if (nodeCount === 0) return { cutVertices: A.empty(), domains: A.empty() };
...
const computeWarnings = (graph: ArchitectureGraph, analysisGraph: AnalysisGraph): Warnings => {
  ...
  const wide = A.empty<WideWarning>();
  graph.layers.forEach((layer) => {
...
const renderNodeClassification = (graph: ArchitectureGraph, analysisGraph: AnalysisGraph): string => {
  ...
  sections.push(
    `  <intermediate n="${intermediateCount}">${grouped.intermediate.map((s) => s.name).join(", ")}</intermediate>`
  );
...
const renderEdges = (graph: ArchitectureGraph, analysisGraph: AnalysisGraph): string => {
  ...
  const edgeLines = ordered.map((item) => {
    const deps = item.layer?.dependencies ?? A.empty();

    const errorTypes = item.layer?.errorTypes ?? A.empty();
...
const renderWarnings = (graph: ArchitectureGraph, analysisGraph: AnalysisGraph): string => {
  ...
  const redundantDesc =
    "Direct dependency exists alongside indirect path. May be intentional (performance) or accidental (refactor remnant).";
  ...
  const wideDesc =
    "Service with many dependencies (≥5). Complex integration point. For VMs: acceptable. For services: may violate SRP.";
  ...
  const parametrizedDesc =
    "Layers created by factory functions. May indicate runtime configuration or non-static dependencies.";
  if (parametrizedLayers.length > 0) {
    sections.push(
      `  <parametrized_layers n="${parametrizedLayers.length}" severity="moderate" description="${parametrizedDesc}">`
    );
...
const renderViolations = (graph: ArchitectureGraph, analysisGraph: AnalysisGraph): string => {
  const violations = checkInvariants(graph, analysisGraph);
...
const renderMetrics = (metrics: GraphMetrics): string => {
  ...
  const densityStatus = metrics.density < 0.2 ? "good" : metrics.density < 0.4 ? "moderate" : "bad";

  const densityMessage =
    metrics.density < 0.2
      ? "Sparse architecture with loose coupling (below 0.2 threshold)"
      : metrics.density < 0.4
        ? "Moderate coupling (0.2-0.4 range)"
        : "Dense architecture with tight coupling (above 0.4 threshold)";

  sections.push(
    `  <density value="${metrics.density.toFixed(3)}" status="${densityStatus}" description="${densityMessage}">${densityMessage}</density>`
  );

  const diameterStatus = metrics.diameter <= 3 ? "good" : metrics.diameter <= 5 ? "moderate" : "bad";

  const diameterMessage =
    metrics.diameter <= 3
      ? "Shallow dependency chains (≤3 hops)"
      : metrics.diameter <= 5
        ? "Moderate depth (4-5 hops)"
        : "Deep dependency chains (>5 hops)";

  sections.push(
    `  <diameter value="${metrics.diameter}" status="${diameterStatus}" description="${diameterMessage}">${diameterMessage}</diameter>`
  );

  const avgDegreeStatus = metrics.averageDegree < 2 ? "good" : metrics.averageDegree < 4 ? "moderate" : "bad";

  const avgDegreeMessage =
    metrics.averageDegree < 2
      ? "Minimal coupling (< 2 connections per service)"
      : metrics.averageDegree < 4
        ? "Moderate connectivity (2-4 connections per service)"
        : "High connectivity (≥4 connections per service)";

  sections.push(
    `  <average_degree value="${metrics.averageDegree.toFixed(2)}" status="${avgDegreeStatus}" description="${avgDegreeMessage}">${avgDegreeMessage}</average_degree>`
  );
...
const renderCommandsSummary = (expanded: boolean): string => {
  ...
  if (expanded) {
    ...
    lines.push(
      "      <description>Shows downstream impact: all services that depend on this (full transitive closure)</description>"
    );
    ...
    lines.push(
      "      <output>Risk level (HIGH/MEDIUM/LOW), all affected services grouped by depth (unlimited depth)</output>"
    );
    lines.push("    </command>");
    ...
    lines.push(
      "      <when_to_use>Understanding module boundaries, planning package splits or microservices</when_to_use>"
    );
    ...
    lines.push(
      "      <when_to_use>Identifying critical infrastructure needing stability and comprehensive testing</when_to_use>"
    );
    ...
    lines.push(
      "    <available>blast-radius, ancestors, common-ancestors, metrics, domains, hot-services, format</available>"
    );
    lines.push("  </available_commands>");
  }
...
export const formatAgentWithHints = (graph: ArchitectureGraph, options: RenderOptions): string => {
  ...
  if (options.showMetrics) {
    ...
    output.push(
      "  <description>Graph metrics include density (coupling), diameter (chain length), and average degree (connections per service)</description>"
    );
  ...
  if (options.showAdvanced) {
    ...
    output.push(
      "  <description>Advanced metrics identify god services, tight clusters, and architectural smells</description>"
    );
  ...
  if (options.showDomains) {
    ...
    output.push(
      "  <description>Domain analysis uses cut vertex detection to identify natural module boundaries and services that bridge separate domains</description>"
    );
  ...
  if (options.showWarnings) {
    ...
    const warnings = computeWarnings(graph, analysisGraph);
    const warningCount = (warnings.redundant?.length ?? 0) + (warnings.hot?.length ?? 0) + (warnings.wide?.length ?? 0);
...
export const formatAdjacencyList = (graph: ArchitectureGraph, showErrors: boolean): string => {
  const analysisGraph = buildAnalysisGraph(graph);
  const ordered = orderServicesByDependencyCount(graph, analysisGraph);
...
export const generateStats = (graph: ArchitectureGraph): string => {
  ...
  const complexLines = stats.complexLayers.map((l) => `- ${l.name}: ${l.dependencies.length} dependencies`);
