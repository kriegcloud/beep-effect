...
**Clarify Intent:**
```
User: "Fix the auth issue"

# BAD - Assumes which issue
Assistant: Let me fix the password validation...

# GOOD - Asks for clarification
Assistant: I see several auth-related areas. Are you referring to:
1. The login timeout on line 42?
2. The session validation in AuthMiddleware?
3. Something else?
```
...
```
# BAD - Large rewrite
- Refactoring 5 files simultaneously
- Changing architecture + fixing bug in one PR

# GOOD - Focused diffs
- Fix the null check in UserService.ts (1 file, 3 lines)
- Then: Refactor UserService separately if needed
```
