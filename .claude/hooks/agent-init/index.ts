...
import { Array as Arr, Config, Console, Effect, FileSystem, Layer, Path, pipe, Schema, ServiceMap } from "effect";
...
const listMemories = pipe(
  Effect.gen(function* () {
    ...
    if (!exists) return "No memories found (vault not initialized).";

    const entries = yield* fs.readDirectory(vaultPath);
    const mdFiles = pipe(
      entries,
      Arr.filter((f: string) => f.endsWith(".md")),
      Arr.take(10)
    );

    if (mdFiles.length === 0) return "Memory vault exists but is empty.";

    return pipe(
      mdFiles,
      Arr.map((f: string) => `  - ${f.replace(".md", "")}`)
    ).join("\n");
  }),
...
export class AgentConfigError extends Schema.TaggedErrorClass<AgentConfigError>(
  "@beep/claude/hooks/agent-init/AgentConfigError"
