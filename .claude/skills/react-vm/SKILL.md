...
declare var ConsentListVM: ServiceMap.Service<ConsentListVM>
interface ConsentListVM {}

// services/ConsentService.ts - shared business logic
export class ConsentService extends ServiceMap.Service<ConsentService, {
...
import { AtomRegistry } from "@effect-atom/atom/Registry"
import { ServiceMap, Layer, Effect, pipe, Data } from "effect"
...
const vmAtom = Atom.family(<Id, Value, E>(key: VmKey<Id, Value, E>) =>
  Atom.make(
    Effect.gen(function* () {
      ...
      return ServiceMap.get(ctx, key.tag)
    })
  )
)

export const useVM = <Id, Value, E>(
  tag: ServiceMap.Service<Value>,
...
describe("WalletVM", () => {
  ...
  const makeVM = () => {
    ...
    const vm = Layer.build(WalletVM.layer).pipe(
      Effect.map((ctx) => ServiceMap.get(ctx, WalletVM.tag)),
      Effect.scoped,
