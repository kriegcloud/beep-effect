...
Each platform context (`NodeServices.layer`, `BunServices.layer`, etc.) provides these services:
...
| **ChildProcessSpawner** | `ChildProcessSpawner` | Spawn and manage child processes |

### Usage Example

```typescript
import { FileSystem, Path, Terminal } from "effect"
...
const buildProject = Effect.gen(function* () {
  ...
  // Use ChildProcess for processes
  const cmd = ChildProcess.make`npm run build`
  const exitCode = yield* ChildProcess.exitCode(cmd)
  return exitCode
})
```
...
1. **Import abstractions, provide implementations**: Application code imports from `effect`, entry points provide platform-specific contexts
2. **One platform layer per runtime**: Use exactly one of `NodeServices.layer`, `BunServices.layer`, etc.
