...
## Scaffold Mode Placeholder Output

**Problem:** Commands return `{ "mode": "scaffold", ... }` JSON instead of
real generated files.

**Cause:** beep-sync is in scaffold mode (`0.0.0-scaffold`). Commands that do
not match a specific POC fixture path fall through to the scaffold stub.

**Fix:** This is expected until P1-P3 implementation is complete. Use the
POC fixtures for testing real behavior.
