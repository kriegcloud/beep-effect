...
import { dual, P, Str } from "@beep/utils";
...
const $I = $AgentsDomainId.create("Telemetry");
 ...
 *     maxTokens: 1000
 *   })
 * }).pipe(Effect.withSpan("chat.completion"))
...
export const addSpanAttributes =
  ...
  <Attributes extends Record<string, any>>(
    ...
    for (const [key, value] of Object.entries(attributes)) {
      if (P.isNotNullish(value)) {
        span.attribute(`${keyPrefix}.${transformKey(key)}`, value);
      }
    }
  };

const addSpanBaseAttributes = addSpanAttributes("gen_ai", Str.camelToSnake)<BaseAttributes>;
const addSpanOperationAttributes = addSpanAttributes("gen_ai.operation", Str.camelToSnake)<OperationAttributes>;
const addSpanRequestAttributes = addSpanAttributes("gen_ai.request", Str.camelToSnake)<RequestAttributes>;
const addSpanResponseAttributes = addSpanAttributes("gen_ai.response", Str.camelToSnake)<ResponseAttributes>;
const addSpanTokenAttributes = addSpanAttributes("gen_ai.token", Str.camelToSnake)<TokenAttributes>;
const addSpanUsageAttributes = addSpanAttributes("gen_ai.usage", Str.camelToSnake)<UsageAttributes>;
...
export const addGenAIAnnotations: {
  ...
  if (P.isNotNullish(options.operation)) addSpanOperationAttributes(span, options.operation);
  if (P.isNotNullish(options.request)) addSpanRequestAttributes(span, options.request);
  if (P.isNotNullish(options.response)) addSpanResponseAttributes(span, options.response);
  if (P.isNotNullish(options.token)) addSpanTokenAttributes(span, options.token);
  if (P.isNotNullish(options.usage)) addSpanUsageAttributes(span, options.usage);
});
