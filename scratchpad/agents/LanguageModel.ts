...
import type { Cause, JsonSchema } from "effect";
import { Context, Effect, FiberSet, Queue, SchemaAST, Sink, Stream } from "effect";
import { constFalse, identity, pipe } from "effect/Function";
import * as R from "effect/Record";
import { CurrentConcurrency } from "effect/References";
...
const $I = $AgentsDomainId.create("LanguageModel");
 ...
 * @since 0.0.0
 */
export class LanguageModel extends Context.Service<LanguageModel, Service>()($I`LanguageModel`) {}
...
export interface Service {
  ...
  readonly generateText: {
    ...
    <
      Options extends {
        readonly toolkit: ToolkitOption<AnyTools>;
      } & NoExcessProperties<GenerateTextOptions<AnyTools>, Options>,
    >(
  ...
  readonly generateObject: <
    ...
    StructuredOutputSchema extends S.Codec<unknown, ObjectEncoded, unknown, unknown>,
    Options extends NoExcessProperties<GenerateObjectOptions<AnyTools, StructuredOutputSchema>, Options>,
    Tools extends Record<string, Tool.Any> = {},
  >(
...
/**
 * A function that transforms a `S.Codec` into a provider-compatible form for structured output generation.
 *
 * **Details**
 *
...
export type CodecTransformer = <T, E, RD, RE>(
  schema: S.ConstraintCodec<T, E, RD, RE>
) => {
  readonly codec: S.ConstraintCodec<T, unknown, RD, RE>;
 ...
 * @since 0.0.0
 */
export interface GenerateObjectOptions<Tools extends Record<string, Tool.Any>, StructuredOutputSchema extends S.Top>
...
export class GenerateTextResponse<Tools extends Record<string, Tool.Any>> {
  ...
  get toolResults(): Array<Response.ToolResultParts<Tools>> {
    return A.filter(this.content, (part) => part.type === "tool-result");
  }
  ...
  get finishReason(): Response.FinishReason {
    ...
    return P.isUndefined(finishPart) ? "unknown" : finishPart.reason;
  }
  ...
  get usage(): Response.Usage {
    const finishPart = A.findFirst(this.content, (part) => part.type === "finish");
    if (O.isNone(finishPart)) {
...
export type ToolkitOption<Tools extends AnyTools, E = never, R = never> = Tools extends unknown
 ...
 * @since 0.0.0
 */
export type ToolkitInput<Tools extends AnyTools, E = never, R = never> =
...
type ExtractErrorFromToolkitOption<ToolkitValue, DisableToolCallResolution extends boolean> =
  ToolkitValue extends Toolkit.WithHandler<infer Tools>
    ? AiError.AiError | (DisableToolCallResolution extends true ? never : Tool.HandlerError<Tools[keyof Tools]>)
...
type ExtractServicesFromToolkitOption<ToolkitValue> =
  ToolkitValue extends Toolkit.WithHandler<infer Tools>
    ...
    : ToolkitValue extends Effect.Effect<Toolkit.WithHandler<infer Tools>, infer _E, infer R>
      ? Tool.HandlerServices<Tools[keyof Tools]> | Tool.ResultDecodingServices<Tools[keyof Tools]> | R
      : never;

type ExtractToolkitResolutionError<ToolkitValue> =
  ToolkitValue extends Effect.Effect<Toolkit.WithHandler<infer _Tools>, infer E, infer _R> ? E : never;

type ExtractToolkitResolutionServices<ToolkitValue> =
  ToolkitValue extends Effect.Effect<Toolkit.WithHandler<infer _Tools>, infer _E, infer R> ? R : never;
...
export const make: (params: {
  ...
  /**
   * A function that transforms a `S.Codec` into a provider-compatible form
  ...
  const getSpanTransformer = Effect.serviceOption(CurrentSpanTransformer).pipe(
    Effect.map(O.orElse(() => parentSpanTransformer))
  );

  const idGenerator = yield* Effect.serviceOption(IdGenerator).pipe(Effect.map(O.getOrElse(() => defaultIdGenerator)));

  const generateText = <
    Options extends NoExcessProperties<GenerateTextOptions<AnyTools>, Options>,
    Tools extends Record<string, Tool.Any> = {},
  >(
  ...
  const streamText: <
    Tools extends Record<string, Tool.Any> = {},
    Options extends NoExcessProperties<GenerateTextOptions<Tools>, Options> = GenerateTextOptions<Tools>,
  >(
    options: Options & GenerateTextOptions<Tools>
  ) => Stream.Stream<Response.StreamPart<Tools>, ExtractError<Options>, ExtractServices<Options>> = Effect.fnUntraced(
    function* <
      Tools extends Record<string, Tool.Any>,
  ...
  >(
    ...
    AiError.AiError | S.SchemaError,
    IdGenerator | Tool.HandlerServices<Tools[keyof Tools]>
  > = Effect.fnUntraced(function* <
    Tools extends Record<string, Tool.Any>,
    Options extends NoExcessProperties<GenerateTextOptions<Tools>, Options>,
  >(options: Options & GenerateTextOptions<Tools>, providerOptions: Mutable<ProviderOptions>) {
    const tracker = O.getOrUndefined(yield* Effect.serviceOption(ResponseIdTracker.ResponseIdTracker));
    ...
    const generateWithNonIncrementalFallback = () => {
      ...
      return P.isNotUndefined(requestOptions.incrementalPrompt)
        ? params
            .generateText(requestOptions)
            .pipe(Effect.catchReason("AiError", "InvalidRequestError", (_) => params.generateText(fallbackOptions)))
    ...
    if (P.isUndefined(options.toolkit)) {
      ...
      if (hasPendingApprovals) {
        return yield* AiError.make({
          module: "LanguageModel",
          method: "generateText",
          reason: AiError.ToolkitRequiredError.make({
            pendingApprovals: pipe(
              [...approved, ...denied],
              A.map((result) => result.toolCall?.name),
      ...
      const ResponseSchema = Response.Part(Toolkit.empty).pipe(S.Array, S.mutable);
      const rawContent = yield* generateWithNonIncrementalFallback();
      const content = yield* S.decodeEffect(ResponseSchema)(rawContent);
      if (P.isNotUndefined(tracker)) {
        const responseMetadata = content.find((part) => part.type === "response-metadata");
        if (P.isNotUndefined(responseMetadata) && P.isNotUndefined(responseMetadata.id)) {
    ...
    const toolkit = yield* resolveToolkit<Tools, ExtractError<Options>, ExtractServices<Options>>(options.toolkit);
    ...
    if (hasPendingApprovals) {
      for (const approval of approved) {
        if (P.isNotUndefined(approval.toolCall) && P.isUndefined(toolkit.tools[approval.toolCall.name])) {
          return yield* AiError.make({
            module: "LanguageModel",
            method: "generateText",
            reason: AiError.ToolNotFoundError.make({
              toolName: approval.toolCall.name,
              availableTools: Struct.keys(toolkit.tools),
            }),
          });
        }
      }
    ...
    {
      const { approved: allResolved, denied: allDenied } = collectToolApprovals(providerOptions.prompt.content);
    ...
    const ResponseSchema = Response.Part(toolkit).pipe(S.Array, S.mutable);
  ...
  >(
    ...
    Stream.Stream<Response.StreamPart<Tools>, AiError.AiError | S.SchemaError, IdGenerator>,
  ...
  >(options: Options & GenerateTextOptions<Tools>, providerOptions: Mutable<ProviderOptions>) {
    ...
    const streamWithNonIncrementalFallback = () => {
      ...
      return P.isNotUndefined(requestOptions.incrementalPrompt)
        ? params
            ...
            .pipe(Stream.catchReason("AiError", "InvalidRequestError", (_) => params.streamText(fallbackOptions)))
    ...
    if (P.isUndefined(options.toolkit)) {
      ...
      const schema = Response.StreamPart(Toolkit.empty).pipe(S.NonEmptyArray);
      const decodeParts = S.decodeEffect(schema);
      return pipe(
        streamWithNonIncrementalFallback(),
        Stream.mapArrayEffect((parts) =>
          decodeParts(parts).pipe(
            P.isNotUndefined(tracker)
              ? Effect.tap((decodedParts) => {
                  for (const part of decodedParts) {
                    if (part.type === "response-metadata" && P.isNotUndefined(part.id)) {
      ...
      ) as Stream.Stream<Response.StreamPart<Tools>, AiError.AiError | S.SchemaError, IdGenerator>;
    }
    ...
    if (Object.values(toolkit.tools).length === 0) {
      ...
      if (hasPendingApprovals) {
        return yield* AiError.make({
          ...
          reason: AiError.ToolkitRequiredError.make({
            pendingApprovals: pipe(
              [...pendingApproved, ...pendingDenied],
              A.map((a) => a.toolCall?.name),
              A.filter(P.isNotUndefined)
            ),
          }),
        });
    ...
    const tools =
      typeof toolChoice === "object" && "oneOf" in toolChoice
        ? R.values(toolkit.tools).filter((tool) => toolChoice.oneOf.includes(tool.name))
        : Object.values(toolkit.tools);
    providerOptions.tools = tools;
    providerOptions.toolChoice = toolChoice;
    ...
    // resolving the tool calls that were generated
    if (options.disableToolCallResolution === true) {
      const schema = S.NonEmptyArray(Response.StreamPart(toolkit));
      const decodeParts = S.decodeEffect(schema);
    ...
    const ResponseSchema = S.NonEmptyArray(Response.StreamPart(toolkit));
    const decodeParts = S.decodeEffect(ResponseSchema);

    // Queue for decoded parts and tool results
    const queue = yield* Queue.make<Response.StreamPart<Tools>, AiError.AiError | Cause.Done | S.SchemaError>();
...
export const generateText: {
  ...
  options: GenerateTextOptions<AnyTools>
): Effect.Effect<GenerateTextResponse<AnyTools>, AiError.AiError, LanguageModel> =>
...
> => Effect.flatMap(Effect.service(LanguageModel), (model) => model.generateObject(options as any)) as any;
...
export const streamText: {
  ...
  >(
    ...
    ExtractServices<Options> | LanguageModel
  >;
} = (options: GenerateTextOptions<AnyTools>): Stream.Stream<Response.StreamPart<{}>, AiError.AiError, LanguageModel> =>
  Stream.unwrap(Effect.map(Effect.service(LanguageModel), (model) => model.streamText(options as any))) as any;
...
const collectToolApprovals = (
  ...
  for (const response of responses) {
    ...
    if (P.isNotUndefined(request)) {
      // Skip if already resolved
      if (P.isNotUndefined(options?.excludeResolved) && toolResultIds.has(request.toolCallId)) {
        continue;
      }
  ...
  for (const message of prompt.content) {
    if (message.role === "assistant") {
      const filteredContent = message.content.filter(
        (part) => part.type !== "tool-approval-request" || !resolvedApprovalIds.has(part.approvalId)
      );
      ...
      const filteredContent = message.content.filter(
        (part) => part.type !== "tool-approval-response" || !resolvedApprovalIds.has(part.approvalId)
      );
      if (filteredContent.length > 0) {
        cleanedMessages.push(
...
const isApprovalNeeded = Effect.fnUntraced(function* <T extends Tool.Any>(
  ...
  messages: ReadonlyArray<Prompt.Message>
): Effect.fn.Return<boolean, S.SchemaError, Tool.HandlerServices<T>> {
  if (P.isUndefined(tool.needsApproval)) {
    return false;
  }

  if (typeof tool.needsApproval === "function") {
    const params = yield* S.decodeUnknownEffect(tool.parametersSchema)(toolCall.params) as any;
...
> => {
  ...
  return Effect.gen(function* () {
    const resolveConcurrency =
      concurrency === "inherit" ? yield* Effect.service(CurrentConcurrency) : (concurrency ?? "unbounded");
...
const createDenialResults = (denials: ReadonlyArray<ApprovalResult>): ReadonlyArray<Prompt.ToolResultPart> => {
  const results: Array<Prompt.ToolResultPart> = [];
  for (const denial of denials) {
    if (P.isNotUndefined(denial.toolCall)) {
      results.push(
...
> => {
  ...
  const resolveConcurrency =
    concurrency === "inherit" ? Effect.service(CurrentConcurrency) : Effect.succeed(concurrency ?? "unbounded");
...
const resolveToolkit = <Tools extends Record<string, Tool.Any>, E, R>(
  ...
  (Effect.isEffect(toolkit) ? toolkit : Effect.succeed(toolkit as unknown as Toolkit.WithHandler<Tools>)) as any;

/** @internal */
export const getObjectName = <StructuredOutputSchema extends S.Constraint>(
  objectName: string | undefined,
  schema: StructuredOutputSchema
): string => {
  if (P.isNotUndefined(objectName)) {
    return objectName;
  }
  if ("identifier" in schema && P.isString(schema.identifier)) {
    return schema.identifier;
  }
  const identifier = SchemaAST.resolveIdentifier(schema.ast);
  if (P.isString(identifier)) {
    return identifier;
  }
  return "generateObject";
};

const resolveStructuredOutput = Effect.fnUntraced(function* <StructuredOutputSchema extends S.Constraint>(
  ...
  if (text.length === 0) {
    return yield* AiError.make({
      module: "LanguageModel",
      method: "generateObject",
      reason: AiError.StructuredOutputError.make({
...
const applySpanTransformer = (
  transformer: O.Option<SpanTransformer>,
  response: ReadonlyArray<Response.AllParts<any>>,
  options: ProviderOptions
): void => {
  if (O.isSome(transformer)) {
