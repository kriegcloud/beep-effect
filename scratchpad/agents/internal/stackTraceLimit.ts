...
const $I = $AgentsDomainId.create("internal/stackTraceLimit");

class ErrorWithStackTraceLimit extends S.Class<ErrorWithStackTraceLimit>($I`ErrorWithStackTraceLimit`)({
  setStackTraceLimit: S.Finite.pipe(S.optionalKey, S.mutableKey),
}) {}
...
export const getStackTraceLimit = (): number | undefined => {
  const err = Error as ErrorWithStackTraceLimit;

  if (P.isUndefined(err)) {
    return undefined;
  }
  return err.setStackTraceLimit;
};
...
export const setStackTraceLimit = (value: number | undefined): void => {
  if (canWriteStackTraceLimit) {
    Object.assign(Error as ErrorWithStackTraceLimit, {
