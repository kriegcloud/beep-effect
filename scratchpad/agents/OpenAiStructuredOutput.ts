 ...
 *
 * OpenAI structured output accepts only a subset of JSON S. This module
 * converts an Effect `S.Codec` into a provider-compatible JSON Schema and
...
import { JsonSchema, SchemaAST, SchemaTransformation } from "effect";
import * as R from "effect/Record";
import * as S from "effect/Schema";
import * as Tool from "./Tool.ts";

/**
 * Converts a `S.Codec` to OpenAI structured-output JSON Schema and a
...
export function toCodecOpenAI<T, E, RD, RE>(
  ...
  codec: S.ConstraintCodec<T, unknown, RD, RE>;
  jsonSchema: JsonSchema.JsonSchema;
} {
  const to = schema.ast;
  const from = recurOpenAI(SchemaAST.toEncoded(to));
  const codec =
    from === to ? schema : S.make<typeof schema>(SchemaAST.decodeTo(from, to, SchemaTransformation.passthrough()));
  ...
  if (Object.keys(document.definitions).length > 0) {
    jsonSchema.$defs = R.map(document.definitions, rewriteOpenAI);
  }
  return { codec, jsonSchema };
}

/**
 * Post-processes the JSON schema produced by `S.toJsonSchemaDocument`,
...
function recurOpenAI(ast: SchemaAST.AST): SchemaAST.AST {
  switch (ast._tag) {
    ...
    case "Undefined":
      return unsupportedAst(
        ast,
        "OpenAI structured output does not support undefined; consider transforming the schema or using a different provider; if using `S.optional`, consider using `S.optionalKey` instead"
      );
    case "Null":
      return ast;
    case "String": {
    ...
    case "Arrays": {
      ...
      if (ast.elements.length > 0) {
        ...
        const propertySignatures = ast.elements.map((e, i) => new SchemaAST.PropertySignature(String(i), e));
        if (ast.rest.length === 1) {
          ...
          if (SchemaAST.isOptional(ps.type)) {
            type = SchemaAST.decodeTo(
              ...
              SchemaTransformation.transformOptional({
                decode: O.filter(P.isNotNull),
                encode: O.orElseSome(() => null),
              })
            );
          }
          if (type === ps.type) {
    ...
    case "Suspend": {
      ...
      if (P.isNotUndefined(cached)) return cached;
...
const get = (
  ...
  return {
    ...
    filters: A.isArrayNonEmpty(filters) ? filters : undefined,
  };
};
...
const getAnnotations = (annotations: S.Annotations.Filter | undefined): Array<Annotation> => {
  const out: Array<Annotation> = [];
  if (annotations !== undefined) {
    const description =
      annotations?.description ??
      (annotations.meta?._tag === "isInt" || annotations.meta?._tag === "isFinite" ? undefined : annotations?.expected);
