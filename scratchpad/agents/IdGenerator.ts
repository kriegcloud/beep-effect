...
const $I = $AgentsDomainId.create("IdGenerator");
 ...
 * @since 0.0.0
 */
export class IdGenerator extends Context.Service<IdGenerator, Service>()($I`IdGenerator`) {}
...
const makeGenerator = ({
  ...
  size = DEFAULT_SIZE,
}: Partial<MakeOptions>): Effect.Effect<string> => {
  ...
  return Effect.gen(function* () {
    ...
    if (P.isUndefined(prefix)) {
      return identifier;
    }
    return `${prefix}${separator}${identifier}`;
  }).pipe(Effect.withSpan("IdGenerator.generateId"));
};
