...
/**
 * Converts a `S.Codec` to Anthropic structured-output JSON Schema and a
...
function recur(ast: SchemaAST.AST): SchemaAST.AST {
  switch (ast._tag) {
    ...
    case "Undefined":
      return unsupportedAst(
        ast,
        "Anthropic structured output does not support undefined; consider transforming the schema or using a different provider; if using `S.optional`, consider using `S.optionalKey` instead"
      );
    case "Null":
      return ast;
    case "String": {
    ...
    case "Arrays": {
      ...
      if (ast.elements.length > 0) {
        ...
        const propertySignatures = A.map(ast.elements, (e, i) => new SchemaAST.PropertySignature(String(i), e));
        if (ast.rest.length === 1) {
...
type Filter = Annotation | { readonly _tag: "filter"; readonly filter: SchemaAST.Filter<any> };
