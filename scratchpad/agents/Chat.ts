...
import { Channel, Chunk, Context, Duration, Effect, Layer, Ref, Semaphore, Stream } from "effect";
import * as S from "effect/Schema";
import type { NoExcessProperties } from "effect/Types";
import type { PersistenceError } from "effect/unstable/persistence/Persistence";
...
const $I = $AgentsDomainId.create("Chat");

type AnyTools = Record<string, Tool.Any>;
type GenerateTextCall<Tools extends Record<string, Tool.Any>, Options> = (
 ...
 * @since 0.0.0
 */
export class Chat extends Context.Service<Chat, Service>()($I`Chat`) {}
...
export interface Service {
  ...
  readonly generateText: {
    ...
    <
      Options extends {
        readonly toolkit: LanguageModel.ToolkitOption<AnyTools>;
      } & NoExcessProperties<LanguageModel.GenerateTextOptions<AnyTools>, Options>,
    >(
      options: Options &
  ...
  readonly generateObject: <
    ...
    ObjectSchema extends S.Codec<unknown, ObjectEncoded, unknown, unknown>,
    Options extends NoExcessProperties<LanguageModel.GenerateObjectOptions<AnyTools, ObjectSchema>, Options>,
  >(
...
const decodeHistory = S.decodeUnknownEffect(Prompt.Prompt);
const encodeHistory = S.encodeUnknownEffect(Prompt.Prompt);
const decodeHistoryJson = S.decodeUnknownEffect(S.fromJsonString(Prompt.Prompt));
const encodeHistoryJson = S.encodeUnknownEffect(S.fromJsonString(Prompt.Prompt));
...
const makeUnsafe = (history: Ref.Ref<Prompt.Prompt>) => {
  ...
  >(
    ...
    Effect.gen(function* () {
      ...
      const generate = LanguageModel.generateText as unknown as GenerateTextCall<Tools, Options>;
      const response = yield* generate({ ...options, prompt });

      const newHistory = Prompt.concat(prompt, Prompt.fromResponseParts(response.content));
      yield* Ref.set(history, newHistory);

      return response;
    }).pipe(semaphore.withPermits(1), Effect.withSpan("Chat.generateText"))) as Service["generateText"];
 ...
 * @since 0.0.0
 */
export const fromExport = (data: unknown): Effect.Effect<Service, S.SchemaError> =>
 ...
 * @since 0.0.0
 */
export const fromJson = (data: string): Effect.Effect<Service, S.SchemaError> =>
 ...
 * @since 0.0.0
 */
export class ChatNotFoundError extends S.ErrorClass<ChatNotFoundError>($I`ChatNotFoundError`)(
  {
    _tag: S.tag("ChatNotFoundError"),
    chatId: S.String,
  },
  $I.annote("ChatNotFoundError", {
    description: "Error raised when a persisted chat cannot be found in the backing persistence store by id.",
  })
) {}
 ...
 * @since 0.0.0
 */
export class Persistence extends Context.Service<Persistence, Persistence.Service>()($I`Persistence`) {}
...
export const makePersisted = Effect.fnUntraced(function* (options: { readonly storeId: string }) {
  ...
  const toPersisted = Effect.fnUntraced(function* (chatId: string, chat: Service, ttl: Duration.Input | undefined) {
    const idGenerator = yield* Effect.serviceOption(IdGenerator.IdGenerator).pipe(
      Effect.map(O.getOrElse(() => IdGenerator.defaultIdGenerator))
    );
    ...
    const saveChat = Effect.fnUntraced(function* (prevHistory: Prompt.Prompt) {
      ...
      if (P.isNotUndefined(lastMessage) && lastMessage.role === "assistant") {
      ...
      const timeToLive = P.isNotUndefined(ttl) ? O.getOrUndefined(Duration.fromInput(ttl)) : undefined;
    ...
    >(
      ...
      Effect.gen(function* () {
        const history = yield* Ref.get(chat.history);
        const generate = chat.generateText as unknown as GenerateTextCall<Tools, Options>;
        return yield* generate(options).pipe(Effect.ensuring(Effect.orDie(saveChat(history))));
  ...
  const getChat = Effect.fnUntraced(
    function* (chatId: string, ttl: Duration.Input | undefined) {
      ...
      if (P.isUndefined(previousHistory)) {
        return yield* ChatNotFoundError.make({ chatId });
      }
