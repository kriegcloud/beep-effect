...
import { A, dual, O, P } from "@beep/utils";
...
const $I = $AgentsDomainId.create("Prompt");
 ...
 * @since 0.0.0
 */
export const ProviderOptions: S.$Record<S.String, S.NullOr<S.Codec<S.Json>>> = S.Record(
  S.String,
  S.NullOr(S.Json)
).pipe(
  $I.annoteSchema("ProviderOptions", {
    description: "Schema for provider-specific options that can be attached to content parts and messages.",
  })
);
 ...
 * @since 0.0.0
 */
export const isPart = (u: unknown): u is Part => P.hasProperty(u, PartTypeId);
...
const BasePart = S.Struct({
  [PartTypeId]: S.Literal(PartTypeId).pipe(
    S.withDecodingDefaultKey(Effect.succeed(PartTypeId), { encodingStrategy: "omit" })
  ),
  options: ProviderOptions.pipe(S.withDecodingDefault(Effect.succeed({}))),
});
...
export const TextPart: S.Struct<{
  readonly type: S.Literal<"text">;
  readonly text: S.String;
  readonly "~effect/ai/Prompt/Part": S.withDecodingDefaultKey<S.Literal<"~effect/ai/Prompt/Part">>;
  readonly options: S.withDecodingDefault<S.$Record<S.String, S.NullOr<S.Codec<S.Json>>>>;
}> = S.Struct({
  ...BasePart.fields,
  type: S.Literal("text"),
  text: S.String,
}).annotate($I.annote("TextPart", { description: "Schema for validation and encoding of text parts." }));
...
export const ReasoningPart: S.Struct<{
  readonly type: S.Literal<"reasoning">;
  readonly text: S.String;
  readonly "~effect/ai/Prompt/Part": S.withDecodingDefaultKey<S.Literal<"~effect/ai/Prompt/Part">>;
  readonly options: S.withDecodingDefault<S.$Record<S.String, S.NullOr<S.Codec<S.Json>>>>;
}> = S.Struct({
  ...
  text: S.String,
}).annotate($I.annote("ReasoningPart", { description: "Schema for validation and encoding of reasoning parts." }));
...
export const FilePart: S.Struct<{
  readonly type: S.Literal<"file">;
  readonly mediaType: S.String;
  readonly fileName: S.optional<S.String>;
  readonly data: S.Union<readonly [S.String, S.Uint8Array, S.URL]>;
  readonly "~effect/ai/Prompt/Part": S.withDecodingDefaultKey<S.Literal<"~effect/ai/Prompt/Part">>;
  readonly options: S.withDecodingDefault<S.$Record<S.String, S.NullOr<S.Codec<S.Json>>>>;
}> = S.Struct({
  ...BasePart.fields,
  ...
  data: S.Union([S.String, S.Uint8Array, S.URL]),
}).annotate($I.annote("FilePart", { description: "Schema for validation and encoding of file parts." }));
 ...
 * @since 0.0.0
 */
export const ToolCallPart: S.Struct<{
  readonly type: S.Literal<"tool-call">;
  readonly id: S.String;
  readonly name: S.String;
  readonly params: S.Unknown;
  readonly providerExecuted: S.withDecodingDefault<S.Boolean>;
  readonly "~effect/ai/Prompt/Part": S.withDecodingDefaultKey<S.Literal<"~effect/ai/Prompt/Part">>;
  readonly options: S.withDecodingDefault<S.$Record<S.String, S.NullOr<S.Codec<S.Json>>>>;
}> = S.Struct({
  ...BasePart.fields,
  ...
  name: S.String,
  params: S.Unknown,
  providerExecuted: S.Boolean.pipe(S.withDecodingDefault(Effect.succeed(false))),
}).annotate($I.annote("ToolCallPart", { description: "Schema for validation and encoding of tool call parts." }));
...
export const ToolResultPart: S.Struct<{
  ...
  isFailure: S.Boolean,
  result: S.Unknown,
}).annotate($I.annote("ToolResultPart", { description: "Schema for validation and encoding of tool result parts." }));
 ...
 * @since 0.0.0
 */
export const ToolApprovalResponsePart: S.Struct<{
  readonly type: S.Literal<"tool-approval-response">;
  readonly approvalId: S.String;
  readonly approved: S.Boolean;
  readonly reason: S.optional<S.String>;
  readonly "~effect/ai/Prompt/Part": S.withDecodingDefaultKey<S.Literal<"~effect/ai/Prompt/Part">>;
  readonly options: S.withDecodingDefault<S.$Record<S.String, S.NullOr<S.Codec<S.Json>>>>;
}> = S.Struct({
  ...BasePart.fields,
  type: S.Literal("tool-approval-response"),
  approvalId: S.String,
  approved: S.Boolean,
  reason: S.optional(S.String),
}).annotate(
  $I.annote("ToolApprovalResponsePart", {
    description: "Schema for validation and encoding of tool approval response parts.",
  })
);
...
export const ToolApprovalRequestPart: S.Struct<{
  readonly type: S.Literal<"tool-approval-request">;
  readonly approvalId: S.String;
  ...
  ...BasePart.fields,
  type: S.Literal("tool-approval-request"),
  approvalId: S.String,
  toolCallId: S.String,
}).annotate(
 ...
 * @since 0.0.0
 */
export const isMessage = (u: unknown): u is Message => P.hasProperty(u, MessageTypeId);
...
const BaseMessage = S.Struct({
  [MessageTypeId]: S.Literal(MessageTypeId).pipe(
    S.withDecodingDefaultKey(Effect.succeed(MessageTypeId), { encodingStrategy: "omit" })
  ),
  options: ProviderOptions.pipe(S.withDecodingDefault(Effect.succeed({}))),
});
 ...
 * @since 0.0.0
 */
export const ContentFromString: S.decodeTo<
  S.NonEmptyArray<
    S.toType<
      S.Struct<{
        readonly type: S.Literal<"text">;
...
> = S.String.pipe(
  S.decodeTo(
    TextPart.pipe(S.toType, S.NonEmptyArray),
    SchemaTransformation.transform({
      decode: (text) => A.of(makePart("text", { text })) as A.NonEmptyReadonlyArray<TextPart>,
      encode: (content) => content[0].text,
    })
  ),
  $I.annoteSchema("ContentFromString", {
    description:
      "Schema that decodes a string into content containing a single TextPart and, when encoding, emits the text value of the first part.",
  })
);

/**
 * Type of content decoded from a string into a single `TextPart`.
 *
 * @category schemas
 * @since 0.0.0
 */
export type ContentFromString = typeof ContentFromString.Type;
 ...
 * @since 0.0.0
 */
export const SystemMessage: S.Struct<{
  readonly role: S.Literal<"system">;
  readonly content: S.String;
  readonly "~effect/ai/Prompt/Message": S.withDecodingDefaultKey<S.Literal<"~effect/ai/Prompt/Message">>;
  readonly options: S.withDecodingDefault<S.$Record<S.String, S.NullOr<S.Codec<S.Json>>>>;
}> = S.Struct({
  ...BaseMessage.fields,
  role: S.Literal("system"),
  content: S.String,
}).annotate($I.annote("SystemMessage", { description: "Schema for validation and encoding of system messages." }));
 ...
 * @since 0.0.0
 */
export const UserMessage: S.Struct<{
  readonly role: S.Literal<"user">;
  readonly content: S.Union<
    readonly [
      S.decodeTo<
        S.NonEmptyArray<
          S.toType<
  ...
  ...BaseMessage.fields,
  role: S.Literal("user"),
  content: S.Union([ContentFromString, S.Array(S.Union([TextPart, FilePart]))]),
}).annotate($I.annote("UserMessage", { description: "Schema for validation and encoding of user messages." }));
...
export const AssistantMessage: S.Struct<{
  readonly role: S.Literal<"assistant">;
  readonly content: S.Union<
    readonly [
      S.decodeTo<
        S.NonEmptyArray<
  ...
  content: S.Union([
    ContentFromString,
    S.Array(S.Union([TextPart, FilePart, ReasoningPart, ToolCallPart, ToolResultPart, ToolApprovalRequestPart])),
  ]),
}).annotate(
  $I.annote("AssistantMessage", { description: "Schema for validation and encoding of assistant messages." })
);
 ...
 * @since 0.0.0
 */
export const ToolMessage: S.Struct<{
  readonly role: S.Literal<"tool">;
  readonly content: S.$Array<S.Union<readonly [typeof ToolResultPart, typeof ToolApprovalResponsePart]>>;
  readonly "~effect/ai/Prompt/Message": S.withDecodingDefaultKey<S.Literal<"~effect/ai/Prompt/Message">>;
  readonly options: S.withDecodingDefault<S.$Record<S.String, S.NullOr<S.Codec<S.Json>>>>;
}> = S.Struct({
  ...BaseMessage.fields,
  role: S.Literal("tool"),
  content: S.Array(S.Union([ToolResultPart, ToolApprovalResponsePart])),
}).annotate($I.annote("ToolMessage", { description: "Schema for validation and encoding of tool messages." }));
 ...
 * @since 0.0.0
 */
export const Message: S.Codec<Message, MessageEncoded> = S.Union([
  SystemMessage,
  UserMessage,
  AssistantMessage,
  ToolMessage,
]).pipe($I.annoteSchema("Message", { description: "Schema for validation and encoding of messages." }));
 ...
 * @since 0.0.0
 */
export const isPrompt = (u: unknown): u is Prompt => P.hasProperty(u, TypeId);
...
const $Prompt = S.declare((u) => isPrompt(u), { identifier: "Prompt" });
 ...
 * @since 0.0.0
 */
export const Prompt: S.Codec<Prompt, PromptEncoded> = S.Struct({
  content: Message.pipe(S.toEncoded, S.Array),
}).pipe(
  S.decodeTo(
    $Prompt,
    SchemaTransformation.transformOrFail({
      decode: (input) =>
        Effect.mapBothEager(SchemaParser.decodeEffect(S.Array(Message))(input.content), {
          onSuccess: makePrompt,
          onFailure: () => new SchemaIssue.InvalidValue(O.some(input.content), { message: "Invalid Prompt messages" }),
        }),
      encode: (prompt) =>
        Effect.mapBothEager(SchemaParser.encodeEffect(S.Array(Message))(prompt.content), {
          onSuccess: (messages) => ({ content: messages }),
          onFailure: () => new SchemaIssue.InvalidValue(O.some(prompt.content), { message: "Invalid Prompt messages" }),
        }),
    })
  ),
  $I.annoteSchema("Prompt", { description: "Schema for AI prompt instances." })
);
 ...
 *
 * // Existing prompt
 * const existingPrompt = Prompt.make("Continue the conversation")
...
const decodeMessagesSync = S.decodeSync(S.Array(Message));
...
export const make = (input: RawInput): Prompt => {
  ...
  if (P.isIterable(input)) {
    return makePrompt(
      decodeMessagesSync(A.fromIterable(input), {
        errors: "all",
      })
    );
  }
