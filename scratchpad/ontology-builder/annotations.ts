...
import {
  ...
  type OntologyMetadataAnnotationPayload,
  OntologyTermName,
} from "./model.js";
import {
  normalizeIriInput,
  normalizeObjectRange,
...
export type OntologyObjectPredicateAnnotationInput = OntologyPredicateAnnotationInput & {
  readonly range: OntologyIriInput | OntologyTermNameInput;
};
...
export const predicateLabel = (termName: OntologyTermName): string => pipe(termName, humanizedTermName, Str.toLowerCase);

export const keyLeafTermName = (identifier: string): OntologyTermName =>
  pipe(identifier, Str.split("."), A.last, O.getOrElse(() => identifier), normalizeTermNameInput);
...
export const makeObjectPredicateDraft = (
  ...
  OntologyObjectPredicateAnnotationDraft.make({
    ...
    description: optionalString(input.description),
    comment: optionalString(input.comment),
    rangeClassIri: normalizeObjectRange(baseIri, input.range),
  });
