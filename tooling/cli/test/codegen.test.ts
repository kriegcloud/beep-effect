...
describe("codegen command", () => {
  it.effect("should dry-run barrel generation for repo-utils", () =>
    Effect.gen(function* () {
      // Resolve relative to test CWD (tooling/cli), so use ../repo-utils
      yield* run(["--package", "../repo-utils", "--dry-run"])
  ...
  it.effect("should skip internal directories and test files", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        yield* fs.writeFileString(
          path.join(tmpDir, "src", "internal", "secret.ts"),
          "export const secret = true\n",
        )

        // Test file (should be skipped)
        yield* fs.writeFileString(
          path.join(tmpDir, "src", "Visible.test.ts"),
