...
describe("Repo/TsConfigIndex.collectTsConfigPaths", () => {
  it.scoped(
    "collects root and utils tsconfig paths including optional variants",
    () =>
      Effect.gen(function* () {
        ...
        if (O.isSome(root)) {
          ...
          deepStrictEqual(
            arr[1]!.endsWith(path_.join("tsconfig.build.json")),
            true,
          );
          deepStrictEqual(
            arr[2]!.endsWith(path_.join("tsconfig.base.json")),
            true,
          );
        }
        ...
        if (O.isSome(utils)) {
          ...
          const base = arr[0];
          deepStrictEqual(
            base.endsWith(path_.join("tooling", "utils", "tsconfig.json")),
            true,
          );
