...
describe("createContentSecurityPolicyHeader", () => {
  ...
  describe("when giving an object", () => {
    ...
    it('should call the second argument function and return a value from the function as object\'s "name" property', () => {
      ...
      expect(createContentSecurityPolicyHeader(dummyOption, properHeaderNameGetterMock)).toHaveProperty(
        "name",
        dummyValue
      );
    });
...
describe("createDirectiveValue", () => {
  let arrayWrapperMock: ReturnType<typeof mock<typeof wrapArray<any>>>;
  beforeEach(() => {
    arrayWrapperMock = mock(wrapArray);
  });
...
describe("convertFetchDirectiveToString", () => {
  ...
  describe("when giving an object which has undefined", () => {
    it("should ignore the properties", () => {
      ...
      expect(
        convertFetchDirectiveToString({ "child-src": undefined, "object-src": undefined, "style-src": "'self'" })
      ).toBe("style-src 'self'");
    });
  });
