...
describe("createForceHTTPSRedirectHeader", () => {
  let headerValueCreatorMock: ReturnType<
    typeof mock<typeof createHSTSHeaderValue>
  >;
  beforeEach(() => {
  ...
  it("should return Some with Strict-Transport-Security header when value is provided", async () => {
    ...
    const result = await runEffect(
      createForceHTTPSRedirectHeader(undefined, headerValueCreatorMock)
    );
  ...
  it("should return None when value creator returns undefined", async () => {
    ...
    const result = await runEffect(
      createForceHTTPSRedirectHeader(false, headerValueCreatorMock)
    );
...
describe("createHSTSHeaderValue", () => {
  ...
  describe("when giving undefined", () => {
    it('should return "max-age" set two years', async () => {
      expect(await runEffect(createHSTSHeaderValue())).toBe(
        `max-age=${secondsOfTwoYears}`
      );
      expect(await runEffect(createHSTSHeaderValue(null as never))).toBe(
        `max-age=${secondsOfTwoYears}`
      );
    });
  });
  ...
  describe("when giving true", () => {
    it('should return "max-age" set two years', async () => {
      expect(await runEffect(createHSTSHeaderValue(true))).toBe(
        `max-age=${secondsOfTwoYears}`
      );
    });
  });
  ...
  describe('when specifying "maxAge" option', () => {
    describe("the number is valid", () => {
      it('should return "max-age" set the number', async () => {
        const dummyAge = 123;
        expect(
          await runEffect(createHSTSHeaderValue([true, { maxAge: dummyAge }]))
    ...
    describe("the number is invalid", () => {
      it("should raise error", () => {
        expect(
          Exit.isFailure(
            runEffectExit(createHSTSHeaderValue([true, { maxAge: Number.NaN }]))
          )
        ).toBe(true);
        expect(
          Exit.isFailure(
  ...
  describe('when specifying "includeSubDomains" option', () => {
    describe("the option is false", () => {
      it('should return only "max-age"', async () => {
        expect(
          await runEffect(
            createHSTSHeaderValue([true, { includeSubDomains: false }])
          )
        ).toBe(`max-age=${secondsOfTwoYears}`);
      });
    });
    ...
    describe("the option is true", () => {
      it('should return "max-age" and "includeSubDomains"', async () => {
        expect(
          await runEffect(
            createHSTSHeaderValue([true, { includeSubDomains: true }])
          )
        ).toBe(`max-age=${secondsOfTwoYears}; includeSubDomains`);
      });
    });
  });
  ...
  describe('when specifying "preload" option', () => {
    describe("the option is false", () => {
      it('should return only "max-age"', async () => {
        expect(
          await runEffect(createHSTSHeaderValue([true, { preload: false }]))
        ).toBe(`max-age=${secondsOfTwoYears}`);
      });
    });
    ...
    describe("the option is true", () => {
      it('should return "max-age" and "preload"', async () => {
        expect(
          await runEffect(createHSTSHeaderValue([true, { preload: true }]))
        ).toBe(`max-age=${secondsOfTwoYears}; preload`);
      });
    });
  });
  ...
  describe("when specifying all options", () => {
      ...
      it('should return only "max-age"', async () => {
        ...
        expect(
          await runEffect(
            createHSTSHeaderValue([
              true,
              { maxAge: dummyAge, includeSubDomains: false, preload: false },
            ])
          )
        ).toBe(`max-age=${dummyAge}`);
      });
    ...
    describe("the options are true", () => {
      it('should return "max-age" and the options', async () => {
        ...
        ).toBe(`max-age=${secondsOfTwoYears}; includeSubDomains; preload`);
        ...
        expect(
          await runEffect(
            createHSTSHeaderValue([
              true,
              { maxAge: dummyAge, includeSubDomains: true, preload: true },
            ])
          )
  ...
  describe("when giving invalid value", () => {
    it("should raise error", () => {
      expect(
        Exit.isFailure(runEffectExit(createHSTSHeaderValue("foo" as never)))
      ).toBe(true);
      expect(
        Exit.isFailure(runEffectExit(createHSTSHeaderValue([] as never)))
      ).toBe(true);
    });
  });
});