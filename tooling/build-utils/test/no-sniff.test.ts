...
} from "@beep/build-utils/secure-headers/no-sniff";
...
describe("createNosniffHeader", () => {
  let headerValueCreatorMock: ReturnType<
    typeof mock<typeof createXContentTypeOptionsHeaderValue>
  >;
  beforeEach(() => {
  ...
  it("should return Some with X-Content-Type-Options header when value is provided", async () => {
    ...
    const result = await runEffect(
      createNosniffHeader(undefined, headerValueCreatorMock)
    );
  ...
  it("should return None when value creator returns undefined", async () => {
    ...
    const result = await runEffect(
      createNosniffHeader(false, headerValueCreatorMock)
    );
...
describe("createXContentTypeOptionsHeaderValue", () => {
  describe("when giving undefined", () => {
    it('should return "nosniff"', async () => {
      expect(await runEffect(createXContentTypeOptionsHeaderValue())).toBe(
        "nosniff"
      );
      expect(
        await runEffect(createXContentTypeOptionsHeaderValue(null as never))
      ).toBe("nosniff");
    });
  });
  ...
  describe("when giving false", () => {
    it("should return undefined", async () => {
      expect(
        await runEffect(createXContentTypeOptionsHeaderValue(false))
      ).toBeUndefined();
    });
  });
  ...
  describe('when giving "nosniff"', () => {
    it('should return "nosniff"', async () => {
      expect(
        await runEffect(createXContentTypeOptionsHeaderValue("nosniff"))
      ).toBe("nosniff");
    });
  });
  ...
  describe("when giving invalid value", () => {
    it("should raise error", () => {
      expect(
        Exit.isFailure(
          runEffectExit(createXContentTypeOptionsHeaderValue(true as never))
        )
      ).toBe(true);
      expect(
        Exit.isFailure(
          runEffectExit(createXContentTypeOptionsHeaderValue("foo" as never))
        )
      ).toBe(true);
      expect(
        Exit.isFailure(
          runEffectExit(createXContentTypeOptionsHeaderValue([] as never))
        )
      ).toBe(true);
    });
  });
