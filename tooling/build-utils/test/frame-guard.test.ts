import { createFrameGuardHeader, createXFrameOptionsHeaderValue } from "@beep/build-utils/secure-headers/frame-guard";
...
describe("createFrameGuardHeader", () => {
  let headerValueCreatorMock: ReturnType<typeof mock<typeof createXFrameOptionsHeaderValue>>;
  beforeEach(() => {
  ...
  it("should return Some with X-Frame-Options header when value is provided", async () => {
    ...
    const result = await runEffect(createFrameGuardHeader(undefined, headerValueCreatorMock));
...
describe("createXFrameOptionsHeaderValue", () => {
  describe("when giving undefined", () => {
    it('should return "deny"', async () => {
      ...
      expect(await runEffect(createXFrameOptionsHeaderValue(null as never))).toBe("deny");
    });
  });

  describe("when giving false", () => {
    it("should return undefined", async () => {
      expect(await runEffect(createXFrameOptionsHeaderValue(false))).toBeUndefined();
    });
  });

  describe('when giving "deny"', () => {
    it('should return "deny"', async () => {
      expect(await runEffect(createXFrameOptionsHeaderValue("deny"))).toBe("deny");
    });
  });

  describe('when giving "sameorigin"', () => {
    it('should return "sameorigin"', async () => {
      expect(await runEffect(createXFrameOptionsHeaderValue("sameorigin"))).toBe("sameorigin");
    });
  });
  ...
  describe('when giving "allow-from" as array', () => {
    ...
    it('should call "encodeStrictURI"', async () => {
      ...
      await runEffect(createXFrameOptionsHeaderValue(["allow-from", { uri }], strictURIEncoderMock));
    ...
    it('should return "allow-from" and the URI', async () => {
      const uri = "https://example.com/";
      expect(await runEffect(createXFrameOptionsHeaderValue(["allow-from", { uri }]))).toBe(`allow-from ${uri}`);
    });
  });

  describe("when giving invalid value", () => {
    it("should raise error", () => {
      expect(Exit.isFailure(runEffectExit(createXFrameOptionsHeaderValue(true as never)))).toBe(true);
      expect(Exit.isFailure(runEffectExit(createXFrameOptionsHeaderValue("foo" as never)))).toBe(true);
      expect(Exit.isFailure(runEffectExit(createXFrameOptionsHeaderValue([] as never)))).toBe(true);
    });
  });
});
