...
describe("createXSSProtectionHeader", () => {
  let headerValueCreatorMock: ReturnType<
    typeof mock<typeof createXXSSProtectionHeaderValue>
  >;
  beforeEach(() => {
  ...
  it("should return Some with X-XSS-Protection header when value is provided", async () => {
    ...
    const result = await runEffect(
      createXSSProtectionHeader(undefined, headerValueCreatorMock)
    );
...
describe("createXXSSProtectionHeaderValue", () => {
  describe("when giving undefined", () => {
    it('should return "1"', async () => {
      ...
      expect(
        await runEffect(createXXSSProtectionHeaderValue(null as never))
      ).toBe("1");
    });
  });
  ...
  describe('when giving "sanitize"', () => {
    it('should return "1"', async () => {
      expect(
        await runEffect(createXXSSProtectionHeaderValue("sanitize"))
      ).toBe("1");
    });
  });
  ...
  describe('when giving "block-rendering"', () => {
    it('should return "1; mode=block"', async () => {
      expect(
        await runEffect(createXXSSProtectionHeaderValue("block-rendering"))
      ).toBe("1; mode=block");
    });
  });
  ...
  describe('when giving "report" as array', () => {
    ...
    it('should return "1; report=" and the URI', async () => {
      const uri = "https://example.com/";
      expect(
        await runEffect(createXXSSProtectionHeaderValue(["report", { uri }]))
      ).toBe(`1; report=${uri}`);
    });
  });
  ...
  describe("when giving invalid value", () => {
    it("should raise error", () => {
      expect(
        Exit.isFailure(
          runEffectExit(createXXSSProtectionHeaderValue(true as never))
        )
      ).toBe(true);
      expect(
        Exit.isFailure(
          runEffectExit(createXXSSProtectionHeaderValue("foo" as never))
        )
      ).toBe(true);
      expect(
        Exit.isFailure(
          runEffectExit(createXXSSProtectionHeaderValue([] as never))
        )
      ).toBe(true);
    });
  });
