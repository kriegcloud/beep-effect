 ...
 * @since 0.0.0
 */
export const FilePathToTsConfigFilePath = FilePath.pipe(S.decodeTo(TsConfigFilePath,
  {
 ...
 * @since 0.0.0
 */
export const FilePathToTypeScriptImplementationFilePath = FilePath.pipe(S.decodeTo(TypeScriptImplementationFilePath,
  {
    decode: SchemaGetter.passthrough({strict: false}),
    encode: SchemaGetter.passthrough({strict: false}),
  },
 ...
 * @since 0.0.0
 */
export const FilePathToTypeScriptDeclarationFilePath = FilePath.pipe(S.decodeTo(TypeScriptDeclarationFilePath,
  {
    decode: SchemaGetter.passthrough({strict: false}),
    encode: SchemaGetter.passthrough({strict: false}),
  },
 ...
 * @since 0.0.0
 */
export const FilePathToTypeScriptFilePath = FilePath.pipe(S.decodeTo(TypeScriptFilePath,
  {
    decode: SchemaGetter.passthrough({strict: false}),
    encode: SchemaGetter.passthrough({strict: false}),
  },
 ...
 * @since 0.0.0
 */
export const TypeScriptImplementationFilePathToSymbolFilePath = TypeScriptImplementationFilePath.pipe(S.decodeTo(SymbolFilePath, {
    decode: SchemaGetter.passthrough({strict: false}),
    encode: SchemaGetter.passthrough({strict: false}),
  }),
...
export const TsMorphScopeEntrypoint = S.Union([
  ...
  .pipe(S.toTaggedUnion("_tag"), SchemaUtils.withStatics(() => {
    const make = (path: string) => {
      return Match.value(path).pipe(Match.when(
        S.is(TsConfigFilePath),
        (path) => new TsMorphScopeEntrypointTsConfig({tsConfigPath: path}),
      ), Match.when(
        S.is(TypeScriptFilePath),
        (path) => new TsMorphScopeEntrypointFile({filePath: path}),
      ), Match.orElseAbsurd)
    }
