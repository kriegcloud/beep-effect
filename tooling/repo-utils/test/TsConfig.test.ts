...
layer(TestLayer)("TsConfig", (it) => {
  describe("collectTsConfigPaths", () => {
    it.effect("should collect tsconfig files for root and workspaces", () =>
      Effect.gen(function* () {
    ...
    it.effect("should find tsconfig files in workspace packages", () =>
      Effect.gen(function* () {
        const configs = yield* collectTsConfigPaths(MOCK_ROOT);
    ...
    it.effect("should find single tsconfig in packages with one config", () =>
      Effect.gen(function* () {
        const configs = yield* collectTsConfigPaths(MOCK_ROOT);
    ...
    it.effect("should return absolute paths", () =>
      Effect.gen(function* () {
        const configs = yield* collectTsConfigPaths(MOCK_ROOT);
    ...
    it.effect("should include all workspace packages", () =>
      Effect.gen(function* () {
        const configs = yield* collectTsConfigPaths(MOCK_ROOT);
