...
import { NoSuchFileError, DomainError, CyclicDependencyError } from "../../src/errors/index.js"
...
describe("Error types", () => {
    ...
    it.effect("should be catchable by tag in Effect", () =>
      Effect.gen(function* () {
        const result = yield* Effect.fail(
          new NoSuchFileError({ path: "/missing", message: "not found" })
        ).pipe(
  ...
  describe("DomainError", () => {
    ...
    it.effect("should be catchable by tag in Effect", () =>
      Effect.gen(function* () {
        const result = yield* Effect.fail(
          new DomainError({ message: "domain fail" })
        ).pipe(
      ...
      Effect.gen(function* () {
        const result = yield* Effect.fail(
          ...
          Effect.catchTag("CyclicDependencyError", (e) =>
            Effect.succeed(`caught ${e.cycles.length} cycle(s)`)
          )
        )
