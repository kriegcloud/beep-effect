...
import { resolveWorkspaceDirs, getWorkspaceDir } from "../src/Workspaces.js"
...
layer(TestLayer)("Workspaces", (it) => {
  describe("resolveWorkspaceDirs", () => {
    it.effect("should resolve all workspace packages", () =>
      Effect.gen(function* () {
    ...
    it.effect("should map names to absolute directory paths", () =>
      Effect.gen(function* () {
        const workspaces = yield* resolveWorkspaceDirs(MOCK_ROOT)
    ...
    it.effect("should return empty HashMap when no workspaces defined", () =>
      Effect.gen(function* () {
        // pkg-a has no workspaces field
        const workspaces = yield* resolveWorkspaceDirs(
          path.resolve(MOCK_ROOT, "packages/pkg-a")
        )
        expect(HashMap.size(workspaces)).toBe(0)
      })
    )

    it.effect("should fail with NoSuchFileError for missing root", () =>
      Effect.gen(function* () {
  ...
  describe("getWorkspaceDir", () => {
    it.effect("should find an existing workspace by name", () =>
      Effect.gen(function* () {
    ...
    it.effect("should return None for a non-existent workspace", () =>
      Effect.gen(function* () {
