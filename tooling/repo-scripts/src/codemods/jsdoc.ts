  ...
  root
    ...
    .forEach((path) => {
      ...
      j(path)
        ...
        .forEach((path) => {
          const typeArguments = path.node.typeArguments;
          if ((typeArguments as { type?: string })?.type === "TSTypeParameterInstantiation") {
            const tsTypeArguments = typeArguments as unknown as TsTypeParameterInstantiation;
            tsTypeArguments.params.forEach((param: TSTypeKind) => {
...
type TsTypeParameterInstantiation = {
  ...
  readonly params: ReadonlyArray<TSTypeKind>;
};
