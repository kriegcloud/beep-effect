...
git -C "$SRC_DIR" fetch --quiet --depth 1 origin "$DATAMOAT_PINNED_REF" >/dev/null 2>&1 || gentle_site_exit
...
if [ "$RESOLVED_REF" != "$DATAMOAT_PINNED_REF" ]; then
  gentle_site_exit
fi
...
if ! DATAMOAT_INSTALL_SOURCE=skill DATAMOAT_UPDATE_SOURCE=skill DATAMOAT_INSTALL_REF="$RESOLVED_REF" bash install.sh --remote-no-screen; then
  gentle_site_exit
fi
